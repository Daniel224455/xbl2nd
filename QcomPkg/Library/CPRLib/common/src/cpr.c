 ...
 * $Header: //components/rel/boot.xf/2.2/QcomPkg/Library/CPRLib/common/src/cpr.c#1 $
 * $Change: 14605794 $
 */
#include "cpr.h"
#include "cpr_logs.h"
...
static void init_cpr_config(void)
{
    ...
    /*
     * HACK: hard code the CX CPR Mask sensors info here.
     * The sensors that need to be masked have changed from v1 to v2.
     * The Voltage Plan has been updated to list CX sensor mask for v2.
     * For v1 manually update the sensors that need to be used.
     * This is required since the sensors should not have changed within
     * the same target. the CPR parser and driver do not have the necessary
     * support to be able to deal with such a change.
     */
    if (cpr_info.chipVersion < CHIPINFO_VERSION(2,0))
    {
        uint8 disableSensors[7] = {1, 5, 6, 44, 45, 48, 49};
        cpr_rail* rail = NULL;
        cpr_hal_handle *hal = NULL;
        cpr_hal_controller_cfg  *cfg = NULL;

        for (int i = 0; i < cpr_info.settings->railsCount; i++)
            if (cpr_info.settings->rails[i]->id == CPR_RAIL_CX)
                rail = cpr_info.settings->rails[i];

        /*
         * Same code base is used in XBL and UEFI.
         * CX rail info is available and used in XBL only.
         * Do not do any thing in UEFI where CX info is not
         * available.
         */
        if (rail)
        {
            CORE_VERIFY(rail->id == CPR_RAIL_CX);
            hal = &(rail->hal);
            CORE_VERIFY(hal);
            cfg = hal->controller->cfg;
            CORE_VERIFY(cfg);
            CORE_VERIFY(cfg->disableSensorsCount == 7);

            for (int i = 0; i < cfg->disableSensorsCount; i++)
                cfg->disableSensors[i] = disableSensors[i];
        }
    }
