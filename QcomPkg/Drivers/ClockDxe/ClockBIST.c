  ...
  if (bBISTPassed == TRUE)
  {
    ...
    else
    {
      ...
      for (i = 0; i < pClock->pDomain->nBSPLen; i++)
      {
        ...
        /* Skip if required voltage corner is NOM(boot corner) until we port VCS driver */
        if (pBSPConfig->eVRegLevel > RAIL_VOLTAGE_LEVEL_NOM)
        {
          continue;
        }
...
)
{
  ...
  ClockNameListType         *pExcludedPowerDomains = NULL;
  ...
  /*-----------------------------------------------------------------------*/
  /* Get the pointer to the List of power domains excluded from enablement.*/
  /*-----------------------------------------------------------------------*/

  eResult = Clock_GetPropertyValue("ExcludedPowerDomains", &PropVal);

    if (eResult == DAL_SUCCESS)
    {
    pExcludedPowerDomains = (ClockNameListType *)PropVal;
  }
      ...
      if (Clock_GetClockId(pDrvCtxt, pBISTDependencies[i].szName, &ClockId) == DAL_SUCCESS)
      {
        ...
        if (pClock == NULL)
        {
          ULOG_RT_PRINTF_2 (pDrvCtxt->hClockBISTLog, "Failed to retrieve clock pointer for ID %d for BIST dependency clock %s",
            (int)ClockId, pBISTDependencies[i].szName);
  ...
  for (i = 0; i < pDrvCtxt->pBSP->nNumPowerDomains; i++)
  {
    ...
    {
      if (Clock_CheckIfExcludedFromBist(pExcludedPowerDomains, pPowerDomain->szName) == TRUE)
      {
        continue;
      }
    }
  ...
  for (nClockIndex = 0; nClockIndex < pDrvCtxt->pBSP->nNumClocks; nClockIndex++)
  {
    ...
    {
      if (Clock_CheckIfExcludedFromBist(pExcludeFromBist, pClock->szName) == TRUE)
      {
        continue;
      }
    }
    ...
    if (pDrvCtxt->pBISTExceptions != NULL)
    {
      if (Clock_CheckBistExceptions(pDrvCtxt, pClock->szName) == TRUE)
      {
  ...
  for (i = 0; i < pDrvCtxt->pBSP->nNumPowerDomains; i++)
  {
    ...
    {
      Clock_DisablePowerDomain(pDrvCtxt, pPowerDomain);
    }
  }

  /*
   * Disable any clocks that needed to be enabled prior to running the BIST.
   */
  if (pBISTDependencies != NULL)
  {
    for (i = 0; pBISTDependencies[i].szName != NULL; i++)
    {
      if (Clock_GetClockId(pDrvCtxt, pBISTDependencies[i].szName, &ClockId) == DAL_SUCCESS)
      {
        ...
        {
          if (Clock_DisableClock(pDrvCtxt, pClock) != DAL_SUCCESS)
          {
            ULOG_RT_PRINTF_1 (pDrvCtxt->hClockBISTLog, "Failed to disable BIST dependency clock %s",
              pBISTDependencies[i].szName);
            DEBUG((EFI_D_WARN, "Failed to disable BIST dependency clock %a\n",pBISTDependencies[i].szName));
          }
        }
      }
      else
