...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 11/23/18   pr       Changes related to assigning TZapps regions to AC_VM_HLOS_UNMAPPED
...
/* 
* Load embedded TA using QSEE scm command
*/
UINT32
QseeActivateTzEmbeddedTASyscall(
  UINT8 *pAppName, 
  ...
  )
{
   ...
   if(AppNameLength > QSEE_MAX_NAME_SIZE || pAppName == NULL)
       return AppId;
   
   Status = QcomScmProtocol->ScmSendCommand(QcomScmProtocol,
                                            APP_START_EMBEDDED_APP_CMD,
                                            &(AppId),
                                            pAppName,
                                            AppNameLength,
                                            NULL,
 ...
 */
EFI_STATUS
QseeAppRegionAssignSyscall(UINT64 applications_region_addr, UINT64 applications_region_size)
{
	...
	hyp_memprot_dstVM_perm_info_t dstVM = {AC_VM_HLOS_UNMAPPED, (VM_PERM_R|VM_PERM_W), (UINT64)NULL, 0};
	UINT64 BufferSize = 0;
	...
	// ShmBridgeFree will be called for this allocation. Allocation will be freed 
	...
	if (AssignBufferPtr != 0)
	{
		...
		// adding 4 bytes for ctx of hyp_memprot_dstVM_perm_info_t to be 64 bits aligned
		p_qsee_syscall->destVMlist = (UINT64)AssignBufferPtr + sizeof(hyp_memprot_ipa_info_t) + sizeof(srcVM) + 4;
		p_qsee_syscall->destVMlistsize = sizeof(hyp_memprot_dstVM_perm_info_t);
		CopyMem((VOID *)p_qsee_syscall->destVMlist, &dstVM, sizeof(hyp_memprot_dstVM_perm_info_t));
		p_qsee_syscall->spare = 0;

		Status = QcomScmProtocol->ScmSipSysCall(QcomScmProtocol, HYP_MEM_PROTECT_ASSIGN, HYP_MEM_PROTECT_ASSIGN_PARAM_ID, Parameters, Results);
		if (EFI_ERROR(Status))
		{
  ...
  )
{
   ...
   DEBUG((EFI_D_INFO, "AppReg Init:%d mS\n", endTime-startTime));
   
   DEBUG((EFI_D_INFO, "SMC command APP_REGION_NOTIFICATION_CMD returned status %x \n",Status));
...
EFI_STATUS ActivateTzEmbeddedSecureImage
(
   IN  EFI_TZE_LOADER_PROTOCOL        *This,
   IN  UINT8                          *pAppName,
   IN  UINT64                         AppNameLength,
   IN BOOLEAN                         ServicesMbn,
   OUT UINT32                         *AppIdPtr
)
{
  ...
  if (ServicesMbn == FALSE)
  { //normal app load
    AppId = QseeActivateTzEmbeddedTASyscall(pAppName, AppNameLength);
    if ((AppId == 0) || (AppId == 0xFFFFFFFF))
    {
      DEBUG ((EFI_D_ERROR, "ActivateTzEmbeddedSecureImage Failed\n"));
      Status = EFI_LOAD_ERROR;
      goto ErrorExit;
    }
    else
