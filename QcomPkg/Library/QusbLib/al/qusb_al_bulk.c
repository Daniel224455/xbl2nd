...
uint32 qusb_al_bulk_handle_rx_zlp_mode(void)
{
  ...
  qusb_log__param(AL_BULK_HANDLE_RX_ZLP_MODE_LOG, qusb_zlp_mode);
...
void qusb_al_bulk_set_zlp_mode(boolean host_sends_zlp)
{
  ...
  qusb_log_bus__param(AL_BULK_SET_ZLP_MODE_LOG, qusb_zlp_mode);
}
...
static void qusb_al_bulk_rx_complete(struct qusb_urb* urb)
{
  ...
  if(NULL == urb)
  {
    ...
    qusb_log_err__param(AL_NULL_POINTER_ERROR_LOG, __LINE__);
    return;
  }

  qusb_log(AL_BULK_RX_COMPLETE_LOG);
  if (QUSB_URB_STATUS_COMPLETE_OK == urb->transfer_status)
  {
    if (urb->actual_length == 0) //ZLP 
    {
      qusb_log(AL_BULK_ZLP_ARRIVED_LOG);
    ...
    { 
      ...
      qusb_log__param(AL_BULK_SHORT_PACKET_RECVD_LOG, total_bytes_received);
    }
    total_bytes_received += urb->actual_length;
    qusb_is_rx_in_progress = FALSE;
    qusb_log__param(TRANSFER_DONE_LOG, total_bytes_received);
  }
  else
  {
  ...
  if (urb->transfer_length == urb->actual_length || short_packet_received)
  {
    ...
    {
      qusb_log__param(AL_BULK_DATA_RECVD_INSTEAD_OF_ZLP_LOG, urb->actual_length);        
      ...
      ((urb->transfer_length % qusb_al_bulk_get_max_packet_size()) == 0))
    {
      if (qusb_zlp_mode == QUSB_LACKS_ZLP) // No ZLP is expected
      {
        ...
        {
          qusb_log__param(AL_BULK_RX_CB_RX_DONE_LOG, total_bytes_received);
    ...
    else
    {
      if (al_bulk_rx_cb != NULL)
      {
        qusb_log__param(AL_BULK_RX_CB_SHORT_PACKET_LOG, total_bytes_received);
        al_bulk_rx_cb(total_bytes_received, BULK_SUCCESS);
        al_bulk_rx_cb = NULL; // use only once 
  ...
  {
    qusb_log__param(TRANSFER_ERROR_LOG, total_bytes_received);
  }
  else // more data to read 
  {
    ...
    qusb_log__param(AL_BULK_RX_MORE_DATA_LOG, remaining_read_size);
...
{
  qusb_log(AL_BULK_TX_COMPLETE_LOG);
  //When TX complete happens, urb can't be NULL, so check for NULL status 
  if(NULL == urb)
  {
    qusb_log_err(AL_NULL_POINTER_ERROR_LOG);
  }
  else
  {
...
{
  qusb_log_bus__param(AL_ENUM_COMPLETE_LOG, enum_flag);
...
static void qusb_al_disconnect_handler(void)
{
  ...
  qusb_log_bus(AL_DISCONNECT_HANDLER_LOG);
}
...
void qusb_al_bulk_init(qusb_max_speed_required_t speed_required)
{
  ...
  qusb_log__param(AL_BULK_INIT_ENTRY_LOG, speed_required);
  ...
  while ( FALSE == qusb_al_bulk_dev->qusb_is_configured )
  {
    if (TRUE == qusb_is_disconnected)
    {
      ...
      qusb_log_bus(AL_BULK_INIT_EXIT_LOG);
      return;
    }
  ...
  qusb_log_bus(SNPS_CONTROLLER_ENUM_COMPLETE_LOG);
  qusb_log_bus(AL_BULK_INIT_EXIT_LOG);
}
    ...
    if(qusb_timer_check_for_expiry())
    {
      ...
      switch(qusb_zlp_mode)
      {
        ...
        case QUSB_SENDS_ZLP:
          ...
          qusb_log_err(ZLP_TIMEOUT_ERROR_LOG); 
          break;
        case QUSB_LACKS_ZLP: 
...
{
  qusb_log_bus(AL_BULK_SHUTDOWN_LOG);
                              ...
                              uint32 * err_code)     // error code returned
{
  ...
  qusb_log__param(AL_BULK_RECEIVE_LOG, len);
  ...
  if (qusb_waiting_for_zlp) // The ZLP buffer has received data due to host skipping sending ZLP
  {
    ...
    qusb_log__param(AL_BULK_RX_ZLP_COPY_LOG, cached_zlp_data_length);
    ...
    if (cached_zlp_data_length < QUSB_MAX_CTRL_PKT_SIZE) // We received a short packet instead of the ZLP so we are done.
    {
      if(rx_cb)
      {
        qusb_log__param(AL_BULK_RX_CB_NO_ZLP_BUT_SHORT_PACKET_LOG, cached_zlp_data_length);
                               ...
                               uint32 * err_code)     // returned error code 
{
  ...
  *err_code = BULK_SUCCESS;
  ...
  qusb_log__param(AL_BULK_TRANSMIT_LOG, (uint32)tx_buf);
  qusb_log__param(TRANSFER_LENGTH_LOG, len);
  ...
  {
    qusb_log_err(AL_BULK_ERR_WRITE_LOG);
    qusb_al_bulk_tx_urb = NULL;
