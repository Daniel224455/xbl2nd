 ...
 ******************************************************************************/
/*==============================================================================
$Header: //components/rel/boot.xf/2.0/QcomPkg/SDM845Pkg/Library/DSFTargetLib/ddrsns/ddrss/src/ddrss_common.c#31 $
$DateTime: 2018/08/21 03:03:15 $
$Author: pwbldsvc $
================================================================================*/
...
{   
    ...
    MC_MR_Write(ddr, channel, chip_select, JEDEC_MR_13, 0x0); 
    /* Initialize MR22 for SoC ODT */
    /* FSP0, MR22 = 0x0 */
    MC_MR_Write(ddr, channel, chip_select, JEDEC_MR_22, 0x0);
    
    /* Set FPS-WR to 1 */
    MC_MR_Write(ddr, channel, chip_select, JEDEC_MR_13, 0x40);
    ...
    if (ddr->shrm_ddr_ptr->detected_ddr_device[0].device_type == DDR_TYPE_LPDDR4X) 
    {
		...
		{
            //if both byte mode
            if((ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_io_width[0] == DEVICE_WIDTH_IS_x8) && (ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_io_width[0] == DEVICE_WIDTH_IS_x8))
            {
                MC_MR_Write(ddr, channel, DDR_CS0, JEDEC_MR_22, 0xB4);
                MC_MR_Write(ddr, channel, DDR_CS1, JEDEC_MR_22, 0xFC);
            }
            //if CS0 is byte mode and CS1 is x16
            else if((ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_io_width[0] == DEVICE_WIDTH_IS_x8) && (ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_io_width[0] == DEVICE_WIDTH_IS_x16))
            {
                MC_MR_Write(ddr, channel, DDR_CS0, JEDEC_MR_22, 0xFC);
                MC_MR_Write(ddr, channel, DDR_CS1, JEDEC_MR_22, 0x34);
            }
            //if CS0 is x16 and CS1 is x8
            else if((ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_io_width[0] == DEVICE_WIDTH_IS_x16) && (ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_io_width[0] == DEVICE_WIDTH_IS_x8))
            {
                MC_MR_Write(ddr, channel, DDR_CS0, JEDEC_MR_22, 0x34);
                MC_MR_Write(ddr, channel, DDR_CS1, JEDEC_MR_22, 0xFC);
            }
            // both x16
			else
			{
				MC_MR_Write(ddr, channel, DDR_CS0, JEDEC_MR_22, 0x34);
				MC_MR_Write(ddr, channel, DDR_CS1, JEDEC_MR_22, 0x3C);                                       
			}
        }
