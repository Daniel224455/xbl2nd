...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 05/02/18   aus     Use secure wdog instead of pmic wdog
...
* 
*/ 
#define SEC_WDT_TIMEOUT_ON   32000
#define SEC_WDT_TIMEOUT_OFF  0

VOID XBLRamDumpMain( VOID )
{
  struct boot_sahara_interface* sbl_sahara_interface = NULL;
  uint32 dload_flag =0x0, timeout = 0;
  ...
  {
    /* Enable Secure WDT only for Minidump case, since full-dumps to eMMC/SDCard/QPST will not complete within PMIC WDG expiry */
    timeout = SEC_WDT_TIMEOUT_ON;
    boot_qsee_enable_secure_wdt(&timeout);
  }
 ...
 /*-----------------------------------------------------------------------
  ...
  /* Disable SEC WDT for QPST case, since QPST attach is manual and may result in WDOG expiry */
  timeout = SEC_WDT_TIMEOUT_OFF;
  boot_qsee_enable_secure_wdt(&timeout);  
