...
typedef enum
{
  // RULE: Do not add lines between ENUM
  // Formula to get ENUM value:
  // QUSB_DEFAULT_LOG's line # + enum value (from log) = line # for ENUM string
  ...
  DCI_HANDLE_SUSPEND_ENTRY__IN_U3,
  DCI_HANDLE_SUSPEND_ENTRY__NOT_IN_U3,
  DCI_HANDLE_SUSPEND_ENTRY__REMOTE_WAKEUP,
  ...
  DCI_CTRL_TRANSFER_DATA_REQUEST_LOG,
...
{
  uint32        tt;     // timetick
  uint16        idx;    // sequence number index
  qusb_log_enum id;     // enum id
  uint32        p0;     // parameter value
} qusb_log_t;
...
#else // QUSB_ENABLE_LOGGING

#define qusb_log(id)
#define qusb_log__param(id, param)

#define qusb_log_bus(id)
#define qusb_log_bus__param(id, param)

#define qusb_log_err(id)
#define qusb_log_err__param(id, param)
