/** @file
*  Copyright (c) 2014-2019, 2022 Qualcomm Technologies, Inc.  All Rights Reserved.
...
/*=============================================================================
 ...
 07/08/19   kpa     Update pre/post console PlatformBds api's
 05/28/19   pa      Added Capsule Update
 03/22/19   kpa     Added ProcessHotkeys, PostFwConfigSec
 03/15/19   kpa     Cleanup redundant debug logs
 01/21/19   kpa     Reorganize platformBdsInitEx
 11/20/18   kpa     Added PlatformBdsProtocol
...
BdsPlatformInit( BDS_INIT_OPTION *InitOption )
{
  ...
  */
  ...
  if (Status != EFI_SUCCESS)
  {
   ...
   QcomBdsDetectBootHotKey(InitOption);
...
{  
  ...
  Status = WaitTillProcessingComplete(FvFileGuid);
  if (EFI_ERROR (Status))
  {
    DEBUG ((EFI_D_ERROR, "Error: WaitTillProcessingComplete Failed - %r\n", Status));
    return Status;    
  } 
    
  Status = GetFvBufferInfo(FvFileGuid, &TempBuffAddr, &TempBuffSize);
  if (EFI_ERROR (Status))
  {
	...
	DEBUG((EFI_D_ERROR, "DecompressFvList BaseAddr: 0x%x Size: 0x%x\n", TempBuffAddr, TempBuffSize));
#endif
...
#ifdef ENABLE_LOCAL_DEBUG_LOGS    
    DEBUG ((EFI_D_ERROR, "FV Launch Wait Delta : %d ms\n", WaitTime));    
#endif
  ...
  )
{
  ...
  #ifdef ENABLE_AUTO_BOOT_OPTIMIZATION
 Status = DecompressFvLibReInit();
  ASSERT_EFI_ERROR (Status);
  
  Status = MountDecompressedFvs(&gCompressedSecondaryFvGuid);
  ASSERT_EFI_ERROR (Status);
  #endif
