...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 03/17/21   pcr     Added changes to handle ssc-qup related clocks/tlmm
 04/16/20   ag		fix memmory leak issue in Free API
...
boolean plat_gpio_enable (plat_device_config *config)
{
	...
	// Update TLMM config with TLMM type as SSC for SSC-QUP instances GPIOs
	 if(config->qupv3_instance == QUP_SSC) {
		config->sda_encoding |= (0x1 << 23);
		config->scl_encoding |= (0x1 << 23);
	}
