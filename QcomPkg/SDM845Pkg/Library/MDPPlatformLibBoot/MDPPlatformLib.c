...
typedef struct
{
  ...
  uint32                        uResetGpio;
  MDPPlatformLABIBBModeType     eOLEDPowerMode;  
...
static MDP_Status Panel_CDP_PeripheralPower(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams, bool32 bPowerUp)
{
  ...
  else if (MDP_PMIC_MODULE_CONTROLTYPE_IBB_LAB_OLED == pPowerParams->ePMICSecondaryPower[eDisplayId])
  {
    if (TRUE == bPowerUp)
    {
       ...
       if (PM_ERR_FLAG__SUCCESS != pm_ibb_set_soft_strt_chgr_resistor(PMIC_PMI_DEV_INDEX, 64))
       {
         MDP_Log_Message(MDP_LOGLEVEL_ERROR, "DisplayLib: Error to set start charging resistor\n");
       }
       ...
       if (MDPPLATFORM_LABIBB_MODE_SWIRE == pPowerParams->eOLEDPowerMode)
       {
         ...
         if (PM_ERR_FLAG__SUCCESS != pm_ibb_config_ibb_ctrl(PMIC_PMI_DEV_INDEX, FALSE, TRUE))
         {
           MDP_Log_Message(MDP_LOGLEVEL_ERROR, "DisplayLib: Error to config IBB control\n");
         }
       }
       ...
       if (PM_ERR_FLAG__SUCCESS != pm_lab_config_pulse_skip_ctrl(PMIC_PMI_DEV_INDEX, 20, 0, 0, TRUE))
       {
         MDP_Log_Message(MDP_LOGLEVEL_ERROR, "DisplayLib: Error to config LAB Pulse skip setting\n");
       }
