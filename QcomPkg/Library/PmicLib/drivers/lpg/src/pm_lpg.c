...
*  &copy; Copyright (c) 2009-2018 Qualcomm Technologies, Inc.  All Rights Reserved.
...
pm_err_flag_type pm_lpg_pwm_output_enable(uint32 pmic_chip, pm_lpg_chan_type pm_lpg_chan, boolean enable)
{
    ...
    else
    {
        ...
        {
            err_flag = pm_comm_write_byte_mask(lpg_ptr->comm_ptr->slave_id, en_pwm_output, 0x2, (pm_register_data_type)enable, 0);
        }
        else
        {
            err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
...
pm_err_flag_type pm_lpg_config_pause_time(uint32 pmic_chip, pm_lpg_chan_type pm_lpg_chan, uint32 ramp_step_duration, uint32 hi_multiplier, uint32 low_multiplier)
{
    ...
    else
    {
        ...
        err_flag |= pm_comm_write_byte(lpg_ptr->comm_ptr->slave_id, pause_hi_multiplier, (pm_register_data_type)(hi_multiplier & 0xFF), 0);
        ...
        err_flag |= pm_comm_write_byte(lpg_ptr->comm_ptr->slave_id, pause_lo_multiplier, (pm_register_data_type)(low_multiplier & 0xFF), 0);
...
pm_err_flag_type pm_lpg_get_status(uint32 pmic_chip, pm_lpg_chan_type pm_lpg_chan, pm_lpg_status_type *lpg_status)
{
    ...
    err_flag |= pm_comm_read_byte(lpg_ptr->comm_ptr->slave_id, pause_hi_multiplier, &data, 0);
    lpg_status->hi_multiplier = data;
    err_flag |= pm_comm_read_byte(lpg_ptr->comm_ptr->slave_id, pause_lo_multiplier, &data, 0);
