...
void ddr_initialize_device(uint32 clk_speed, boolean dload_mode)
{
  ...
  {    
    ...
    else
    {
      ...
      snprintf(ddr_log_string, 200, "Freeze IOs = 0x%x, 0x%x, 0x%x, 0x%x, DBG PDC Tr = 0x%x, First Pass expire = 0x%x, First Pass complete = 0x%x",
      ...
      {
        if (ddrsns_share_data->shrm_ddr_ptr->misc.chip_version >= 0x201)
        {
