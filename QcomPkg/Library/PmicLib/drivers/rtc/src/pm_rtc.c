...
pm_err_flag_type pm_rtc_alarm_set_ms_time(uint32 pmic_chip, pm_rtc_time_type  *time_ptr)
{
  pm_err_flag_type    err_flag    = PM_ERR_FLAG_SUCCESS;

  uint32 time_sec = 0;

  uint32 number_of_bytes = 4;

  uint8 periph_index = PM_RTC_ALARM;
  ...
  pm_register_address_type alrm_data0 = rtc_ptr->rtc_reg_table->base_address + 
                                        (pm_register_address_type)(rtc_ptr->rtc_reg_table->peripheral_offset * periph_index) + 
                                        rtc_ptr->rtc_reg_table->alrm_data0;

  pm_rtc_hal_rtc_get_time(pmic_chip, &hal_time);

  time_sec = hal_time.sec + time_ptr->sec + ((time_ptr->msec + 999) /1000) ; //+999/1000 to round up millisec to second

  err_flag = pm_comm_write_byte_array(rtc_ptr->comm_ptr->slave_id, alrm_data0, number_of_bytes, 
                                      (pm_register_data_type *)&time_sec, 0);
...
pm_err_flag_type pm_rtc_alarm_enable(uint32 pmic_chip, boolean  enable)
{
  ...
  if(!enable) {
	  pm_register_address_type reg_alrm_clear = rtc_ptr->rtc_reg_table->base_address + 
										   (pm_register_address_type)(rtc_ptr->rtc_reg_table->peripheral_offset * periph_index) + 
										   rtc_ptr->rtc_reg_table->alarm_clear;

	  err_flag = pm_comm_write_byte_mask(rtc_ptr->comm_ptr->slave_id, reg_alrm_clear, 0x01, 0x01, 0);
   }

  pm_register_address_type alrm_en_ctl = rtc_ptr->rtc_reg_table->base_address + 
                                         (pm_register_address_type)(rtc_ptr->rtc_reg_table->peripheral_offset * periph_index) + 
                                         rtc_ptr->rtc_reg_table->alrm_en_ctl;

  err_flag = pm_comm_write_byte_mask(rtc_ptr->comm_ptr->slave_id, 
                                     alrm_en_ctl, mask, data, 0);
