...
----------      ---   -----------------------------------------------------------
2018-02-20      jt    GetLUWriteProtectInfo should return 0 entries if wp cleared
   ...
   else if (0 == info.bLUWriteProtect) 
   {
      ...
      if (EFI_SUCCESS != Status)
      {
         DEBUG ((EFI_D_ERROR, "GetLbaWriteProtect: UFSGetSecureWpConfigBlockt failed "
                              "with return value of 0x%x(%r)\n", Status, Status));
         return Status; 
      }

      /* Once the configuration block is updated, we won't have NumEntries = 0.
         If we see that NumEntries = 1 and the first entry disabled, then we know
         that the ClearWriteProtect has happened and so we essentially
         have NumEntries = 0 */
