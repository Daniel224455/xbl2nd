...
#define   CLOCK_SILVER_NOMINAL_PERF_LEVEL       15 // 1516800UL OSM perf level (1516.8 MHz)
#define   CLOCK_GOLD_NOMINAL_PERF_LEVEL         16 // 1574400ULOSM perf level (300 MHz)
...
#define MDSSMDP_DEFAULT_FREQUENCY_HZ              412500 * 1000 // Nominal Frequency
...
)
{
  ...
  if (bPLLEnable)
  {
    eResult = Clock_SetCpuPerfLevel(CLOCK_APPS_CPU_C4, CLOCK_CLUSTER_LOW_PERF_LEVEL, &pnFrequencyHz); // set Gold to 300Mhz
  }
...
)
{
  ...
  {
    eResult =  Clock_SetCpuPerfLevel(CLOCK_APPS_CPU_C4, CLOCK_GOLD_NOMINAL_PERF_LEVEL, &pnFrequencyHz);
    if (eResult != DAL_SUCCESS)
    {
