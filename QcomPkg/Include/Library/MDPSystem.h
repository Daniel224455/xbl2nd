...
/* Maximum size of the cache table to save variables
 */
#define MDP_VAR_TABLE_MAX_SIZE        16
...
typedef enum
{
  MDP_VARIABLE_FLAG_NONE                = 0x00000000,
  MDP_VARIABLE_FLAG_NON_VOLATILE        = 0x00000001,
} MDPVariableFlagType;

/* MDP Variable Attrib Type
 */
typedef enum
{
  MDP_VARIABLE_ATTRIB_FIRMWARE_ENV      = 0,
  MDP_VARIABLE_ATTRIB_BOOT_SERVICE      = 1
} MDPVariableAttribType;

/* Display variable cache table
 */
typedef struct {
  CHAR16                 *pVariableName;          /* Variable name */
  void                   *pVariableValue;         /* Variable value */
  UINTN                   uDataSize;              /* Variable size */
  MDPVariableAttribType   eAttrib;                /* Variable attrib */
} DisplayVariableType;
