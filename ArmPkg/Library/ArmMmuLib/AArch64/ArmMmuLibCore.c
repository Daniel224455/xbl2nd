...
{
  INT64 FnAddr = (INT64)GetBlockEntryListFromAddress;
  
  if (!ArmMmuEnabled())
    return TRUE;

  // Text section
  if (ABS(FnAddr - (INT64)RegionStart) <= SIZE_2MB)
  {
    return FALSE; 
  }

  // Stack
  if (ABS((INT64)&FnAddr - (INT64)RegionStart) <= SIZE_2MB)
  {
    return FALSE;
  }

  // Page Tables
  if (ABS((INT64)BlockEntry - (INT64)RegionStart) <= SIZE_2MB)
  {
    return FALSE;
  }
