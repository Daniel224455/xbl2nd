...
const MDP_DisplayCtrlPathMapType gMDP_DisplayControlPathMap[MDP_DISPLAY_MAX][MDP_DUALPIPE_NUM_MIXERS] =
{
  ...
  {
    {HAL_MDP_SOURCE_PIPE_VIG_2, HAL_MDP_CONTROL_PATH_1,    HAL_MDP_LAYER_MIXER_2,    HAL_MDP_DESTINATION_PIPE_2,    HAL_MDP_PINGPONG_2},    // Main control Path in single pipe or Index 0 path in dual pipe
    {HAL_MDP_SOURCE_PIPE_VIG_3, HAL_MDP_CONTROL_PATH_1,    HAL_MDP_LAYER_MIXER_3,    HAL_MDP_DESTINATION_PIPE_3,    HAL_MDP_PINGPONG_3},    // Index 1 path in dual pipe
  },
  ...
  {
    {HAL_MDP_SOURCE_PIPE_DMA_0, HAL_MDP_CONTROL_PATH_2,    HAL_MDP_LAYER_MIXER_3,    HAL_MDP_DESTINATION_PIPE_3,    HAL_MDP_PINGPONG_3},    // Main control Path in single pipe or Index 0 path in dual pipe
    {HAL_MDP_SOURCE_PIPE_DMA_1, HAL_MDP_CONTROL_PATH_2,    HAL_MDP_LAYER_MIXER_4,    HAL_MDP_DESTINATION_PIPE_4,    HAL_MDP_PINGPONG_4},    // Index 1 path in dual pipe
  }, 
...
const MDP_DisplayInterfaceMapType gMDP_InterfaceModeMap[MDP_DISPLAY_CONNECT_MAX][MDP_DUALPIPE_NUM_MIXERS] =
{
  ...
  },
  {
    ...
    {HAL_MDP_INTERFACE_4,     HAL_MDP_INTERFACE_MODE_VIDEO,   HAL_MDP_INTERFACE_STANDARD_DP},    // MDP_DISPLAY_CONNECT_DP_0
  },
  {
  ...
  {
    {HAL_MDP_INTERFACE_0,     HAL_MDP_INTERFACE_MODE_VIDEO,   HAL_MDP_INTERFACE_STANDARD_DP},    // MDP_DISPLAY_CONNECT_DP_1
...
static MDP_Status UpdateDisplayControlpathInfo(MDP_Panel_AttrType *pPanelConfig)
{
        ...
        (NULL != psMDPHwPrivateInfo->pDeviceCaps->pPingPongCaps))
    {
        ...
        else
        {
            ...
            {
                if (pPanelConfig->sDSCDesc.bDSCEnable &&
                    pPanelConfig->sDSCDesc.bDSCLayerMixSplit)
                {
...
static void InterfaceFetchStartConfig(MDP_Panel_AttrType *pPanelConfig, HAL_MDP_Interface_ProgFetchStartConfigType *pProgFetchStartConfig)
{
  if ((NULL != pProgFetchStartConfig) && (NULL != pPanelConfig))
  {
    ...
    uint32   uVFrontPorch      = pPanelConfig->sActiveTiming.uVsyncFrontPorchLines;
    ...
    if (bEnablePFS)
    {
      ...
      if ((pPanelConfig->sDSCDesc.bDSCEnable) &&
         (pPanelConfig->sDSCDesc.uDSCCompressionRatio))
      {
        // In DSC mode, the interface is only a pushing a compressed frame
        uHorzActive  = MDP_CEIL(uHorzActive, pPanelConfig->sDSCDesc.uDSCCompressionRatio);
      }
...
static MDP_Status SetupInterface(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  DSCDescType                                *pDSCDesc = &pPanelConfig->sDSCDesc;
  ...
  for (uI = 0; uI < pPanelConfig->uNumInterfaces; uI++)
  {
    ...
    /* DSC related */
    sInterfaceTiming.bDSCEnable               = pDSCDesc->bDSCEnable;
    sInterfaceTiming.uPixelPerClock           = pDSCDesc->uPixelPerClock;
    sInterfaceTiming.uCompressionRatio        = pDSCDesc->uDSCCompressionRatio;
    sInterfaceTiming.bDTOEnable               = pDSCDesc->bDTOEnable;
      ...
      case MDP_DISPLAY_CONNECT_SECONDARY_DSI_CMD:
        {
          ...
          /* Check for DSI DSC */
          if ( TRUE == pPanelConfig->sDSCDesc.bDSCEnable)
          {
            if (pPanelConfig->uAttrs.sDsi.uDSCCompressionRatio)
            {
              ...
              sInterfaceTiming.uVisibleWidthInPixels    = MDP_CEIL(sInterfaceTiming.uVisibleWidthInPixels, pPanelConfig->uAttrs.sDsi.uDSCCompressionRatio);
            }
            else
            {
      ...
      if (MDP_STATUS_OK == eStatus)
      {
        ...
        switch (pPanelConfig->ePhysConnect)
        {
          ...
          case MDP_DISPLAY_CONNECT_DP_1:
          {
              ...
              sPingpongConfig.pDSCConfig->bEnableDSCEnc = FALSE;

              if (TRUE == pPanelConfig->sDSCDesc.bDSCEnable)
              {
                 if (pPanelConfig->sDSCDesc.uDSCProfileID < MDP_DSC_PROFILEID_MAX)
                 {
                  ...
                  (MDP_INTERFACE_SINGLE    == pPanelConfig->uNumInterfaces))
              {
                 ...
                 if (HAL_MDSS_STATUS_SUCCESS != HAL_MDP_PingPong_Setup(pPanelConfig->sDisplayControlPath[MDP_SLAVE_PATH_INDEX].ePingPongId, &sPingpongConfig, 0x00))
                 {
                     eStatus = MDP_STATUS_FAILED;     
                 }
              }
...
static MDP_Status SetupControlPath(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  for (uInterfaceId = 0; uInterfaceId < pPanelConfig->uNumMixers; uInterfaceId++)
  {
    ...
    if (pPanelConfig->sDSCDesc.bDSCEnable)
    {
      ...
      /* Configure DSC registers */
      HAL_MDP_DSC_Setup(sActiveModulesConfig.uDSCId, pPanelConfig->sDSCDesc.pDSCEncCfg, 0x00);
    }
...
*
****************************************************************************/
static MDP_Status DisableControlPath(MDP_Panel_AttrType *pPanelConfig)
{
  MDP_Status                                      eStatus = MDP_STATUS_OK;
  ...
  for (uInterfaceId = 0; uInterfaceId < pPanelConfig->uNumMixers; uInterfaceId++)
  {
    ...
    // 3D Mux ( Only for Dual Pipe Mode )
    if (MDP_DUALPIPE_NUM_MIXERS == pPanelConfig->uNumMixers)
    {
       // Setup 3D Mux
       b3DMuxEnable = MDP_GET_DUALPIPEMUX(pPanelConfig->ePhysConnect);
       ...
       if ((MDP_DISPLAY_CONNECT_PRIMARY_DSI_VIDEO   == pPanelConfig->ePhysConnect) ||
           ...
           (MDP_DISPLAY_CONNECT_SECONDARY_DSI_CMD   == pPanelConfig->ePhysConnect))
       {
          b3DMuxEnable = pPanelConfig->uAttrs.sDsi.uDSC3DMux;
       }
    ...
    // disable the control path
    MDP_OSAL_MEMZERO(&sControlPathConfig, sizeof(HAL_MDP_ControlPathConfigType));
    sControlPathConfig.psMixerConfig      = &sMixerConfig;
    sControlPathConfig.psModulesConfig    = &sActiveModulesConfig;

    uFlags |= HAL_MDP_FLAGS_CONTROL_PATH_RELEASE_RESOURCES;

    // tear down the control path
    if (HAL_MDSS_STATUS_SUCCESS != HAL_MDP_ControlPath_Setup(pPanelConfig->sDisplayControlPath[uInterfaceId].eControlPathId, 
                                                             &sControlPathConfig,
                                                             uFlags))
    {
...
MDP_Status  MDPStopPipe(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  else
  {
    ...
    HAL_MDP_Interface_TEEnableType         sTeEnable;
    uint32                                 uI;
    HAL_MDP_InterfaceInfoParamsType        sInterfaceStatus;
    HAL_MDP_Interface_EngineStatusType     sEngineStatus; 

   if (MDP_STATUS_OK != (eStatus = DisableControlPath(pPanelConfig)))
   {
       MDP_Log_Message(MDP_LOGLEVEL_WARN, "MDPLib: StopControlPath failed!\n");
   }
    ...
    for (uI = 0; uI < pPanelConfig->uNumInterfaces; uI++)
    {
      ...
      MDP_OSAL_MEMZERO(&sVsyncEnable, sizeof(HAL_MDP_Interface_VsyncEnableType));    
      MDP_OSAL_MEMZERO(&sTeEnable, sizeof(HAL_MDP_Interface_TEEnableType));
      MDP_OSAL_MEMZERO(&sInterfaceStatus, sizeof(HAL_MDP_InterfaceInfoParamsType));
      ...
      {
          MDP_Log_Message(MDP_LOGLEVEL_WARN, "MDPLib: HAL_MDP_Interface_Setup failed!\n");
      }
      ...
      sInterfaceStatus.pEngineStatus = &sEngineStatus;

      /*
       * wait for timing engine to stop completely 
       * 40 ms is enough to cover a frame time
       */
      uCnt = 4;
      while(uCnt--)
      {
         MDP_OSAL_DELAYMS(10);
         if (HAL_MDSS_STATUS_SUCCESS == HAL_MDP_Interface_GetProperty(pPanelConfig->sDisplayControlPath[uI].eInterfaceId, &sInterfaceStatus, 0))
         {
             if (!sInterfaceStatus.pEngineStatus->bIsEngineOn &&
                  sInterfaceStatus.pEngineStatus->bIsInterfaceDone)
             {
                break;
             }
         }
