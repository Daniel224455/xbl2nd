...
)
{
  ...
  switch(eCore)
  {
    ...
    case HAL_CLK_APCS_CPU_CLUSTER1_C2:
      /* Program the hardware. */
      HWIO_OUTFI(APSS_OSM_RAIL1_CLKDOM0_COREd_PERF_STATE_DESIRED, 2, OS_REQUEST, nIndex);
      break;

    case HAL_CLK_APCS_CPU_CLUSTER1_C3:
      /* Program the hardware. */
      HWIO_OUTFI(APSS_OSM_RAIL1_CLKDOM0_COREd_PERF_STATE_DESIRED, 3, OS_REQUEST, nIndex);
      break;
...
//HWIO_OUTF(NPU_CC_CAL_DP_CRC_SID_FSM_CTRL,      SID_FSM_CTRL, 0x00800000);
//HWIO_OUTF(NPU_CC_CAL_DP_CRC_MND_CFG,           MND_CFG,      0x00015011);
} /* HAL_clk_InitMiscGfxRegisters */
