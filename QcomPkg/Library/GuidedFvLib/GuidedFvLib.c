...
EFI_STATUS LoadAndAuthenticateFV(EFI_GUID *FvGuid, CHAR16* PartitionLabel)
{
  ...
  Status = AuthenticateElf(ReadBuffer, ImageSize, FvGuid, &FvBase);
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN, " __FIX__SDM855 ERROR: Authentication of FV failed \n"));
...
//    return Status;    
  ...
  if (!GuidedFvIsMounted(FvGuid))
  {
    DEBUG((EFI_D_WARN,  "%a : Unable to validate mounted FV \n",__func__));
    return EFI_SUCCESS;
  }
