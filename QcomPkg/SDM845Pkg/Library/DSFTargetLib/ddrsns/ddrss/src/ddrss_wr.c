...
$Header: //components/rel/boot.xf/2.2/QcomPkg/SDM845Pkg/Library/DSFTargetLib/ddrsns/ddrss/src/ddrss_wr.c#6 $
$DateTime: 2018/06/15 05:00:47 $
$Author: pwbldsvc $
================================================================================================================================================================
          ...
          for(cs = 0; cs < NUM_CS; cs++)
          {
              ...
              if((chip_select >> cs) & 0x1)
              {
                ...
                ddr_printf(DDR_NORMAL,"Frequency, %d, Channel, %d, Rank, %d,\n\n",clk_freq_khz, ch , cs);
                  ...
                  MC_MR_Write (ddr, CH_1HOT(ch), CS_1HOT(cs), JEDEC_MR_13, MR13_value);
...
// ============================================================================================================================
    ...
    //{
		...
		if(sed_enable)
		{
			   ...
			   , v_pr[mid_v][0/*phy_x*/][EYEW] * FINE_STEP_IN_PS, v_pr[mid_v][0/*phy_x*/][EYEW]
			   , clk_freq_khz, ch, cs );
			   
			   ddr_printf(DDR_NORMAL, "\nSmall Eye SCREEN 1: WR MINIMUM EYE WIDTH = %d (ps)= %d (f steps), Frequency = %d, Channel = %d, Rank = %d, Byte = 1\n"
			   , v_pr[mid_v][1/*phy_x*/][EYEW] * FINE_STEP_IN_PS, v_pr[mid_v][1/*phy_x*/][EYEW]
			   , clk_freq_khz, ch, cs );
			   
			   min_eye_width_mask_top_Vref =((mid_v + (training_params_ptr->wr_dqdqs.min_eye_width_mask_top)/v_step) < 78/v_step ) ?  (mid_v + (training_params_ptr->wr_dqdqs.min_eye_width_mask_top)/v_step) : 78/v_step;
			...
			if( ((vmax[1] < mid_v) || (((vmax[1] - mid_v)*v_step) <   training_params_ptr->wr_dqdqs.min_eye_width_mask_top/*MIN_EYE_WIDTH*/))&& (training_params_ptr->wr_dqdqs.min_eye_width_mask_top_enable==1) )
			  {
				min_eye_width_mask_top_Vref =((mid_v + (training_params_ptr->wr_dqdqs.min_eye_width_mask_top)/v_step) < 78/v_step ) ?  (mid_v + (training_params_ptr->wr_dqdqs.min_eye_width_mask_top)/v_step) : 78/v_step;
			...
			#if DSF_SMALL_EYE_DETECTION_LOG                
			  min_eye_width_mask_bottom_Vref = ((mid_v - (training_params_ptr->wr_dqdqs.min_eye_width_mask_bottom)/v_step) < 0 ) ?  0 : mid_v - (training_params_ptr->wr_dqdqs.min_eye_width_mask_bottom)/v_step;
			...
			if( ((mid_v < vmin[1]) || ((( mid_v-vmin[1] )*v_step) <  training_params_ptr->wr_dqdqs.min_eye_width_mask_bottom)) && (training_params_ptr->wr_dqdqs.min_eye_width_mask_bottom_enable==1))
			  {
				min_eye_width_mask_bottom_Vref= ((mid_v - (training_params_ptr->wr_dqdqs.min_eye_width_mask_bottom)/v_step) < 0 ) ?  0 : mid_v - (training_params_ptr->wr_dqdqs.min_eye_width_mask_bottom)/v_step;
    ...
    if(sed_enable)
	{
		...
		mid_v_p60 = mid_v + (60/(v_step*3.6));
        mid_v_n60 = (mid_v < (60/(v_step*3.6))) ? 0 : mid_v - (60/(v_step*3.6));
