...
)
{
  ...
  for (nIndex = 0; nIndex < pBSP->nNumSources; nIndex++)
  {
    ...
    CLOCK_FLAG_CLEAR(pBSP->aSources[nIndex], SOURCE_CALIBRATED);
    CLOCK_FLAG_CLEAR(pBSP->aSources[nIndex], INITIALIZED);
  }
...
)
{
  ...
  if ((pClockDomain->nReferenceCount + pClockDomain->nReferenceCountSuppressible) == 1 &&
      ...
      !CLOCK_FLAG_IS_SET(pClockDomain, DFS_CONTROLLED))
  {
    Clock_ConfigClockMux(pClockDomain,
...
)
{
  ...
  if ((pClockDomain->nReferenceCount + pClockDomain->nReferenceCountSuppressible) == 1)
  {
    ...
    if (pClockDomain->pActiveMuxConfig &&
        !CLOCK_FLAG_IS_SET(pClockDomain, DFS_CONTROLLED))
    {
      Clock_ConfigClockMux(pClockDomain, &pDrvCtxt->pBSP->SafeMuxConfig);
    }
  }
...
/* =========================================================================
**  Function : Clock_ConfigClockMux
** =========================================================================*/
/*
  See ClockDriver.h.
*/

DALResult Clock_ConfigClockMux
(
  ClockDomainNodeType               *pDomain,
...
)
{
  ...
  if (CLOCK_FLAG_IS_SET(pDomain, DONT_FORCE_DYNAMIC_SWITCH))
  {
    HAL_clk_ConfigClockMux(&pDomain->HALDomain, pConfig);
  }
  else
  {
    HAL_clk_ConfigClockMux(&pDomain->HALDomain, pConfig);
//  HAL_clk_ConfigClockMuxDynamic(&pDomain->HALDomain, pConfig);  // Revert usage of this function to fix CR 2210201
  }

  return DAL_SUCCESS;

} /* END Clock_ConfigClockMux */
...
)
{
  ...
  if (bSlewing)
  {
     ...
     *    If the source (i.e., nMuxSel) is specified as -1 (all 'F's), then
     *    the client is requesting a voltage update only and no MUX update.
     *    This special processing is sometimes required when an external
     *    source is configured outside the clock driver, yet the client
     *    requests a voltage update based on a hinted frequency configuration.
     *    See function Clock_SelectExternalSource().
     */
    if (!CLOCK_FLAG_IS_SET(pDomain, DFS_CONTROLLED) &&
        pNewConfig->HALConfig.nMuxSel != 0xFF &&
        pNewConfig->HALConfig.nDiv2x > pActiveConfig->HALConfig.nDiv2x &&
        !bStub)
    {
      Clock_ConfigClockMux(pDomain, &pNewConfig->HALConfig);
    }
  }
...
)
{
  ...
  if (pNewConfig != pDomain->pActiveMuxConfig)
  {
    ...
    if (eResult == DAL_SUCCESS)
    {
      ...
      if (pDrvCtxt->bQdssEnabled)
      {
        PEP_CLK_TRACE_CLOCK_FREQ(pClock->HALClock.nDebugMuxSel, nResultFreq * nMult);
      }
      #else
...
/*
  ...
  the client is requesting a voltage update only and no MUX reconfiguration.
  This special processing is sometimes required when an external source is
  configured outside the clock driver, yet the client requests a voltage
  update based on a hinted frequency configuration.
*/
...
)
{
  ...
  /*
   * Don't force dynamic switches if external sources are used, since we
   * aren't in control of them and thus can't guarantee they'll be on
   * while the mux is switched.
   */
  CLOCK_FLAG_SET(pDomain, DONT_FORCE_DYNAMIC_SWITCH);
  ...
  if (CLOCK_FLAG_IS_SET(pClock, LOG_STATE_CHANGE) ||
      CLOCK_GLOBAL_FLAG_IS_SET(LOG_CLOCK_STATE_CHANGE))
  {
    ULOG_RT_PRINTF_8 (pDrvCtxt->hClockLog,
                      "%s (ExternalSource: %d, %d, %d, %d, %d) @ %lu Hz, @ %d voltage",
                      pClock->szName,
                      nSource, nDiv2x, nM, nN, n2D,
                      nFreqHz,
                      eVRegLevel);
  }
...
)
{
      ...
      !CLOCK_GLOBAL_FLAG_IS_SET(DO_NOT_DISABLE_POWER_DOMAINS))
  {
    ...
    {
      /*
       * Make sure the power domain is not still enabled in HW control mode, or is
       * marked to ignore the disabled status.
       */
      if (pPowerDomain->nReferenceCountHW == 0 && !CLOCK_FLAG_IS_SET(pPowerDomain, IGNORE_OFF_STATUS))
      {
        /*
...
)
{
      ...
      !CLOCK_GLOBAL_FLAG_IS_SET(DO_NOT_DISABLE_POWER_DOMAINS))
  {
    ...
    {
      /*
       * Make sure the power domain is not still enabled in SW control mode, or is
       * marked to ignore the disabled status.
       */
      if (pPowerDomain->nReferenceCount == 0 && !CLOCK_FLAG_IS_SET(pPowerDomain, IGNORE_OFF_STATUS))
      {
        /*
         * Wait for power domain to turn off before removing the parent vote,
         * since the rail is still required during the collapse sequence.
