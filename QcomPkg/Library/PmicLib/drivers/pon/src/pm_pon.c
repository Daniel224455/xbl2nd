...
#define SEC_MOR_SDAM_FLAG_ADDR 0xB177
#define PMIO_PON_GEN2_ON_REASON3_ADDR  0xB27A
/*===========================================================================
...
pm_pon_init(void)
{
  ...
  uint8   periph_type = 0;
  uint8 flag = 0;
  pm_register_address_type on_reason_addr;
  ...
  /** for WP target sec mor feature leads to additional hard reset overwriting the PON reason history
   ** check SDAM flag to get the correct reason 
   ** if sec mor flag is set to 0xA5 read on reason from previous history(0xB27A) **/
  err_flag |= pm_comm_read_byte(PMIC_A_SLAVEID_PRIM, SEC_MOR_SDAM_FLAG_ADDR, &flag, 0);
  on_reason_addr = (flag == 0xA5)? PMIO_PON_GEN2_ON_REASON3_ADDR : PMIO_PON_GEN2_ON_REASON_ADDR;
  
  /* Read ON reason and write the DVDD_RB_SPARE register bit <0> to 1 if Warm Reset occured */
  err_flag |= pm_comm_read_byte(PMIC_A_SLAVEID_PRIM, on_reason_addr, warm_reset_reason, 0);
...
pm_err_flag_type pm_pon_smpl_enable(uint32 pmic_device_index, boolean enable)
{
  ...
  {
    err_flag = pm_comm_write_byte_mask(slave_id, PMIO_PON_GEN2_SMPL_CTL_ADDR, PMIO_PON_GEN2_SMPL_EN_BMSK, data, 0);
  }
