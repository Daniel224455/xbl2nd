...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 05/30/19	vk		 Remove DummyKeyCheckflag check 
...
)
{
  ...
  *InformationBlock = AllocatePool(sizeof(ADAPTER_INFO_PLATFORM_SECURITY));
  ...
  // Check 5: IsProductionDevice test and DummyKeyCheck Test
  ...
  // Check 6: DummyKey Check 

	dummykeycheckflag = DummyKeyCheck(); // True implies Secure device
   if(proddevicecheckflag && dummykeycheckflag)
   {
	//AsciiPrint("dummykeycheckflag TRUE\r\n");
	DEBUG((EFI_D_WARN, "On Production Device dummykey checkflag TRUE\r\n"));
   }    
	else
   {
...
BOOLEAN DummyKeyCheck(VOID)
{
  ...
  UINT8 Result[AES256_KEY_SIZE] = {0x1E, 0x34,  0xE, 0xB4, 0x92, 0x66, 0xB3, 0xCD, 0xEC, 0x32, 0x72,  0x0, 0xB4, 0x2B, 0x89, 0x9C, 0x2A, 0x96, 0xE8, 0xDC, 0xAF, 0x2D, 0x35, 0x82, 0xB2, 0x35, 0xBD, 0x61, 0x79, 0x40, 0x68, 0xAE};
  ...
  {
	//DEBUG((EFI_D_WARN, "Response and output buffers are matched... \r\n"));
    retVal = FALSE;
  }
ErrorExit:
