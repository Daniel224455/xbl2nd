...
{
  qusb_log_err(QUSB_DCI_SELECT_UTMI_CLK_LOG);
...
boolean qusb_dci_dp_gnd_check(void)
{
  ...
  if (is_dp_low == 0)
  {
    qusb_log_err__param(DCI_SKIP_FEDL_LOG, is_dp_low);
    return FALSE;
  }

  //D+ is connected to the GND
  qusb_log_err__param(DCI_ENTER_FEDL_LOG, is_dp_low);
  return TRUE;
}
...
void qusb_dci_eud_phy_tune(void)
{
  ...
  for (; index < QUSB_DCI_EUD_PHY_REG_ARRAY_CNT; index++)
  {
    ...
    //If North_BG_BYPASS == 0, old device (no Patch). change PLL_BAIS to old value 0x20
    if (DIV4(HWIO_PERIPH_SS_QUSB2PHY_QUSB2PHY_PLL_BIAS_CONTROL_2_OFFS) == array->phy_addr)
    {
      if(0 == HWIO_PERIPH_SS_PHY_REFGEN_NORTH_BG_CTRL_INM(HWIO_PERIPH_SS_PHY_REFGEN_NORTH_BG_CTRL_BANDGAP_BYPASS_BMSK))
      {
        out_byte(&(array->phy_val), 0x20); 
      }
    }
