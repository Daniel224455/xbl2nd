...
/*=============================================================================
 ...
 --------     ---     -----------------------------------------------------------
 03/22/17     pbitra  For CLS platform, turning display panel on only before booting 
                      to HLOS, not turning display panel before shutdown
 10/13/17     va      Display splash logo if dead battery charging is perfomed
11/09/17      pbitra  Updated Power key press behavior
 10/16/17     pbitra  Added support for CLS platform
...
)
{
  ...
  {
  DEBUG((EFI_D_WARN, "QcomChargerApp:: %a CLS platform detected. Locating SmartChargerProtocol \r\n", __FUNCTION__));
    
  Status = gBS->LocateProtocol( &gSmartChargerProtocolGuid, NULL, (VOID **)&pQcomChargerProtocol );
  }
  else
  {
  ...
  if(EFI_QCOM_CHARGER_ACTION_DEBUG_BOARD_GOOD_TO_BOOT != ChargingAction)
  {
    ...
    switch(ChargingAction)
    {
      ...
      case EFI_QCOM_CHARGER_ACTION_DEBUG_BOARD_GOOD_TO_BOOT:
           DEBUG((EFI_D_WARN, "QcomChargerApp:: QcomChargerApp_Start: Battery level is good to boot to HLOS. \r\n"));
           QcomChargerAppEvent_HandleLPMDisplay(FALSE); /* Set Display Power State ON */
...
EFI_STATUS QcomChargerApp_Initialize( EFI_QCOM_CHARGER_ACTION_TYPE ChargingAction )
{
  ...
  {
    DEBUG((EFI_D_WARN, "QcomChargerApp:: %a Entering LPM \r\n", __FUNCTION__));  
