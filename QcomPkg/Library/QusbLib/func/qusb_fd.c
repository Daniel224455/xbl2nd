...
static void qusb_fd_update_pid_and_serial_number_string(uint16 product_id, uint8 serial_number)
{
  ...
  else
  {
    ...
    qusb_log_err(HLOS_PASSED_NULL_BUFFER_PORT_HOP_LOG);
    return; 
  }
...
{
  qusb_log(HDLC_TX_CALLBACK_LOG);
  if(NULL == urb)
  {
    qusb_log_err(FD_NULL_POINTER_ERROR_LOG);
    return; 
  }
...
static void qusb_rx_callback(struct qusb_urb* urb)
{
  if(NULL != urb)
  {
    qusb_log__param(HDLC_RX_CALLBACK_ACTUAL_LENGTH_LOG, urb->actual_length);
    qusb_log__param(HDLC_RX_CALLBACK_TRANSFER_LENGTH_LOG, urb->transfer_length);
    user_rx_callback(urb);
  }
  else
  {
    qusb_log_err__param(FD_NULL_POINTER_ERROR_LOG, __LINE__);
    return;
  }
}
...
static void qusb_notify_cb(struct qusb_dcd_dsc_device* dev, qusb_notify_events_t event)
{
  ...
  qusb_log(HDLC_NOTIFY_CB_LOG);
  switch (event) 
  {
    case QUSB_DCD_EVT_CONFIG:
      qusb_log_bus(NOTIFY_EVT_CFG_LOG);
      qusb_fd_init_urb();
    ...
    case QUSB_DCD_EVT_UNCONFIG:
      qusb_log_bus(NOTIFY_EVT_UNCFG_LOG);
      if (user_enum_complete != NULL)
      {
    ...
    {
      qusb_log_bus__param(NOTIFY_EVT_SPEED_LOG, dev->speed);
    ...
    {
      qusb_log_bus(NOTIFY_EVT_DISCONNECT_LOG);
    ...
    {
      // Sequence to be followed on Suspend
      qusb_log_bus(NOTIFY_EVT_SUSPEND_LOG);
    }
    break;
...
{
  qusb_log_bus__param(FD_HDLC_SET_CORE_LOG, core_id);
  //for synopsys controller, make sure that the correct core_id is set
  if (core_id != DCI_SNPS_CORE)
  {
    qusb_log_err__param(FD_CORE_ID_WRONG_LOG, __LINE__); 
  }
  else
  {
...
{
   qusb_log_bus__param(FD_HDLC_SHUTDOWN_LOG, sw_device.shutdown_mode);
...
void qusb_fd_tx_byte(unsigned char chr)
{
  ...
  qusb_log__param(FD_TX_BYTE_LOG, chr); 
...
void qusb_fd_tx_pkt(unsigned char *pkt, uint32 len)
{
  ...
  qusb_log__param(FD_HDLC_TX_PKT_LOG, len);
  while ( QUSB_URB_STATUS_ACTIVE == urb->transfer_status ) 
  {
    qusb_dcd_poll(&sw_device);
  ...
  qusb_log__param(TRANSFER_LENGTH_LOG, urb->transfer_length);
  // send urb 
  qusb_submit_urb(urb);
...
int qusb_fd_is_tx_active(void)
{
  ...
  {
    qusb_log(FD_HDLC_IS_TX_ACTIVE_LOG);
    return 1;
  }
  qusb_log(FD_HDLC_IS_TX_NOT_ACTIVE_LOG);
  return 0;
}
...
uint32 qusb_fd_rx_pkt(void *buff_ptr, uint32 len)
{
  ...
  qusb_log__param(FD_HDLC_RX_PKT_LOG, len);
  ...
  urb->transfer_length = len;
  qusb_log__param(SUBMIT_RX_URB_LOG, len); 
  // send urb 
  qusb_submit_urb(urb);
...
void qusb_fd_rx_completed(struct qusb_urb* urb)
{
  if(NULL != urb)
  {
    qusb_log__param(FD_HDLC_RX_COMPLETED_LOG, urb->actual_length);
    urb->actual_length = 0;
  }
  else
  {
...
{
  qusb_log__param(FD_SET_BUFFER_MODE_LOG, buffer_mode); 
...
static void qusb_rx_callback_int_buf_mode(struct qusb_urb* urb)
{
  qusb_log(RX_CALLBACK_INT_BUF_MODE_LOG);
  if(NULL != urb)
  {
...
void qusb_rx_completed_int_buf_mode(struct qusb_urb* urb)
{
  if(NULL != urb)
  {
    qusb_log__param(RX_COMPLETED_INT_BUF_MODE_LOG, urb->actual_length);
