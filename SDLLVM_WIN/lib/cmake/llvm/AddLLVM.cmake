...
macro(add_llvm_library name)
  ...
  # QUIC code
  elseif( LIBCLANG_BUILD_STATIC AND LLVM_INSTALL_TOOLCHAIN_ONLY AND NOT LLVM_TOOLCHAIN_TOOLS MATCHES "(.*)${name}(.*)" )
    message(STATUS "Not installing library ${name}: ${name} not in ${LLVM_TOOLCHAIN_TOOLS}.")
    set_property(GLOBAL APPEND PROPERTY LLVM_EXPORTS ${name})
 # End of QUIC code
  elseif(NOT _is_gtest)
    if (NOT LLVM_INSTALL_TOOLCHAIN_ONLY OR ${name} STREQUAL "LTO" OR ${name} STREQUAL "LLVMAE")
...
macro(add_llvm_loadable_module name)
  ...
  if(NOT TARGET ${name})
    ...
    if( EXCLUDE_FROM_ALL )
      set_target_properties( ${name} PROPERTIES EXCLUDE_FROM_ALL ON)
    # QUIC code
    elseif( LLVM_INSTALL_TOOLCHAIN_ONLY AND NOT LLVM_TOOLCHAIN_TOOLS MATCHES "(.*)${name}(.*)" )
      message(STATUS "Not installing loadable_module ${name}: ${name} not in ${LLVM_TOOLCHAIN_TOOLS}.")
    # End of QUIC code
    else()
...
macro(add_llvm_tool name)
  ...
  if (LLVM_IS_${name}_TOOLCHAIN_TOOL GREATER -1 OR NOT LLVM_INSTALL_TOOLCHAIN_ONLY)
    if( LLVM_BUILD_TOOLS )
      #QUIC code
      list(FIND LLVM_PROGRAM_PREFIX_EXCLUSION_LIST ${name} prefix_component)
      if (LLVM_PROGRAM_PREFIX AND prefix_component EQUAL -1)
        install(PROGRAMS ${CMAKE_BINARY_DIR}/${LLVM_CFG_INTDIR}/${LLVM_TOOLS_INSTALL_DIR}/${name}${CMAKE_EXECUTABLE_SUFFIX}
          DESTINATION ${LLVM_TOOLS_INSTALL_DIR}
          COMPONENT ${name}
          RENAME ${LLVM_PROGRAM_PREFIX}${name}${CMAKE_EXECUTABLE_SUFFIX})
      # End of QUIC code
      else()
        install(TARGETS ${name}
          EXPORT LLVMExports
  ...
  # QUIC code
    ...
    message(STATUS "Not installing tool ${name}: ${name} not in ${LLVM_TOOLCHAIN_TOOLS}.")
  # End of QUIC code
  endif()
  if( LLVM_BUILD_TOOLS )
    set_property(GLOBAL APPEND PROPERTY LLVM_EXPORTS ${name})
...
function(set_exported_symbol_file target in_exports_file out_exports_file)

  if(MSVC)
    set(out_exports_file_style "windows-def")
  else(MSVC)
    set(out_exports_file_style "version-script")
  endif(MSVC)

  add_custom_command(OUTPUT ${out_exports_file}.tmp
    # Generate tablegen output in a temporary file.
    COMMAND ${PYTHON_EXECUTABLE}
    ${LLVM_MAIN_SRC_DIR}/utils/gen-native-exports.py
    ${in_exports_file}
    ${out_exports_file}.tmp
    ${out_exports_file_style}

    # The file in LLVM_TARGET_DEFINITIONS may be not in the current
    # directory and local_tds may not contain it, so we must
    # explicitly list it here:
    DEPENDS ${LLVM_MAIN_SRC_DIR}/utils/gen-native-exports.py ${in_exports_file}
    COMMENT "Building ${out_exports_file}..."
    )
  add_custom_command(OUTPUT ${out_exports_file}
    ...
    ${out_exports_file}.tmp
    ${out_exports_file}
    DEPENDS ${out_exports_file}.tmp
    COMMENT ""
    )

  set_source_files_properties(${out_exports_file}
    PROPERTIES GENERATED 1)

  add_custom_target(generate_exports
    DEPENDS ${out_exports_file})
  add_dependencies(${target} generate_exports)
  ...
  if(NOT MSVC)
    set_target_properties(${target} PROPERTIES
      LINK_FLAGS "-Wl,--version-script,${out_exports_file}")
  endif(NOT MSVC)
...
function(llvm_install_symlink name dest)
  ...
  # QUIC code
  # Override the symbolic links with prefix name
  list(FIND LLVM_PROGRAM_PREFIX_EXCLUSION_LIST ${name} prefix_component)
  list(FIND LLVM_PROGRAM_PREFIX_EXCLUSION_LIST ${dest} prefix_symlink)
  if (LLVM_PROGRAM_PREFIX)
    # If the tool in not present in exclusion list
    if (prefix_component EQUAL -1)
      set(full_name ${LLVM_PROGRAM_PREFIX}${name}${CMAKE_EXECUTABLE_SUFFIX})
    endif()
    # If the symlink is not presentin exclusion list
    if (prefix_symlink EQUAL -1)
      set(full_dest ${LLVM_PROGRAM_PREFIX}${dest}${CMAKE_EXECUTABLE_SUFFIX})
    endif()

    # Overwrite full_name if it symlink is invoked second time
    list(FIND LLVM_TOOLCHAIN_TOOLS ${name} install_tools_symlink)
    if (install_tools_symlink AND ${name} MATCHES ${LLVM_PROGRAM_PREFIX})
      set(full_name ${name}${CMAKE_EXECUTABLE_SUFFIX})
    endif()
  endif()
  # End of QUIC code
