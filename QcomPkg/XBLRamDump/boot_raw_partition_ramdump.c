...
Copyright 2013-2015, 2017-2019 by Qualcomm Technologies, Inc.  All Rights Reserved.
============================================================================*/
...
01/03/19   vk          Close partition before reset
...
void boot_ram_dump_to_raw_parition()
{
  ...
  /* Only perform ram dump if uefi dump cookie is set AND 
     memory debug is allowed. use shared variable rather than rely on shared imem value*/  
  if((boot_get_shared_uefi_ram_dump_magic() == BOOT_RAW_RAM_DUMP_MAGIC_NUM) &&
    ...
    if(trans_if != NULL)
    {
      ...
      {        
        ...
        */
        ...
        /* close parition before reset */
        boot_flash_dev_close_image(&trans_if);

        DL_LOG("RawDump complete, reset device");
