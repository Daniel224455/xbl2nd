...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 30/01/17   ska     Test code enhance
...
extern i2c_status i2c_bus_scan (uint32 i2c_core_number);
...
i2c_status i2c_test_eeprom (VOID)
{
  ...
  DEBUG((EFI_D_ERROR, "i2c_read : backing up eeprom data"));
  ...
  DEBUG((EFI_D_ERROR, "i2c_read :backup successful\n"));
 
  DEBUG((EFI_D_ERROR, "i2c_write :Writing Data to Eeprom"));
  ...
  DEBUG((EFI_D_ERROR, "i2c_write :Data write successful\n"));

  gBS->Stall (PcdGet32(EEPROMWriteCycleTime));

  DEBUG((EFI_D_ERROR, "i2c_read :Reading back to check for written data"));
  ...
  DEBUG((EFI_D_ERROR, "i2c_write : restoring backed up data to eeprom"));
  ...
  i2c_test_hex_dump (bbuf, DATA_SIZE);
  DEBUG((EFI_D_ERROR, "i2c_write : Data restore successful\n"));
  ...
  if(i == DATA_SIZE)
  {
    DEBUG((EFI_D_ERROR, "*******I2C DATA VERIFICATION PASSED**********\n"));
  }
...
error:
  ...
  i2c_bus_scan (PcdGet32(EEPROMI2CCore));
