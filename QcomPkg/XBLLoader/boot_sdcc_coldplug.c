...
Copyright 2010-2015,2017-2019 by Qualcomm Technologies, Inc.  All Rights Reserved.
=============================================================================*/
...
02/06/19   vk      Add check if ram dump init completed for NVMe RAM dump
...
)
{
  ...
  {
    boot_shared_functions_type * function_table = (boot_shared_functions_type *)SCL_SBL1_SHARED_FUNCTIONS_TABLE_BASE;
    ...
    /* For RAM dump to NVMe 
     * If SPI_NOR raw dumps are taken to NVMe. use shared variable rather than rely on shared imem value*/
    if ((sbl_partitions[coldplug_id].chdl == NULL) && 
        (fastboot_option == SPI_FLASH) &&
        (boot_get_shared_uefi_ram_dump_magic() == BOOT_RAW_RAM_DUMP_MAGIC_NUM) &&
        (function_table->ramdump_image_init_done == TRUE)
        )
    {
      status=boot_coldplug_open_device(COLDPLUG_DEVICE_NVME, 1);
      if (status != COLDPLUG_RESULT_SUCCESS)
      {
        boot_log_message("NVME open failed ");
          return INVALID_FILE_HANDLE;
      }

      sbl_partitions[coldplug_id].chdl =
        boot_coldplug_open_gpt_partition (COLDPLUG_DEVICE_NVME,
                                          dev_loc,
                                          (struct coldplug_guid *)partition_id_ptr);
