...
--------   ---     ------------------------------------------------------------
04/18/19   vb      Fix for CR2425146 - nvme on-storage dump not getting collected in case of a mini dump
02/07/19   vk      Update field name
...
boolean boot_shared_functions_init(void)
{
  ...
  shared_functions_table->nvme_read = &nvme_read;
  shared_functions_table->nvme_write = &nvme_write;
  shared_functions_table->nvme_flush = &nvme_flush;
  shared_functions_table->nvme_open = &nvme_open;
  shared_functions_table->nvme_close = &nvme_close;
  shared_functions_table->nvme_get_namespace_sizes = &nvme_get_namespace_sizes;
  shared_functions_table->nvme_get_device_info = &nvme_get_device_info;
 
  shared_functions_table->ramdump_image_init_done = TRUE;
...
{
  return shared_functions_table->Clock_PCIeInit(eClock, ePerfLevel);
}
...
pm_err_flag_type pm_pwm_enable_pwm(uint32 pmic_chip, pm_pwm_resource_type pwm_rsrc, boolean enable)
{
   return shared_functions_table->pm_pwm_enable_pwm( pmic_chip, pwm_rsrc, enable);
}

pm_err_flag_type pm_pwm_set_pwm_value( uint32 pmic_chip, pm_pwm_resource_type pwm_rsrc, uint16 pwm_value)
{
   return shared_functions_table->pm_pwm_set_pwm_value( pmic_chip, pwm_rsrc, pwm_value);
}

pm_err_flag_type pm_pwm_clock_config( uint32 pmic_chip, pm_pwm_resource_type pwm_rsrc, pm_pwm_bit_size_type bit_size, pm_pwm_clock_freq_type clk_freq, uint32 pre_div, uint32 exp)
{
   return shared_functions_table->pm_pwm_clock_config( pmic_chip, pwm_rsrc, bit_size, clk_freq, pre_div, exp);
}

pm_err_flag_type pm_pwm_glitch_removal(uint32 pmic_chip, pm_pwm_resource_type pwm_rsrc, boolean glitch_removal)
{
   return shared_functions_table->pm_pwm_glitch_removal(pmic_chip, pwm_rsrc, glitch_removal);
}

pm_err_flag_type pm_pwm_config_dtest(uint32 pmic_chip, pm_pwm_resource_type pwm_rsrc, pm_pwm_dtest dtest, uint32 set_value)
{
   return shared_functions_table->pm_pwm_config_dtest(pmic_chip, pwm_rsrc, dtest, set_value);
}
...
pm_err_flag_type pm_gpio_config_digital_output(uint8 pmic_chip, pm_gpio_perph_index gpio, pm_gpio_out_buffer_config_type  out_buffer_config, pm_gpio_volt_src_type  voltage_source, pm_gpio_src_config_type  source, pm_gpio_out_buffer_drv_strength_type  out_buffer_strength, boolean  out_inversion)
{
  return shared_functions_table->pm_gpio_config_digital_output(pmic_chip, gpio, out_buffer_config, voltage_source, source, out_buffer_strength, out_inversion);
}
...
void boot_set_shared_uefi_ram_dump_magic()
{
  return shared_functions_table->boot_set_shared_uefi_ram_dump_magic();
}

uint32 boot_get_shared_uefi_ram_dump_magic()
{
  return shared_functions_table->boot_get_shared_uefi_ram_dump_magic();
}
