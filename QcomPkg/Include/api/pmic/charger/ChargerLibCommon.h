...
/*===========================================================================
 ...
 --------   ---     -----------------------------------------------------------
 11/09/17   pbitra  Added Charger presence and battery presence APIs
 10/16/17   pbitra  Added support for CLS platform SmartCharger
 10/11/17   dc      Rerun APSD and reconfigure USB ICL in every periodical check
 09/22/17   va      Read logfs partition size as per alloted from target specific file, enable BMD 
...
#define DEFAULT_LOGFILE_SIZE   8630272 // 7MB
...
#define _ULOG(Expression)   ULogPrint Expression
...
#if !defined(SMARTDEBUG_TIME)
  #define SMARTDEBUG_TIME(Expression)          \
    do {                           \
         DEBUG ((EFI_D_WARN, "%lld, ", SmartChargerLib_CalculateTime())); \
        _DEBUG (Expression);       \
      }while (FALSE)
#endif


#if !defined(SMARTCHARGER_DEBUG)
  #define SMARTCHARGER_DEBUG(Expression)        \
    do {                           \
      if (SmartChargerLib_PrintDebugMsg ()) {  \
         DEBUG ((EFI_D_WARN, "%lld, ", SmartChargerLib_CalculateTime())); \
        _DEBUG (Expression);       \
      }                            \
      if(SmartChargerLib_PrintDebugMsgToFile()) {  \
        _ULOG (Expression);         \
      }                            \
    } while (FALSE)
  #define SMARTCHARGER_FILE_DEBUG(Expression)        \
    do {                           \
      if(SmartChargerLib_PrintDebugMsgToFile()) {  \
        _ULOG (Expression);         \
      }                            \
  ...
  /*Use below for File debug a log with Always on UART messeages */
  #define SMARTCHARGER_FILE_UART_DEBUG(Expression)      \
    do {               \
      SMARTDEBUG_TIME(Expression);    \
      if(SmartChargerLib_PrintDebugMsgToFile()) {  \
      _ULOG (Expression);     \
      }                \
    } while (FALSE)
  #define SMARTCHARGER_UART_DEBUG(Expression)        \
    do {                           \
      if (SmartChargerLib_PrintDebugMsg ()) {  \
         DEBUG ((EFI_D_WARN, "%lld, ", SmartChargerLib_CalculateTime())); \
        _DEBUG (Expression);       \
...
EFI_STATUS SmartChargerLibCommon_Init( chargerlib_cfgdata_type *pChargerLibConfig );
...
EFI_STATUS SmartChargerLib_GetBatteryPresence(BOOLEAN *BatteryPresence);

BOOLEAN SmartChargerLib_PrintDebugMsg(void);

BOOLEAN SmartChargerLib_PrintDebugMsgToFile(void);
...
/*Enables WDOG if charging is already enabled*/
EFI_STATUS SmartChargerLib_EnWdogIfCharging(void);
...
EFI_STATUS SmartChargerLib_GetErrorAction( CHARGERLIB_CHARGING_ERROR_TYPES  ChargingError, CHARGERLIB_ERROR_ACTION_TYPE *pErrorAction);

EFI_STATUS SmartChargerLib_GetMaxUsbCurrent( UINT32 *pMaxCurrent);

EFI_STATUS SmartChargerLib_GetBatteryStatus(chargerlib_batt_status_info *BatteryStatus);
...
EFI_STATUS SmartChargerLib_GetChargingPath(CHARGERLIB_ATTACHED_CHGR_TYPE *ChargingPath);
...
EFI_STATUS SmartChargerLib_GetChargerPresence(BOOLEAN *bChargerPresent);

EFI_STATUS SmartChargerLib_GetBatteryPresence(BOOLEAN *bBatteryPresent);
