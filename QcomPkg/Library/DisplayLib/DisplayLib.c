...
static EFI_STATUS DisplayDxeSetMode(MDP_Display_IDType eDisplayId, MDP_DetectParamType  *pDisplayModes, uint32 uFlags)
{
   ...
   {
      MDP_Log_Message(MDP_LOGLEVEL_ERROR, "DisplayLib: Failed to set property while forcing mode\n");
   }
  ...
  uint32                        uFlags)
{
  ...
  else
  {
    ...
    sInitParam.aSupportedDisplays[MDP_DISPLAY_PRIMARY] = TRUE;
    ...
    // Initialize the MDP 
    else if (MDP_STATUS_OK != MDPInit(&sInitParam, 0x0))
    {
    ...
    else
    {
      ...
      if (TRUE == sInitParam.aSupportedDisplays[MDP_DISPLAY_PRIMARY])
      {
        ...
        {
          MDP_Log_Message(MDP_LOGLEVEL_INFO, "Displaylib: Common display panel power up failed!\n");
          eStatus = MDP_STATUS_HW_ERROR;
        }
        ...
        else if (MDP_STATUS_OK != MDPPower(MDP_DISPLAY_PRIMARY, &sPowerParams, POWERCONFIG_FLAGS_PANEL_POWER))
        {
        ...
        else if (TRUE == sDetectParams.bDisplayDetected)
        {
           ...
           if (MDP_STATUS_OK != DisplayDxeSetMode(MDP_DISPLAY_PRIMARY, &sDetectParams, 0x0))
           {
               MDP_Log_Message(MDP_LOGLEVEL_ERROR, "Displaylib: Failed to add/filter new DisplayDxe modes!\n");
           }
           ...
           {
              eStatus = MDP_STATUS_HW_ERROR;
              MDP_Log_Message(MDP_LOGLEVEL_ERROR, "Displaylib: Failed to POWER_STATE!\n");
           }
        }
