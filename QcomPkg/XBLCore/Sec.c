...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 10/25/17   bd      Configure HW WDG if ENABLE_RAMDUMP is defined
...
EFI_STATUS EFIAPI LZ4DecompressLibConstructor (VOID);
...
void UefiBootContinue (void* arg)
{
  ...
  DEBUG((EFI_D_INFO, "UefiBootContinue : [ %d] DecompressFirstFvStart \n", GetTimerCountms()));
  DecompressFirstFv ();
  DEBUG((EFI_D_INFO, "UefiBootContinue : [ %d] DecompressFirstEnd \n", GetTimerCountms()));
  ...
  Status = WatchDogSetBiteTimer(SEC_TO_MSEC(WDOG_BITE_TIME_MAX_SECS));
  if(Status == EFI_SUCCESS)
  {
  ...
  {
    DEBUG((EFI_D_ERROR, "Error: Unable to set WDG Bite Timer\n"));
  }
#endif  
  SetUefiCrashCookie();
