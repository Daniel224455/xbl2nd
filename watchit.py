...
# --------   ---     -----------------------------------------------------------
# 01/28/19   kpa     Update build file hash
...
build_files_sha1=[["buildex.py", "d13972c60dcabd9355ab90ea424382fa893f5207", True]]

if (platform.system().lower() == "linux"):
  # folder_files_sha structure, <Folder Name>, <Hash>, <Mandatory Existance>
  folder_files_sha=[["boot_images/QcomPkg/Include/Protocol", "0", True]]
else:
  print "Running watchit on windows is not supported, get the hash in Linux machine"
...
def main():
  ...
  parser.add_option("-p", "--protocol-dir-hash",
                    action="store",
                    dest="protocol_dir_hash",
                    default="0",
                    help="Hash value for /Include/Protocol");
  ...
  if (options.enable_all_features or options.enable_Folder_hash_check):
      try:
        ...
        folder_files_sha[0][1] = options.protocol_dir_hash
...
def folder_files_check(openssl_path):
  ...
  for current_entry in folder_files_sha:
    ...
    print current_entry[0] + " Hash: " + folder_hash_value
    
    # Check if hash value is present in output
    if not current_entry[1] == folder_hash_value:
      print 'Failure is due to update in directory: ' + current_entry[0]
      raise NameError("FAILED")
