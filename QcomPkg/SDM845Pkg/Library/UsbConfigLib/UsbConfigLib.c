  ...
  )
{
  ...
  if (CoreNum >= USB_CORE_MAX_NUM_SDM_845) {
    Status = EFI_INVALID_PARAMETER;
  ...
  if (IS_CLS_PLAT(PlatformInfo.platform)) {
      ...
      if (!EFI_ERROR(Status) && (UefiVar != 0)) {
     	...
     	// Enable static configuration for Device Mode
      	...
      	gUsbConfigDevice[USB_CORE_0_SDM_845].UsbCoreInterface->ModeType = USB_DEVICE_MODE;
      }
      else
      {
      	// Enable static configuration for Host Mode
      	gUsbConfigDevice[USB_CORE_0_SDM_845].EnableStaticCfg.Enabled = TRUE;
      	gUsbConfigDevice[USB_CORE_0_SDM_845].EnableStaticCfg.UsbMode = USB_HOST_MODE_XHCI;
      	gUsbConfigDevice[USB_CORE_0_SDM_845].UsbCoreInterface->ModeType = USB_HOST_MODE;

      	// Start Controller in host mode
      	DBG(EFI_D_WARN, "Start Controller");
      	Status = UsbStartController((QCOM_USB_CORE_NUM)USB_CORE_0_SDM_845, USB_HOST_MODE_XHCI);
      	ERR_CHK("Start controller failed with error: %r on core %d", Status, USB_CORE_0_SDM_845);
    	}
  	}
  else {
    ...
    if (!FeaturePcdGet(HostModeSupported)) {
      ...
      Status = UsbPortGetRtConnectionMode(gUsbConfigDevice[USB_CORE_0_SDM_845].UsbPortHandle, &PortMode);
      ERR_CHK(" Cannot Get Connection Mode for Core %d: %r", USB_CORE_0_SDM_845, Status);
      ...
      if (gUsbConfigDevice[USB_CORE_0_SDM_845].UsbCoreInterface->ModeType == USB_HOST_MODE) {
  ...
  )
{
  ...
  UsbCoreIfc = gUsbConfigDevice[USB_CORE_0_SDM_845].UsbCoreInterface;
  ...
  if (!FeaturePcdGet(HostModeSupported)) {
    ...
    if (gUsbConfigDevice[USB_CORE_0_SDM_845].EnableStaticCfg.Enabled == TRUE &&
        gUsbConfigDevice[USB_CORE_0_SDM_845].EnableStaticCfg.UsbMode == USB_HOST_MODE_XHCI) {
