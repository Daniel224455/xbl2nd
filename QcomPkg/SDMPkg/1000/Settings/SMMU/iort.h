//#########################################################################
//#########################################################################
//#          THIS IS AN AUTOGENERATED FILE, DO NOT EDIT MANUALLY          #
//#########################################################################
//#########################################################################


#pragma pack(1)

#include "Platform.h"
//#define offsetof(s,m)   (UINT64)&(((s *)0)->m)

typedef struct _INTERRUPT {
		UINT32 GSIV;
		UINT32 InterruptFlags;
}INTERRUPT;



typedef struct _SIDMAPPING {
		UINT32 InputBase;
		UINT32 NumIDs;
		UINT32 OutputBase;
		UINT32 OutputReference;
		UINT32 Flags;
}SIDMAPPING;



typedef struct _SMMUV2NODE_SMMU_APPS{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT64 BaseAddress;
	UINT64 Span;
	UINT32 Model;
	UINT32 Flags;
	UINT32 GlobalIntOffset;
	UINT32 NumContextInterrupts;
	UINT32 ContextIntOffset;
	UINT32 NumPMUInterrupts;
	UINT32 PMUIntOffset;
	UINT32 NSGIRPT_GSIV;
	UINT32 NSGIRPT_FLAGS;
	UINT32 NSGCFGIRPT_GSIV;
	UINT32 NSGCFGIRPT_FLAGS;
	INTERRUPT ContextInterrupts[128];
	INTERRUPT PMUInterrupts[9];
	SIDMAPPING SIDMappings[1];
}SMMUV2NODE_SMMU_APPS;

#define SMMUV2NODE_SMMU_APPS_VAR  {		\
	.Type	=	3,		\
	.Length	=	sizeof(SMMUV2NODE_SMMU_APPS),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	1,		\
	.MappingsOffset	=	offsetof(SMMUV2NODE_SMMU_APPS,SIDMappings),		\
	.BaseAddress	=	0x15000000,		\
	.Span	=	0x100000,		\
	.Model	=	3,		\
	.Flags	=	3,		\
	.GlobalIntOffset	=	offsetof(SMMUV2NODE_SMMU_APPS,NSGIRPT_GSIV),		\
	.NumContextInterrupts	=	128,		\
	.ContextIntOffset	=	offsetof(SMMUV2NODE_SMMU_APPS,ContextInterrupts),		\
	.NumPMUInterrupts	=	9,		\
	.PMUIntOffset	=	offsetof(SMMUV2NODE_SMMU_APPS,PMUInterrupts),		\
	.NSGIRPT_GSIV	=	97,		\
	.NSGIRPT_FLAGS	=	0,		\
	.NSGCFGIRPT_GSIV	=	0,		\
	.NSGCFGIRPT_FLAGS	=	0,		\
	.ContextInterrupts	={		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	129,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	130,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	131,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	132,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	133,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	134,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	135,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	136,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	137,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	138,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	139,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	140,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	141,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	142,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	143,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	144,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	145,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	146,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	147,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	148,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	149,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	150,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	213,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	214,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	215,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	216,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	217,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	218,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	219,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	220,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	221,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	222,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	223,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	224,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	347,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	348,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	349,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	350,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	351,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	352,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	353,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	354,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	355,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	356,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	357,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	358,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	359,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	360,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	361,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	362,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	363,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	364,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	365,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	366,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	367,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	368,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	369,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	370,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	371,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	372,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	373,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	374,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	375,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	376,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	377,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	427,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	428,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	429,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	430,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	431,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	432,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	433,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	434,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	435,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	436,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	437,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	438,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	439,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	440,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	441,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	442,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	443,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	444,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	445,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	738,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	739,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	740,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	741,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	742,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	743,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	744,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	745,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	746,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	747,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	675,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	674,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	673,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	672,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	800,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	801,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	802,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	803,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	804,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	805,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	806,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	807,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	808,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	809,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	810,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	811,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	812,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	813,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	814,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	815,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	816,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	817,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	818,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	819,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	820,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	821,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	822,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	823,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	824,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	825,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	826,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	827,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	828,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	829,		\
		},		\
	},		\
	.PMUInterrupts	={		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	100,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	101,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	102,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	103,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	104,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	105,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	126,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	127,		\
		},		\
		{		\
		.InterruptFlags	=	0,		\
		.GSIV	=	128,		\
		},		\
	},		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	16,		\
		.OutputBase	=	0xA0,		\
		.OutputReference	=	0,		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_SDC2{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[1];
}NAMEDNODE_SDC2;

#define NAMEDNODE_SDC2_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_SDC2),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	1,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_SDC2,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	0,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.SDC2",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x6A0,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_SDC4{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[1];
}NAMEDNODE_SDC4;

#define NAMEDNODE_SDC4_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_SDC4),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	1,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_SDC4,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	0,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.SDC4",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x6C0,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_UFS{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[3];
}NAMEDNODE_UFS;

#define NAMEDNODE_UFS_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_UFS),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	3,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_UFS,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	1,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.UFS0",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x2E0,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x10,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x300,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x20,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x0,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_PCI0{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[1];
}NAMEDNODE_PCI0;

#define NAMEDNODE_PCI0_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_PCI0),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	1,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_PCI0,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	1,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.PCI0",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0,		\
		.NumIDs	=	128,		\
		.OutputBase	=	0x1d80,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_PCI1{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[1];
}NAMEDNODE_PCI1;

#define NAMEDNODE_PCI1_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_PCI1),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	1,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_PCI1,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	1,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.PCI1",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0,		\
		.NumIDs	=	128,		\
		.OutputBase	=	0x1d00,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_PCI2{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[1];
}NAMEDNODE_PCI2;

#define NAMEDNODE_PCI2_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_PCI2),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	1,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_PCI2,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	1,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.PCI2",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0,		\
		.NumIDs	=	128,		\
		.OutputBase	=	0x1c80,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_PCI3{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[1];
}NAMEDNODE_PCI3;

#define NAMEDNODE_PCI3_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_PCI3),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	1,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_PCI3,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	1,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.PCI3",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0,		\
		.NumIDs	=	128,		\
		.OutputBase	=	0x1e00,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_USB3_0{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[1];
}NAMEDNODE_USB3_0;

#define NAMEDNODE_USB3_0_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_USB3_0),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	1,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_USB3_0,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	0,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.USB3-0",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x140,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_USB3_1{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[1];
}NAMEDNODE_USB3_1;

#define NAMEDNODE_USB3_1_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_USB3_1),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	1,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_USB3_1,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	0,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.USB3-1",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x160,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_USB3_2{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[1];
}NAMEDNODE_USB3_2;

#define NAMEDNODE_USB3_2_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_USB3_2),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	1,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_USB3_2,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	0,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.USB3-2",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x60,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_MDP{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[8];
}NAMEDNODE_MDP;

#define NAMEDNODE_MDP_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_MDP),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	8,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_MDP,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	0,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.MDP",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x800,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x1,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x820,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x2,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x801,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x3,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0x821,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x4,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0xC00,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x5,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0xC20,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x6,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0xC01,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x7,		\
		.NumIDs	=	1,		\
		.OutputBase	=	0xC21,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_QGP0{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[2];
}NAMEDNODE_QGP0;

#define NAMEDNODE_QGP0_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_QGP0),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	2,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_QGP0,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	0,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.QGP0",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	0x1,		\
		.OutputBase	=	0x4C3,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x1,		\
		.NumIDs	=	0x1,		\
		.OutputBase	=	0x4D6,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_QGP1{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[2];
}NAMEDNODE_QGP1;

#define NAMEDNODE_QGP1_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_QGP1),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	2,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_QGP1,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	0,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.QGP1",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	0x1,		\
		.OutputBase	=	0x603,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x1,		\
		.NumIDs	=	0x1,		\
		.OutputBase	=	0x616,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _NAMEDNODE_QGP2{
	UINT8 Type;
	UINT16 Length;
	UINT8 Revision;
	UINT32 Reserved;
	UINT32 NumberofMappings;
	UINT32 MappingsOffset;
	UINT32 NodeFlags;
	UINT32 CacheCoherency;
	UINT8 AllocationHints;
	UINT16 Reserved1;
	UINT8 MemAccessFlags;
	UINT8 DeviceMemAddressSize;
	UINT8 DevObjectName[32];
	SIDMAPPING SIDMappings[2];
}NAMEDNODE_QGP2;

#define NAMEDNODE_QGP2_VAR  {		\
	.Type	=	1,		\
	.Length	=	sizeof(NAMEDNODE_QGP2),		\
	.Revision	=	1,		\
	.Reserved	=	0,		\
	.NumberofMappings	=	2,		\
	.MappingsOffset	=	offsetof(NAMEDNODE_QGP2,SIDMappings),		\
	.NodeFlags	=	0,		\
	.CacheCoherency	=	0,		\
	.AllocationHints	=	0,		\
	.Reserved1	=	0,		\
	.MemAccessFlags	=	0,		\
	.DeviceMemAddressSize	=	36,		\
	.DevObjectName	=	"\\_SB_.QGP2",		\
	.SIDMappings	={		\
		{		\
		.InputBase	=	0x0,		\
		.NumIDs	=	0x1,		\
		.OutputBase	=	0x7A3,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
		{		\
		.InputBase	=	0x1,		\
		.NumIDs	=	0x1,		\
		.OutputBase	=	0x7B6,		\
		.OutputReference	=	offsetof(IORT,SMMUV2Nodes_smmu_apps),		\
		.Flags	=	0,		\
		},		\
	},		\
}

typedef struct _IORT{
	UINT32 Signature;
	UINT32 Length;
	UINT8 Revision;
	UINT8 Checksum;
	UINT8 OEMID[6];
	UINT64 OEMTableID;
	UINT32 OEMRevision;
	UINT32 CreatorID;
	UINT32 CreatorRevision;
	UINT32 NumberofIORTNodes;
	UINT32 IORTNodesOffset;
	UINT32 Reserved;
	SMMUV2NODE_SMMU_APPS SMMUV2Nodes_smmu_apps;
	NAMEDNODE_SDC2 NamedNodes_sdc2;
	NAMEDNODE_SDC4 NamedNodes_sdc4;
	NAMEDNODE_UFS NamedNodes_ufs;
	NAMEDNODE_PCI0 NamedNodes_pci0;
	NAMEDNODE_PCI1 NamedNodes_pci1;
	NAMEDNODE_PCI2 NamedNodes_pci2;
	NAMEDNODE_PCI3 NamedNodes_pci3;
	NAMEDNODE_USB3_0 NamedNodes_usb3_0;
	NAMEDNODE_USB3_1 NamedNodes_usb3_1;
	NAMEDNODE_USB3_2 NamedNodes_usb3_2;
	NAMEDNODE_MDP NamedNodes_mdp;
	NAMEDNODE_QGP0 NamedNodes_qgp0;
	NAMEDNODE_QGP1 NamedNodes_qgp1;
	NAMEDNODE_QGP2 NamedNodes_qgp2;
}IORT;

IORT IORT_TABLE = {		
	.Signature	=	'TROI',		
	.Length	=	sizeof(IORT),		
	.Revision	=	0,		
	.Checksum	=	0,		
	.OEMID	=	ACPI_OEM_ID,		
	.OEMTableID	=	ACPI_OEM_TABLE_ID,		
	.OEMRevision	=	ACPI_OEM_REVISION,		
	.CreatorID	=	ACPI_CREATOR_ID,		
	.CreatorRevision	=	ACPI_CREATOR_REVISION,		
	.NumberofIORTNodes	=	15,		
	.IORTNodesOffset	=	offsetof(IORT,Reserved)+4,		
	.Reserved	=	0,		
	.SMMUV2Nodes_smmu_apps	=	SMMUV2NODE_SMMU_APPS_VAR		,
	.NamedNodes_sdc2	=	NAMEDNODE_SDC2_VAR		,
	.NamedNodes_sdc4	=	NAMEDNODE_SDC4_VAR		,
	.NamedNodes_ufs	=	NAMEDNODE_UFS_VAR		,
	.NamedNodes_pci0	=	NAMEDNODE_PCI0_VAR		,
	.NamedNodes_pci1	=	NAMEDNODE_PCI1_VAR		,
	.NamedNodes_pci2	=	NAMEDNODE_PCI2_VAR		,
	.NamedNodes_pci3	=	NAMEDNODE_PCI3_VAR		,
	.NamedNodes_usb3_0	=	NAMEDNODE_USB3_0_VAR		,
	.NamedNodes_usb3_1	=	NAMEDNODE_USB3_1_VAR		,
	.NamedNodes_usb3_2	=	NAMEDNODE_USB3_2_VAR		,
	.NamedNodes_mdp	=	NAMEDNODE_MDP_VAR		,
	.NamedNodes_qgp0	=	NAMEDNODE_QGP0_VAR		,
	.NamedNodes_qgp1	=	NAMEDNODE_QGP1_VAR		,
	.NamedNodes_qgp2	=	NAMEDNODE_QGP2_VAR		,
};

#pragma pack()
