...
STATIC BOOLEAN MutexInitialized = FALSE;
mutex_t *MemoryMutxPtr =NULL;

STATIC 
EFI_STATUS
InitNamedLock(
  CONST CHAR8 *LockStrID,
  mutex_t ** MLockPtr 
)
{
  UINT64 LockID = 0;
  ...
  if (LockStrID == NULL || MLockPtr == NULL )
  {
  ...
  *MLockPtr = init_mutex (LockID);
  ...
  if (*MLockPtr == NULL)
  {
    DEBUG ((EFI_D_ERROR, "Error: Unable to Acquire Memory Mutex \n")); 
    return EFI_ABORTED;
  }
  ...
  )
{
  ...
  {
    Status = InitNamedLock ("MEMMUTEX", &MemoryMutxPtr);
    if (EFI_ERROR (Status))
    {
...
{
  if(MutexInitialized == TRUE)
    mutex_acquire(MemoryMutxPtr);
