...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 07/27/18   pm       USB CX Vote from Nominal to SVS_MIN to support Low power in charging Power-Off Scenario
...
{
  {
    ...
    .arbData.type3 = 
    {
      .uIb = 74000000,        /**< Instantaneous bandwidth in Bytes/second - SVS_MIN*/
  ...
  },
  {
    ...
    {
      .uIb = 74000000,        /**< Instantaneous bandwidth in Bytes/second - SVS_MIN */
      .uAb = 0,               /**< Arbitrated bandwidth in Bytes/second - SVS_MIN*/
      .uLatencyNs = 0         /**< Latency requirement in nanoseconds */
    }
  }
};
  ...
  )
{
  ...
  if (!gPmicNpaClientSS1) {
    ...
    // is PMI8998, use default SS1 LDO vote for primary USB core.
    // If PMI reports other result (no PMI8998), then use alternative SS3 LDO vote
    if (EFI_ERROR(Status) || PmiDeviceInfo.PmicModel == EFI_PMIC_IS_PMI8998)
    {      
    ...
    {
      DBG(EFI_D_ERROR, "PMI8998 not detected");
      gPmicNpaClientSS1 = npa_create_sync_client(
  ...
  if (!gPmicNpaClientSS2) {
    gPmicNpaClientSS2 = npa_create_sync_client(
                            PMIC_NPA_GROUP_ID_USB_SS2, //PMIC_NPA_GROUP_ID_USB_SS2 - Connect to the usb ss2 resource
                            "usb_ss2",                 //client name "usb_ss2"
                            NPA_CLIENT_REQUIRED);      //NPA_CLIENT_REQUIRED: Request must honor request
  }
  ...
  )
{
  ...
  if (IS_CLS_PLAT(PlatformInfo.platform)) {
    ...
    if(UsbCore->CoreType == USB_CONFIG_SSUSB1)
      Status = gPwrCtrlProtocol->GetVbusSrcOkStatus(USB_CORE_0_SDM_845, (BOOLEAN *)VbusSt);
    else if(UsbCore->CoreType == USB_CONFIG_SSUSB2)
      Status = gPwrCtrlProtocol->GetVbusSrcOkStatus(USB_CORE_1_SDM_845, (BOOLEAN *)VbusSt);
    else
    {
      Status = EFI_INVALID_PARAMETER;
      DBG(EFI_D_WARN, "Invalid Core Type");
  ...
  )
{
  ...
  if(UsbCore->CoreType == USB_CONFIG_SSUSB1) {
    Status = EnableVbusSS(USB_CORE_0_SDM_845);
    ERR_CHK("Failed to initialize VbusSS for core 1");
  }
  else if(UsbCore->CoreType == USB_CONFIG_SSUSB2) { 
    Status = EnableVbusSS(USB_CORE_1_SDM_845);
  ...
  )
{
  ...
  else if(CoreNum == USB_CORE_1_SDM_845) {
  ...
  if(FlagToCallAPIOnlyOnce == 0 && CoreNum < USB_CORE_MAX_NUM_SDM_845) {
