...
DALResult DAL_AdcDeviceAttach(DALDEVICEID DevId, DalDeviceHandle **phAdcDev)
{
   ...
   if (CHIPINFO_FAMILY_SDM855 == DalChipInfo_ChipFamily())
   {
      ChipInfoIdType eChipId = DalChipInfo_ChipId();

      if (eChipId == CHIPINFO_ID_SA8150 || eChipId == CHIPINFO_ID_SA8150P)
      {
         pszAdcBspPropName = "ADC_BSP_AU";
      }
      else if (pm_get_pmic_model(1) == PMIC_IS_PM855B)
      {
         pszAdcBspPropName = "ADC_BSP_855_PM855_PM855B_PM855L";
      }
   }
