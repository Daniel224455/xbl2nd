...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 04/09/18   sb        changed some log levels to prevent unnecessary logging during bootup
 01/25/18   cs        change normal case log to level INFO
 09/27/17   va        Fixing Array bound for SRAM data
 09/22/17   va        return error on read for write transaction
                                               ...
                                               UINT8 WriteValue, UINT8 Offset, UINT8 mask)
{
  ...
  {
    PMIC_DEBUG(( EFI_D_WARN, "PmicDxe:: %a Single Ima Read Addr = %d Failed = %r \r\n", __FUNCTION__, WriteAddress, Status));
    return Status;
  }
...
EFI_STATUS PmicFgSram_SramHealthy(UINT32 PmicDeviceIndex)
{
  ...
  {
    PMIC_DEBUG(( EFI_D_INFO, "PmicDxe:: %a SRAM contents are good \r\n", __FUNCTION__, Status));
...
EFI_STATUS PmicFgSram_VerifySramContent(UINT32 PmicDeviceIndex, BOOLEAN *UnExpectedMatch)
{
  ...
  *UnExpectedMatch = FALSE;
  ...
  {
    PMIC_DEBUG(( EFI_D_INFO, "PmicDxe:: %a FgUnUsedData = 0x%x \r\n", __FUNCTION__, FgUnUsedData));
  }
