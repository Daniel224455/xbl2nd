...
)
{
  ...
  /*set alarm time and then enable */
  err_flag = pm_rtc_alarm_set_ms_time(PMIC_A, (pm_rtc_time_type *)TimePtr); 

  err_flag |= pm_rtc_alarm_enable(PMIC_A, TRUE);    
...
)
{
  ...
  errFlag = pm_rtc_alarm_enable(PMIC_A, Enable);    
