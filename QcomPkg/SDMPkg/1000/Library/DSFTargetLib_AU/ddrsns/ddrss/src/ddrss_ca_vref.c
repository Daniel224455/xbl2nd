                                   ...
                                   ) 
{
  ...
  //uint8 freq_eq_max_training = ddr->shrm_ddr_ptr->misc.max_ddr_frequency == clk_freq_khz;
	//boolean ca_vref_fail_flag  = FALSE; //removed due to compiler error
  for(ch = 0; ch < NUM_CH; ch++)
    {
      ...
      for(cs = 0; cs < NUM_CS; cs++) {
        if((chip_select >> cs) & 0x1) {
		 ...
		 //ca_vref_fail_flag removed due to compiler error :: set but never used	  
                                   ...
                                   )
{
		 ...
		 vref_value ++) 
    {
        ...
        // Low speed
        ddr_external_set_clk_speed (ddr->training_params.low_freq_in_khz);
