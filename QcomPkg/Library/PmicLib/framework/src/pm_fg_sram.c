...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 10/17/17   czq     Move FG SRAM related WA functions to this file
 ...
 */ 
 ...
 { 
  ...
  /* this function is used to read read PBS OTP address 0x5F8 which have VBAT gain to compare with SRAM value.
   If not match, write OTP reading to sram */ 

pm_err_flag_type pm_sbl_copy_vbat_gain_from_pbs_otp_to_sram()
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    uint8 data_otp[4];       // Used to get the data read from PBS OTP
    uint32 data_sram = 0;      // Used to get the data read from FG SRAM
    uint16 otp_address_fg_start = 0x5F8; // VBATT gain address in PBS OTP
	uint16 sram_vadc_gain_addr = 0x0; 
    pm_pbs_info_type pbs_info;
    uint8 device_index = 1; // PMi8998
    uint8 slave_id = device_index * 2;
    uint8 data = 0;  // for temporarily reading parameter
    ...
    // Start PBS OTP reading on given address
    ...
    err_flag = pm_pbs_config_access(slave_id, otp_address_fg_start, PM_PBS_ACCESS_READ_SINGLE);
    if(err_flag != PM_ERR_FLAG_SUCCESS)
    {
        return err_flag;
    }

    err_flag = pm_comm_read_byte_array(slave_id, PMIO_PBS_CORE_MEM_INTF_RD_DATA0_ADDR, 4, data_otp, 0);
    if(err_flag != PM_ERR_FLAG_SUCCESS)
    {
        return err_flag;
    }
    ...
    // End of PBS OTP reading

    err_flag = pm_get_pbs_info(device_index, &pbs_info);
    if(err_flag != PM_ERR_FLAG_SUCCESS)
    {
        return err_flag;
    }

    // adjust vadc gain by decrement3 if TP version between 29 and 31. vadc located at the lowest byte, so decrease directly.
    if((pbs_info.test_pgm_rev >=29) && (pbs_info.test_pgm_rev <=31))
    {
		if(data_otp[0] > 3)
		{
			data_otp[0] = data_otp[0] - 3;
		}
		else
		{
			data_otp[0] = 0;
    ...
    // read data from FG SRAM address 0
    err_flag = PmicFgSram_ImaRead(device_index, sram_vadc_gain_addr, &data_sram); 
    if(err_flag != PM_ERR_FLAG_SUCCESS) 
    { 
        return err_flag; 
    } 

    // Compare data from SRAM address 0 and data from PBS OTP, if not equal, write PBS OTP value (with adjust if necessary) into SRAM 0
    if(data_sram != *(uint32*)data_otp)
    {
        //Write into SRAM 0
		err_flag = PmicFgSram_ImaWriteSingle(device_index, sram_vadc_gain_addr, *(uint32*)data_otp);
		pm_log_message("Vo doesn't equal to Vs, write Vo into FG SRAM");
    }
...
/** 
 pm_sbl_check_fg_sram_status() 
   
 @brief 
 Verify if SRAM content are health  
 */ 
 pm_err_flag_type pm_sbl_check_fg_sram_status(uint32 pmic_index) 
 { 
   boolean     unexpected_match = FALSE;
   pm_err_flag_type  err_flag = PM_ERR_FLAG__SUCCESS;  
   
   err_flag = pm_fg_sram_verify_content(pmic_index, &unexpected_match);
   ...
   { 
	 ...
	 boot_hw_reset(BOOT_DVDD_HARD_RESET_TYPE);
   }  
