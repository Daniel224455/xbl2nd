...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 09/15/20   rj      Added GetUFSBootLun to get active LUN no.
...
)
{
  ...
  for (j = 0; j < NumberOfSdccHandles; j++)
  {
    ...
    Status = CardInfo->GetBootLU(CardInfo, &BootLUN);

    if (Status != EFI_SUCCESS)
    {
       DEBUG((EFI_D_ERROR,"Failed to get BootLU! status: %d\n", Status));
       goto GetUfsBootLUExit;
    }

    *LUN = BootLUN;
    DEBUG((EFI_D_INFO,"GetUFSBootLun, get boot lun : %u\n", BootLUN));
...
/**
  Returns a list of BlkIo handles based on required criteria
  It checks active UFS boot lun, then set the root device to specfied LUN.
  SelectionAttrib : Bitmask representing the conditions that need
                    to be met for the handles returned. Based on the
                    selections filter members should have valid values.
  FilterData      : Instance of Partition Select Filter structure that
  ...
  )
{
  ...
  Status = GetBlkIOHandles(SelectionAttrib, FilterData, HandleInfoPtr, MaxBlkIopCnt);

  /* Handle two handles for A/B partiitons with same guid */
  if (Status == EFI_SUCCESS &&
       boot_from_ufs() &&
       *MaxBlkIopCnt == 2 &&
       (SelectionAttrib & BLK_IO_SEL_MATCH_PARTITION_TYPE_GUID) != 0)
  {
    Status = GetUFSBootLUN(&BootLunNumber);
    if (Status == EFI_SUCCESS)
    {
      if (BootLunNumber == LunNumber1)
      {
          DEBUG((EFI_D_INFO, "GetBlkIOHandlesExt, boot lun is 1, select the first one\n"));
          *MaxBlkIopCnt = 1;
      }
      else if (BootLunNumber == LunNumber2)
      {
          DEBUG((EFI_D_INFO, "GetBlkIOHandlesExt, boot lun is 2, select the second one\n"));
          HandleInfoPtr[0].Handle = HandleInfoPtr[1].Handle;
          HandleInfoPtr[0].BlkIo = HandleInfoPtr[1].BlkIo;
          HandleInfoPtr[0].PartitionInfo = HandleInfoPtr[1].PartitionInfo;

          *MaxBlkIopCnt = 1;
      }
      else
      {
          DEBUG((EFI_D_ERROR, "GetBlkIOHandlesExt, invalid boot lun :%u\n", BootLunNumber));
      }
    }
  }
  ...
  IN UINT32 BufferSize)
{
  ...
  if(Status != EFI_SUCCESS)
  {
	DEBUG((EFI_D_ERROR, "LocateProtocol: ChipInfo unsuccessful! status: %d\n", Status));
    goto ErrorExit;
  }
  ...
  if(Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_ERROR,"Failed to get MSM serial number\n"));
    goto ErrorExit;
  }
  ...
  Status = pEfiHash2Protocol->HashUpdate(pEfiHash2Protocol, (UINT8 *)&chip_serial_num_expanded, sizeof(chip_serial_num_expanded));
  if (Status != EFI_SUCCESS) {
...
SaveTZAppLogs(VOID)
{
  ...
  DEBUG(( EFI_D_ERROR, "TZApp Log Buffer: (Size = %d)\r\n",BufferSize));
  DEBUG(( EFI_D_ERROR, "-- START -- \r\n"));

  for(Offset=0;Offset<(BufferSize); )
  {
    ...
    DEBUG(( EFI_D_ERROR, "%a", (CHAR8 *)((UINTN)OutputBuffer + Offset)));
    Offset += (BytesPerLoop);
  }
