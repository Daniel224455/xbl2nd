/*=============================================================================
 ...
 Copyright (c)  2013-2016, 2024 Qualcomm Technologies, Inc. 
 All Rights Reserved.
...
*/
/*===========================================================================*/
static int32 smem_part_process_item
(
  smem_partition_info_type              *info,
...
)
{
  ...
  if ((info->offset_free_uncached > info->offset_free_cached) ||
      (info->offset_free_uncached < sizeof(smem_partition_header_type)) ||
  ...
  /* Point to first item */
  item_hdr = (smem_partition_allocation_header_type *)
                ((uint8 *)info->header + sizeof(smem_partition_header_type));

  /* Set limit of scan */
  limit_addr = (uint8 *)info->header + info->offset_free_uncached;

  /* Scan from uncached side of heap */
  while ((uint8*)item_hdr + sizeof(smem_partition_allocation_header_type) <= limit_addr)
  {
   ...
   * Point to first item */
  item_hdr = (smem_partition_allocation_header_type *)
                ((uint8 *)info->header + info->size - padding_header -
                sizeof(smem_partition_allocation_header_type));

  /* Set limit of scan */
  limit_addr = (uint8 *)info->header + info->offset_free_cached;

  /* Scan from cached side of heap */
  while (((uint8 *)item_hdr >= limit_addr) && 
         (((uint8 *)item_hdr + sizeof(smem_partition_allocation_header_type)) <=
          ((uint8 *)info->header + info->size)))
  {
    /* Process the next item, and check if it is the item we are searching
     * for. */
    ret = smem_part_process_item(
...
void smem_part_init(void)
{
  ...
  for (i = 0; i < toc->num_entries; i++)
  {
    ...
    if (size != 0 && size < smem_size && remote_host != SMEM_INVALID_HOST )
    {
