...
void qusb_dci_handle_u1_u2_enable(struct qusb_dcd_dsc_device* dev, qusb_dci_u1_u2_event_type event, boolean enable)
{
  ...
  switch(event)
  {
    ...
    default:
      qusb_log_err__param(DCI_DEFAULT_CASE_LOG, __LINE__);
  }
}
...
{ 
   switch(dev->test_mode) 
   {
     ...
     default:
       qusb_log_err__param(DCI_TEST_MODE_INVALID_LOG, dev->test_mode);
       break;
  }
}
                       ...
                       qusb_dci_callback_table_t *callback_table_ptr)
{
  ...
  qusb_log_bus(DCI_INIT_LOG);

  if (NULL == dcd_context_ptr || NULL == callback_table_ptr)
  {
    qusb_log_err__param(DCI_NULL_POINTER_ERROR_LOG, __LINE__);
    return DCI_ERROR; 
  }
...
void qusb_dci_core_init(struct qusb_dcd_dsc_device* dcd_context_ptr)
{
  ...
  qusb_log_bus(DCI_CORE_INIT_LOG);
  ...
  {
    qusb_log_err(DCI_CORE_INIT_WRITE_EP_CMD_ERROR_LOG);
  } 
...
static uint32 qusb_dci_write_ep_cmd(uint32 ep, uint32 dir, qusb_ep_cmd_t *ep_cmd, uint8 *rsc_idx_ptr)
{
  ...
  {
    qusb_log_err__param(DCI_EP_CMD_ACTIVE_ERROR_LOG, ep_index);
  }
  ...
  switch (ep_cmd->template.cmd.reg.cmd_typ) 
  {
    ...
    default:
    qusb_log_err__param(DCI_EP_CMD_INVALID_ERROR_LOG, ep_cmd->template.cmd.reg.cmd_typ);
    ...
    if (!(reg_val & HWIO_USB3_PRI_DEPCMDPAR2_REGS_p_DEPCMD_CMDACT_BMSK))
    {
      if((NULL != rsc_idx_ptr) && (ep_cmd->template.cmd.reg.cmd_typ == QUSB_EP_CMD_START_XFER_VAL))
      {
        ...
        *rsc_idx_ptr = (uint8)(cmd_params & QUSB_EP_XFER_RSC_INDEX_BMASK);
        qusb_log__param(EP_COMMAND_COMPLETE_LOG, *rsc_idx_ptr);
      }
      return DCI_OK;
    }
...
static uint32 qusb_dci_write_dev_cmd(qusb_dev_cmd_t *dev_cmd)
{
  ...
  {
    qusb_log_err(DCI_DEV_CMD_ACTIVE_ERROR_LOG);
  }
  ...
  switch(dev_cmd->template.cmd.reg.cmd_typ)
  {
    ...
    break;
    default:
    qusb_log_err__param(DCI_DEV_CMD_INVALID_ERROR_LOG, dev_cmd->template.cmd.reg.cmd_typ);
    return QUSB_ERR_INVALID_COMMAND;			
  }
...
void qusb_dci_event_buffer_restore(void)
{
  ...
  qusb_log_bus__param(DCI_EVENT_BUFFER_RESTORE_LOG, qusb_evt_buf_info.cur_pos);
...
static void qusb_dci_event_buffer_init(void)
{
  ...
  qusb_log_bus(DCI_EVENT_BUFFER_INIT_LOG);
...
{
  qusb_log_bus__param(DCI_SET_RUN_STOP_LOG, set);
  ...
  else
  {
    ...
    switch(depevt->ep_xfer_event)
    {
      ...
      case QUSB_EVT_EP_CMD_COMPLETE:
        qusb_log_bus__param(EP_COMMAND_COMPLETE_INTR_LOG, ep_num);
      break;
    }
  }
}
...
static void qusb_dci_handle_dev_event(struct qusb_event_devt  *devevt)
{
  ...
  qusb_log_bus__param(DCI_HANDLE_DEV_EVENT_LOG, devevt->dev_event);
  ...
  switch(devevt->dev_event)
  {
    ...
    case QUSB_EVT_DEV_ERRATIC_ERROR:
      qusb_log_err(DCI_ERRATIC_ERROR_EVENT_LOG);
    break;
    ...
    case QUSB_EVT_DEV_EVENT_BUFFER_OVERFLOW:
      qusb_log_err(DCI_EVT_BUFFER_OVERFLOW_EVENT_LOG);
...
boolean qusb_vbus_detect(void)
{
  ...
  {
    qusb_log_err__param(DCI_PMIC_API_FAILURE_LOG, pm_status);
...
void qusb_dci_poll_events(void)
{
  ...
  if(QUSB_DCI_PLATFORM_SILICON == qusb_dci_get_platform())
  {
    ...
    if(vbus_state != qusb_vbus_detect())
    {
      ...
      qusb_log_err__param(DCI_VBUS_STATE_CHANGE_LOG, vbus_state);
      if (vbus_state)
      {
        qusb_uart_log("VBUS High!", 0);
      }
      else
      {
        qusb_uart_log("VBUS Low", 0);
      }

      if(OFF == vbus_state)
      {
        qusb_log_err(DCI_VBUS_REMOVAL_LOG);
        qusb_dci_handle_usb_disconnect();
      }
  ...
  while (evt_byte_count > 0) 
  {
    ...
    {
      qusb_log_err__param(DCI_EVENT_ERROR_LOG, event.devt.dev_spec_event);
    }
  ...
  switch (ep_evt_type) 
  {
    case QUSB_EVT_EP_XFER_COMPLETE:
      ...
      if (dcd_dev->ctrl_state == EP0_STATE_SETUP_TRB)
      {
        ...
        qusb_log_bus__param(XFER_COMPLETE_SETUP_LOG, depevt->event_status);
        qusb_dci_handle_transfer_complete(depevt);
      ...
      else if (dcd_dev->ctrl_state == EP0_STATE_DATA_IN_PROGRESS)
      {
        ...
        {
          qusb_log_bus__param(XFER_COMPLETE_DATA_LOG, depevt->event_status);
          qusb_dci_handle_transfer_complete(depevt);
      ...
      {
        qusb_log_bus__param(XFER_COMPLETE_NOT_EXPECTED_IN_EP0_STATE_DATA_LOG, depevt->event_status);
      }
      /* status stage complete */
      else if (dcd_dev->ctrl_state == EP0_STATE_STATUS_2 ||
                dcd_dev->ctrl_state == EP0_STATE_STATUS_3)
      {
        qusb_log_bus__param(XFER_COMPLETE_STATUS_LOG, depevt->event_status);
        qusb_dci_handle_transfer_complete(depevt);
        dcd_dev->ctrl_state = EP0_STATE_SETUP_TRB;
        /* start a new setup */
        qusb_dci_ep0_start_xfer(dcd_dev);
    ...
    case QUSB_EVT_EP_XFER_NOT_READY: 
      ...
      */
      if(dcd_dev->ctrl_state == EP0_STATE_SETUP_TRB)
      {
        qusb_log_bus__param(XFER_NOT_READY_IN_SETUP_STAGE_ERROR_LOG, dir);
        dcd_dev->ctrl_state = EP0_STATE_SETUP_TRB;
      }
      else if(dcd_dev->ctrl_state == EP0_STATE_DATA)
      {
        ...
        {
          qusb_log_bus__param(XFER_NOT_READY_DATA_LOG, dir);
          ...
          {
            qusb_log_bus__param(XFER_NOT_READY_INCORRECT_DIR_ERROR_LOG, dcd_dev->ctrl_data_dir);
        ...
        {
          qusb_log_bus__param(XFER_NOT_READY_STATUS_IN_DATA_STAGE_ERROR_LOG, depevt->event_status);
          dcd_dev->ctrl_state = EP0_STATE_SETUP_TRB;
          qusb_dci_stall_endpoint(ep, DIR_RX); 
        }
        else
        {
          qusb_log_bus__param(INVALID_EVENT_STATUS_ERROR_LOG, depevt->event_status);
        }
      }
      else if(dcd_dev->ctrl_state == EP0_STATE_DATA_IN_PROGRESS)
      {
        if (depevt->event_status == QUSB_DDEPEVT_STATUS_CONTROL_DATA)
        {
          qusb_log_bus__param(XFER_NOT_READY_DATA_STILL_IN_PROGRESS_LOG, depevt->event_status);
        }
        else
        {
          qusb_log_bus__param(INVALID_EVENT_STATUS_ERROR_LOG, depevt->event_status);
      ...
      else if (dcd_dev->ctrl_state == EP0_STATE_STATUS_2)
      {
        ...
        {
          qusb_log_bus__param(XFER_NOT_READY_STATUS2_LOG, dir);
          if(dir == DIR_TX)
          {
            qusb_dci_submit_request(ep, dir);
          }
          ...
          {
            qusb_log_bus__param(XFER_NOT_READY_INCORRECT_DIR_STATUS2_ERROR_LOG, dcd_dev->ctrl_data_dir);
            dcd_dev->ctrl_state = EP0_STATE_SETUP_TRB;
            qusb_dci_stall_endpoint(ep, DIR_RX); 
          }
        }
        ...
        {
          qusb_log_bus__param(XFER_NOT_READY_DATA_IN_STATUS2_STAGE_ERROR_LOG, depevt->event_status);
          dcd_dev->ctrl_state = EP0_STATE_SETUP_TRB;
          qusb_dci_stall_endpoint(ep, DIR_RX); 
        }
      }
      else if (dcd_dev->ctrl_state == EP0_STATE_STATUS_3)
      {
        if (depevt->event_status == QUSB_DDEPEVT_STATUS_CONTROL_STATUS)
        {
          qusb_log_bus__param(XFER_NOT_READY_STATUS3_LOG, dir); 
          if(dcd_dev->ctrl_data_dir != dir)
          {
            qusb_dci_submit_request(ep, dir);  
          }
          ...
          {
            qusb_log_bus__param(XFER_NOT_READY_INCORRECT_DIR_STATUS3_ERROR_LOG, dcd_dev->ctrl_data_dir);
            if(qusb_set_stall_on_xfer_nrdy_wrong_dir == TRUE)
            {
              dcd_dev->ctrl_state = EP0_STATE_SETUP_TRB;
              qusb_dci_stall_endpoint(ep, DIR_RX); 
        ...
        {
          qusb_log_bus__param(XFER_NOT_READY_DATA_IN_STATUS3_STAGE_ERROR_LOG, depevt->event_status);
          dcd_dev->ctrl_state = EP0_STATE_SETUP_TRB;
          qusb_dci_stall_endpoint(ep, DIR_RX); 
        }
      }
    ...
    case QUSB_EVT_EP_CMD_COMPLETE:
      qusb_log_bus__param(EP_COMMAND_COMPLETE_INTR_LOG, ep);
    break;

    default:
       qusb_log_err__param(DCI_DEFAULT_CASE_LOG, ep_evt_type);
    break;
  }
  return;
}
...
static void qusb_dci_handle_transfer_complete(struct qusb_event_depevt *depevt)
{
  ...
  {
    qusb_log_err__param(DCI_EVT_BUFFER_OVERFLOW_EVENT_LOG, __LINE__);
    return;
  }
  ...
  if(NULL == urb_ptr)
  {
    qusb_log_err(DCI_HANDLE_TRANSFER_COMPLETE_URB_PTR_NULL_ERROR_LOG);
    return;
  }
...
static void qusb_dci_handle_usb_reset(void)
{
  ...
  qusb_log_bus__param(DCI_HANDLE_USB_RESET_LOG, dcd_dev->ctrl_state);
...
static void qusb_dci_handle_connect_done(void)
{
  ...
  qusb_log_bus__param(DCI_HANDLE_CONNECT_DONE_LOG, dcd_dev->ctrl_state);

  if( dcd_dev->speed > QUSB_SUPER_SPEED)
  {
    qusb_log_err__param(DCI_UNSUPPORTED_SPEED_LOG, dcd_dev->speed);
  }
  ...
  for (dir =0 ; dir < DCI_MAX_DIRECTION; dir++) 
  {
    ...
    if(DCI_OK != qusb_dci_write_ep_cmd(ep0, dir, &ep_cmd, NULL))
    {
      qusb_log_err(DCI_HANDLE_CONNECT_DONE_WRITE_EP_CMD_ERROR_LOG);
    }
  ...
  if (dcd_dev->ctrl_state != EP0_STATE_SETUP_TRB)
  {
    ...
    qusb_log_err__param(DCI_CTRL_SM_ERROR_LOG, dcd_dev->ctrl_state);
...
static void qusb_dci_handle_usb_disconnect(void)
{
  ...
  uint8  ep, dir;
  struct qusb_dcd_dsc_device *dcd_dev = dci_core_info[DCI_SNPS_CORE].dcd_context_ptr;
  qusb_log_bus(DCI_HANDLE_USB_DISCONNECT_LOG);
...
static qusb_link_state_t qusb_dci_get_link_state(void)
{
  ...
  qusb_log_bus__param(DCI_GET_LINK_STATE_LOG, link_state);
  return link_state;
}
...
static void qusb_dci_handle_link_status_change(uint32 state)
{
  ...
  qusb_log_bus__param(DCI_HANDLE_LINK_STATUS_CHANGE_LOG, dcd_dev->link_state);
...
void qusb_dci_handle_suspend_entry(void)
{
  ...
  */
  ...
  if(link_state == LS_U3)
  {
    qusb_log_bus(DCI_HANDLE_SUSPEND_ENTRY__IN_U3);
    qusb_uart_log("suspended", 0);
    ...
    if ( dcd_dev->remote_wakeup_enabled == TRUE)
    {
      ...
      *(uint32 *)QUSB_REMOTE_WAKEUP_COOKIE = QUSB_WAKEUP_COOKIE_VAL;
      ...
      qusb_log_bus(DCI_HANDLE_SUSPEND_ENTRY__REMOTE_WAKEUP);
      qusb_uart_log("remote_wakeup", 0);
  ...
  {    
    qusb_log_bus(DCI_HANDLE_SUSPEND_ENTRY__NOT_IN_U3);
  }
}
...
{
  struct qusb_dcd_dsc_device *dcd_dev = dci_core_info[DCI_SNPS_CORE].dcd_context_ptr;
  qusb_log_bus(DCI_HANDLE_WAKEUP_LOG);
  qusb_uart_log("resume", 0);
...
{
  qusb_log_bus__param(DCI_SET_ADDRESS_LOG, address);
...
uint32 qusb_dci_stall_endpoint(uint8 ep, uint8 dir)
{
  ...
  qusb_log_bus__param(DCI_STALL_ENDPOINT_LOG, ep);
  if (NULL == dcd_dev)
  {
    qusb_log_err__param(DCI_NULL_POINTER_ERROR_LOG, __LINE__);
    return(DCI_ERROR);
  ...
  if((ep >= DCI_MAX_ENDPOINTS) || (dir >= DCI_MAX_DIRECTION))
  {
    qusb_log_err__param(DCI_MEMORY_OUT_OF_BOUNDS_ERROR_LOG, __LINE__);
    return(DCI_ERROR);
  }
  memset(&ep_cmd, 0, sizeof(ep_cmd));
...
uint32 qusb_dci_unstall_endpoint(uint8 ep, uint8 dir)
{
  uint32 status=DCI_OK;
  qusb_ep_cmd_t ep_cmd;
  qusb_log_bus__param(DCI_UNSTALL_ENDPOINT_LOG, ep);
  if((ep < DCI_MAX_ENDPOINTS) && (dir < DCI_MAX_DIRECTION))
  {
    memset(&ep_cmd, 0, sizeof(ep_cmd));
                                  ...
                                  boolean zero_length_transfer)
{
  ...
  {
    qusb_log_err(DCI_CONFIG_EP_WRITE_EP_CMD_ERROR_LOG);
  }
...
static uint32 qusb_dci_submit_trb_data(uint8 ep, uint8 dir)
{
  ...
  {
    qusb_log_err(DCI_SUBMIT_TRB_DATA_WRITE_EP_CMD_ERROR_LOG);
    return (DCI_ERROR);
  }
...
static void qusb_dci_prepare_trbs(uint32 ep, uint32 dir, uint64 buf_addr, uint32 byte_count, uint8 send_zlp, uint8 trb_cntrl)
{
  ...
  do
  {
    ...
    {
      trb->buf_ptr_lo = (uint32)( (uint64)(trb_ring->buf)  & 0xFFFFFFFF );
      trb->buf_ptr_hi = (uint32)( ((uint64)(trb_ring->buf)  >> 32) & 0xFFFFFFFF );
    }
    else
    {
         ...
         ||((ep == 0) && ( dir == DIR_TX)&& ((xfer_len % qusb_dci_ep0_max_packet_size) == 0))))
  {
    ...
    trb->sid_sofn = 0;
    trb->rsvd2    = 0;
    qusb_log__param(DCI_PREPARE_TRB_ZLP_LOG, i);
  }
...
static uint32 qusb_dci_transfer_data_request(uint8 ep, uint8 dir,void* buffer_ptr,uint32 byte_count, uint8 send_zlp, trb_ctl_type_t trb_ctrl)
{
  if (ep == 0)
  {
    qusb_log__param(DCI_CTRL_TRANSFER_DATA_REQUEST_LOG, byte_count);
  }
  else
  {
    qusb_log__param(DCI_TRANSFER_DATA_REQUEST_LOG, byte_count);
  }
  
  /* max transfer to be less than to fit in num of TRBs */
  if (byte_count > (DCI_MAX_TRBS * QUSB_TRB_MAX_BUFFER_SIZE)) 
  {
    qusb_log_err__param(DCI_TRANSFER_SIZE_TOO_LARGE_LOG, byte_count);
...
static uint32 qusb_dci_submit_request(uint8 ep, uint8 dir)
{
  ...
  qusb_log_bus__param(DCI_CTRL_XFER_TO_HW_LOG, QUSB_EP_INDEX(ep, dir));
...
uint32 qusb_dci_cancel_ep0(void)
{
  ...
  {
    qusb_log_err(DCI_CANCEL_TRANSFER_WRITE_EP_CMD_ERROR_LOG);
  }
  return 0;
}
...
uint32 qusb_dci_cancel_transfer(uint8 ep, uint8 dir)
{
  ...
  if(NULL == urb_ptr)
  {
    qusb_log_err(DCI_CANCEL_TRANSFER_URB_NULL_ERROR_LOG);
    return DCI_ERROR;
  }

  res_index = urb_ptr->transfer_index;
  qusb_log__param(DCI_CANCEL_TRANSFER_LOG, ep);
  if((urb_ptr->transfer_status == QUSB_URB_STATUS_ACTIVE)&& (res_index != QUSB_RSC_IDX_NOT_SET))
  {
    ...
    if(ep == 0)
    {
      qusb_log__param(DCI_CANCEL_TRANSFER_LOG, (1+ep+dir));
      qusb_log__param(DCI_PARAM_LOG, res_index);
      if (dir == DIR_RX)
      {
        qusb_uart_log("endxfer EP0 OUT", 0);
      }
      else
      {
        qusb_uart_log("endxfer EP0 IN", 0);
      }
    }
    urb_ptr->transfer_status = QUSB_URB_STATUS_COMPLETE_ERROR;
  }
...
uint32 qusb_dci_get_speed(void)
{
  ...
  qusb_log__param(DCI_GET_SPEED_LOG, speed);
  return(speed);
}
//============================================================================
/**
...
{
  qusb_log_bus__param(DCI_SET_MAX_PACKET_SIZE_LOG, max_packet_size);
...
static uint32 qusb_dci_get_max_packet_size(void)
{
  qusb_log__param(DCI_GET_MAX_PACKET_SIZE_LOG, qusb_dci_max_packet_size);
...
void qusb_dci_handle_set_select(void)
{
  ...
  qusb_log_bus(DCI_HANDLE_SET_SELECT_LOG);
...
boolean qusb_dci_skip_enumeration(void)
{
  ...
  {
    qusb_log_bus(DCI_SKIP_ENUMERATION_LOG);
    return TRUE;
  }
  else
  {
    qusb_log(DCI_PERFORM_ENUMERATION_LOG);
    return FALSE;
  }
}
