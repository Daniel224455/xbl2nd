...
void    HAL_clk_ConfigClockMuxDynamic   ( HAL_clk_ClockDomainDescType *pmClockDomainDesc, const HAL_clk_ClockMuxConfigType *pmConfig );
...
Description:
  ...
  This function will result in either a static or dynamic switch, depending
  on whether the RCG is currently on. If the RCG is off, then the switch will
  be static and not require either source (old or new) to be on. If the RCG
  is on, then the switch will be dynamic and both sources (old and new) must
  be on. This function is only safe to call if the RCG can't turn on/off
  in the middle of the update. If it does, then the RCG can lockup, the
  switch can be silently ignored, or the switch will be applied but
  mis-configured. HAL_clk_ConfigClockMuxDynamic is generally preferred,
  since it prevents these race conditions.
...
HAL_clk_ConfigClockMuxDynamic

Description:
  Configures clock MUX for the given clock domain.

  This function forces a dynamic switch, which is typically accomplished by
  forcing the RCG root on while the update is in progress. Both sources
  (old and new) are required to be on before calling this function. This
  function prevents a number of issues that can arise when the RCG turns on
  in the middle of an update and is generally safer than HAL_clk_ConfigClockMux.
