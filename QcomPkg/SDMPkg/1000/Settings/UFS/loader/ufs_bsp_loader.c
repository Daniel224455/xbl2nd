...
---------------------------------------------------------------------- 
2020-12-10   gk      Add clock/PMIC voting for secondary UFS for auto target
...
2018-05-11   jt      Add autoH8 function 
...
void ufs_bsp_clk_set (uint8_t hostid, uint32_t mode)
{
  ...
  if (hostid == 1)
  {
   if (mode == UFS_CLOCK_MODE_OFF) {
	  (void) Clock_UFS2CardInit (CLOCK_BOOT_PERF_NONE);	  
   }
   else if (mode == UFS_CLOCK_MODE_ON) {
	  (void) Clock_UFS2CardInit (CLOCK_BOOT_PERF_NOMINAL);	  
   }
  }
  else if(hostid == 0)
  {
...
ufs_bsp_err_type ufs_bsp_volt_set(uint8_t hostid, ufs_bsp_volt_mode mode)
{
   ...
   if(1 == hostid)
   {
      err_flag = pm_ldo_sw_mode(PMIC_A, PM_LDO_11, PM_SW_MODE_NPM);
      err_flag |= pm_ldo_volt_level(PMIC_A, PM_LDO_11, 1200 * 1000, TRUE);
      err_flag |= pm_ldo_sw_enable(PMIC_A,PM_LDO_11,PM_ON,TRUE);

      err_flag |= pm_ldo_sw_mode(PMIC_E, PM_LDO_7, PM_SW_MODE_NPM);
      err_flag |= pm_ldo_volt_level(PMIC_E, PM_LDO_7, 1800 * 1000, TRUE);
      err_flag |= pm_ldo_sw_enable(PMIC_E,PM_LDO_7,PM_ON,TRUE);

      err_flag |= pm_ldo_sw_mode(PMIC_E, PM_LDO_10, PM_SW_MODE_NPM);
      err_flag |= pm_ldo_volt_level(PMIC_E, PM_LDO_10, 2904 * 1000, TRUE);
      err_flag |= pm_ldo_sw_enable(PMIC_E,PM_LDO_10,PM_ON,TRUE);
