 ...
 *
 * Copyright (c) 2014-2016, 2018-2019 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
...
----------  ---     ----------------------------------------------------- 
2019-06-26  jt      Add capability check for UHS-1 enablement
   ...
   for ( i = 0; i < SDCC_SD_DETECT_MAX_RETRIES; i++ )
   {
      ...
      { 
         if ( sdcc_cmd.resp[0] & SDCC_HC_CCS )
         {
            ...
            /* SD memory card. If S18A bit is set and capabilities */
            /* indicate 1.8V support, the card is a UHS-1 card. */
            if ( ( sdcc_cmd.resp[0] & SDCC_S18A ) && 
                 ( handle->hdev->pslot->capabilities.HAL_sdhci_Reg_40_42 & HAL_SDHCI_CAPS_VOLTAGE_SUPPORT_1_8V_BMSK ) )
            {
