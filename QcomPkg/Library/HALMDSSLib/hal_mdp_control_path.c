                                                     ...
                                                     uint32                           uFlags)
{
  ...
  if (HAL_MDP_DSC_NONE != pActiveModulesConfig->uDSCId)
  {
    ...
    if (HAL_MDP_FLAGS_CONTROL_PATH_RELEASE_RESOURCES & uFlags)
    {
        out_dword(HWIO_MDP_DSC_CTL_DSC_0_CTL_ADDR + uDSCCtlOffset, HWIO_MDP_DSC_CTL_DSC_0_CTL_INPUT_XBAR_MUX_SEL_BMSK);   /* disable */
    }
    else
    {
  ...
  if (pActiveModulesConfig->u3DMuxId >= HAL_MDP_3DMUX_0 &&
      pActiveModulesConfig->u3DMuxId <= HAL_MDP_3DMUX_2)
  {
