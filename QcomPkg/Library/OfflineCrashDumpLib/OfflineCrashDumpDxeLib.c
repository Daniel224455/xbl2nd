...
/*=======================================================================
 ...
 --------   ----    ---------------------------------------------------
 02/07/19   na      Add force option for on-storage dumps for secure boot testing
...
STATIC BOOLEAN ForceOnStorageDump = FALSE;
...
UpdateOfflineCrashDumpConfigTable( VOID )
{
 ...
 if (!IsAbnormalResetOccurredValid(AbnormalResetOccurredVal))
 {
  ...
  Status = gRT->GetVariable (ENABLE_ONSTORAGE_DUMP_VARNAME,
                             ...
                             &DataSize,
                             &Data);
  if (((Status == EFI_SUCCESS) && (Data == 0x1)) || (ForceOnStorageDump == TRUE)) 
  {
    DEBUG((EFI_D_WARN, "Enabling On-Storage dump\r\n"));
  }
  else
  {
