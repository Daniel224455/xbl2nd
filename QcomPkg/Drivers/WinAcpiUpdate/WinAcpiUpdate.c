...
)
{
	...
	*/
	...
	if (EFI_ERROR(Status)){
		DEBUG((EFI_D_WARN, "[WinAcpiUpdate] Error finding STA method.\n"));
	}
	if (FALSE == isSTA) {
...
)
{
	...
	if (isSTA){
		...
		DEBUG((EFI_D_INFO, "[WinAcpiUpdate] Global variable found!\n"));
...
)
{
	...
	DEBUG((EFI_D_INFO, "[WinAcpiUpdate] Creating UEFI variable... \r\n"));
  ...
  IN UINTN TableKey )
{
	...
	if (EFI_ERROR(Status)) {
		...
		if (Status == EFI_NOT_FOUND){
			...
			DEBUG((EFI_D_INFO, "[WinAcpiUpdate]UpdateDsdtTable: InitFeatureVariable: Created uefi variable with value (0x%x) \r\n", gfeatureBitmask));
		}
	...
	DEBUG((EFI_D_INFO, "[WinAcpiUpdate] gRT->GetVariable: Got uefi variable with value (0x%x) \r\n", gfeatureBitmask));
...
static VOID DisableCPUs(ACPI_MULTIPLE_APIC_DESCRIPTION_TABLE *Blob, UINT8 CPUMask)
{
	...
	UINT32 i=0;
	num=GetTotalNumCPU();
	DEBUG((EFI_D_INFO, "[WinAcpiUpdate]DisableCPUs: CPUMask (0x%x) \r\n", CPUMask));
	for(i=0;i<num;i++)
	{
		if(CPUMask&(Mask<<i))
		{
			if(Blob->LocalGic[i].Flags)
			{
				...
				DEBUG((EFI_D_INFO, "[WinAcpiUpdate]DisableCPUs: Disabling Processor 0x%x Blob->LocalGic[i].Flags =0x%x \r\n",
...
)
{
	...
	else
	{
		...
		DEBUG((EFI_D_INFO, "[WinAcpiUpdate] AcpiMADTCallback: MADT Address (0x%p), Size (0x%08x) \r\n", Madt,*AcpiTableSize));
	}

	CPUDisableMask = (UINT8) GetDefaultDefectivesMask(); // this will be used as fall back mask 
	DEBUG((EFI_D_INFO, "[WinAcpiUpdate] AcpiMADTCallback: Start with Default CPUDisableMask (0x%x) \r\n", CPUDisableMask));
...
*/	
	...
	{
		DEBUG((EFI_D_INFO, "[WinAcpiUpdate]AcpiMADTCallback: GetDefectiveCPUs returned defectiveCPUsBitMask (0x%x) \r\n", defectiveCPUsBitMask));
		CPUDisableMask = (UINT8) defectiveCPUsBitMask;
		DEBUG((EFI_D_INFO, "[WinAcpiUpdate]AcpiMADTCallback: CPUDisableMask (0x%x) after updating with defective CPUs \r\n", CPUDisableMask));
	}
	//CPUDisableMask = 96; //0x60 or 01100000, which is test value to disable 2 of the gold cores (5 and 6) out of 8 total cores
	DEBUG((EFI_D_INFO, "[WinAcpiUpdate]AcpiMADTCallback: Calling DisableCPUs with CPUDisableMask (0x%x)\r\n", CPUDisableMask));
	...
	switch(defectivePart)
		{
			case EFICHIPINFO_PART_GPU:
  			...
  			case EFICHIPINFO_PART_DISPLAY:
				...
				DEBUG((EFI_D_INFO, "[WinAcpiUpdate]AcpiMADTCallback: Marking EFI_FEATURE_GPU for disabling. Updated defectivePartsBitMask to (0x%x) \r\n", defectivePartsBitMask));
				break;
	  		case EFICHIPINFO_PART_CAMERA:
				...
				DEBUG((EFI_D_INFO, "[WinAcpiUpdate]AcpiMADTCallback: Marking EFI_FEATURE_FRONT_CAMERA and EFI_FEATURE_BACK_CAMERA for disabling. Updated defectivePartsBitMask to (0x%x) \r\n", defectivePartsBitMask));
				break;
	  		case EFICHIPINFO_PART_AUDIO:
				defectivePartsBitMask |= (UINT32)(0x1 << EFI_FEATURE_AUDIO);
				DEBUG((EFI_D_INFO, "[WinAcpiUpdate]AcpiMADTCallback: Marking EFI_FEATURE_AUDIO for disabling. Updated defectivePartsBitMask to (0x%x) \r\n", defectivePartsBitMask));
				break;
			default:
				break;
		}
...
GetDefectiveParts(VOID)
{
	EFI_STATUS Status = EFI_SUCCESS;
	UINT32 nMask = 0x0;
	DEBUG((EFI_D_INFO, "[WinAcpiUpdate]GetDefectiveParts: Creating UEFI variable... \r\n"));
...
)
{
   ...
   {
	 DEBUG((EFI_D_INFO, "[WinAcpiUpdate]UpdateACPIEntry: Locate ACPI Protocol succeeded, Status = (0x%x)\r\n", Status));
   }
   ...
   {
	 DEBUG((EFI_D_INFO, "[WinAcpiUpdate]UpdateACPIEntry: ACPI MADT Callback Register succeeded, Status = (0x%x)\r\n", Status));
   }
   ...
   {
	 DEBUG((EFI_D_INFO, "[WinAcpiUpdate]UpdateACPIEntry: Locate ChipInfo Protocol succeeded, Status = (0x%x)\r\n", Status));
   }
