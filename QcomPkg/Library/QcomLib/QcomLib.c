...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 07/24/18   bd      Improve uefi sec app logging
 03/21/18   kpa     Added new API MountFatPartitionExt
 01/25/18	cs		changing Mounting FAT volume log message level to INFO since it is normal case
 10/26/17	mt		Added function to save TZ log  buffer
...
SaveTZAppLogs(VOID)
{
  ...
  CHAR16		LogBufFile[MAX_LEN] = L"TZAppLogs.txt";
  UINT32		DumpToLogFS = 0, BytesPerLoop = 0;
  UINTN     Offset = 0;
  ...
  Status = GetConfigValue ("DumpUefiSecAppLogsToLogFs", &DumpToLogFS);
  if ((Status != EFI_SUCCESS) || (DumpToLogFS == 0))
    goto Exit;
  ...
  Status = WriteFile (LogBufFile,
                      ...
                      (UINT8*)OutputBuffer,
                      BufferSize);  
 DEBUG(( EFI_D_ERROR, "Write Completed for WriteFile \n"));
  if (EFI_ERROR(Status))
  {
     DEBUG(( EFI_D_ERROR, "Write TZApp logs to FAT partition failed, Status =  (0x%x)\r\n", Status));
  }
