...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 14/03/18   mt      Add UefiMemMapCmd 
 11/02/17	  mt		  Added TPL check in BdsStartEfiApplication
  ...
  )
{
  ...
  if (Status == EFI_BUFFER_TOO_SMALL) {
    ...
    if (OrigMemMap != NULL) {
      ...
      Status = gBS->GetMemoryMap (&MemMapSize, MemMap, &MapKey, &DescriptorSize, &DescriptorVersion);
      if (!EFI_ERROR (Status)) {
	  	DEBUG((EFI_D_ERROR, "UEFI Memory Map\n-----------------------------------------------\n"));
        for (Index = 0; Index < MemMapSize/DescriptorSize; Index++) {
          DEBUG((EFI_D_ERROR, "%a  PhysicalStart = %,10lx  VirtualStart = %lx \n",memType[MemMap->Type], MemMap->PhysicalStart, MemMap->VirtualStart));
...
CheckPoolUsage()
{
  ...
  if (Status == EFI_BUFFER_TOO_SMALL) {
    ...
    if (OrigMemMap != NULL) {
      ...
      if (!EFI_ERROR (Status)) {
        for (Index = 0; Index < MemMapSize/DescriptorSize; Index++) {
          Arr[MemMap->Type] += MemMap->NumberOfPages;
          MemMap = NEXT_MEMORY_DESCRIPTOR (MemMap, DescriptorSize);
        }
        
        if(Arr[0] > PcdGet32 (PcdMemoryTypeEfiReservedMemoryType))
          DEBUG((EFI_D_ERROR, "%a Pages used= %d exceeds the limit %d\n", memType[0], Arr[0], PcdGet32 (PcdMemoryTypeEfiReservedMemoryType)));
        if(Arr[1] > PcdGet32 (PcdMemoryTypeEfiLoaderCode))
          DEBUG((EFI_D_ERROR, "%a Pages used= %d exceeds the limit %d\n", memType[1], Arr[1], PcdGet32  (PcdMemoryTypeEfiLoaderCode)));
        if(Arr[2] > PcdGet32 (PcdMemoryTypeEfiLoaderData))
          DEBUG((EFI_D_ERROR, "%a Pages used= %d exceeds the limit %d\n", memType[2], Arr[2], PcdGet32 (PcdMemoryTypeEfiLoaderData)));
        if(Arr[3] > PcdGet32 (PcdMemoryTypeEfiBootServicesCode))
          DEBUG((EFI_D_ERROR, "%a Pages used= %d exceeds the limit %d\n", memType[3], Arr[3], PcdGet32 (PcdMemoryTypeEfiBootServicesCode)));
        if(Arr[4] > PcdGet32 (PcdMemoryTypeEfiBootServicesData))
          DEBUG((EFI_D_ERROR, "%a Pages used= %d exceeds the limit %d\n", memType[4], Arr[4], PcdGet32 (PcdMemoryTypeEfiBootServicesData)));
        if(Arr[5] > PcdGet32 (PcdMemoryTypeEfiRuntimeServicesCode))
          DEBUG((EFI_D_ERROR, "%a Pages used= %d exceeds the limit %d\n", memType[5], Arr[5], PcdGet32 (PcdMemoryTypeEfiRuntimeServicesCode)));
        if(Arr[6] > PcdGet32 (PcdMemoryTypeEfiRuntimeServicesData))
          DEBUG((EFI_D_ERROR, "%a Pages used= %d exceeds the limit %d\n", memType[6], Arr[6], PcdGet32 (PcdMemoryTypeEfiRuntimeServicesData)));
        if(Arr[9] > PcdGet32 (PcdMemoryTypeEfiACPIReclaimMemory))
          DEBUG((EFI_D_ERROR, "%a Pages used= %d exceeds the limit %d\n", memType[9], Arr[9], PcdGet32 (PcdMemoryTypeEfiACPIReclaimMemory)));
        if(Arr[10] > PcdGet32 (PcdMemoryTypeEfiACPIMemoryNVS))
          DEBUG((EFI_D_ERROR, "%a Pages used= %d exceeds the limit %d\n", memType[10], Arr[10], PcdGet32 (PcdMemoryTypeEfiACPIMemoryNVS)));
    }
