...
void LLCC_init (BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel)
{
	...
	LLCC_configure_beac_credit(ddr);
...
void LLCC_cacheable_mode_override_ctrl (CACHEABLE_MODE cachable_mode)
{
	...
	reg_offset_llcc_feac = REG_OFFSET_LLCC_FEAC_BROADCAST;
...
void LLCC_configure_beac_credit (BOOT_DDR_STRUCT *ddr)
{
    ...
    reg_offset_llcc_beac = REG_OFFSET_LLCC_BEAC_BROADCAST;
...
void LLCC_ram_init (BOOT_DDR_STRUCT *ddr)
{
   ...
   reg_offset_llcc_drp = REG_OFFSET_LLCC_DRP_BROADCAST;
   reg_offset_llcc_trp = REG_OFFSET_LLCC_TRP_BROADCAST;
   reg_offset_llcc_pmgr = REG_OFFSET_LLCC_PMGR_BROADCAST;
   reg_offset_llcc_feac = REG_OFFSET_LLCC_FEAC_BROADCAST;
   ...
   HWIO_OUTXF (reg_offset_llcc_drp, LLCC_DRP_PCB_WAKEUP_SEL, SEL, all_pcb_mask); 
   HWIO_OUTXF  (reg_offset_llcc_drp, LLCC_DRP_PCB_CMD, WAKEUP_COMMAND, 0x1);
   
   // check each pcb are wakeup state
   while ((HWIO_INX (reg_offset_llcc_drp, LLCC_DRP_PCBSLP_STATUS) & ddr->shrm_ddr_ptr->misc.pcb_status_mask) != 0x0); 
...
void LLCC_PXI_flush (BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel, uint16 start_inx, uint16 end_inx, uint16 way_vec)
{
    ...
    for (ch = 0 ; ch < NUM_CH ; ch++) 
    {
        ...
        if(((channels >> ch) & 0x1) == 0x0) continue;
         ...
         reg_offset_llcc_trp = REG_OFFSET_LLCC_TRP_CH(ch);
...
void LLCC_PXI_TCM_mode_ctrl (BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel, uint8 act_en) 
{
	...
	reg_offset_llcc_trp = REG_OFFSET_LLCC_TRP_BROADCAST;
	reg_offset_llcc_fewc = REG_OFFSET_LLCC_FEWC_BROADCAST;
	reg_offset_llcc_feac = REG_OFFSET_LLCC_FEAC_BROADCAST;
...
//================================================================================================//
// Apply magic number to LLCC CSR
//================================================================================================//
void LLCC_magic_number(BOOT_DDR_STRUCT *ddr) 
{   
    if (ddr->jtag_connected == 0) {
        // Apply magic number to LLCC CSR if JTAG is dis-connected
        // Workaround of LLCC power collapse issue (QCTDD04043297) for Napali V1  //channel number changed for Poipu
        HWIO_OUTX (REG_OFFSET_LLCC_DRP_CH(7), LLCC_DRP_ECC_ERROR_INJECTION_1, SHUB_COLLAPSE_MAGIC_NUM);
    }
}
