...
ddr_freq_plan_entry* ddr_freq_plan_entry_ptr = NULL;
/*==============================================================================
                                   FUNCTIONS
==============================================================================*/
...
{
  boolean freeze_io_not_set = 0;
  if(clock_configured == TRUE)
  {
    freeze_io_not_set  = !(HWIO_INF(DDRCC_CH01_DDRCC_PCC_PC_STA, PC_FREEZE_IO_0) || HWIO_INF(DDRCC_CH01_DDRCC_PCC_PC_STA, PC_FREEZE_IO_1) ||
                          HWIO_INF(DDRCC_CH23_DDRCC_PCC_PC_STA, PC_FREEZE_IO_0) || HWIO_INF(DDRCC_CH23_DDRCC_PCC_PC_STA, PC_FREEZE_IO_1));
  }
  ddr_set_sdi_struct();

  /*1.memset the 5KB dataram pointer to zero only if clock_configured is FALSE
    2.Skip memset if clock_configured is TRUE
  */
  ...
  /* Below are the checks used to determine if we would fully initialize the DDR or go to Pass 3, 
  in both those cases we need to memset the ddr struct to have a clean start of ddr initialization
  1. Cold boot case - Download mode is not set
  2. SDI cookie indicates Pass 3
  3. DBC PDC was triggered for SDI
  4. First Pass timer expired and freeze IOs were not set: For <= v2.0, we would go to Pass 3 in this case and for >= v2.0.1, we would do HAL_DDR_Init */
...
/*   if ((!dload_mode) || (HWIO_INF(AOSS_CC_RESET_FSM_STATUS, DBG_PDC_TRIGGER)) ||
      (HWIO_INF(AOSS_CC_RESET_FSM_STATUS, FIRST_PASS_EXPIRE) && freeze_io_not_set)) */
...
uint32 ddr_get_training_checksum(uint32 serial_number)
{
  ...
  if (&(ddrsns_share_data->training_results) != NULL)
  {
    ...
    for(i = 0; i < clk_plan_tbl_size; i++) {
	  ...
	  if ((ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.max_ddr_frequency) > (ddrsns_share_data->training_results.stored_ddr_fmax))
  {
    ddr_printf (DDR_NORMAL, "DDR: Training is required- stored DDR max frequency is less than the max DDR frequency from DCB");
    return 0;
  }
...
boolean ddr_set_training_checksum(uint32 serial_number)
{
  ...
  if (&(ddrsns_share_data->training_results) != NULL)
  {
	...
	ddrsns_share_data->training_results.stored_ddr_fmax = ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.max_ddr_frequency;
...
boolean ddr_get_shared_info_details(ddr_details_entry_ptr_type ddr_details_entry_ptr)
{
      ...
      ddr_details_entry_ptr->num_channels = ddrsns_share_data->shrm_ddr_ptr->misc.num_channel;
	  if(ddrsns_share_data->shrm_ddr_ptr->ddr_size_info.ddr_cs1_mb[ddr_details_entry_ptr->num_channels-1]!=0){
		  num_of_ranks = 2;
	  }	  
	  else{
		  num_of_ranks = 1;
	  ...
	  for(channel_no = 0, channel = 1; channel_no < ddrsns_share_data->shrm_ddr_ptr->misc.num_channel; channel_no++, channel = channel << 1)
	  {
	  ...
	  {
		  ddr_details_entry_ptr->ddr_freq_tbl.ddr_freq[clk_idx].freq_khz = ddrsns_share_data->aop_params.mc_states[clk_idx].freq_khz;
		  ddr_details_entry_ptr->ddr_freq_tbl.ddr_freq[clk_idx].enable   = ddrsns_share_data->aop_params.mc_states[clk_idx].freq_enable;
	  }
	  /* fill the current ddr freq addr to UEFI SMEM */
	  ddr_details_entry_ptr->ddr_freq_tbl.num_ddr_freqs = MAX_NUM_CLOCK_PLAN;
