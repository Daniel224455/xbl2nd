...
#define DALPLATFORMINFO_TYPE_MTP_SUBTYPE_RB3 7
typedef enum
{
    PM_SMB_NAME_INVALID,
    PM_SMB_NAME_138x,
...
static boolean gSmbSupported = TRUE;
/*===========================================================================
...
pm_device_pre_init(void)
{
  ...
  if (DAL_SUCCESS == DAL_DeviceAttach(DALDEVICEID_PLATFORMINFO, &phPFormInfo))
  {
     DalPlatformInfo_GetPlatformInfo(phPFormInfo, &PlatformInfo);
  }
   //For RB3 Platform do not do SMB_INIT
  if(DALPLATFORMINFO_TYPE_MTP == PlatformInfo.platform && DALPLATFORMINFO_TYPE_MTP_SUBTYPE_RB3 == PlatformInfo.subtype)
  {
    gSmbSupported = FALSE;	
  }
  else
  ...
  if(PM_SMB_NAME_INVALID == pm_smb_name && TRUE == gSmbSupported)
  { 
    err_flag |= smb_init();
  }
...
pm_sbl_chg_pre_init (void)
{
  ...
  platform_type = DalPlatformInfo_Platform();
  if(platform_type == DALPLATFORMINFO_TYPE_QRD || platform_type == DALPLATFORMINFO_TYPE_HDK)
  {
