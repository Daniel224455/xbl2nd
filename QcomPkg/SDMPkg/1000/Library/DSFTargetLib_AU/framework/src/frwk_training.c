...
)
{
  ...
  else
  {
    ...
    extern  uint32  training_prfs_table[MAX_TRAINING_FREQ_NUM];
    extern  uint32  training_freq_idx[MAX_TRAINING_FREQ_NUM];
    ...
    /*Logging pre_training_array timing and number of accesses for each training*/
    print_training_time(pre_training_array , array_size);
    ...
    /*Logging do_training_array timing and number of accesses for each training*/
    print_training_time(do_training_array , array_size);
    ...
    frwk_loop_training_seq(ddr, 
                           ...
                           training_params_ptr,
                           local_vars);    
    /*Logging post_training_array timing and number of accesses for each training*/
    print_training_time(post_training_array , array_size);
...
{
  uint32 i, dep_struct_idx, start_time, end_time;
  ...
  if (!training_complete[proc[struct_index].index])
  {
    ...
    {      
      ...
      else
      {
        ...
        proc[struct_index].num_of_times_executed++;
        start_time = ddr_external_get_time();
        (proc[struct_index].func_ptr)(ddr, 
                                      ddr->shrm_ddr_ptr->misc.total_channels, 
                                      ddr->shrm_ddr_ptr->detected_ddr_device[TARGET_ACTIVE_CHANNEL].populated_chipselect, 
                                      freq, 
                                      prfs_index, 
                                      training_params_ptr,
                                      local_vars);
        training_complete[proc[struct_index].index] = 1;
        end_time = ddr_external_get_time() ;
        proc[struct_index].trng_total_time =  proc[struct_index].trng_total_time + (end_time - start_time);   
        break;
      }
    }
  }
...
* 
*/
void print_training_time(frwk_training_struct *proc , uint32 array_size )
{
  uint32 struct_index;
  
    while (proc->func_ptr != NULL )
    {
      struct_index = getStructIndex (proc, array_size, proc->func_ptr);
      ddr_printf (DDR_BOOTLOG,"\n Training : %s() ,no_of_times_executed : %d ,total_time : %d", proc[struct_index].func_name,proc[struct_index].num_of_times_executed,proc[struct_index].trng_total_time);
      proc++;
    }
}/*print_training_time*/