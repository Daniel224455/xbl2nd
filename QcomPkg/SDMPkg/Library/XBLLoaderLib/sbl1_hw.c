...
--------   ---       ----------------------------------------------------------
10/12/18   dj	     uncomment reset cookie to instruct TZ do reset
09/11/18   daison    Added DRE Init sequence
...
{
  boolean dre_status = FALSE, dre_test_status = FALSE;  
...
if(sbl1_hw_get_au_power_status())
{
  ...
  {  
      /* Initialize DRE */
      boot_log_message("sbl1_dre_init, Start");
      boot_log_start_timer();
      dre_status = boot_dre_init();
	  boot_log_stop_timer("sbl1_dre_init, Delta");
	  dre_test_status = boot_dre_test();
      /* if DRE init or test fail, Assert*/
      if(dre_status == FALSE || dre_test_status ==FALSE)
      {
          BL_VERIFY(FALSE, BL_ERROR_GROUP_BOOT);
      }
  }
}
...
{
   pm_err_flag_type pm_device_init, pm_driver_init, pm_sbl_chg_init;

  //load second stage devcf gdata via xblconfig
...
void sbl1_ddr_set_params(  bl_shared_data_type *bl_shared_data  )
{
  ...
  if(platform != DALPLATFORMINFO_TYPE_VIRTIO)
  {
	...
	boot_ddr_set_params((boot_dload_is_dload_mode_set()) == TRUE, TRUE); 
...
**  Function :  sbl1_hw_get_au_power_status
...
*
*/
uint32 sbl1_hw_get_au_power_status( void )
{
   return((HWIO_QFPROM_RAW_FEAT_CONFIG_ROW1_MSB_INM(HWIO_QFPROM_RAW_FEAT_CONFIG_ROW1_MSB_AUTOMOTIVE_BIT_FUNCTIONAL_BMSK))>>HWIO_QFPROM_RAW_FEAT_CONFIG_ROW1_MSB_AUTOMOTIVE_BIT_FUNCTIONAL_SHFT);
} 
