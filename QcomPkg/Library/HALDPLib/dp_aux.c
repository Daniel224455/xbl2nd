...
**********************************************************************************************************************/
static DP_Status DP_PackCmd(DP_DeviceIDType       eDeviceId,
...
**********************************************************************************************************************/
static DP_Status DP_WaitAUXDone(DP_DeviceIDType      eDeviceId)
{
  ...
  { 
    HAL_DP_Interrupt_GetStatus((HAL_DP_InterruptType *) &uIntrStatus, 0);

    if (uIntrStatus & DP_AUX_INTERRUPT)
    {
      HAL_DP_AUX_ClearTransaction();
      ...
      HAL_DP_Interrupt_Clear((HAL_DP_InterruptType) uIntrStatus, 0);
                              ...
                              DP_ReadPacketType       *psReadPacket)
{
  ...
  {    
    ...
    eStatus = DP_PackCmd(eDeviceId, &sPackCmd);
        ...
        (FALSE             == psReadPacket->bQueueOnly))
    {
      ...
      for (;;)
      {
        for (uRetryNum = 0; uRetryNum <= DP_AUX_MAX_I2C_RETRY; uRetryNum++)
        {
          ...
          /* Send the command */
          HAL_DP_Write(&sHALWritePacket, 0);

          eStatus =  DP_WaitAUXDone(eDeviceId);
          if (DP_STATUS_SUCCESS == eStatus)
          {
            break;
          }
        ...
        else
        {
          ...
          uAuxPhyCfgIdx++;
          if (HAL_MDSS_STATUS_SUCCESS != HAL_DP_PHY_AUXConfig(&sAuxPhyCfg))
          {
          ...
          /* Reset AUX HW */
          HAL_DP_Reset(HAL_DP_MODULE_TYPE_AUX, 0);
        }
      }
