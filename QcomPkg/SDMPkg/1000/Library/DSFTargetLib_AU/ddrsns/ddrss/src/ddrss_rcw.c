...
//	uint8 num_cs			= ddr->shrm_ddr_ptr.detected_ddr_device.populated_chipselect;
    ...
    // Skip RCW training if not enabled in training_params
    
	//TBD TODO: needs to be removed once DCB is fixed
	//if(training_params_ptr->RCW_Training_En == 0)
    //    return;
	
	//Determine the rcw clock frequency index from the clock plan
    for(rcw_start_clk_idx = 0; rcw_start_clk_idx < ddr->shrm_ddr_ptr->ddr_runtime.ddr_num_clock_levels; rcw_start_clk_idx++)
    {
    ...
    // Skip RCW training if start frequency is greater than max
    if(ddr->shrm_ddr_ptr->ddr_runtime.clock_plan[rcw_start_clk_idx].clk_freq_in_khz > ddr->shrm_ddr_ptr->ddr_runtime.max_ddr_frequency)
    {
        return;
    }
    
    // Initialize RCW params before starting RCW training 
	for(freq_index = rcw_start_clk_idx ; freq_index < ddr->shrm_ddr_ptr->ddr_runtime.ddr_num_clock_levels; freq_index++ )
	{
          ...
          ddrss_rcw_local_vars *local_vars) 
{
    ...
    MC_Cmd_Set_Cfg(ch_offset, CS_1HOT(cs), 0xA, CMD_MPC_RDDQCAL, 0x0, 0x0);
    MC_Cmd_Stagger_Cfg(ch_offset, 0xA, 0x0, 0x0, 0x0);
...
//	}
	...
	MC_MR_Write (ch_offset, CS_1HOT(cs), JEDEC_MR_13, (MR13_BASE)|0xC2);
...
if(! all_found)
{
	...
	MC_MR_Write (ch_offset, CS_1HOT(cs), JEDEC_MR_13,(MR13_BASE)|0xC0);
...
{	
	...
	MC_DTC_base  = REG_OFFSET_MC_DTC_CH(ch);
	...
	for (byte_lane = 0; byte_lane < dq; byte_lane++) 
    {
		if (byte_not_done[byte_lane])		
		{
			...
			MC_Timing_Recalc_Update (REG_OFFSET_MC_CH(ch)); 
		}
	}
}
...
void set_rcw_cdc_delay ( uint32 rcw_delay,  uint16 coarse_fine, uint16 mode, uint8 ch,  uint8 byte_not_done[NUM_DQ_PCH], PHY_BUS dq)
{
   ...
   for (byte_lane = 0; byte_lane < dq; byte_lane++) {
      ...
      DDR_PHY_hal_cfg_cdc_slave_rcw(dq0_ddr_phy_base + (byte_remapping_table[ch][byte_lane] * DDR_PHY_OFFSET), rcw_delay, coarse_fine, mode);
   }
}
   	...
   	//clear counters
		for (indx = 0; indx < dq ; indx++) 
   	 	...
   	 	for (lp = 0; lp < loop_cnt; lp++)  
	 	{
			...
			MC_Cmd_Trigger (REG_OFFSET_MC_CH(ch), 0x0400); // Set the trigger on  CABO_SHKE_CMD_SET_CFG_9,10,11
			//***MC_MR_Read(ch, CS_1HOT(cs), JEDEC_MR_5);
...
void  read_rcw_status (uint8 *byte_one_cnt, uint8 *byte_zero_cnt, PHY_BUS dq, uint8 ch)
{
 ...
 for (byte_indx =0; byte_indx < dq; byte_indx++) {
      ...
      DDR_PHY_rd_mon_status(dq0_ddr_phy_base + (byte_remapping_table[ch][byte_indx] * DDR_PHY_OFFSET)  ,
                         &pass[byte_indx]);
	  (pass[byte_indx] == 1)?
...
void read_tDQSCK( uint8 ch, uint8 cs, uint8 byte_lane, ddrss_rcw_local_vars *local_vars, uint8 coarse_fine_sel)
{
    ...
    if  (coarse_fine_sel == RCW_COARSE_DELAY) 
    {
        ...
        local_vars->rcw_coarse_cdc[ch][cs][byte_lane]  = DDR_PHY_hal_sta_rcw_coarse(dq0_ddr_phy_base + (byte_remapping_table[ch][byte_lane] * DDR_PHY_OFFSET),0);// Read from only Rank0 fields 
        local_vars->rcw_num_cycles[ch][cs][byte_lane] = DDR_PHY_hal_sta_rcw_num_cycle(MC_DTC_base);
    }
    else
    {
         local_vars->rcw_fine_cdc[ch][cs][byte_lane]    = DDR_PHY_hal_sta_rcw_fine(dq0_ddr_phy_base   + (byte_remapping_table[ch][byte_lane] * DDR_PHY_OFFSET),0);// Read from only rank0 fields
    }
}
...
//uint8  freq_index            =  0;
    ...
    for (byte_lane = 0; byte_lane < dq; byte_lane++) 
    {
		...
		{
		    #if DSF_RCW_TRAINING_LOG
		      ddr_printf(DDR_NORMAL,"Ch = %d,Rank = %d, PHY_Byte = %d: \n", ch, rank, byte_remapping_table[ch][byte_lane]);
            #endif
            ...
            {                                          //                   = b1  for cs = DDR_CS0 
			    tDQSCK_in_ps[rank][byte_lane] = ((local_vars->rcw_num_cycles[ch][rank][byte_lane] * rcw_period) +
                  ...
                  ddr_printf(DDR_NORMAL, "Num_cycles: %d, Coarse_cdc:  %d, Fine_cdc: %d, search_value in ps = %d \n",
                                            ...
                                            tDQSCK_in_ps[rank][byte_lane]+rcw_period) ;
                #endif
            }
		}
    ...
    for (prfs_idx = RCW_TRAINING_TABLE_START_INDX; prfs_idx <= HIGHEST_TRAINING_TABLE_INDX; prfs_idx++)
		{			
            ...
            for(rank = 0; rank< NUM_CS; rank++){
                if ((populated_chipselect >> rank) & 0x1)  // populated_chipset = b11 for cs = DDR_CS_BOTH,
                {
                   ddr_printf(DDR_NORMAL, "PRSF = %d  Channel %d  rank = %d  PHY_byte_lane = %d:\t\ttDQSCKin_full_cycle = %d\n",
                                            prfs_idx,
                                            ch, 
                                            rank,
                                            byte_remapping_table[ch][byte_lane],
                  ...
                  // ============================================================================================================================
                     for(rank = 0; rank< NUM_CS; rank++)
                     {
                        ...
                        g_sed_struct_val.training = "RCW";
                        g_sed_struct_val.param_name = "TDQSCK";
                        g_sed_struct_val.param_steps = tDQSCK_perCS_full_cycle [rank];
                        g_sed_struct_val.param_value = (tDQSCK_in_ps[rank][byte_lane]+ rcw_period);
                        ...
                        g_sed_struct_val.freq = ddr->shrm_ddr_ptr->misc.training_freq_in_khz [HIGHEST_TRAINING_TABLE_INDX - prfs_idx];
                        g_sed_struct_val.ch = ch;
                        g_sed_struct_val.cs = rank;
                        g_sed_struct_val.byte_lane = byte_lane;
						if (((populated_chipselect >> rank) & 0x1) && 
                            (((tDQSCK_in_ps[rank][byte_lane]+ rcw_period) <= training_params_ptr->ddr_abort.min_tDQSCK) ||
                            ((tDQSCK_in_ps[rank][byte_lane]+ rcw_period) >= training_params_ptr->ddr_abort.max_tDQSCK)))
                          {
                              g_abort_flag = 1;
                              training_params_ptr->small_eye_abort = 1;
                          }
					 if(((g_abort_flag)&&(training_params_ptr->ddr_abort.rcw_range_enable)) || (g_sed_flag))
						 {
                  ...
                  // ============================================================================================================================
			        ...
			        DDR_PHY_hal_cfg_rcw_QFI_ext_V2( dq0_ddr_phy_base +( byte_remapping_table[ch][byte_lane] * DDR_PHY_OFFSET),populated_chipselect, tDQSCK_perCS_full_cycle,  prfs_idx);
