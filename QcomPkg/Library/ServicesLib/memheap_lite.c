...
static void mem_heap_get_random_num(void*  random_ptr, int random_len)
{
 ...
 if(PRNGML_ERROR_NONE != PrngML_getdata_lite(((uint8*)random_ptr), random_len))
  ...
  if(random_len == 4)
  {
	...
	*ran_num = 0xabcddcbc - magic_num_index;
  }
  else if(random_len == 2)
  {
    uint16 *ran_num = (uint16 *)random_ptr;
	*ran_num = 0xabcd - magic_num_index;
  }
#endif
}
...
)
{
  ...
  boot_log_message("mem_deinit_heap");

  /*verify if this is genioun heap deinit*/
  if (heap_ptr->magic_num_index < MAX_HEAP_INIT && heap_ptr->magic_num)
  {
  ...
  MEMHEAP_ASSERT(magic_num_index != 0);

  debug_deinit++;

  magic_num[heap_ptr->magic_num_index] = 0xdeadcc03 - heap_ptr->magic_num_index;
...
)
{
  ...
  boot_log_message("mem_force_deinit_heap");
