...
--------   ---     --------------------------------------------------------------------------
04/12/18   pbitra  Updates to let display turn on, returning EFI_ChargerExIsPowerOk TRUE for CLS platform
03/14/18   pbitra  Updates to let display turn on, to display BDS menu on screen
12/15/17   dc      Change the behavior of IsOffModeCharging. Return EFI_NOT_SUPPORTED for non CLS platform 
11/09/17   pbitra  Added support for CLS platform
...
)
{
  ...
  Status |= pPmicPwRonProtocol->GetPonReason(PM_DEVICE_0, &PonReasonType);
	if((Status == EFI_SUCCESS) && (PonReasonType.KPDPWR || PonReasonType.HARD_RESET))
  {
  ...
  {
    DEBUG(( EFI_D_INFO, "ChargerExProtocol:: %a (%d) Platform detected. Off Mode Charging is not supported. \r\n", __FUNCTION__, PlatformType));
    return EFI_UNSUPPORTED;
  }
}
