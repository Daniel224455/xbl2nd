  ...
  )
{
  ...
  //Assume always connected for pre-si platforms else detect cable status through PMIC
  //interface
  if (!(Usbfn->PlatType.platform == EFI_PLATFORMINFO_TYPE_UNKNOWN ||
        Usbfn->PlatType.platform == EFI_PLATFORMINFO_TYPE_RUMI    ||
        Usbfn->PlatType.platform == EFI_PLATFORMINFO_TYPE_ADP     ||
        Usbfn->PlatType.platform == EFI_PLATFORMINFO_TYPE_VIRTIO  ||
        Usbfn->PlatType.platform == EFI_PLATFORMINFO_TYPE_OEM))
  {
    // Get cable state
  ...
  )
{
  ...
  if(TRUE == IsAttached) {
    ...
    // Override charger type to be SDP in compliance mode.
    if (Usbfn->CoreNum == USB_CORE_0) {
      UefiVarSize = sizeof(UefiVar);
      Status = gRT->GetVariable(L"UsbfnForceComplianceMode", &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
      if (!EFI_ERROR(Status) && (UefiVar != 0)) {
        DBG(EFI_D_WARN, "USB Device mode compliance override set");
