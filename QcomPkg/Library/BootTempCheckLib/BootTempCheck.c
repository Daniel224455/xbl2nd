...
**
** ========================================================================= */
#define BOOT_TEMP_MSG_LEN 256
static char boot_temp_message[BOOT_TEMP_MSG_LEN];

static volatile uint32 uLoopCount = 0;
   ...
   for ( ; ; )
   {
      ...
      if (tsensResult == TSENS_SUCCESS)
      {
         ...
         if (nDegC > pBootTempCheckBsp->nUpperThresholdDegC)
         {
            ...
            {
               snprintf(boot_temp_message, BOOT_TEMP_MSG_LEN,
              "Temperature is %d degC(too high ) , Going for shutdown", (nDegC) );
               boot_log_message(boot_temp_message);
         ...
         else if (nDegC < pBootTempCheckBsp->nLowerThresholdDegC)
         {
            ...
            {
              snprintf(boot_temp_message, BOOT_TEMP_MSG_LEN,
              "Temperature is %d degC(too low ) , Going for shutdown", (nDegC) );
              boot_log_message(boot_temp_message);
         ...
         else
         {
		   ...
		   {
             snprintf(boot_temp_message, BOOT_TEMP_MSG_LEN,
             "Temperature is %d degC  within the Limits ,continuing boot process , BTM loop count is  %d", nDegC ,uLoopCount );
             boot_log_message(boot_temp_message);
		   }
