                            ...
                            bool32                  bEnable)
{
  ...
  /* 
     ...
     */
  uRegVal = in_dword(uPclkOffset + HWIO_DP_P0CLK_DSC_DTO_ADDR);
  uRegVal = HWIO_OUT_FLD(uRegVal, DP_P0CLK_DSC_DTO, OVERRIDE_ACK, (bEnable ? 0 : 1));
  uRegVal = HWIO_OUT_FLD(uRegVal, DP_P0CLK_DSC_DTO, OVERRIDE_ACK_VALUE, 0);
  out_dword(uPclkOffset + HWIO_DP_P0CLK_DSC_DTO_ADDR, uRegVal);
}
                            ...
                            HAL_DP_StreamAttribType *psStreamAttrib)
{
  ...
  uRegVal = HWIO_OUT_FLD(uRegVal, DP_CONFIGURATION_CTRL, STATIC_DYNAMIC_COUNTER, psStreamAttrib->bSynchronousClk ? 1 : 0);
  uRegVal = HWIO_OUT_FLD(uRegVal, DP_CONFIGURATION_CTRL, ENHANCED_FRAMING,       psStreamAttrib->bEnhancedFraming ? 1 : 0);
  uRegVal = HWIO_OUT_FLD(uRegVal, DP_CONFIGURATION_CTRL, ASSR,                   psStreamAttrib->bASSR ? 1 : 0);
                   ...
                   HAL_DP_MSAPacketType *psMainStream)
{
   ...
   */
  out_dword(uOffset + HWIO_DP_SOFTWARE_MVID_ADDR, psMainStream->uSWMVid * 2);
