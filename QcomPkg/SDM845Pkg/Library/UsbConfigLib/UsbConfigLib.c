...
/*=============================================================================
 ...
 09/29/17   psapin   Add host mode support for CLS secondary port
 ...
 07/31/17   psapin   Add SMMU Stage 1 Support
...
CHAR16                     
gVariableName[USB_CORE_MAX_NUM_SDM_845][MAX_VARIABLE_NAME_SIZE] = {L"UsbConfigPrimaryPort", 
                                                                   L"UsbConfigSecondaryPort"};

UINT8 
gDefaultVariableVal[USB_CORE_MAX_NUM_SDM_845] =  {USB_DUAL_ROLE_SDM_845,  //Auto Detection For Primary port
                                                  USB_DEVICE_MODE_SDM_845}; //Device Mode For Secondary Port

//Variable to set Static Configuration based on first time cable connected
BOOLEAN 
gDualRoleEnable[USB_CORE_MAX_NUM_SDM_845]     =  {FALSE, FALSE};
  ...
  )
{
  ...
  switch (CoreType)
  {
    ...
    default:
      ...
      *CoreNum = (QCOM_USB_CORE_NUM) USB_CORE_MAX_NUM_SDM_845;
    break;
  }
  ...
  )
{
  ...
  if (CoreNum >= (QCOM_USB_CORE_NUM)USB_CORE_MAX_NUM_SDM_845) {
    Status = EFI_INVALID_PARAMETER;
  ...
  )
{
  ...
  default: 
    Status = EFI_INVALID_PARAMETER;
    *CoreType = USB_CONFIG_SSUSB1;
    ERR_CHK("GetUsbHostConfig: Error - CoreNum is %d, but expect < %d", CoreNum, USB_CORE_MAX_NUM_SDM_845);
  }
  ...
  )
{
  ...
  if (gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_845].PeripheralHandle != NULL)
  {
  ...
  gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_845].UsbConfig = gUsbConfigTemplate;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_845].UsbConfig.coreNum = (QCOM_USB_CORE_NUM)USB_CORE_MAX_NUM_SDM_845;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_845].UsbConfig.modeType = USB_INVALID_MODE;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_845].HostHandle = NULL;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_845].PeripheralHandle = NULL;
  ...
  if (!EFI_ERROR(Status) && (UefiVar != 0)) {
    DBG(EFI_D_WARN, "USB force High Speed mode only set for Display port");
    gHighSpeedOnly = TRUE;
  }
  
  // Install the protocol on PeripheralHandle, this is used as an indicator that
  // the UsbConfigInit has been called. 
  Status = gBS->InstallMultipleProtocolInterfaces(&gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_845].PeripheralHandle,
    &gQcomUsbConfigProtocolGuid, &gUsbConfigDevice[USB_CORE_MAX_NUM_SDM_845].UsbConfig,
    NULL);
  ...
  for (Index = 0; Index < USB_CORE_MAX_NUM_SDM_845; Index++) {
  ...
  */
  ...
  if (IS_CLS_PLAT(PlatformInfo.platform)) { 
    ...
    gDefaultVariableVal[USB_CORE_1_SDM_845] = USB_HOST_MODE_SDM_845;
  }
  
  // Check if host mode is supported in UEFI.
  if (!FeaturePcdGet(HostModeSupported)) {
    gUsbConfigDevice[USB_CORE_0_SDM_845].EnableStaticCfg.Enabled = TRUE;
    gUsbConfigDevice[USB_CORE_0_SDM_845].EnableStaticCfg.UsbMode = USB_DEVICE_MODE_SS;
  } 
  else {
    for (Index = 0; Index < USB_CORE_MAX_NUM_SDM_845; Index++) {
      UefiVarSize = sizeof(UefiVar);
      ...
      switch(UefiVar) { 
        case USB_HOST_MODE_SDM_845:
          DBG(EFI_D_WARN, "Host Mode override set on Port Number: %d", Index);
        ...
        case USB_DEVICE_MODE_SDM_845:
          DBG(EFI_D_WARN, "Device Mode override set on Port Number: %d", Index);
          gUsbConfigDevice[Index].EnableStaticCfg.Enabled = TRUE;
      ...
      if (Index == USB_CORE_0_SDM_845 && gUsbConfigDevice[Index].UsbCoreInterface->ModeType == USB_HOST_MODE) {
      ...
      else if (Index == USB_CORE_1_SDM_845 && gUsbConfigDevice[Index].UsbCoreInterface->ModeType == USB_HOST_MODE)
      {
  ...
  )
{
  ...
  if ((CoreNum >= (QCOM_USB_CORE_NUM)USB_CORE_MAX_NUM_SDM_845) || (ModeType >= USB_INVALID_MODE))
  {
    Status = EFI_INVALID_PARAMETER;
  ...
  )
{
  ...
  if ((CoreNum == USB_CORE_0 || CoreNum == USB_CORE_1) && (ModeType != USB_HOST_MODE_XHCI) && (ModeType != USB_DEVICE_MODE_SS))
  {
    Status = EFI_INVALID_PARAMETER;
  ...
  if (gUsbConfigDevice[CoreNum].EnableStaticCfg.Enabled) {
    if (ModeType != gUsbConfigDevice[CoreNum].EnableStaticCfg.UsbMode) {
      DBG(EFI_D_INFO, "Static Config Mode (%d) overrides Client Selected Mode (%d)",
  ...
  )
{
  ...
  // Just return success with Max number of core supported by SDM845
  *MaxCoreNum = USB_CORE_MAX_NUM_SDM_845;
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  *Count = USB_CORE_MAX_NUM_SDM_845;
  ...
  )
{
  ...
  if (CoreNum >= (QCOM_USB_CORE_NUM)USB_CORE_MAX_NUM_SDM_845 || !ModeType) {
    Status = EFI_INVALID_PARAMETER;
    ERR_CHK("invalid parameter");
  }
  ...
  IN QCOM_USB_CORE_NUM    CoreNum)
{
  ...
  //As there is no Lane number for Micro B USB seocnadry Port - Hardcoded, return Success
  if(CoreNum == USB_CORE_1_SDM_845)
  {
    Status = EFI_SUCCESS;
  ...
  )
{
  ...
  // Polling only for core 0 - Hardcoded
  for (Index = 0; Index < USB_CORE_1_SDM_845; Index++) {
