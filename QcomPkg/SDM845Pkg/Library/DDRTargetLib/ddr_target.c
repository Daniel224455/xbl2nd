...
void ddr_target_init()
{
  ...
  if (platform != DALPLATFORMINFO_TYPE_RUMI)
  {
    pm_smps_sw_enable_status (2, PM_SMPS_3,&on_off);
...
boolean ddr_post_init()
{
  ...
  /* Disable 100 MHz from the clock plan */
  ddrsns_share_data->aop_params.mc_states[1].freq_enable = FALSE;
...
void ddr_post_setup()
{
/* With DSF v242.3 onwards, the set min max functionality has been enabled through the DCB Excel files */
#if 0
   uint8 min_ddr_freq = 0;
...
/*					   SCMO CS0  SCMO CS1	       rank_detected  remap_size  BIMC Remapper CS0   BIMC Remapper CS1 */
  ...
  /* 2GB Dual Rank 	 { 0x0,		0x80000000  } */   {DDR_CS_BOTH,   0x800,    0x100000000,		         0},
  /* 3GB Single Rank { 0x0		N/A         } */   {DDR_CS0 ,      0xC00,     0x80000000,		         0},
  /* 3GB Dual Rank	 { 0x0		0x80000000  } */   {DDR_CS_BOTH,   0xC00,    0x100000000,		         0},
  /* 4GB Single Rank { 0x0		N/A	        } */   {DDR_CS0,      0x1000,     0x80000000,		         0},
  /* 4GB Dual Rank	 { 0x0		0x80000000  } */   {DDR_CS_BOTH,  0x1000,    0x100000000,		         0},
  ...
  /* 10GB Dual Rank   { 0x0,   0x200000000 } */ 	{DDR_CS_BOTH,  0x2800,    0x80000000,    			 0}
...
void ddr_remapper(void)
{
  ...
  */
  ...
  if(chip_select == DDR_CS_BOTH){
	  if (ddr_system_size.ddr_cs0[0] == ddr_system_size.ddr_cs1[0])
	  {
		remap_size /= 2;
	  }
	  else
	  {
		  remap_size = ddr_system_size.ddr_cs0[0]+ddr_system_size.ddr_cs0[1]+ddr_system_size.ddr_cs0[2]+ddr_system_size.ddr_cs0[3];
	  }
