...
void LLCC_init (BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel)
{
    ...
    // Apply magic number to LLCC CSR if JTAG is connected
    // Workaround of LLCC power collapse issue (QCTDD04043297) for Napali V1
    if ((ddr->shrm_ddr_ptr->misc.platform_id == MSM_VERSION_NAPALI) && \
        (ddr->shrm_ddr_ptr->misc.chip_version == 0x0100)) {
        LLCC_magic_number(ddr);
    }
}
...
void LLCC_PXI_TCM_mode_ctrl (BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel, uint8 act_en) 
{
	...
	if (act_en)
	{
		...
		if(ddr->shrm_ddr_ptr->misc.chip_version == 0x0100) // Napali V1
		{
...
{   
    if (ddr->jtag_connected == 0) {
        ...
        // Workaround of LLCC power collapse issue (QCTDD04043297) for Napali V1
        HWIO_OUTX (REG_OFFSET_LLCC_DRP(0), LLCC_DRP_ECC_ERROR_INJECTION_1, SHUB_COLLAPSE_MAGIC_NUM);
    }
}
