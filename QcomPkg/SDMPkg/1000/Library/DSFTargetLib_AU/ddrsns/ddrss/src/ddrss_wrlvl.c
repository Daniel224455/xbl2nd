...
{  
	...
	freq_eq_max_training          = training_params_ptr->WRLVL_Training_Switchboard[prfs_index][_WRLVL_CF];
...
void DDRSS_wrlvl_seq ( BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel, uint8 cs, uint32 freq, training_params_t *training_params_ptr, uint8 prfs_index)
{
...
void DDRSS_wrlvl_ca(BOOT_DDR_STRUCT *ddr, uint8 ch, uint8 cs, uint32 freq, training_params_t *training_params_ptr, uint8 prfs_index, uint8 MR2_opcode)
{
  ...
  //-------------------------------------------------------------------------------------
   ...
   //  wrlvl_fine_cdc_step    = training_params_ptr->wrlvl.fine_cdc_step;  
	...
	MC_MR_Write (REG_OFFSET_MC_CH(ch), CS_1HOT(cs), JEDEC_MR_2, (1 << 7) | MR2_opcode);
	...
	//disable CA and CS OEs, enable pull down
	HWIO_OUTXF (ca0_ddr_phy_base + (ca * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_CMIO_PAD_MODE_CFG, SW_CA_PAD_OE_OVR_MODE_DQ, 0x3FF);
	HWIO_OUTXF (ca0_ddr_phy_base + (ca * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_PAD_CNTL_0_CFG, PULL, 0x1);	
	...
	//enable CA and CS OEs, disable pull down
	HWIO_OUTXF (ca0_ddr_phy_base + (ca * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_CMIO_PAD_MODE_CFG, SW_CA_PAD_OE_OVR_MODE_DQ, 0x240);
	HWIO_OUTXF (ca0_ddr_phy_base + (ca * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_PAD_CNTL_0_CFG, PULL, 0x0);	
	...
	MC_MR_Write (REG_OFFSET_MC_CH(ch), CS_1HOT(cs), JEDEC_MR_2, MR2_opcode);
...

{
    ...
    uint32 DQ_PERIODIC_TRAINING_CLK     = 0;
    uint32 DQ_TIMESTAMP_COUNTER_OFFSET  = 0;
    uint8  coarse_delay_ca[NUM_CA_PCH] = {0};
    uint8  coarse_ca_dly_fine_steps[NUM_CA_PCH] = {0};
    uint8  coarse_dqs_dly_fine_steps[NUM_DQ_PCH] = {0};
    int  wrlvl_ck_dqs_delta          = 0;
    int  wrlvl_ck_dqs_delta_ps       = 0;
    //uint8 ca_delay[NUM_CA_PCH]          = {0};   // CA Write level coarse delay
  ...
  else
  {
    ...
    // Disable PAD SW override 
    HWIO_OUTXF (dq0_ddr_phy_base + (dq * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_CMIO_PAD_MODE_CFG, SW_PAD_MODE_DQS, 0);
    HWIO_OUTXF (dq0_ddr_phy_base + (dq * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_CMIO_PAD_MODE_CFG, SW_PAD_MODE_DQ, 0x200);
    ...
    // Disable DQ PAD IE
    HWIO_OUTXF (dq0_ddr_phy_base + (dq * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_CMIO_PAD_IE_CFG, SW_PAD_MODE_IE_DQ, 0x200);
    HWIO_OUTXF (dq0_ddr_phy_base + (dq * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_CMIO_PAD_IE_CFG, SW_PAD_ENABLE_IE_DQ, 0); 
    ...
    ddr_printf(DDR_NORMAL,"\n    WRLVL ch %d rank %d DQ %d coarse = %d  fine = %d  Total = %d ps",
                ...
                for (ca=0;ca<NUM_CA_PCH;ca++) 
                {
                    ...
                    if(cs ==0)
                    {
                     coarse_delay_ca[ca] = HWIO_INXF (ca0_ddr_phy_base  + (ca * DDR_PHY_OFFSET), DDR_PHY_DDRPHY_CMCDCWRLVL_CTL_CFG, HP_COARSE_DELAY_RANK_0);
                    }
                    else
                    {
					 coarse_delay_ca[ca] = HWIO_INXF (ca0_ddr_phy_base  + (ca * DDR_PHY_OFFSET), DDR_PHY_DDRPHY_CMCDCWRLVL_CTL_CFG, HP_COARSE_DELAY_RANK_1);
                    }            
                     coarse_ca_dly_fine_steps[ca] = coarse_delay_ca[ca]* FINE_STEPS_PER_COARSE;
                     coarse_dqs_dly_fine_steps[dq] = coarse_dqs_delay[dq]* FINE_STEPS_PER_COARSE;
                      
                     wrlvl_ck_dqs_delta = (coarse_ca_dly_fine_steps[ca]) - (coarse_dqs_dly_fine_steps[dq]  + fine_dqs_delay[dq]);
                     wrlvl_ck_dqs_delta_ps = ((coarse_delay_ca[ca] * COARSE_STEP_IN_PS) -((coarse_dqs_delay[dq] * COARSE_STEP_IN_PS)+(fine_dqs_delay[dq] * FINE_STEP_IN_PS)));

                        g_sed_struct_val.training = "WRLVL";
                        g_sed_struct_val.param_name = "WRLVL_CK_DQS_DELTA";
                        g_sed_struct_val.param_steps = wrlvl_ck_dqs_delta;
                        g_sed_struct_val.param_value = wrlvl_ck_dqs_delta_ps;
                        ...
                        if ((((g_abort_flag)&& (training_params_ptr->ddr_abort.wrlvl_delta_enable)) || 
                             (g_sed_flag)))// &&
