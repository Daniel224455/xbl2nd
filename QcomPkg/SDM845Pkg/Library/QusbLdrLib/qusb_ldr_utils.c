...
static boolean qusb_fedl_is_chgr_sdp(void)
{
  ...
  while (--charger_det_retry_cnt)
  {
    ...
    if (PM_ERR_FLAG__SUCCESS != pm_err)
    {
      ...
      qusb_log_err__param(DCI_SKIP__CHGR_PORT_DET_FAIL_LOG, pm_err);
      return FALSE;
    }
    ...
    if (0 == typec_status.vbus_sts)
    {
      ...
      qusb_log_err__param(DCI_SKIP__CHGR_PORT_VBUS_LOW_LOG, !typec_status.vbus_sts);
      return FALSE;
    }
  ...
  if (PM_SCHG_USB_APSD_RESULT_STATUS_INVALID == usb_charger_type)
  {
    ...
    qusb_log_err(DCI_SKIP__CHGR_PORT_DET_FAIL_LOG);
    return FALSE;
  }

  if (PM_SCHG_USB_APSD_RESULT_STATUS_SDP_CHARGER != usb_charger_type)
  {
    qusb_log_err__param(DCI_SKIP__CHGR_PORT_NOT_SDP_LOG, usb_charger_type);
    return FALSE;
  }
...
{ 
  // DBG: qusb_uart_log("eud_init_start", 0);
  ...
  // DBG: qusb_uart_log("eud_init_finish", 0);
