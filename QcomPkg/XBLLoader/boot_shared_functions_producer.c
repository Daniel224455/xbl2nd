...
02/07/19   vk      Add error handling
10/06/18   vk      Add functions initialized by RAM dump
...
static boot_shared_functions_type * function_table = (boot_shared_functions_type *)SCL_SBL1_SHARED_FUNCTIONS_TABLE_BASE;

static void boot_ramdump_image_init_check (void)
{
  BL_VERIFY ((function_table->ramdump_image_init_done == TRUE), (BL_ERR_RAM_DUMP_INIT_CHECK_FAIL | BL_ERROR_GROUP_BOOT));
}

#ifndef FEATURE_DEVICEPROGRAMMER_IMAGE
uint32 nvme_get_device_info (uint32 id, struct nvme_device_info *info)
{
  boot_ramdump_image_init_check();
  return function_table->nvme_get_device_info(id, info);
}

struct nvme_handle* nvme_open (uint32 id, uint32 namespace_num, uint32 *error_num)
{
  boot_ramdump_image_init_check();
  return function_table->nvme_open ( id, namespace_num, error_num);
}

uint32 nvme_close (struct nvme_handle *handle)
{
  boot_ramdump_image_init_check();
  return function_table->nvme_close (handle);
}

uint32 nvme_get_namespace_sizes (struct nvme_handle *handle, struct nvme_namespace_sizes *info)
{
  boot_ramdump_image_init_check();
  return function_table->nvme_get_namespace_sizes (handle, info);
}

uint32 nvme_read (struct nvme_handle *handle, void *buff, uint64 lba, uint64 num_blocks)
{
  boot_ramdump_image_init_check();
  return function_table->nvme_read (handle, buff, lba, num_blocks);
}

uint32 nvme_write (struct nvme_handle *handle, void *buff, uint64 lba, uint64 num_blocks)
{
  boot_ramdump_image_init_check();
  return  function_table->nvme_write (handle, buff, lba, num_blocks);
}

uint32 nvme_flush(struct nvme_handle *handle)
{
  boot_ramdump_image_init_check();
  return  function_table->nvme_flush(handle);
}
...
boot_shared_functions_register(void)
{
  ...
  function_table->pm_pwm_enable_pwm    = &boot_pm_pwm_enable_pwm;
  function_table->pm_pwm_set_pwm_value = &boot_pm_pwm_set_pwm_value;
  function_table->pm_pwm_clock_config  = &boot_pm_pwm_clock_config;
  function_table->pm_pwm_glitch_removal= &boot_pm_pwm_glitch_removal;
  function_table->pm_pwm_config_dtest  = &boot_pm_pwm_config_dtest;
  ...
  function_table->pm_gpio_config_digital_output = &pm_gpio_config_digital_output;
  ...
  function_table->Clock_PCIeInit = &Clock_PCIeInit;
  
  /* These are filled by RamDump image */
  function_table->nvme_read = NULL;
  function_table->nvme_write = NULL;
  function_table->nvme_flush = NULL;
  function_table->nvme_open = NULL;
  function_table->nvme_close = NULL;
  function_table->nvme_get_namespace_sizes = NULL;
  function_table->nvme_get_device_info = NULL;
  
  function_table->boot_set_shared_uefi_ram_dump_magic = &boot_set_shared_uefi_ram_dump_magic;
  function_table->boot_get_shared_uefi_ram_dump_magic = &boot_get_shared_uefi_ram_dump_magic;
  ...
  function_table->ramdump_image_init_done = 0;
}
