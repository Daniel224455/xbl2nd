...
extern EFI_PLATFORMINFO_PLATFORM_INFO_TYPE  gPlatformInfo;
...
)
{
    ...
    *pIsChargingSupported = FALSE;
	...
	CHARGER_DEBUG(( EFI_D_WARN, "QcomChargerDxe:: %a PlatformType = %d, Platform SubType = %u \r\n", 
						__FUNCTION__, gPlatformInfo.platform, gPlatformInfo.subtype));
    
	Status = ChargerLib_IsChargingSupported(pIsChargingSupported);
	/* Return False if Platform type is MTP and Subtype is RB3 HW */
	if ((gPlatformInfo.platform == EFI_PLATFORMINFO_TYPE_MTP) && (gPlatformInfo.subtype == EFI_PLATFORMINFO_TYPE_MTP_SUBTYPE_RB3)) {
		*pIsChargingSupported = FALSE;
		CHARGER_DEBUG(( EFI_D_WARN, "QcomChargerDxe:: %a PlatformType is RB3 \r\n", __FUNCTION__));
	}
