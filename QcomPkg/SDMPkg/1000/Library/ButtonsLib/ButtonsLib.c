...
//KeyMap for CDP
KEY_TYPE KeyMap_1000_CDP[ NUMBER_OF_KEYS] =
{
   PWR, VOL_UP, VOL_DOWN, HOME, NONE
};

//KeyMap for MTP and FLUID
KEY_TYPE KeyMap_1000_MTP_FLUID[ NUMBER_OF_KEYS] =
{
   PWR, VOL_UP, VOL_DOWN, HOME, NONE
};

//KeyMap for LIQUID
KEY_TYPE KeyMap_1000_LIQUID[ NUMBER_OF_KEYS] =
{
   PWR, VOL_UP, VOL_DOWN, HOME, NONE
};

//KeyMap for ADP
KEY_TYPE KeyMap_1000_ADP[ NUMBER_OF_KEYS] =
{
   PWR, VOL_UP, VOL_DOWN, HOME, NONE
};

//KeyMap for QRD
KEY_TYPE KeyMap_1000_QRD[ NUMBER_OF_KEYS] =
{
   PWR, VOL_UP, VOL_DOWN, NONE, NONE
};

//KeyMap for CLS
KEY_TYPE KeyMap_1000_CLS[ NUMBER_OF_KEYS] =
{
   PWR, VOL_UP, VOL_DOWN, NONE, NONE
};
   ...
   )
{
   ...
   case EFI_PLATFORMINFO_TYPE_CDP:
      CopyMem( pKeyMap, KeyMap_1000_CDP, NUMBER_OF_KEYS*sizeof(KEY_TYPE) );
      break;
   case EFI_PLATFORMINFO_TYPE_MTP:
   case EFI_PLATFORMINFO_TYPE_FLUID:
      CopyMem( pKeyMap, KeyMap_1000_MTP_FLUID, NUMBER_OF_KEYS*sizeof(KEY_TYPE) );
      break;
    case EFI_PLATFORMINFO_TYPE_ADP:
#ifdef ENABLE_AUTO_PLAT
    case EFI_PLATFORMINFO_TYPE_OEM:
#endif
      CopyMem( pKeyMap, KeyMap_1000_ADP, NUMBER_OF_KEYS*sizeof(KEY_TYPE) );
      break;
   case EFI_PLATFORMINFO_TYPE_LIQUID:
      CopyMem( pKeyMap, KeyMap_1000_LIQUID, NUMBER_OF_KEYS*sizeof(KEY_TYPE) );
      break;
   case EFI_PLATFORMINFO_TYPE_QRD:
      CopyMem( pKeyMap, KeyMap_1000_QRD, NUMBER_OF_KEYS*sizeof(KEY_TYPE) );
      break;
   case EFI_PLATFORMINFO_TYPE_CLS:
      CopyMem( pKeyMap, KeyMap_1000_CLS, NUMBER_OF_KEYS*sizeof(KEY_TYPE) );
      break;
   default:
      Status = EFI_INVALID_PARAMETER;
      goto ErrorExit;
...
**/
EFI_STATUS EnableInput ( UINT16 GpioNumber )
{
  EFI_STATUS Status = EFI_SUCCESS;

  if(PmicGpioProtocol)
  {
     Status = PmicGpioProtocol->ConfigDigitalInput( 0,
...
EFI_STATUS ConfigureButtonGPIOs ( VOID )
{
   ...
   // volume up
   Status = EnableInput(VOLUME_UP_BUTTON_GPIO);
   if ( EFI_ERROR (Status) )
   {
   ...
   if( (PlatformType == EFI_PLATFORMINFO_TYPE_MTP)||
       ...
       (PlatformType == EFI_PLATFORMINFO_TYPE_OEM))
   {
     // Camera Snapshot
     if (CAMERA_SNAPSHOT_BUTTON_GPIO != NOT_SUPPORTED) 
     {
       Status = EnableInput(CAMERA_SNAPSHOT_BUTTON_GPIO);
       if ( EFI_ERROR (Status) )
       {
   ...
   {
     //home
     Status = EnableInput(HOME_BUTTON_GPIO);
     if ( EFI_ERROR (Status) )
     {
