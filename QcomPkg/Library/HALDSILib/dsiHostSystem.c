  ...
  else
  {
    ...
    */
    ...
    else
    {
      ...
      /* Setup clock divisor for escape clock */
      if ((psDisplayConfig->sDisplayInfo.uEscapeSource  == 0) &&
          (psDisplayConfig->sDisplayInfo.uEscapeDivisor >  0))
      {
        // Source from CXO and setup the divider
        MDPClockEntry *pClockList  = MDPGetClockList(eDSIClockId);
        pClockList[0].nFreqHz      = ((DSIPLL_REF_CLOCK * 2) + (psDisplayConfig->sDisplayInfo.uEscapeDivisor / 2)) / psDisplayConfig->sDisplayInfo.uEscapeDivisor;
      }
      else if (psDisplayConfig->sDisplayInfo.uEscapeDivisor > 0)
      {
