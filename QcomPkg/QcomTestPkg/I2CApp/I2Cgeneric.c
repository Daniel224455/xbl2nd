...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 08/01/18	nm		core number as per ipcat
 07/31/18	nm		Shows detect in help
...
static void i2c_test_cmd_help (void)
{
  ...
  DEBUG((EFI_D_ERROR,"                             **This is offset by 1 from the IPCAT number,\n"));
  DEBUG((EFI_D_ERROR,"                             which starts from 0**\n"));
  ...
  DEBUG((EFI_D_ERROR,"  -d, --detect <0/1>         To scan the entire bus for any live slaves\n"));
  ...
  DEBUG((EFI_D_ERROR,"  // Perform a Bus Scan\n"));
  DEBUG((EFI_D_ERROR,"  start I2CTest -c 9 -d 1\n\n"));
}
...
i2c_status i2c_bus_scan (uint32 i2c_core_number)
{
  ...
  istatus = i2c_open((i2c_instance) (i2c_core_number), &i2c_handle);
...
i2c_status i2c_test_slave (UINT32 bus_scan)
{
  ...
  /* Initialize the I2C Core for eeprom */
  istatus = i2c_open((i2c_instance) (sctxt.i2c_core_number), &i2c_handle);
  if (I2C_SUCCESS != istatus)
  {
    DEBUG((EFI_D_ERROR, "Failed to initialize I2C %d\n", istatus));
    goto error;
