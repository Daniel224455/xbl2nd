...
pm_err_flag_type pm_sbl_chg_check_weak_battery_status(uint32 device_index)
{
   ...
   while( vbatt_weak_status == TRUE )  //While battery is in weak state
   {
      ...
      err_flag |= pm_sbl_schg_config_red_led(device_index, toggle_led, TRUE);

   }//while

   toggle_led = FALSE;
   err_flag |= pm_sbl_schg_config_red_led(device_index, toggle_led, FALSE); //Ensure LED is turned of at the end of SBL charging
...
pm_app_auto_power_on_check(uint32 device_index)
{
   ...
   if (err_flag != PM_ERR_FLAG_SUCCESS)
   {
      pm_log_message("ERROR, pm_schg_misc_get_sysok_reason()");
      return err_flag;
   }

   if(((*((uint8*)&pon_reason))==0x20) && (sysok_reason == PM_SCHG_MISC_SYSOK_REASON__USBIN) &&(on_reason.warm_seq == FALSE))
   {
