/** @file RpmhDxe.c
  ...
  Copyright (c) 2016-2017, Qualcomm Technologies, Inc. All rights 
  reserved. 
...
* @return       EFI_SUCCESS
 */
STATIC void EFI_rpmh_psleep(psleep_cb_mode_enum mode)
{
  switch(mode)
  {
    case PSLEEP_CB_C4D4_ENTER :
      rpmh_enter_sleep(RSC_DRV_HLOS);
      break;
    case PSLEEP_CB_C4D4_EXIT :
...
EFI_STATUS EFIAPI RpmhDxeEntryPoint(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
  ...
  if(GuidHob != NULL)
  {
    ...
    psleep_ptr->RegisterSleepCb((psleep_cb_fnc)EFI_rpmh_psleep, NULL);
  }
