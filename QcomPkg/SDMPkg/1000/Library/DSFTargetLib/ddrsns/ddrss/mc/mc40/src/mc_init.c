...
void MC_Config(BOOT_DDR_STRUCT *ddr)
{
    DDRSS_set_config(ddr, REG_OFFSET_MC_BROADCAST,
        (uint32(*)[2])((size_t)(ddr->dcb_ptr) + (*ddr->dcb_ptr)[MC_CFG_IDX][0]));

    // Set LOAD_CONFIG = 1 for each module for above settings to take effect
    // No LOAD_CONFIG for DTC since timing parameters will be re-calculated in frequency switch
    
    HWIO_OUTX(REG_OFFSET_MC_GLOBAL_BROADCAST, GLOBAL_LOAD_CONFIG, 0x1); 
    while(HWIO_INX(REG_OFFSET_MC_GLOBAL_BROADCAST, GLOBAL_LOAD_CONFIG) != 0x0);

    HWIO_OUTX(REG_OFFSET_MC_ISU_BROADCAST, ISU_LOAD_CONFIG, 0x1); 
    while(HWIO_INX(REG_OFFSET_MC_ISU_BROADCAST, ISU_LOAD_CONFIG) != 0x0);

    HWIO_OUTX(REG_OFFSET_MC_CBU_TOP_BROADCAST, CBU_TOP_LOAD_CONFIG, 0x1); 
    while(HWIO_INX(REG_OFFSET_MC_CBU_TOP_BROADCAST, CBU_TOP_LOAD_CONFIG) != 0x0);

    HWIO_OUTX(REG_OFFSET_MC_MPE_BROADCAST, MPE_LOAD_CONFIG, 0x1); 
    while(HWIO_INX(REG_OFFSET_MC_MPE_BROADCAST, MPE_LOAD_CONFIG) != 0x0);

    HWIO_OUTX(REG_OFFSET_MC_SHKE_BROADCAST, SHKE_LOAD_CONFIG, 0x1); 
    while(HWIO_INX(REG_OFFSET_MC_SHKE_BROADCAST, SHKE_LOAD_CONFIG) != 0x0);

    HWIO_OUTX(REG_OFFSET_MC_OSU_BROADCAST, OSU_LOAD_CONFIG, 0x1); 
    while(HWIO_INX(REG_OFFSET_MC_OSU_BROADCAST, OSU_LOAD_CONFIG) != 0x0);
}

void MC_Pre_Init_Setup_1(BOOT_DDR_STRUCT *ddr)
{
    uint8  clk_idx         = 0;
    uint8  params_idx      = 0;
    uint8  odt_en          = 0;   
    ...
    uint8  wr_dbi_en       = 0;
    uint32 clk_in_ps       = 0;   
    uint32 reg_offset_dtc  = REG_OFFSET_MC_DTC_BROADCAST;
...
void MC_Pre_Init_Setup_2(BOOT_DDR_STRUCT *ddr)
{
    ...
    MC_Cmd_Set_Cfg(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, 0, CMD_POWER_DOWN_PDE, 0x0, 0x0);
    MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_BROADCAST, 0, 0x0, 0x0, 0x0);

    // 5. Configure the HW to turn CK OFF that got turned ON as part of freq switch
    MC_Cmd_Set_Cfg(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, 1, CMD_INIT_CK_OFF, 0x0, 0x0);
    MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_BROADCAST, 1, 0x0, 0x0, 0x0);
    ...
    MC_Cmd_Trigger(REG_OFFSET_MC_BROADCAST, 0x3); // Selecting commands 0-1
...
void MC_Memory_Device_Init(BOOT_DDR_STRUCT *ddr)
{
    uint8  clk_idx     = 0;
    uint8  latency_idx = 0;
    uint8  params_idx  = 0;
    ...
    MC_Cmd_Set_Cfg(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, 0, CMD_NOP, 0, 0);
    // clk_sel = 0x1 -> sleep clk (32KHz), interval = 0x40 -> 64*(1/32KHz) = 2ms
    // clk_sel = 0x1 -> sleep clk (32KHz), interval = 0x41 -> 65*(1/32KHz) > 2ms
    MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_BROADCAST, 0, 0x0, WAIT_SLEEP_CLOCK, 0x41);
    ...
    MC_Cmd_Set_Cfg(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, 1, CMD_INIT_CK_ON, 0, 0); 
    // clk_sel = 0x2 -> core clk, interval = 0x5 -> 5tCK
    MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_BROADCAST, 1, 0x0, WAIT_CORE_CLOCK, 0x5); 

    // 9. Program the CKE to be asserted high and configure the HW to wait for 
    // tINIT5 (2us) before executing event in step 10
    MC_Cmd_Set_Cfg(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, 2, CMD_POWER_DOWN_PDX, 0, 0); 
    // clk_sel = 0x0 -> XO clk (19.2MHz), interval = 0x27 -> 39*(1/19.2MHz) = 2us 
    MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_BROADCAST, 2, 0x0, WAIT_XO_CLOCK, 0x27); 

    // 10. Program the MRW execution to update nWR, RL/WL and DBI in the DRAM 
    // as per the DDR frequency
    MC_Cmd_Set_Cfg(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, 4, CMD_MRW, JEDEC_MR_2,
        ddr->shrm_ddr_ptr->ddr_runtime.dram_latency[latency_idx].MR2);
    MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_BROADCAST, 4, 0x0, 0x0, 0x0);
    MC_Cmd_Set_Cfg(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, 5, CMD_MRW, JEDEC_MR_1,
    MC_MR1_Value(ddr->shrm_ddr_ptr, REG_OFFSET_MC_BROADCAST, latency_idx, params_idx));
    MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_BROADCAST, 5, 0x0, 0x0, 0x0);
    MC_Cmd_Set_Cfg(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, 6, CMD_MRW, JEDEC_MR_3,
        ddr->shrm_ddr_ptr->ddr_runtime.mc_freq_switch[params_idx].MR3);
    MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_BROADCAST, 6, 0x0, 0x0, 0x0);

    // 11. Program for the ZQ Calibration Start command and 
    // configure HW to wait for tZQCAL (1us) time before executing step 12
    MC_Cmd_Set_Cfg(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, 13, CMD_MPC_ZQCAL_START, 0, 0); 
    // stagger_ranks_en = 0x1 -> for shared ZQ resistor 
    // clk_sel = 0x0 -> XO clk (19.2MHz), interval = 0x14 -> 20*(1/19.2MHz) = 1us = tZQCAL_in_ps
    MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_BROADCAST, 13, 0x1/* for shared ZQ resistor */, WAIT_XO_CLOCK, 0x14); 

    // 12. Program for the ZQ Calibration Latch command */
    MC_Cmd_Set_Cfg(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, 14, CMD_MPC_ZQCAL_LATCH, 0, 0); 
    MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_BROADCAST, 14, 0x0, 0x0, 0x0);
    ...
    for(ch = 0; ch < NUM_CH; ch++)
    {
        ...
        MC_Cmd_Trigger(REG_OFFSET_MC_CH(ch), 0x6077);
    }
    ...
    MC_MR_Write(REG_OFFSET_MC_BROADCAST, DDR_CS_BOTH, JEDEC_MR_22, 0x0);   
}
                                  ...
                                  uint8 cs, uint32 mr8_value)
{
   ...
   if (device_io_width == DEVICE_WIDTH_IS_x16) { /* x16 mode */
      ddr->shrm_ddr_ptr->detected_ddr_device[ch].device_density_in_Gb[cs]  = lpddr_size_table[device_density][0][0] >> 7;
   }
   else if (device_io_width == DEVICE_WIDTH_IS_x8) { /* x8 mode */
      ddr->shrm_ddr_ptr->detected_ddr_device[ch].device_density_in_Gb[cs]  = lpddr_size_table[device_density][1][0] >> 7;
   }
...
void MC_DDR_Topology_Detection(BOOT_DDR_STRUCT *ddr, DDR_CHIPSELECT chip_select)
{
    uint8  ch              = 0;
    uint8  cs              = 0; 
    ...
    for(ch = 0; ch < NUM_CH; ch++)
    {
        ...
        reg_offset_mpe  = REG_OFFSET_MC_MPE_CH(ch);
        ...
        for(cs = 0; cs < NUM_CS; cs++)
        {
            ...
            MC_Cmd_Set_Cfg(REG_OFFSET_MC_CH(ch), cs_1hot, 0, CMD_MRR, JEDEC_MR_8, 0);
            MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_CH(ch), 0, 0, 0, 0);   
            ...
            MC_Cmd_Trigger(REG_OFFSET_MC_CH(ch), 0x1);
            ...
            {
                ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].num_rows[cs]        = 0;
                ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].num_cols[cs]        = 0;
                ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].num_banks[cs]       = 0;
                ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].device_io_width[cs] = 0;
                ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].device_density[cs]  = 0;
                
                // Disable CKE to the unpopulated rank
                MC_Cmd_Set_Cfg(REG_OFFSET_MC_CH(ch), cs_1hot, 0, CMD_POWER_DOWN_PDE, 0, 0); 
                MC_Cmd_Stagger_Cfg(REG_OFFSET_MC_CH(ch), 0, 0x0, 0x0, 0x0);
                MC_Cmd_Trigger(REG_OFFSET_MC_CH(ch), 0x1);
            }
            else
            {
                ...
                if(cs == 0)
                {
                    ...
                    ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].manufacturer_id =
                          (MC_MR_Read(REG_OFFSET_MC_CH(ch), DDR_CS0, JEDEC_MR_5) & 0xFF);
                }
                else if(cs == 1)
                {
                ...
                ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].revision_id_1[cs] =
                    (MC_MR_Read(REG_OFFSET_MC_CH(ch), cs_1hot, JEDEC_MR_6) & 0xFF);
                    
                // Read revision ID-2 (MR7)
                ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].revision_id_2[cs] =
                    (MC_MR_Read(REG_OFFSET_MC_CH(ch), cs_1hot, JEDEC_MR_7) & 0xFF);
                    
#ifdef VELOCE
               // For Veloce simulation, MR8 always returns 0.  Force the DDR size to a good value.
               mr8_value = 0x10;  
#endif // #ifdef VELOCE							   
                              
               MC_DDR_Geometry_Detection(ddr, ch_index, cs, mr8_value);
         ...
         ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].populated_chipselect = populated_chipselect;
    ...
    for(ch = 0; ch < NUM_CH; ch++)
    {
        ...
        if(ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].populated_chipselect != populated_chipselects_all_ch)
        {
            ddr_printf(DDR_ERROR, "\nDDR ABORT: RANKS DETECTED NOT THE SAME FOR ALL CHANNELS, Channel = %d, Populated CS = %d\n", ch, ddr->shrm_ddr_ptr->detected_ddr_device[ch_index].populated_chipselect);
            ddr_abort();
        }
	ch_index++;
    ...
    ddr->shrm_ddr_ptr->misc.not_frd = populated_chipselects_all_ch;
}
...
void MC_DDR_Addr_Setup (BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel, DDR_CHIPSELECT chip_select)
{
    ...
    {
       ch_index = 0;
       if ((chip_select & CS_1HOT(cs)) != 0)
       { // Rank exists   
          // Assume the same density for Rank 0 in all channels
          num_rows        = ddr->shrm_ddr_ptr->detected_ddr_device[TARGET_ACTIVE_CHANNEL].num_rows[cs];
          num_cols        = ddr->shrm_ddr_ptr->detected_ddr_device[TARGET_ACTIVE_CHANNEL].num_cols[cs];
          num_banks       = ddr->shrm_ddr_ptr->detected_ddr_device[TARGET_ACTIVE_CHANNEL].num_banks[cs];
          device_density  = ddr->shrm_ddr_ptr->detected_ddr_device[TARGET_ACTIVE_CHANNEL].device_density[cs];
          device_io_width = ddr->shrm_ddr_ptr->detected_ddr_device[TARGET_ACTIVE_CHANNEL].device_io_width[cs];
          ...
          if (cs == 0)
          {
             ...
             for(ch = 0; ch < NUM_CH; ch++)
             {
                // Skip channels which are not enabled
                 ...
                 ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_mb[ch_index]       = size_in_mb; 
                 ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_io_width[ch_index] = device_io_width; 
                 ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_addr[ch_index]     = base_addr;
				 active_num_ch++;
                                 ch_index++;
             }
          ...
          else if (cs == 1)
          {
             ...
             base_addr = MC_BASE_ADDR + ch_cs0_size * active_num_ch;
             //Updated ddr_size_info struct for Rank1
             for (ch = 0; ch < NUM_CH; ch++) {
                if (((channels >> ch) & 0x1) == 0x1) {
                   ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_mb[ch_index]       = size_in_mb; 
                   ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_io_width[ch_index] = device_io_width; 
                   ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_addr[ch_index]     = base_addr;
				    ch_index++;
          ...
          switch(active_num_ch)
		  {
            case 8 : interleave_shift = 3;
               break;
         ...
         addr_base_9_2 = (base_addr >> interleave_shift) >> 26;
          ...
          reg_offset_global_sec = REG_OFFSET_MC_GLOBAL_SEC_BROADCAST;
          ...
          MC_Timing_Recalc_Update(REG_OFFSET_MC_BROADCAST);
...
void MC_Post_Init_Setup (BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel)
{
   ...
   vendor = ddr->shrm_ddr_ptr->detected_ddr_device[TARGET_ACTIVE_CHANNEL].manufacturer_id;
   // Check mode register 0 to see if the device supports LPDDR4Y
   mr0_value = MC_MR_Read (REG_OFFSET_MC_BROADCAST, DDR_CS0, JEDEC_MR_0);
   ...
   if (vendor == SAMSUNG)
   {
      ...
      {
         ddr->shrm_ddr_ptr->ddr_runtime.lpddr4y_support &= LPDDR4Y_LOW_SPEED_CA_BUFFER | LPDDR4Y_SINGLE_ENDED_CLOCK | LPDDR4Y_SINGLE_ENDED_WRITE_DQS | LPDDR4Y_SINGLE_ENDED_READ_DQS;
      }
      else
      {
         // Some Samsung devices will support a subset of LPDDR4Y.  Check MR7 to see.
		 mr7_value = MC_MR_Read (REG_OFFSET_MC_BROADCAST, DDR_CS0, JEDEC_MR_7);
   ...
   for(ch = 0; ch < NUM_CH; ch++)
   {
         // Skip channels which are not enable
		    if(((channels >> ch) & 0x1) == 0x0) continue;
			
			      reg_offset_shke = REG_OFFSET_MC_SHKE_CH(ch);

               HWIO_OUTX (reg_offset_shke, SHKE_ARB_GLOBAL_TIMER_MATCH, (0x40*ch));
               HWIO_OUTX (reg_offset_shke, SHKE_ARB_GLOBAL_TIMER_HP_MATCH, (0x20 + (0x40*ch)));

   }
   
   reg_offset_shke = REG_OFFSET_MC_SHKE_BROADCAST;
   reg_offset_mpe  = REG_OFFSET_MC_MPE_BROADCAST;
   ...
   for (cs = 0; cs < NUM_CS; cs++) {
      if ((chip_select & CS_1HOT(cs)) != 0) { // Rank exists   
         ...
         if (device_density >= 0x3) {
            ...
            MC_MR_Write(REG_OFFSET_MC_BROADCAST, CS_1HOT(cs), JEDEC_MR_4, 0xB);
         }
      } // end of if(rank exists)
   } // end of for(cs)
   ...
   /*Initialize the pasr cmd logging rolling buffer with 0 */	
   for(i = 0; i < 4; i++){
	  ddr->shrm_ddr_ptr->misc.pasr_cmd_logging[i] = 0;   
   }	   
...
void MC_Post_Training_Init_Setup (BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel, uint8 enable)
{
   DDR_CHIPSELECT chip_select  = 0;
...
//   MC_Watchdog_Reset_Setup (ddr, chip_select);
}
