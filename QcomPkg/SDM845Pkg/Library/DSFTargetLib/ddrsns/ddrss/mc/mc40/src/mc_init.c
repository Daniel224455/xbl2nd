...
void MC_Memory_Device_Init (BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel)
{
   ...
   do { 
      if ((ch == BROADCAST_CH_NUM) || (((channel >> ch) & 0x1) == 0x1)) {
         ...
         MC_Cmd_Stagger_Cfg(ch, 0, 0x0, WAIT_SLEEP_CLOCK, 0x41);
                                  ...
                                  uint8 cs, uint32 mr8_value)
{
   ...
   if (device_io_width == DEVICE_WIDTH_IS_x16) { /* x16 mode */
      ...
      // Check if this rank supports byte mode
      if((BYTE_MODE_SUPPORTED_CHIPSELECTS & CS_1HOT(cs)) == 0)
      {
         ddr_printf(DDR_ERROR, "\nDDR ABORT: CHIP SELECT DOES NOT SUPPORT BYTE MODE, Channel = %d, CS = %d\n", ch, cs);
         ddr_abort();
      }
...
void MC_DDR_Addr_Setup (BOOT_DDR_STRUCT *ddr, DDR_CHANNEL channel, DDR_CHIPSELECT chip_select)
{
   ...
   if((ddr->shrm_ddr_ptr->detected_ddr_device[0].manufacturer_id == SAMSUNG) && \
                                (ddr->shrm_ddr_ptr->detected_ddr_device[0].device_io_width[0]== DEVICE_WIDTH_IS_x8))
    {
        //We need to set RL and MR different for a 10GB samsung device  
        //Calculate the size of the detected DDR 
        if((num_ch*(ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_mb[0]+ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_mb[0])) == 0x2800)
        {
            ddr->shrm_ddr_ptr->ddr_runtime.dram_latency[6].RL_DBI_Off_x8 = 32;
            ddr->shrm_ddr_ptr->ddr_runtime.dram_latency[6].MR2 = 0x35;
        }
        else //ddr size is not 10 GB, we need to abort
        {
            ddr_abort();
        }
    }    
