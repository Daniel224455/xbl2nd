...
/*=============================================================================
 ...
 --------     ---     -----------------------------------------------------------
 06/01/18     pbitra  Replaced missing call to QcomChargerAppEvent_DisplayPanelOn 
                      after LPM exit call
 03/22/18     pbitra  Separated display ON functionality from Exit LPM function
...
//This function gets called before exiting the charging loop, either to shutdown or to boot to HLOS.
...
EFI_STATUS QcomChargerAppEvent_ReportStatusCode(EFI_GUID gCurrentGUID, LPM_STATE LpmState)
{
  ...
  IN EFI_GUID                 CallerId = gQcomChargerProtocolGuid ;
  ...
  if (EFI_PLATFORMINFO_TYPE_CLS == PlatformType )
  {
	  CallerId = gSmartChargerProtocolGuid ;
