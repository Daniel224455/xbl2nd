...
#define MASK_ERROR_BYTE 0xFF000000
...
uint32 corrupt_err_code[] = {BL_ERROR_GROUP_BOOTELF,
        BL_ERROR_GROUP_SECURITY,
				BL_ERROR_GROUP_CRYPTO,
				BL_ERROR_GROUP_SECIMG};
...
)
{
  ...
  {
    if((err_code & MASK_ERROR_BYTE) == corrupt_err_code[i])
      return TRUE;
  }
...
{   
	  if(boot_check_error_code(err_code) == TRUE)
	  {
	  	    partition_type = boot_check_backup_partition_cookie();

		    if (partition_type == PARTITION_DEFAULT)
		    {
    	     boot_set_backup_partition_cookie(PARTITION_B);
		       boot_log_message("Switching to boot chain B.");
		       boot_hw_reset(BOOT_HARD_RESET_TYPE);
		    }

		    else if(partition_type == PARTITION_B)
		    {
     	    boot_set_backup_partition_cookie(PARTITION_A);
		      boot_log_message("Switching to boot chain A.");
		      boot_hw_reset(BOOT_HARD_RESET_TYPE);
		    }

		    else
		    {
    	      boot_log_message("Both A and B corrupted. Going into DLOAD mode.");
		    }
	  }
}
  ...
  */
  if((MAX_BOOTLOG_SECONDARY_ENTRIES + 1) >= error_handler_entry_count)
  {
    ...
    {
	  if ((exc_context_ptr->exc_cookie == SBL1_EXC_CONTEXT_VALID_COOKIE) && (ramdump_allowed))
  	  {
      ...
      fp =  boot_crash_dump_data.regs[29];
  	  ...
  	  uint64 i = 0;
      while((fp!=0 && i < MAX_CALLSTACK_PRINT)&&(fp < SBL1_STACK_BASE)&&(fp > SCL_SBL1_STACK_BASE)){
      i++;
