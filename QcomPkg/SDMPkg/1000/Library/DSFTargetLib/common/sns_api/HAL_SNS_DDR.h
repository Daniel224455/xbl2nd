...
// Used as register block node for register value save/restore compressed address list
typedef struct
{
    uint8 bases;
    uint8 offsets;
...
struct ddr_misc
{
  ...
  uint32 dcb_crc;
  uint32 not_frd;
  uint32 pll_precal_completed;
  uint16 pasr_cmd_logging[4];
};
...
/*******************************/
/*** MC MR CMD HISTORY Structure ***/
/*******************************/
/**
 * Pointers pointing to SHRM DRAM address 
 * used to log last 32 issued MR CMDs and 
 * recent values of all 41 MR CMDs
 */

struct mc_mr_cmd_hist
{
  uint32  *mr_cmd_buffer_ptr;      
  uint8  *mr_cmd_list_ptr;      
};
...
typedef struct
{
  ...
  uint32                                phy_core_version;               /**< Records PHY core version */
  uint32                                mc_core_version;                /**< Records MC core version */
  uint32                                llcc_core_version;              /**< Records LLCC core version */
  struct detected_ddr_device_params     detected_ddr_device[DDR_MAX_NUM_CH];         /**< Detected DDR device parameters for every channel */
  ...
  struct mc_mr_cmd_hist                 mc_mr_cmd_logging;              /**< struct to store MR CMDs */
} SHRM_DDR_STRUCT;
...
typedef struct
{
  ...
  REG_BLOCK_COUNTS                  addr_list_save_restore[DDRSS_SAVE_RESTORE_MAX_ADDR / sizeof(REG_BLOCK_COUNTS)]; /**< Compressed address list for training results save and restore */
} BOOT_DDR_STRUCT;
