...
#define LOCK_DET_BMSK       HWIO_SILVER_PLL_MODE_PLL_LOCK_DET_BMSK
#define MODE_CTL_MSK        0x7
#define GFMUX_SRC_SEL_BMSK  0x3


/* HSR v7.0 */
#define USER_CTL_SETTINGS       0x00100008

#define CONFIG_CTL_U1_SETTINGS  0x04300152
#define CONFIG_CTL_HI_SETTINGS  0x00000011
#define CONFIG_CTL_LO_SETTINGS  0x10000927

#define TEST_CTL_U1_SETTINGS    0x00000000
...
boolean Clock_SetCPUPerfLevel(ClockBootPerfLevelType eCPUPerfLevel)
{
  ...
  CPU_Cfg = &cfg->CPU_Cfg[nVer][eCPUPerfLevel];
  if ( Clock_ConfigAPSSClock(CPU_Cfg, HWIO_SILVER_PLL_MODE_ADDR) == FALSE ) return FALSE;
  Clock_APPSCL0SpeedKHz = CPU_Cfg->nFreqKHz;

  /* Set L3 speed */
  if ( Clock_ConfigAPSSClock(&cfg->L3_Cfg[nVer][eCPUPerfLevel], HWIO_L3_PLL_MODE_ADDR) == FALSE ) return FALSE;
...
)
{
  ...
  outp32( (nPLLModeAddr+CONFIG_CTL_U1_OFFS),  CONFIG_CTL_U1_SETTINGS );

  outp32( (nPLLModeAddr+TEST_CTL_LO_OFFS),  TEST_CTL_LO_SETTINGS );
  outp32( (nPLLModeAddr+TEST_CTL_HI_OFFS),  TEST_CTL_HI_SETTINGS );
  outp32( (nPLLModeAddr+TEST_CTL_U1_OFFS),  TEST_CTL_U1_SETTINGS );
  ...
  if( bConfigOnly == FALSE) 
  {
    ...
    while( (inp32(nPLLModeAddr) & LOCK_DET_BMSK) == 0 )
    {
      busywait(1);
    }
