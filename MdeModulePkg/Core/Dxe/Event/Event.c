  ...
  )
{
  ...
  EFI_STATUS            Status;
  CHAR8                 *pName = NULL; 
  VOID                  *Base = NULL, *NotifyAddress = NULL;
  UINTN                 Size = 0; 
#endif
  CoreAcquireEventLock ();
  ...
  while (!IsListEmpty (Head)) {
    ...
    UINT64 perf1 =0, perf2 = 0;

    perf1 = GetPerformanceCounter();
    NotifyAddress = Event->NotifyFunction;
#endif
    Event->NotifyFunction (Event, Event->NotifyContext);
#if defined QCOM_EDK2_PATCH
    UINT8 i =0;
    perf2 = GetPerformanceCounter();
    ...
    { 
      ...
      { 
        ...
        if(pName != NULL)
          pName = AsciiStrStr(pName, "AARCH64");
        if(NotifyAddress >= Base && NotifyAddress <= (Base + Size))
        if(PrintFlag)
        DEBUG ((DEBUG_ERROR,"[0x%x] DxeName:%-032a \t Size 0x%x \t Time:%d \n", Base, pName, Size, (perf2 - perf1)));
       }
        DebugTablePtr++;
    }
#endif
