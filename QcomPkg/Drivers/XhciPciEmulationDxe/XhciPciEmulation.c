...
#define XHC_BAR_MEM_IDX             0      // BAR 0 (for 32bit regs), Memory base address
  ...
  )
{
  ...
  if (Supports == NULL && Resources == NULL) {
    //DBG_PRINT(EFI_D_ERROR,"PciIoGetBarAttributes, bad input %d", BarIndex); 
    return EFI_INVALID_PARAMETER;
  }

  if ((BarIndex >= PCI_MAX_BAR) || BarIndex > XHC_BAR_MEM_IDX) {  //currently only support Memory read/Write
    //DBG_PRINT(EFI_D_ERROR,"PciIoGetBarAttributes, Not support idx %d", BarIndex); 
    return EFI_UNSUPPORTED;
  }

  // Supports not use in this xhcipciemuation
  //if (Supports ?!= NULL) {
  //  *Supports = NULL;
  //}
  ...
  if (Resources != NULL) {
    ...
    *Resources   = Descriptor;
    ...
    Descriptor->AddrRangeMin = Private->RootBridge.MemoryStart;
    Descriptor->AddrLen      = Private->RootBridge.MemorySize;
    Descriptor->AddrRangeMax = Private->RootBridge.MemoryStart + Private->RootBridge.MemorySize;
    ...
    DBG_PRINT(EFI_D_WARN,"PciIoGetBarAttributes idx %d, type %d", BarIndex, Descriptor->ResType); 
  }
  return EFI_SUCCESS;  
