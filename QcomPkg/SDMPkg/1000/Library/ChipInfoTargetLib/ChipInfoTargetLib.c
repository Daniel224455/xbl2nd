...
void ChipInfo_InitChipset(ChipInfoCtxtType *pChipInfoCtxt)
{
  ...
  {
    /*
     * One or more of the CPUs is defective. Since sub-binning
     * support is not needed for SDM1000, mark all necessary cores
     * as defective if the top-level fuse is blown.
     *
     * Clients are expecting the second nibble to represent "gold" cores,
     * and the first nibble to represent "silver". Poipu will not consider silver
     * cores for partial binning, so only mark the gold cores as defective.
     * See CR 2205991.
     */
