...
/* Global Variables */
void *p_mdp_domain = NULL, *p_sdc_domain = NULL/*, *p_usb_domain = NULL*/, *p_ufs_domain = NULL/*, *p_pcie_domain = NULL*/;
...
*/
    ...
    // MDP Domain configuration
    if(HAL_IOMMU_ERR_OK != hal_iommu_domain_configure(p_mdp_domain, &non_cca_domain_cfg)) {
        return EFI_NOT_READY;
    }
...
/*    
    ...
    ret = hal_iommu_domain_attach_device(p_usb_domain, (void *)"\\_SB_.USB3-1", 0x0, 0x0);
    if(ret != HAL_IOMMU_ERR_OK) {
        return EFI_NOT_READY;
    }
*/
...
HAL_IOMMUResult_t IDIOM_domain_lookup(void**p_domain_local, void *client_handle)
{
	...
	if(strcmp((void *)"\\_SB_.MDP",client_handle ) == 0)
	{
		*p_domain_local = p_mdp_domain;
	}
	else if( strcmp((void *)"\\_SB_.SDC2",client_handle ) == 0 ||  strcmp((void *)"\\_SB_.SDC4",client_handle ) == 0)
	{
