...
void DDR_PHY_hal_cfg_rx_cal (BOOT_DDR_STRUCT *ddr, uint8 ch)
{
	...
	uint32 ch_1hot = ddr->shrm_ddr_ptr->misc.enabled_channels;
	uint8 ch_idx = 0;
	uint32 reg_offset_ddr_phy;
	uint32 master_slave_pair[2];
	
	if (ch%2==0)
	{
	master_slave_pair[0] = ch;
	master_slave_pair[1] = ch+1;
	}
	else
	{
	master_slave_pair[0] = ch;
	...
	{
		if(ch_1hot & (1 << (master_slave_pair[ch_idx])))
		{
			reg_offset_ddr_phy = REG_OFFSET_DDR_PHY_CH(master_slave_pair[ch_idx]);
			HWIO_OUTXF(reg_offset_ddr_phy + CA0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_2_CFG, HPRX_CAL_EN, 1);
			HWIO_OUTXF(reg_offset_ddr_phy + DQ0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_2_CFG, HPRX_CAL_EN, 1);
			HWIO_OUTXF(reg_offset_ddr_phy + DQ1_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_2_CFG, HPRX_CAL_EN, 1);
		}
	}
	reg_offset_ddr_phy = REG_OFFSET_DDR_PHY_CH(ch);

	HWIO_OUTXF(reg_offset_ddr_phy + CA0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_CFG, IO_CAL_FF_TIMER_EN, 0);
	HWIO_OUTXF(reg_offset_ddr_phy + CA0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_CFG, CAL_USE_LAST, 0x0);
	HWIO_OUTXF(reg_offset_ddr_phy + CA0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_CFG, MARGIN_LOAD, 0x0);
	HWIO_OUTXF(reg_offset_ddr_phy + CA0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_CFG, NWAIT_TIME, 0x80);
	HWIO_OUTXF(reg_offset_ddr_phy + CA0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_CFG, PWAIT_TIME, 0x80);
	HWIO_OUTXF(reg_offset_ddr_phy + CA0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_CFG, CAL_NOW, 1);
	HWIO_OUTXF(reg_offset_ddr_phy + CA0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_CFG, CAL_NOW, 0);
	while(HWIO_INXF(reg_offset_ddr_phy + CA0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_TOP_1_STA, IOCAL_DONE) == 0);
	...
	for(ch_idx = 0; ch_idx < 2; ch_idx++)
	{
		...
		{
			reg_offset_ddr_phy = REG_OFFSET_DDR_PHY_CH(master_slave_pair[ch_idx]);
			HWIO_OUTXF(reg_offset_ddr_phy + CA0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_2_CFG, HPRX_CAL_EN, 0);
			HWIO_OUTXF(reg_offset_ddr_phy + DQ0_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_2_CFG, HPRX_CAL_EN, 0);
			HWIO_OUTXF(reg_offset_ddr_phy + DQ1_DDR_PHY_OFFSET, DDR_PHY_DDRPHY_IOCTLR_CTRL_2_CFG, HPRX_CAL_EN, 0);
		}
	}
#endif
}
