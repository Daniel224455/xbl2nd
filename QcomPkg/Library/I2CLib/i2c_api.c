...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 30/01/17   ska     Added changes for raising TPL
...
//    boolean iface_inited = FALSE;
    ...
    I2C_LOG(LEVEL_INFO, "[I2C] ++open core %d", instance);
    ...
    {
        i_status = I2C_ERROR_INVALID_PARAMETER;
        I2C_LOG(LEVEL_ERROR, "[I2C] i2c_open : instance %d , invalid param", instance);
		goto exit;
    }
	...
	*i2c_handle = NULL;
    ...
    if (dcfg == NULL)
    {
        ...
        I2C_LOG(LEVEL_ERROR, "[I2C] i2c_open : instance %d , plat_get_device_config() fail", instance);
        goto exit;
    }
    ...
    if (h_ctxt == NULL)
    {
        ...
        I2C_LOG(LEVEL_ERROR, "[I2C] i2c_open : instance %d , alloc_hw_ctxt() fail", instance);
        goto exit;
    }
    ...
    if (h_ctxt->core_ref_count == 0)
    {
        ...
        {
            I2C_LOG(LEVEL_ERROR, "[I2C] i2c_open : instance %d , init_hw_ctxt() fail", instance);
            goto exit;
        }
    }
    ...
    if (c_ctxt == NULL)
    {
        ...
        I2C_LOG(LEVEL_ERROR, "[I2C] i2c_open : instance %d , plat_mem_alloc() fail", instance);
        goto exit;
    }
    ...
    *i2c_handle = c_ctxt;
    ...
    I2C_LOG(LEVEL_INFO, "[I2C] --open handle 0x%08X", c_ctxt);

exit: 

    if (I2C_ERROR(i_status))
    {
        I2C_LOG(LEVEL_ERROR, "[I2C] i2c_open : failed with status : 0x%x", i_status);
...
)
{
    ...
    I2C_LOG(LEVEL_INFO, "[I2C] ++i2c_read : handle 0x%x called", i2c_handle);
	...
	if(!desc) {
        ...
        I2C_LOG(LEVEL_ERROR, "[I2C] i2c_read : handle 0x%x , plat_sys_mem_alloc() failed", i2c_handle);
		goto exit;
	}
...
exit:
    ...
    I2C_LOG(LEVEL_INFO, "[I2C] --i2c_read : handle 0x%x , exit with result %d", i2c_handle, result);
	return result;
...
)
{
    ...
    I2C_LOG(LEVEL_INFO, "[I2C] ++i2c_write : handle 0x%x called", i2c_handle);
	...
	if(!desc) {
		result = I2C_ERROR_MEM_ALLOC_FAIL;
        I2C_LOG(LEVEL_ERROR, "[I2C] i2c_write : handle 0x%x , plat_sys_mem_alloc() failed", i2c_handle);
		goto ON_EXIT;
	}
	memset(desc,0,sizeof(i2c_descriptor));
	...
	{
		result = I2C_ERROR_MEM_ALLOC_FAIL;
        I2C_LOG(LEVEL_ERROR, "[I2C] i2c_write : handle 0x%x , buff alloc failed", i2c_handle);
		goto ON_EXIT;
	}
...
ON_EXIT:
    ...
    I2C_LOG(LEVEL_INFO, "[I2C] --i2c_write : handle 0x%x , exit with result %d", i2c_handle, result);
        ...
        )
{
    ...
    // log parameters
    I2C_LOG(LEVEL_VERBOSE, "[I2C] bus_frequency_khz %d",
            config->bus_frequency_khz);
    I2C_LOG(LEVEL_VERBOSE, "[I2C] slave_address 0x%02x",
            config->slave_address);
    I2C_LOG(LEVEL_VERBOSE, "[I2C] smbus_mode %d\n",
            config->mode);
    I2C_LOG(LEVEL_VERBOSE, "[I2C] slave_max_clock_stretch_us %d",
            config->slave_max_clock_stretch_us);
    I2C_LOG(LEVEL_VERBOSE, "[I2C] core_configuration1 %d",
            config->core_configuration1);
    I2C_LOG(LEVEL_VERBOSE, "[I2C] core_configuration2 %d",
            config->core_configuration2);
    I2C_LOG(LEVEL_VERBOSE, "[I2C] num_descriptors %d",
            num_descriptors);
    I2C_LOG(LEVEL_VERBOSE, "[I2C] func 0x%08x: ctxt 0x%08x",
            func, ctxt);
    ...
    for (i = 0; i < num_descriptors; i++)
    {
        ...
        I2C_LOG(LEVEL_INFO, "[I2C] [%d] buffer 0x%08x: length %d: flags 0x%08x",
...
)
{
    ...
    I2C_LOG(LEVEL_INFO, "[I2C] ++ i2c_transfer : handle 0x%x called", i2c_handle);
    ...
    if (i2c_validate_transfer_params(
                ...
                ctxt) == FALSE)
    {
        I2C_LOG(LEVEL_ERROR, "[I2C] i2c_transfer : i2c_validate_transfer_params() failed");
        return I2C_ERROR_INVALID_PARAMETER;
    }
...
//            plat_signal_de_init (t_ctxt->signal);
    ...
    I2C_LOG(LEVEL_INFO, "[I2C] -- i2c_transfer : handle 0x%x , exit with result %d", i2c_handle, i_status);
