/** @file
  ...
  Copyright (c) 2011-2019, Qualcomm Technologies Inc. All rights
  reserved. 
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 08/16/19   rc      Added GPIOConfig to NoPull for Compal GPIO
 03/08/19   vb      Added AmlUpdatePlst variable
 02/04/19   rajb    Updated log mask
 01/08/19   vb      Added AmlUpdateSidt variable
 31/08/18   rajb    Updated log mask
  ...
  )
{
  ...
  if EFI_ERROR (Status)
  {
    DEBUG((ACPI_PLATFORM_DEBUG_PRINT,"[AcpiPlatformDxeInitialize]: LocateProtocol Failed\r\n"));
  }
  ...
  if EFI_ERROR (Status)
  {
    DEBUG((ACPI_PLATFORM_DEBUG_PRINT,"[AcpiPlatformDxeInitialize]: GetPlatformInfo Failed\r\n"));
  }

  if (PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_CLS && PlatformInfo.subtype == 0x1)
  {
    // Initialize TLMM
    ...
    if (EFI_ERROR (Status)) 
    {
      DEBUG ((EFI_D_WARN, "[AcpiPlatformDxeInitialize] %a: LocateProtocol gEfiTLMMProtocolGuid Protocol failed 0x%x\r\n", __FUNCTION__, Status));
    }
    Status = gTLMMProtocol->ConfigGpio(SDX55_PRESENT_GPIO_CFG, TLMM_GPIO_ENABLE);
    Status = gTLMMProtocol->ConfigGpio(HST_PRESENT_GPIO_CFG, TLMM_GPIO_ENABLE);
    Status = gTLMMProtocol->ConfigGpio(PCIE_PLAT_CFG_0, TLMM_GPIO_ENABLE);
    Status = gTLMMProtocol->ConfigGpio(PCIE_PLAT_CFG_1, TLMM_GPIO_ENABLE);
  }
