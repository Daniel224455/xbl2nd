        ...
        for(cs = 0; cs < NUM_CS; cs++)
        {
          if((chip_select >> cs) & 0x1)
          {
            ...
            {                            
              ...
              DDRSS_DIT_Runtime(ddr, ch, cs, low_freq_clk_khz, 1);
              #if DSF_WR_DQDQS_TRAINING_TDQS2DQ_CAL_LOG
			  ddr_printf(DDR_NORMAL, "DIT runtime count: %u at freq: %u\n", training_data_ptr->results.dit.dit_runtime_count, low_freq_clk_khz);
              #endif  
              ...
              DDRSS_wr_dqdqs_lpddr4_tdqs2dq_cal (ddr, 
                                              ...
                                              training_params_ptr, 
                                              low_freq_clk_khz);
              ddr_printf(DDR_NORMAL, "\nEND: WR training for band0/1 on Ch: %u Rank: %u\n", ch, cs);
            }
                                           ...
                                           uint32 clk_freq_khz)
{
   ...
   // uint16               scaling_period = 0;
    ...
    device_width = ddr->shrm_ddr_ptr->detected_ddr_device[TARGET_ACTIVE_CHANNEL].device_io_width[cs]; // find the device width per rank
    ...
    if (dit_count[ch][cs][0] != 0)
    {
      ...
      #if DSF_WR_DQDQS_TRAINING_TDQS2DQ_CAL_LOG
      ddr_printf(DDR_NORMAL, "Die 0 DIT count: %u\n", dit_count[ch][cs][0]);
      #endif   
      if(device_width == DEVICE_WIDTH_IS_x8)
      {
        ...
        #if DSF_WR_DQDQS_TRAINING_TDQS2DQ_CAL_LOG
        ddr_printf(DDR_NORMAL, "Die 1 DIT count: %u\n", dit_count[ch][cs][1]);
        #endif   
      }
      else
      {
    ...
    if(device_width == DEVICE_WIDTH_IS_x8)
    {
        ddr_printf(DDR_NORMAL, "tDQSDQ for CH = %d, CS = %d, die1 = %d \n",ch,cs,tdqs2dq[1]);
         ...
         // ============================================================================================================================
              ...
              if (device_width == DEVICE_WIDTH_IS_x8)
              {
                  g_sed_struct_val.param_value = tdqs2dq[1];
              }
              else
              {
                  g_sed_struct_val.param_value = tdqs2dq[0];
              }                         
           //if(g_sed_flag || g_abort_flag)
           {
       
                 g_sed_struct_val.training = "WR_TDQS2DQ";
               g_sed_struct_val.param_name = "TDQS2DQ";
               g_sed_struct_val.param_steps = dit_runtime_count;
               //g_sed_struct_val.param_value = tmp_string_p;
               ...
               if(((tdqs2dq[0] || tdqs2dq[1]) <= training_params_ptr->ddr_abort.min_tdqs2dq) &&
                  ((tdqs2dq[0] || tdqs2dq[1]) >= training_params_ptr->ddr_abort.max_tdqs2dq))
                {
                  g_abort_flag = 1;
                  training_params_ptr->small_eye_abort = 1;                        
                }
               if (((g_abort_flag)&&(training_params_ptr->ddr_abort.tdqs2dq_range_enable))|| (g_sed_flag))
                {   
    ...
    {        
        for(freq_inx = 0; freq_inx <= 0; freq_inx++) // For band0 only
        {
            ...
            if(ddr->shrm_ddr_ptr->detected_ddr_device[TARGET_ACTIVE_CHANNEL].populated_chipselect == DDR_CS_BOTH)
            {
