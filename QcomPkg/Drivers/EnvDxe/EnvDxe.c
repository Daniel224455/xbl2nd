...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 11/09/17    bd       Add option to enable HS UART
 10/25/17    bd       Add support for HW WDG 
  ...
  )
{
  ...
  gBS->SetTimer (
                  PetTimerEvent,
                  TimerPeriodic,
                  EFI_TIMER_PERIOD_SECONDS(WDOG_PET_TIME_IN_SECS)
                  );  
  DEBUG((EFI_D_INFO, " WDG timer pet handler \n"));                  
  
}

VOID
WdgPetTimerCallbackCancel (
  IN EFI_EVENT    Timer,
  IN VOID         *Context
  )
{
...
STATIC EFI_STATUS RegisterWdgPetTimer(VOID)
{
  EFI_STATUS  Status;    
  ...
  WdgPetTimerCallback ( PetTimerEvent, NULL);
...
VOID EnvExitBootServicesCallback(IN EFI_EVENT Event, IN VOID *Context)
{
  ...
  WdgPetTimerCallbackCancel (PetTimerEvent,NULL);
#endif    
  ...
  if (HsUartBaudRateVar != 0) { 
    ...
    Status = SerialPortSetAttributes(&BaudRate, NULL, NULL, NULL, NULL, NULL);
	
	if(Status != EFI_SUCCESS) {
      DEBUG((EFI_D_ERROR, " SerialPortSetAttributes fail to set 3Mbps Baudrate, Status =  (0x%x)\r\n", Status));
	}
  }  
...
/* Enable Watchdog only if enabled in DSC and we're in Debug environment */
  ...
  if (EFI_ERROR(Status))
    DEBUG((EFI_D_ERROR, "Error: Unable to register periodic timer for Watchdog pet\n"));   
#endif
