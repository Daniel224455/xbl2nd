...
Requirements for threshold:
  1.      “Microsoft recommends systems shipped with Windows Mobile set the NoPPIClear flag to a default value of TRUE.  
          Microsoft recommends systems shipped with Windows Desktop set the NoPPIClear flag to a default value of FALSE.”    
  2.      “If a system sets the NoPPIClear flag to a default value of TRUE, the system must implement PPI operations 17 (SetNoPPIClear_False) and 18 (SetNoPPIClear_True).”  
          If clear is issued when PPI is on, a BIOS prompt should be used for confirmation. If PPI is on and is to be turned off, a BIOS prompt should be used for confirmation
...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 04/09/19   pj       Optimized ProcessMor
 12/20/18   shl      Converted to use TPM protocol.
 06/04/18   vk       Change Debug policy Name
 04/29/15   sm       Removed iMEM PPI address
 09/03/14   sk       Updated logic for threshold requirements
 12/03/14   sk       Changed library to protocol
...
extern int is_morppi_memclear_512bytes_possible(void);
extern void morppi_memclear_cached_aligned512bytes(uint64 addr, uint64 size);
...
#define MOR_PPI_FLAG_DEFAULT        0x01      // New Threshold requirement for windows mobile. Default should be 1
...
// Tpm protocol pointer
STATIC EFI_TPM_PROTOCOL   *pTpmProtocol = NULL;

extern EFI_GUID gQcomTokenSpaceGuid;

// Function Declarations
EFIAPI EFI_STATUS ProcessMorPpi(IN EFI_QCOM_MORPPI_PROTOCOL *This );

// Function Table
EFI_QCOM_MORPPI_PROTOCOL MorPpiProtocolImpl =
{
  ProcessMorPpi
};
...
)
{
  ...
  if( pTpmProtocol == NULL ) 
  {
    ...
    if(EFI_ERROR (Status) )
    {
      DEBUG(( EFI_D_ERROR, "Locate Tpm protocol failed, Status = (0x%p)\r\n", Status));
      goto ErrorExit;
    }
  }

  Status = pTpmProtocol->SubmitCommand(pTpmProtocol, cmdSize, cmd, &respSize, resp);
  if (Status != EFI_SUCCESS) 
  {
     DEBUG(( EFI_D_ERROR, " pTpmProtocol->SubmitCommand failed, Status = (0x%p)\r\n", Status));
...
EFIAPI EFI_STATUS ProcessMor( )
{
  ...
  UINTN                   Index;
  EFI_TPL                 OriginalTPL;
  UINT64                  PhyMemStart = 0;
  UINT64                  MemSize = 0;
  BOOLEAN                 Use512BytesDCZVA = FALSE;
  ...
  if (Status == EFI_BUFFER_TOO_SMALL)
  {
    ...
    if( MemoryMap != NULL )
    {
      ...
      if ( Status == EFI_SUCCESS )
      {
        ...
        if((EFI_PAGE_SIZE == SIZE_4KB) && (is_morppi_memclear_512bytes_possible()))
          Use512BytesDCZVA = TRUE;
        ...
        for (Index = 0; Index < MemoryMapSize/DescriptorSize; Index++)
        {
          ...
          {
            PhyMemStart = MemoryMapTemp->PhysicalStart;
            MemSize = MemoryMapTemp->NumberOfPages*EFI_PAGE_SIZE;
            
            if(Use512BytesDCZVA)
            {
              morppi_memclear_cached_aligned512bytes(PhyMemStart, MemSize);
            }
            else
            {
              ZeroMem((VOID *)(UINTN)(PhyMemStart), MemSize);
            }
            
            DEBUG((EFI_D_INFO, "VirtualStart: 0x%x; PhysicalStart: 0x%x; MemSize: 0x%x\n", MemoryMapTemp->VirtualStart, MemoryMapTemp->PhysicalStart, MemSize));
...
/**
 	...
 	Variable Name: "CurrentActivePolicy"
...
BOOLEAN IsDebugMode( VOID)
{
    ...
    Status = gRT->GetVariable (L"CurrentActivePolicy", &DebugModeVariableGuid,
...
EFIAPI EFI_STATUS ProcessPpi( )
{
  ...
  switch( PendingReq )
  {
    ...
    case 17:
      // If 'SetNoPPIClear_False' is issued when PPI is on, a BIOS prompt should be used for confirmation. If not, just set the flag to FALSE
      if( (IsDebugMode() == TRUE) && (DisplayPpiFlag == FALSE) )
      {
      ...
      // Set NoPpiClearFlag to FALSE with physical presence (BIOS prompt)
      // Bring up user confirmation screen
      gST->ConOut->SetAttribute(gST->ConOut, (EFI_WHITE | EFI_BACKGROUND_BLUE) );
      // draw blue screen 
      gST->ConOut->ClearScreen (gST->ConOut);
  
      SizeOfPtrArray = sizeof( NoPpiClearLinePtr1 )/sizeof(NoPpiClearLinePtr1[0]);
      for( i=0; i<SizeOfPtrArray; i++)
      {
        AsciiPrint((const CHAR8 *)NoPpiClearLinePtr1[i]); 
      }
    ...
    case 18:
      // If 'SetNoPPIClear_True' is issued when PPI is on, a BIOS prompt should be used for confirmation. If not, just set the flag to TRUE
      if( (IsDebugMode() == TRUE) && (DisplayPpiFlag == FALSE) )
      {
      ...
      // Set NoPpiClearFlag to TRUE with physical presence (BIOS prompt)
      // Bring up user confirmation screen
      gST->ConOut->SetAttribute(gST->ConOut, (EFI_WHITE | EFI_BACKGROUND_BLUE) );
      // draw blue screen 
      gST->ConOut->ClearScreen (gST->ConOut);
...
EFIAPI EFI_STATUS ProcessMorPpi(IN EFI_QCOM_MORPPI_PROTOCOL *This )
{
  ...
  switch(Status)
  {
    ...
    case EFI_NOT_FOUND:
      ...
      // Default value of NoPPIClear for Windows Mobile should be TRUE
...
/**
  MorPpiDxe driver initialization, it is the entry point of MorPpiDxe driver.
  ...
  )
{
  ...
  // Install MorPpiDxe protocols. 
  Status = gBS->InstallMultipleProtocolInterfaces (
                      &ImageHandle,
                      &gQcomMorPpiProtocolGuid,
                      (void **)&MorPpiProtocolImpl,
                       NULL
                    );
  if (EFI_ERROR (Status))
  {
    DEBUG(( EFI_D_INFO, "MorPpiDxeInitialize: Install Protocol failed, Status = (0x%p)\r\n", Status));
    goto ErrorExit;
  }
