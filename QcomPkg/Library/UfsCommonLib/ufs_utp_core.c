...
----------   ---     --------------------------------------------------- 
2021-06-21   bb      Added print for HS gear info 
...
2018-01-19   jt      Add invalidate after read operation  
...
{
   int32_t rc = EOK;
   uint8_t hs_series = UFS_DEFAULT_HS_SERIES;
   ...
   // Skip gear switch for primary UFS if required
   ...
   // Restrict gear to the maximum HS gear allowed 
   if (hba->max_hsgear_device < hba->dal_cfg.gear) 
   {
      hba->dal_cfg.gear = hba->max_hsgear_device; 
   }

   // If we are in HSG3, but UFS 3.0 support is present, use HSG4. 
   if ((hba->dal_cfg.enable_hs == UFS_USES_HS_GEAR) && (hba->dal_cfg.gear == 3) && 
       (hba->max_hsgear_device > 3) && (hba->plat_cfg.g4_support[hba->ufs_core_id])) 
   {
      hba->dal_cfg.gear = 4; 
   }

   // If Rate A is needed, change the hs_series 
   if (hba->ratea_override)
   {
      hs_series = 0x1; 
   }

   // If PWM mode enabled, adjust phy termination/pwr_mode values
   if (0 == hba->dal_cfg.enable_hs) {
      phy_termination = 0; 
      pwr_mode = 0x22;
   }
   ...
   lanes = hba->dal_cfg.num_lanes < lanes ? hba->dal_cfg.num_lanes : lanes;
   ...
   rc = ufs_utp_uic_getset (hba, UICCMDR_DME_SET, MIBattr(PA_RxGear, 0), hba->dal_cfg.gear, NULL);
   if (rc != EOK) {
   ...
   rc = ufs_utp_uic_getset (hba, UICCMDR_DME_SET, MIBattr(PA_TxGear, 0), hba->dal_cfg.gear, NULL);
   if (rc != EOK) {
      ufs_error_log(hba, 0x0031);
      return rc;
   }

  snprintf(log_buf, 64, "UFS GEAR: %d", hba->dal_cfg.gear);
  ufs_bsp_log_error(log_buf);
   ...
   // Set the HS Series
   rc = ufs_utp_uic_getset (hba, UICCMDR_DME_SET, MIBattr(PA_HSSeries, 0), hs_series, NULL);
   if (rc != EOK) {
   ...
   if (hba->dal_cfg.gear == 4) {
      rc = ufs_utp_uic_getset(hba, UICCMDR_DME_PEER_GET, MIBattr(RX_HS_ADAPT_INITIAL_Capability, GenSelectRx(0)), 0, &hs_adapt_capability); 
      if (rc != EOK) {
         ufs_error_log(hba, 0x0049); 
         return rc; 
      }

      rc = ufs_utp_uic_getset(hba, UICCMDR_DME_SET, MIBattr(PA_PeerRxHsAdaptInitial, 0), hs_adapt_capability, NULL); 
      if (rc != EOK) {
         ufs_error_log(hba, 0x004a); 
         return rc; 
      }

      rc = ufs_utp_uic_getset(hba, UICCMDR_DME_SET, MIBattr(PA_TxHsAdaptType, 0), UFS_PA_INITIAL, NULL); 
      if (rc != EOK) {
   ...
   {
      rc = ufs_utp_uic_getset(hba, UICCMDR_DME_SET, MIBattr(PA_TxHsAdaptType, 0), UFS_PA_NO_ADAPT, NULL);
      if (rc != EOK) {
...
int32_t ufs_utp_init (ufs_host_t *hba)
{
   ...
   if (hba->has_dal_cfg == 0)
   {
      ...
      /* If there is no Gear 4 support, restrict the gear to HSG3 */
      if ((dal_cfg.enable_hs == UFS_USES_HS_GEAR) && (0 == hba->plat_cfg.g4_support[hba->ufs_core_id]))
      {
         hba->dal_cfg.gear = (dal_cfg.gear > 3) ? 3 : hba->dal_cfg.gear; 
      }
...
SKIP_LINK_STARTUP:
   // Check to see whether we can use HSG4  
   if (!hba->has_hsg4_check) {
      // Check max HS gear capability for the device
      rc = ufs_utp_uic_getset (hba, UICCMDR_DME_PEER_GET, 
                               MIBattr(TX_HSGEAR_Capability, 0), 
                               0, &hba->max_hsgear_device); 
      if (rc != EOK) {
         ufs_error_log(hba, 0x0097);
         return rc;
      }
#ifdef ENABLE_AUTO_PLAT
      // For Auto target, set max gear as 3 only and remove voltage switch. Once HLOS comes up, it will enable Gear 4, if supported
	  hba->max_hsgear_device = (hba->max_hsgear_device > 3) ? 3 : hba->max_hsgear_device;
      hba->plat_cfg.low_volt_support = 0;
#endif
      if (hba->max_hsgear_device == 4) {
         hba->plat_cfg.g4_support[hba->ufs_core_id] = 1; 
      }
      else
      {
         hba->plat_cfg.g4_support[hba->ufs_core_id] = 0; 
      }

      if (hba->plat_cfg.g4_support[hba->ufs_core_id] && hba->dal_cfg.enable_hs && 
          (hba->dal_cfg.gear >= 3)) {

         // If device is capable of HSG4, change the DAL cfg to use it. 
         if (hba->max_hsgear_device > 3) {
            hba->dal_cfg.gear = hba->max_hsgear_device;  

            // Now, check to see if the PHY initialization needs to be done 
            // again with the HSG4 settings. 
            rc = ufs_utp_uic_getset (hba, UICCMDR_DME_GET, 
                                     MIBattr(TX_HSGEAR_Capability, GenSelectTx(0)), 
                                     0, &val); 
            if (rc != EOK) {
               ufs_error_log(hba, 0x009c);
               return rc;
            }

            if (val < hba->dal_cfg.gear) {
               hba->has_hsg4_check = 1;
...
{
   boolean rateb = (UFS_DEFAULT_HS_SERIES == RATE_B), gear4 = FALSE; 
   uint16_t flags = 0, offs = 0; 
   uint32_t i = 0; 
   uint8_t val; 

   // Gear 4 until HW version 0x4003 needs Rate A 
   gear4 = ((hba->dal_cfg.enable_hs) && (hba->plat_cfg.g4_support[hba->ufs_core_id])) ? TRUE : FALSE; 
   if (rateb) {
      rateb = ((hba->ufs_hw_version < 0x4003) && (gear4 == TRUE)) ? FALSE : TRUE; 
   }
   if (rateb == FALSE) {
      hba->ratea_override = 1; 
   }
   ...
   for (i = 0; i < table_size; i++) {
      ...
      if (flags != 0) {
         ...
         if (((flags & VER_MASK) >> VER_SHFT) == CONTROLLER_VER_4_0) {
            if (hba->ufs_hw_version != 0x4000) { 
               continue;
            }
         }
         else if (((flags & VER_MASK) >> VER_SHFT) == CONTROLLER_VER_4_1) {
            if (hba->ufs_hw_version != 0x4001) {
               continue; 
            }
         }
      }
