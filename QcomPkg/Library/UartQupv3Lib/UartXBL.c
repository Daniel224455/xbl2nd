...
$Header: //components/rel/boot.xf/2.2/QcomPkg/Library/UartQupv3Lib/UartXBL.c#3 $
...
RETURN_STATUS EFIAPI
uart_set_baud_rate(IN UINT32 baud_rate)
{
   ...
   if( uart_clock_open(uart_ctxt, freq)== UART_ERROR)
   {
      return RETURN_UNSUPPORTED;
   }
...
uart_power_off(void)
{
   ...
   if(REG_IN(base + GENI4_CFG, GENI_STATUS) & 0x41000)                     //GENI_STATUS & 0x41000 this gives GENI RX engine status weather its Active or not. 
   {
      REG_OUT(base + GENI4_DATA, GENI_S_CMD_CTRL_REG, 0x4);                 //Need to stop Rx engine before Disabling serial clock.  
																		    //0x04 cancel Geni current read command which eventually stop RX engine.  
      do
      {
