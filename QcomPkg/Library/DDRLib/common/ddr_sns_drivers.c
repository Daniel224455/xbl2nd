...
#define GRANULE_256MB 256
#define GRANULE_512MB 512
...
uint32 ddr_get_max_channels(void)
{
  /*This check is specifically for Poipu V2 .
    return number of channels based on the value in TCSR_TIMEOUT_INTR_STATUS register calculated earlier*/
    
  if(ddrsns_share_data->shrm_ddr_ptr->misc.platform_id == MSM_VERSION_POIPU && ddrsns_share_data->shrm_ddr_ptr->misc.chip_version == 0x0200)
  {
    //This check is only for Poipu V2 and to determine number of channel based on TCSR register 
      return ddrsns_share_data->shrm_ddr_ptr->misc.num_channel;    
  } 
  return DDR_MAX_NUM_CH; /*--> 4 for Hana and 8 for Poipu V1*/
}
...
{
  /*If each DDR rank size <= 2GB (0x800) then granule size will be 256MB, else it is 512MB*/	
  if (ddr_system_size.ddr_cs0[0] <= 0x800) 
  {	   
     /* Size of granule is in MB*/
     *granule_size = GRANULE_256MB;	
  }
  else	
  {    
     *granule_size = GRANULE_512MB;		
  }
}
