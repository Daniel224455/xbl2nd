...
/*=====================================================================================
 ...
 09/22/17     va      Enable BMD explicitly
...
EFI_PLATFORMINFO_PLATFORM_INFO_TYPE  gPlatformInfo;
...
EFI_STATUS ChargerPlatform_Init( VOID )
{
  ...
  if ((Status != EFI_SUCCESS)|| (pPlatformInfoProtocol==NULL))
  {
    return Status;
  }

  if(pPlatformInfoProtocol->GetPlatformInfo(pPlatformInfoProtocol, &gPlatformInfo)!= EFI_SUCCESS)
  {
    return Status;
  }
  
  CHARGER_DEBUG(( EFI_D_WARN, "QcomChargerDxe:: PlatformType = %d, Platform SubType = %u \r\n",
				gPlatformInfo.platform, gPlatformInfo.subtype));
