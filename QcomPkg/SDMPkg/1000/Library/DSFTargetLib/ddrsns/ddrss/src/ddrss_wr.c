...
#else
    MC_Periodic_Auto_Refresh_Ctrl(ddr, chip_select, 1);
#endif
  ...
  for(ch = 0; ch < NUM_CH; ch++)
    {
        ...
        if(((channels >> ch) & 0x1) == 0x0) continue; 
          for(cs = 0; cs < NUM_CS; cs++)
          {
               ...
               {                               
                 ...
                 if((training_params_ptr->WR_Training_Switchboard[prfs_index][_V_FSP0]==1) || 
                      (training_params_ptr->WR_Training_Switchboard[prfs_index][_V_FSP1]==1))
                 {
                      //=================================================================
                    //copy dram vref from rank 0 to rank 1
                    DDRSS_cpy_dram_wr_vref_r0tor1(ddr, ch );
                      //=================================================================
                 }
               }
               else 
              ...
              if((chip_select >> cs) & 0x1)
              {
                ...
                // seq_wait(200,NS);
                 ...
                 MC_MR_Write (REG_OFFSET_MC_CH(ch), CS_1HOT(cs), JEDEC_MR_13, (MR13_BASE)|MR13_value);
        ...
        if (training_params_ptr->Training_pivot_plots_Switchboard[prfs_index][_WR_P]==1)
        {
            uint64 tmp = 0;
    ...
    MC_Periodic_Auto_Refresh_Ctrl(ddr, chip_select, 0);
                            ...
                            uint8 prfs_index)
{
  ...
  uint8 freq_eq_max_training = (training_params_ptr->WR_Training_Switchboard[prfs_index][_C_PB]==1);
    ...
    if(
	    (training_params_ptr->WR_Training_Switchboard[prfs_index][_C_PB  ]==1)||
	    (training_params_ptr->WR_Training_Switchboard[prfs_index][_C_CF  ]==1)||
	    (training_params_ptr->WR_Training_Switchboard[prfs_index][_DCC   ]==1)||
	    (training_params_ptr->WR_Training_Switchboard[prfs_index][_V_FSP1]==1)||
	    (training_params_ptr->WR_Training_Switchboard[prfs_index][_V_FSP0]==1)
		)
    {
       
        for (byte=0;byte<NUM_DQ_PCH;byte++)
        { 
          HWIO_OUTXF (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET), DDR_PHY_DDRPHY_TOPCTRL1EXT_PRFS_2_CFG, PERIODIC_TRAINING_CLK_ON, 0x1);
         ...
         //Enable WR_RANK_UPDATE_EN for 768MHz only during the training. 
         // this Change is required for loading the values in WR_CMCDC register on to CDC ckt. 
         //Not required for other frequencies because this setting is one time setting from SWC.
         // HWIO_OUTX (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_QFI2EXT_PRFS_2_CNTL_1_CFG,0x00041);// 768MHz
        // HWIO_OUTX (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_QFI2EXT_PRFS_3_CNTL_1_CFG,0x00041);//1017MHz
         // HWIO_OUTX (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_QFI2EXT_PRFS_4_CNTL_1_CFG,0x00041);//1296MHz
    ...
    if(training_params_ptr->WR_Training_Switchboard[prfs_index][_V_FSP1]==1)
    {
    ...
    if(training_params_ptr->WR_Training_Switchboard[prfs_index][_C_PB]==1)//highest frequency
    {
    ...
    if(training_params_ptr->WR_Training_Switchboard[prfs_index][_C_CF]==1)//high frequency (other than highest)
    {
    ...
    {
        //scaling
        ddr_printf(DDR_NORMAL,"\nCopied Vref from highest Freq\n");
    }

    // Disable WR training settings
    for (byte=0;byte<NUM_DQ_PCH;byte++)
    {
      HWIO_OUTXF (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET), DDR_PHY_DDRPHY_TOPCTRL1EXT_PRFS_2_CFG, PERIODIC_TRAINING_CLK_ON, 0x0);//0x1: previous value Sagar
      HWIO_OUTXF (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET), DDR_PHY_DDRPHY_TOPCTRL1EXT_PRFS_3_CFG, PERIODIC_TRAINING_CLK_ON, 0x0);
      HWIO_OUTXF (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET), DDR_PHY_DDRPHY_TOPCTRL1EXT_PRFS_4_CFG, PERIODIC_TRAINING_CLK_ON, 0x0);
      HWIO_OUTXF (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET), DDR_PHY_DDRPHY_TOPCTRL1EXT_PRFS_5_CFG, PERIODIC_TRAINING_CLK_ON, 0x1);
      HWIO_OUTXF (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET), DDR_PHY_DDRPHY_TOPCTRL1EXT_PRFS_6_CFG, PERIODIC_TRAINING_CLK_ON, 0x1);
      HWIO_OUTXF (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET), DDR_PHY_DDRPHY_TOPCTRL1EXT_PRFS_7_CFG, PERIODIC_TRAINING_CLK_ON, 0x1);
      
	  //FPM Mode: Enable
	//  DDR_PHY_WR_FpmMode_En_Dis( dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET), 1 );   
	  //  HWIO_OUTX (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_QFI2EXT_PRFS_2_CNTL_1_CFG,0x000000); //0x000041
      //HWIO_OUTX (dq0_ddr_phy_base + (byte * DDR_PHY_OFFSET),DDR_PHY_DDRPHY_QFI2EXT_PRFS_3_CNTL_1_CFG,0x000000);
                                       ...
                                       uint8 prfs_index)
{
  ...
  //uint8                *compare_result;
    ...
    if((prfs_index == MAX_TRAINING_FREQ_INDEX) || (clk_freq_khz <= LOWEST_TRAINED_FPRS_FREQ))
    {
        ...
        MC_MR_Write (REG_OFFSET_MC_CH(ch), CS_1HOT(cs), JEDEC_MR_14, MR14_value);
    }
    ...
    for(half_cycle_count = (max_half_cycle_count + 1); half_cycle_count > 0;)
    {
        ...
        for(coarse_cdc_value = max_coarse_value; coarse_cdc_value >= min_coarse_value; coarse_cdc_value--)
        {
            ...
            for(loop_count = 0; loop_count < training_params_ptr->wr_dqdqs.max_loopcnt; loop_count++)
            {
                ...
                fail_cell.w |=  DDRSS_mem_read_Get_Fail_pat(ch,cs);
...
// ============================================================================================================================
  ...
  extern     uint8 dq_dbi_bit   ;
   ...
   uint32         pr_max_stop[3][NUM_DQ_PCH] = {{0,0},{0,0},{0,0}};
    ...
    //uint8       dbi_x;
    //uint8       dd_offset=8;  //old, from Napali
	//uint8       dd_offset=4;  //previous from Hana
    //uint16         dcccsr=0;
#if DSF_WR_DQDQS_TRAINING_LOG_IN   
   uint32   pr_bus_eyew[NUM_DQ_PCH]  = {0,0};
   uint32   pr_bus_skwr[NUM_DQ_PCH]  = {0,0};
#endif
    int                           dq_dcc_adj_cur[NUM_DQ_PCH] = {0} ;
    int                           dqsdcc_adj_cur[NUM_DQ_PCH] = {0} ;
    int                           dq_dcc_adj_otp[NUM_DQ_PCH] = {0} ;
    int                           dqsdcc_adj_otp[NUM_DQ_PCH] = {0} ;
    int                           dq_dcc_adj_set[NUM_DQ_PCH] = {0} ;
    int                           dqsdcc_adj_set[NUM_DQ_PCH] = {0} ;
...
// ============================================================================================================================
    ...
    DDRSS_cdcref_init_wr(ddr, local_vars->cdc_ref);    //cdc ruler init
    // ============================================================================================================================
...
// ---------------------------------------------------------------------------------
        for (knob = 0;  knob < sweep_size;  knob++)
        {
            ...
            {
                DDRSS_mem_write (ch,cs);
                fail_pat    =   DDRSS_mem_read_Get_Fail_pat(ch,cs);
    ...
    for (knob=0; knob < sweep_size-1; knob++)
    {
        for (phy_x = 0; phy_x < dq; phy_x++)
        {
            ddr_printf (DDR_NORMAL, "PHY_DQ%d,",byte_remapping_table[ch][phy_x]);
    ...
    for (phy_x = 0; phy_x < dq; phy_x++)
    {
        ...
        // ============================================================================================================================
        if(training_params_ptr->WR_Training_Switchboard[prfs_indx][_DCC   ]==1)
        { 
	    
		dqsdcc_adj_cur[phy_x] =  DDRSS_wr_csr2dcc( DDR_PHY_hal_wr_io_dqsdcc_get_CSR ( reg_offset_ddr_dq[byte_remapping_table[ch][phy_x]], cs, prfs_indx));
        dqsdcc_adj_otp[phy_x] =  (pr_max_midp[1/*ODD*/][phy_x]-pr_max_midp[0/*even*/][phy_x]);
		//dqsdcc_adj_otp[phy_x] =  (pr_max_midp[0/*even*/][phy_x]-pr_max_midp[1/*ODD*/][phy_x]);  //wrong direction
		dqsdcc_adj_set[phy_x] = dqsdcc_adj_cur[phy_x] + dqsdcc_adj_otp[phy_x];
		//DDR_PHY_hal_wr_io_dqsdcc_CSR(reg_offset_ddr_dq[phy_x], cs, prfs_indx,DDRSS_wr_dcc2csr( dqsdcc_adj_set[phy_x]));

        dq_dcc_adj_cur[phy_x] =  DDRSS_wr_csr2dcc(  DDR_PHY_hal_wr_t90_dq_dcc_get_CSR ( reg_offset_ddr_dq[byte_remapping_table[ch][phy_x]], prfs_indx)    );  
		dq_dcc_adj_otp[phy_x] =  (pr_max_eyew[1/*ODD*/][phy_x]-pr_max_eyew[0/*even*/][phy_x]);
		//dq_dcc_adj_otp[phy_x] =  (pr_max_eyew[0/*even*/][phy_x]-pr_max_eyew[1/*ODD*/][phy_x]);  //wrong direction
		dq_dcc_adj_set[phy_x] = dq_dcc_adj_cur[phy_x] + dq_dcc_adj_otp[phy_x];
		//DDR_PHY_hal_wr_t90_dq_dcc_CSR(reg_offset_ddr_dq[phy_x], prfs_indx,DDRSS_wr_dcc2csr( dq_dcc_adj_set[phy_x]));
        }
		...
		midp[phy_x] = (local_vars->cdc_ref[midp[phy_x] ][FINE_L] <  WRITE_FINE_CDC_MIN) ? (midp[phy_x] + (WRITE_FINE_CDC_MIN - local_vars->cdc_ref[midp[phy_x] ][FINE_L])) : midp[phy_x];
		midp[phy_x] = (local_vars->cdc_ref[midp[phy_x] ][FINE_L] >  WRITE_FINE_CDC_MAX) ? (midp[phy_x] - (local_vars->cdc_ref[midp[phy_x] ][FINE_L] - WRITE_FINE_CDC_MAX)) : midp[phy_x];
			
		
		
        // ============================================================================================================================
        // Write the trained coarse and fine center to the active CDC register
        DDR_PHY_hal_cfg_cdc_slave_wr(reg_offset_ddr_dq[byte_remapping_table[ch][phy_x]],
        ...
        DDR_PHY_hal_cfg_cdcext_slave_wr(reg_offset_ddr_dq[byte_remapping_table[ch][phy_x]] ,
                                        cs,//cs
                                        local_vars->cdc_ref[ midp[phy_x] ][COARSE_L],
                                        local_vars->cdc_ref[ midp[phy_x] ][FINE_L  ],
                                        prfs_indx
       ...
       //                                local_vars->cdc_ref[mid_tmp ][FINE_L  ]);
        ...
        for (bit = 0; bit < BYTE_SIZE; bit++)
        {
            ...
            remapped_bit  =        bit_remapping_bimc2phy_DQ[ch][byte_remapping_table[ch][phy_x]][bit];
            DDR_PHY_hal_cfg_pbit_dq_delay(reg_offset_ddr_dq[byte_remapping_table[ch][phy_x]],
                                            remapped_bit,
                                            1, // 1 for Tx
                                            cs,//cs
                                            perbit[phy_x][bit]);
...
// ============================================================================================================================
    ...
    for (phy_x = 0; phy_x < dq; phy_x++)
    {
        ddr_printf (DDR_NORMAL, "PHY_DQ%d,knob, %d, coarse, %d, fine, %d, delay, %d,\n"
                                , byte_remapping_table[ch][phy_x]
                                , midp[phy_x]
                                , local_vars->cdc_ref[ midp[phy_x] ][COARSE_L]
                                , local_vars->cdc_ref[ midp[phy_x] ][  FINE_L]
    ...
    //    for (phy_x = 0; phy_x < dq; phy_x++)
    //    {
    //        ddr_printf(DDR_NORMAL,"DQ%d,\ndqsdcc_adj_cur= %4d, dqsdcc_adj_otp= %4d, dqsdcc_adj_set= %4d, \ndq_dcc_adj_cur= %4d, dq_dcc_adj_otp= %4d, dq_dcc_adj_set= %4d, \n"
 	//    		                 , phy_x
    //                             , dqsdcc_adj_cur[phy_x]
    //                             , dqsdcc_adj_otp[phy_x]
    //                             , dqsdcc_adj_set[phy_x]
    //                             , dq_dcc_adj_cur[phy_x]
    //                             , dq_dcc_adj_otp[phy_x]
    //                             , dq_dcc_adj_set[phy_x]
    //                              );
    //    }//phy_x
	//}
    // ============================================================================================================================
...
// ============================================================================================================================
  ...
  FAILCELL                          fail_cell;
    uint8     pr[3][NUM_DQ_PCH][NUM_PR_PARAMS];
    uint8        vref_trained = 0;
    uint32        vref_trained_mV = 0;
    uint8 set_up =0;
    uint8 hold =0;
    ...
    for (phy_x = 0; phy_x < dq; phy_x++)
    {
             DDRSS_passing_region_scan(  &pr[rf_x][phy_x][STRT],
                                         &pr[rf_x][phy_x][STOP],
                                         fail_histogram[rf_x][phy_x],
                                    0,sweep_size);
             pr[rf_x][phy_x][MIDP] = (pr[rf_x][phy_x][STOP]+pr[rf_x][phy_x][STRT])>>1;
	     		 pr[rf_x][phy_x][MIDP] = (local_vars->cdc_ref[ pr[rf_x][phy_x][MIDP]][FINE_L] < WRITE_FINE_CDC_MIN) ? (pr[rf_x][phy_x][MIDP] + (WRITE_FINE_CDC_MIN - local_vars->cdc_ref[ pr[rf_x][phy_x][MIDP]][FINE_L])) : pr[rf_x][phy_x][MIDP];
	     	     pr[rf_x][phy_x][MIDP] = (local_vars->cdc_ref[ pr[rf_x][phy_x][MIDP]][FINE_L] > WRITE_FINE_CDC_MAX) ? (pr[rf_x][phy_x][MIDP] - (local_vars->cdc_ref[ pr[rf_x][phy_x][MIDP]][FINE_L] - WRITE_FINE_CDC_MAX)) : pr[rf_x][phy_x][MIDP];
             pr[rf_x][phy_x][EYEW] =  pr[rf_x][phy_x][STOP]-pr[rf_x][phy_x][STRT];
		}
	}
    ...
    for (phy_x = 0; phy_x < dq; phy_x++)
    {
		...
		{ 
		dqsdcc_adj_cur[phy_x] =  DDRSS_wr_csr2dcc(  DDR_PHY_hal_wr_io_dqsdcc_get_CSR ( reg_offset_ddr_dq[byte_remapping_table[ch][phy_x]], cs, prfs_indx)    );
        dqsdcc_adj_otp[phy_x] =   pr[1/*ODD*/][phy_x][MIDP]-pr[0/*even*/][phy_x][MIDP];
		dqsdcc_adj_set[phy_x] = dqsdcc_adj_cur[phy_x] + dqsdcc_adj_otp[phy_x];
		DDR_PHY_hal_wr_io_dqsdcc_CSR( reg_offset_ddr_dq[byte_remapping_table[ch][phy_x]], cs, prfs_indx, DDRSS_wr_dcc2csr(dqsdcc_adj_set[phy_x]));

        dq_dcc_adj_cur[phy_x] =  DDRSS_wr_csr2dcc(  DDR_PHY_hal_wr_t90_dq_dcc_get_CSR ( reg_offset_ddr_dq[byte_remapping_table[ch][phy_x]], prfs_indx)    );  
		dq_dcc_adj_otp[phy_x] =  (pr[1/*ODD*/][phy_x][EYEW]-pr[0/*even*/][phy_x][EYEW]);
		dq_dcc_adj_set[phy_x] = dq_dcc_adj_cur[phy_x] + dq_dcc_adj_otp[phy_x];
		//DDR_PHY_hal_wr_t90_dq_dcc_CSR(reg_offset_ddr_dq[phy_x], prfs_indx,DDRSS_wr_dcc2csr( dq_dcc_adj_set[phy_x]));
       }
        ...
        // ============================================================================================================================ 
			   ...
			   // ************************************************************************************************  //
			   //  print DQS/DQ DCD for 2100MHz 
			   //************************************************************************************************ //
			   if ( prfs_indx == MAX_TRAINING_FREQ_INDEX)
			   {
				  g_sed_struct_val.training = g_str_write;
                  g_sed_struct_val.param_name = "WR_DQS_DCD";
                  g_sed_struct_val.param_steps = dqsdcc_adj_otp[phy_x];
                  g_sed_struct_val.param_value = dqsdcc_adj_otp[phy_x] * FINE_STEP_IN_PS;
                  ...
                  g_sed_struct_val.cs = cs;
                  g_sed_struct_val.byte_lane = phy_x;
                    //abort for wr dqsdcc_adj_otp
                    if (dqsdcc_adj_otp[phy_x] >= (training_params_ptr->ddr_abort.max_dqs_cm_dcc))
				   ...
				   DDRSS_SED_prints_struct();  
				       ...
				       g_sed_struct_val.param_name = "WR_DQ_DCD";
                       g_sed_struct_val.param_steps = dq_dcc_adj_otp[phy_x];
                       g_sed_struct_val.param_value = (dq_dcc_adj_otp[phy_x] * FINE_STEP_IN_PS);
                       //abort for wr dq_dcc_adj_otp
                       if (dq_dcc_adj_otp[phy_x] >= (training_params_ptr->ddr_abort.max_dqs_cm_dcc))
                       {  
                        g_abort_flag = 1;
                        training_params_ptr->small_eye_abort = 1;
                       }
					  if (((g_abort_flag) &&(training_params_ptr->ddr_abort.max_dqs_cm_dcc_enable)) ||(g_sed_flag))
				       {
				      DDRSS_SED_prints_struct();  
				       }
	            }
			   ...
			   // ************************************************************************************************  //
			   // END print DQS/DQ DCD for 2100MHz 
			   //************************************************************************************************ //

				// This should not happen at 1804 (or Max freq.) before Vref training
                 vref_trained = DDRSS_get_wr_vref(ddr, ch, cs, phy_x, clk_freq_khz);
                 vref_trained_mV = DDRSS_convert_vref_steps_to_mv(vref_trained, DIR_WR, clk_freq_khz);
                 set_up = pr[2][phy_x][MIDP]- pr[2][phy_x][STRT];
                 hold = pr[2][phy_x][STOP]- pr[2][phy_x][MIDP];
			
                if((training_params_ptr->WR_Training_Switchboard[prfs_indx][_V_FSP0]==0) && 
                   (training_params_ptr->WR_Training_Switchboard[prfs_indx][_V_FSP1]==0))
                {

                  g_sed_struct_val.training = g_str_write;
                  g_sed_struct_val.param_name = "WR_EYE_WIDTH";
                  g_sed_struct_val.param_steps = pr[2][phy_x][EYEW];
                  g_sed_struct_val.param_value = pr[2][phy_x][EYEW] * FINE_STEP_IN_PS;
                  g_sed_struct_val.param_units = g_str_unit_ps;
                  g_sed_struct_val.vref_type = "VREF_TRAIN";
                  g_sed_struct_val.vref_steps = vref_trained;
                  g_sed_struct_val.vref_value = vref_trained_mV;
                  g_sed_struct_val.freq = clk_freq_khz;
                  g_sed_struct_val.ch = ch;
                  g_sed_struct_val.cs = cs;
                  g_sed_struct_val.byte_lane = phy_x;
                      //abort for eye width
				     if (pr[2][phy_x][EYEW] < training_params_ptr->ddr_abort.min_wr_eye_width)			   
                      {  
                       g_abort_flag = 1;
                       training_params_ptr->small_eye_abort = 1;
                      }
					  if (((g_abort_flag) && (training_params_ptr->ddr_abort.min_wr_eye_width_enable)) || (g_sed_flag))
					  {
						DDRSS_SED_prints_struct();  
					  }
                      g_sed_struct_val.param_name = "WR_EYE_WIDTH_SETUP";
                      g_sed_struct_val.param_steps = set_up;
                      g_sed_struct_val.param_value = (set_up * FINE_STEP_IN_PS);
                      //abort for wr setup
                      if ( (set_up) <= training_params_ptr->ddr_abort.min_wr_setup )
                      {  
                       g_abort_flag = 1;
                       training_params_ptr->small_eye_abort = 1;
                      }
					  if (((g_abort_flag) && (training_params_ptr->ddr_abort.min_wr_setup_enable)) || (g_sed_flag))
					  {
						DDRSS_SED_prints_struct();  
					  }
                      g_sed_struct_val.param_name = "WR_EYE_WIDTH_HOLD";
                      g_sed_struct_val.param_steps = hold;
                      g_sed_struct_val.param_value = (hold * FINE_STEP_IN_PS);
                      //abort for wr hold
                      if ((hold) <= training_params_ptr->ddr_abort.min_wr_hold)
                      {  
                       g_abort_flag = 1;
                       training_params_ptr->small_eye_abort = 1;
                      }
					  if (((g_abort_flag) &&(training_params_ptr->ddr_abort.min_wr_hold_enable)) || (g_sed_flag))
				      {
					    DDRSS_SED_prints_struct();  
				      }
                      //  g_sed_struct_val.param_name = "WR_DQS_DCD";
    ...
    // Write the trained coarse and fine center to the active CDC register
    DDR_PHY_hal_cfg_cdc_slave_wr(reg_offset_ddr_dq[byte_remapping_table[ch][phy_x]],
                                local_vars->cdc_ref[ pr[2][phy_x][MIDP] ][COARSE_L],
                                0x1,  // 0 for fine_delay 1 for coarse
                                HP_MODE,
                                cs);
    DDR_PHY_hal_cfg_cdc_slave_wr(reg_offset_ddr_dq[byte_remapping_table[ch][phy_x]],
                                local_vars->cdc_ref[ pr[2][phy_x][MIDP] ][FINE_L  ],
                                0x0, // 0 for fine_delay 1 for coarse
                                HP_MODE,
                                cs);
    // Write the trained coarse and fine to the EXT registers
...
// ============================================================================================================================
    for (phy_x = 0; phy_x < dq; phy_x++)
    {
        ddr_printf(DDR_NORMAL, "    PHY_DQ%d,strt= %d, stop= %d , eyew= %d (%d ps) ,\n"
			                 , byte_remapping_table[ch][phy_x]
			                 , pr[2][phy_x][STRT]
			                 , pr[2][phy_x][STOP]
			                 , pr[2][phy_x][EYEW]
			                 , local_vars->cdc_ref[   pr[2][phy_x][EYEW]   ][DELAY_L]);
        ddr_printf (DDR_NORMAL  , "    PHY_DQ%d,midp, %d, coarse, %d, fine, %d, Delay(ps), %d,  \n"
                                , byte_remapping_table[ch][phy_x]
                                , pr[2][phy_x][MIDP]
                                , local_vars->cdc_ref[ pr[2][phy_x][MIDP] ][COARSE_L]
                                , local_vars->cdc_ref[ pr[2][phy_x][MIDP] ][  FINE_L]
                                , local_vars->cdc_ref[ pr[2][phy_x][MIDP] ][ DELAY_L]);
    }//phy_x
    // ============================================================================================================================
    if(training_params_ptr->WR_Training_Switchboard[prfs_indx][_DCC   ]==1)
    {  
        for (phy_x = 0; phy_x < dq; phy_x++)
        {
            ddr_printf(DDR_NORMAL,"PHY_DQ%d,\ndqsdcc_adj_cur= %4d, dqsdcc_adj_otp= %4d, dqsdcc_adj_set= %4d, \ndq_dcc_adj_cur= %4d, dq_dcc_adj_otp= %4d, dq_dcc_adj_set= %4d, \n"
 	    		                 , byte_remapping_table[ch][phy_x]
                                 , dqsdcc_adj_cur[phy_x]
                                 , dqsdcc_adj_otp[phy_x]
                                 , dqsdcc_adj_set[phy_x]
...
//#if DSF_SMALL_EYE_DETECTION_LOG
  ...
  uint8 set_up =0;
  uint8 hold =0;    
  uint8 height_in_steps =0;
  uint32 height_in_mV =0;
  uint8 height_mid_top=0;
  uint8 height_mid_bottom =0;
  uint32 height_mid_top_mV=0;
  uint32 height_mid_bottom_mV=0;
  uint8 top_eye_width =0;
  uint8 bottom_eye_width =0;
  uint8 vref_trained =0;
  uint32 vref_trained_mV=0;  
...
// ============================================================================================================================
         ...
         vref_x += training_params_ptr->wr_dqdqs.vref_step)
    {
        ...
        for (phy_x = 0; phy_x < dq ; phy_x++)
        {
            ...
            if (max_v_eyew[phy_x] < v_pr[s_vref_x][phy_x][EYEW] )
            {
                ...
                v_th[phy_x]      =   (max_v_eyew[phy_x]*8)/10;  //threshold at 80%
                if (v_th[phy_x] < training_params_ptr->ddr_abort.min_wr_eye_width)//This will guarantee all WR EYEs meet the MIN EYE requirements
                {
                    v_th[phy_x] = training_params_ptr->ddr_abort.min_wr_eye_width;
                }
				
				if(sed_enable==0) {  //tparrett WR 2D Height abort fix
    ...
    //{
            ...
            vref_x += training_params_ptr->wr_dqdqs.vref_step)
        {
 		s_vref_x=vref_x/v_step;
            ...
            if ((v_pr[s_vref_x][0/*phy_x*/][EYEW] >= training_params_ptr->ddr_abort.min_wr_eye_width /*MIN_EYE_WIDTH*/ )&& 
                (v_pr[s_vref_x][1/*phy_x*/][EYEW] >= training_params_ptr->ddr_abort.min_wr_eye_width /*MIN_EYE_WIDTH*/ ))
            {
                v_hist[1][s_vref_x]=0;
            }
            else
        ...
        mid_v       = ((vmin[0]+vmax[0])>>1);//div2
        vref_data   = vknob2vrange(mid_v*v_step);
        fsp_op = (clk_freq_khz > ODT_FSP_EN_FREQ_THRESHOLD) ? 1 : 0 ;
        MC_MR_Write (REG_OFFSET_MC_CH(ch), CS_1HOT(cs), JEDEC_MR_14, vref_data);
        training_data_ptr->results.wr_dram.wr_dram_vref[ch][cs][0][fsp_op] = vref_data;
        min_eye_width_mask_top_Vref = ((mid_v*v_step) + training_params_ptr->ddr_abort.min_wr_eye_width_mask_top) > (VREF_RANGE_THRESHOLD + VREF_WR_LP4_MAX_VALUE) ? (VREF_RANGE_THRESHOLD + VREF_WR_LP4_MAX_VALUE) :((mid_v*v_step) + training_params_ptr->ddr_abort.min_wr_eye_width_mask_top);
        min_eye_width_mask_bottom_Vref = (mid_v*v_step) < (training_params_ptr->ddr_abort.min_wr_eye_width_mask_bottom) ? 0 :((mid_v*v_step) - training_params_ptr->ddr_abort.min_wr_eye_width_mask_bottom);
       // set_up = (v_pr[mid_v][phy_x][MIDP]- v_pr[mid_v][phy_x][STRT]);
        //hold = (v_pr[mid_v][phy_x][STOP]- v_pr[mid_v][phy_x][MIDP]);
        vref_trained = (mid_v*v_step); 
        vref_trained_mV = DDRSS_convert_vref_steps_to_mv((mid_v*v_step), DIR_WR, clk_freq_khz);
        top_eye_width = v_pr[min_eye_width_mask_top_Vref][phy_x][EYEW];
        bottom_eye_width = v_pr[min_eye_width_mask_bottom_Vref][phy_x][EYEW];
        height_in_steps = (vmax[2] - vmin[2]) * v_step;
        height_in_mV = (height_in_steps * VREF_WR_LP4x_STEP_IN_MV); 
        height_mid_top = (vmax[2]- mid_v)* v_step;
		height_mid_top_mV = (height_mid_top * VREF_WR_LP4x_STEP_IN_MV);
 		height_mid_bottom = (mid_v - vmin[2])* v_step;
        height_mid_bottom_mV = (height_mid_bottom * VREF_WR_LP4x_STEP_IN_MV);
		
        
        //ZERO EYE WIDTH ABORT
        if( (vmax[0] == 0) || (mid_v >= MAXVREF))
        {
                g_abort_flag = 1;            
                training_params_ptr->small_eye_abort = 1;

                  g_sed_struct_val.training = g_str_write;
                  g_sed_struct_val.param_name = "ZERO_EYE_WIDTH";
                  g_sed_struct_val.param_steps = 0;
                  g_sed_struct_val.param_value = 0;
                  g_sed_struct_val.param_units = g_str_unit_ps;
                  g_sed_struct_val.vref_type = "VREF_TRAIN";
                  g_sed_struct_val.vref_steps =  (mid_v*v_step);
                  g_sed_struct_val.vref_value = DC;
                  g_sed_struct_val.freq = clk_freq_khz;
                  g_sed_struct_val.ch = ch;
                  g_sed_struct_val.cs = cs;
        ...
        for (phy_x =0 ; phy_x < dq; phy_x++)
        {
		   ...
		   // mid_v_p0      = ((vmin[0]+vmax[0])>>1);//div2
		   // //mid_v_p1      = ((vmin[1]+vmax[1])>>1);//div2
           // dqsdcc_adj_otp =   v_pr[mid_v/*ODD*/][phy_x][MIDP]- v_pr[mid_v_p0/*even*/][phy_x][MIDP];
		   // dq_dcc_adj_otp =  (v_pr[mid_v/*ODD*/][phy_x][EYEW]- v_pr[mid_v_p0/*even*/][phy_x][EYEW]); 
			
			//dqsdcc_adj_otp[phy_x] =   v_pr[vmin[1]/*ODD*/][phy_x][MIDP]- v_pr[vmin[0]/*even*/][phy_x][MIDP];
		    //dq_dcc_adj_otp[phy_x] =  (v_pr[vmin[1]/*ODD*/][phy_x][EYEW]- v_pr[vmin[0]/*even*/][phy_x][EYEW]); 
			
                  g_sed_struct_val.training = g_str_write;
                  g_sed_struct_val.param_name = "WR_EYE_WIDTH";
                  g_sed_struct_val.param_steps = v_pr[mid_v][phy_x][EYEW];
                  g_sed_struct_val.param_value = v_pr[mid_v][phy_x][EYEW] * FINE_STEP_IN_PS;
                  g_sed_struct_val.param_units = g_str_unit_ps;
                  g_sed_struct_val.vref_type = "VREF_TRAIN";
                  g_sed_struct_val.vref_steps = vref_trained;
                  g_sed_struct_val.vref_value = vref_trained_mV;
                  ...
                  g_sed_struct_val.byte_lane = phy_x;
                      //abort for eye width
                      if ((v_pr[mid_v][phy_x][EYEW])< (training_params_ptr->ddr_abort.min_wr_eye_width))
                      {  
                       g_abort_flag = 1;
                       training_params_ptr->small_eye_abort = 1;
                      }
					  if (((g_abort_flag) &&(training_params_ptr->ddr_abort.min_wr_eye_width_enable)) || 
					       (g_sed_flag))
					  {
						DDRSS_SED_prints_struct();  
					  }
					  ...
					  if (((g_abort_flag) &&(training_params_ptr->ddr_abort.min_wr_setup_enable)) || 
					       (g_sed_flag))
					  {
						DDRSS_SED_prints_struct();  
					  }
					  ...
					  if (((g_abort_flag) &&(training_params_ptr->ddr_abort.min_wr_hold_enable)) ||
      					  (g_sed_flag))
					  {
						DDRSS_SED_prints_struct();  
					  }
                ...
                if(prfs_indx == MAX_TRAINING_FREQ_INDEX)
                {
                   ...
                   //  g_sed_struct_val.param_value = (dqsdcc_adj_otp/*[phy_x]*/* FINE_STEP_IN_PS);
                   //  //abort for wr dqsdcc_adj_otp
                   //  if ((((g_abort_flag)&&(dqsdcc_adj_otp/*[phy_x]*/ >= (training_params_ptr->ddr_abort.max_dqs_cm_dcc_enable))) || 
                   //        (g_sed_flag)))
                   //  {  
                   //   g_abort_flag = 1;
                   //   training_params_ptr->small_eye_abort = 1;
                   ...
                   //  g_sed_struct_val.param_value = (dq_dcc_adj_otp/*[phy_x]*/ * FINE_STEP_IN_PS);
                   //  //abort for wr dq_dcc_adj_otp
                   //  if ((((g_abort_flag)&&(dq_dcc_adj_otp/*[phy_x]*/ >=(training_params_ptr->ddr_abort.max_dqs_cm_dcc_enable))) || 
                   //        (g_sed_flag)))
                   //  {  
                   //   g_abort_flag = 1;
                   //   training_params_ptr->small_eye_abort = 1;
				   ...
				   //  }
                    ...
                    g_sed_struct_val.param_name = "WR_EYE_WIDTH_MASK_TOP";
                    g_sed_struct_val.param_steps = top_eye_width;
                    g_sed_struct_val.param_value = top_eye_width * FINE_STEP_IN_PS;
                    g_sed_struct_val.param_units = g_str_unit_ps;
                    g_sed_struct_val.vref_type = "VREF_TOP";
                    g_sed_struct_val.vref_steps =  min_eye_width_mask_top_Vref;
                    g_sed_struct_val.vref_value = DC;
					//abort for top eye width
					if ((vmax[1] < mid_v) || (((vmax[1] - mid_v)*v_step) <   training_params_ptr->ddr_abort.min_wr_eye_width_mask_top/*MIN_EYE_WIDTH*/))                      
                      {  
                       g_abort_flag = 1;
                       training_params_ptr->small_eye_abort = 1;
                      }
					  if (((g_abort_flag) && (training_params_ptr->ddr_abort.min_wr_eye_width_mask_top_enable)) || 
					      (g_sed_flag))
					  {
						DDRSS_SED_prints_struct();  
					  }						  
						g_sed_struct_val.param_name = "WR_EYE_WIDTH_MASK_BOT";
                      g_sed_struct_val.param_steps = bottom_eye_width;
                      g_sed_struct_val.param_value = bottom_eye_width * FINE_STEP_IN_PS;
                      g_sed_struct_val.vref_type = "VREF_BOTTOM";
                      g_sed_struct_val.vref_steps =  min_eye_width_mask_bottom_Vref;
					  //abort for bottom eye width
                      if ((mid_v < vmin[1]) || ((( mid_v-vmin[1] )*v_step) <  training_params_ptr->ddr_abort.min_wr_eye_width_mask_bottom))
                      {  
                       g_abort_flag = 1;
                       training_params_ptr->small_eye_abort = 1;
                      }
					  if (((g_abort_flag) &&(training_params_ptr->ddr_abort.min_wr_eye_width_mask_bottom_enable))|| 
					      (g_sed_flag))
					  {
						DDRSS_SED_prints_struct();  
					  }	
                        ...
                        g_sed_struct_val.param_name = "WR_EYE_HEIGHT";
                        g_sed_struct_val.param_steps = height_in_steps;
                        g_sed_struct_val.param_value = height_in_mV;
							g_sed_struct_val.param_units = g_str_unit_mV;
                        g_sed_struct_val.vref_type = "HEIGHT";
                        g_sed_struct_val.vref_steps = DC;
                        g_sed_struct_val.vref_value = DC;
                        g_sed_struct_val.byte_lane = phy_x;
                       if((vmax[2] < vmin[2]) || (((vmax[2]-vmin[2])*v_step ) < training_params_ptr->ddr_abort.min_wr_eye_height))
                            
                          {
                               g_abort_flag = 1;
                               training_params_ptr->small_eye_abort = 1;                        
                          }
                        if (((g_abort_flag) &&(training_params_ptr->ddr_abort.min_wr_eye_height_enable))|| 
						     (g_sed_flag))
						  {
						   DDRSS_SED_prints_struct();  
					      }
							g_sed_struct_val.param_name = "WR_EYE_HEIGHT_MID_TOP";
                        g_sed_struct_val.param_steps = height_mid_top;
                        g_sed_struct_val.param_value = height_mid_top_mV;
							g_sed_struct_val.param_units = g_str_unit_mV;
                        g_sed_struct_val.vref_type = "HEIGHT";
                        g_sed_struct_val.vref_steps = DC;
                        g_sed_struct_val.vref_value = DC;
                        ...
                        if (((g_abort_flag) &&(training_params_ptr->ddr_abort.min_wr_eye_height_enable))|| 
						     (g_sed_flag))
						  {
						   DDRSS_SED_prints_struct();  
							...
							g_sed_struct_val.param_name = "WR_EYE_HEIGHT_MID_BOT";
                        g_sed_struct_val.param_steps = height_mid_bottom;
                        g_sed_struct_val.param_value = height_mid_bottom_mV;
							g_sed_struct_val.param_units = g_str_unit_mV;
                        g_sed_struct_val.vref_type = "HEIGHT";
                        g_sed_struct_val.vref_steps = DC;
                        g_sed_struct_val.vref_value = DC;
    ...
    for (phy_x = 0; phy_x < dq ; phy_x++)
    {        
		mid_cdc[phy_x] = v_pr[mid_v][phy_x][MIDP];
        mid_cdc[phy_x] = (local_vars->cdc_ref[mid_cdc[phy_x] ][FINE_L] <  WRITE_FINE_CDC_MIN) ? (mid_cdc[phy_x] + (WRITE_FINE_CDC_MIN - local_vars->cdc_ref[mid_cdc[phy_x] ][FINE_L])) : mid_cdc[phy_x];
		mid_cdc[phy_x] = (local_vars->cdc_ref[mid_cdc[phy_x] ][FINE_L] >  WRITE_FINE_CDC_MAX) ? (mid_cdc[phy_x] - (local_vars->cdc_ref[mid_cdc[phy_x] ][FINE_L] - WRITE_FINE_CDC_MAX)) : mid_cdc[phy_x];
				
#if DSF_WR_DQDQS_TRAINING_LOG_IN
        ddr_printf (DDR_NORMAL, "PHY_DQ, %d,   \n",byte_remapping_table[ch][phy_x]);
        ddr_printf (DDR_NORMAL, "vmin, %d,vmax, %d  mid_v(Dec): %d, vref_data(Hex): %x   \n",vmin[0]*v_step,vmax[0]*v_step,mid_v*v_step,vref_data );
        ddr_printf (DDR_NORMAL, "mid_cdc, %d, coarse, %d, fine, %d, delay, %d, \n"
            ...
            // ============================================================================================================================
             ...
             mid_v_p60 = (mid_v *v_step) + (60/VREF_WR_LP4x_STEP_IN_MV);
             mid_v_n60 = ((mid_v * v_step) < (60/VREF_WR_LP4x_STEP_IN_MV)) ? 0 : ((mid_v * v_step) - (60/VREF_WR_LP4x_STEP_IN_MV));
             mid_v_p60_mv = DDRSS_convert_vref_steps_to_mv(mid_v_p60 , DIR_WR, clk_freq_khz);
             mid_v_n60_mv = DDRSS_convert_vref_steps_to_mv(mid_v_n60 , DIR_WR, clk_freq_khz);

                 if ((prfs_indx == MAX_TRAINING_FREQ_INDEX) && (g_sed_flag)&&(sed_enable))
                 {
                    g_sed_struct_val.training = g_str_write;
                    g_sed_struct_val.param_name = "WR_EYE_WIDTH_SIPI_+60mv";
                    g_sed_struct_val.param_steps = v_pr[mid_v_p60][phy_x][EYEW];
                    g_sed_struct_val.param_value = v_pr[mid_v_p60][phy_x][EYEW] * FINE_STEP_IN_PS;
                    g_sed_struct_val.param_units = g_str_unit_ps;
                    g_sed_struct_val.vref_type = "VREF_SIPI+";
                    g_sed_struct_val.vref_steps = mid_v_p60;
                    g_sed_struct_val.vref_value = mid_v_p60_mv;
                    g_sed_struct_val.freq = clk_freq_khz;
                    g_sed_struct_val.ch = ch;
                    g_sed_struct_val.cs = cs;
                    g_sed_struct_val.byte_lane = phy_x;
                    DDRSS_SED_prints_struct();
                    g_sed_struct_val.param_name = "WR_EYE_WIDTH_SIPI_-60mv";
                    g_sed_struct_val.param_steps = v_pr[mid_v_n60][phy_x][EYEW];
                    g_sed_struct_val.param_value = v_pr[mid_v_n60][phy_x][EYEW] * FINE_STEP_IN_PS;
                    g_sed_struct_val.vref_type = "VREF_SIPI-";
                    g_sed_struct_val.vref_steps = mid_v_n60;
                    g_sed_struct_val.vref_value = mid_v_n60_mv;
                    DDRSS_SED_prints_struct();
    ...
    //if(SCALED_FREQ_EN) //TODO : remove once scaling is fixed
    {
      for (scale = 1; scale <= NUM_SCALE_BANDS; scale++)
      {
        // Calculate the scaled PRFS index
        scaled_prfs_indx = prfs_indx - scale ;
        target_freq      = SCALE_FREQ(scaled_prfs_indx); //scaled_prfs_indx
        period =(CONVERT_CYC_TO_PS / clk_freq_khz);
		target_period = (CONVERT_CYC_TO_PS / target_freq);
        T4 = period/4;		
        for (phy_x = 0; phy_x < dq; phy_x++)
        { 
          scaled_cdc_delay_in_ps = ((target_period/4)+ ((local_vars->cdc_ref[mid_cdc[phy_x] ][DELAY_L]) - (T4)));
#if DSF_WR_DQDQS_TRAINING_LOG_IN          
...
// ============================================================================================================================
  ...
  memset(local_vars->failcount_histogram, 0,3 * NUM_DQ_PCH * PINS_PER_PHY * HISTOGRAM_SIZE);
// ======================================      cdc ref                   =================================
    DDRSS_cdcref_init_wr(ddr, local_vars->cdc_ref);
// ============================================================================================================================
// ============================================================================================================================
...
// ============================================================================================================================
    ...
    for (phy_x = 0; phy_x < dq; phy_x++)
    {
        ...
        ddr_printf (DDR_NORMAL, "=================PHY_DQ_%d,\n",byte_remapping_table[ch][phy_x]);
        // ============================================================================================================================
        for (bit = 0; bit < BYTE_SIZE; bit++)
        {
            remapped_bit  =  bit_remapping_bimc2phy_DQ[ch][byte_remapping_table[ch][phy_x]][bit] ;
        ...
        // ============================================================================================================================
        ddr_printf (DDR_NORMAL, "%s\n","perbit Delay");
        ddr_printf (DDR_NORMAL, "   wr_dq_dcc[%d][%d]=, %d ,\n"  ,cs ,byte_remapping_table[ch][phy_x],     dq_dcc_adj[phy_x]);
        ddr_printf (DDR_NORMAL, "   wr_dqsdcc[%d][%d]=, %d ,\n"  ,cs ,byte_remapping_table[ch][phy_x],     dqsdcc_adj[phy_x]);
        ddr_printf (DDR_NORMAL, "   pr_bus_eyew[phase0][%d]=, %d ,\n",byte_remapping_table[ch][phy_x], pr_bus_eyew[0][phy_x]);
        ddr_printf (DDR_NORMAL, "   pr_bus_skwr[phase0][%d]=, %d ,\n",byte_remapping_table[ch][phy_x], pr_bus_skwr[0][phy_x]);
        ddr_printf (DDR_NORMAL, "   pr_bus_eyew[phase1][%d]=, %d ,\n",byte_remapping_table[ch][phy_x], pr_bus_eyew[1][phy_x]);
        ddr_printf (DDR_NORMAL, "   pr_bus_skwr[phase1][%d]=, %d ,\n",byte_remapping_table[ch][phy_x], pr_bus_skwr[1][phy_x]);
        ddr_printf (DDR_NORMAL, "pr_bus_eyew[bothphase][%d]=, %d ,\n",byte_remapping_table[ch][phy_x], pr_bus_eyew[2][phy_x]);
        ddr_printf (DDR_NORMAL, "pr_bus_skwr[bothphase][%d]=, %d ,\n",byte_remapping_table[ch][phy_x], pr_bus_skwr[2][phy_x]);
    }//phy_x
    return(1);
}// END OF FUNCTION
...
int DDRSS_wr_csr2dcc ( uint16 dcc_csr)
{
    int dcc_adj = 0;
...
int DDRSS_set_cdcext_slave_wr(BOOT_DDR_STRUCT *ddr, uint8 ch, uint8 cs, uint8 dq, uint32 freq_khz, uint8 coarse_delay, uint8 fine_delay)
{
    ...
    if (freq_khz > ddr->shrm_ddr_ptr->ddr_runtime.clock_plan[training_data_ptr->results.dit.dit_acq_ndx].clk_freq_in_khz)
	{
		DDRSS_Periodic_Training_Calc_And_Copy_To_SHRM(ddr, ch, 1<<cs);// takes a bit mask for CS
	}
...
uint8 DDRSS_get_active_wr_vref(uint8 cs, uint8 ch, uint8 dq)
{
	...
	return vrange2vknob(MC_MR_Read(REG_OFFSET_MC_CH(ch), CS_1HOT(cs), JEDEC_MR_14));
}

int DDRSS_get_wr_vref(BOOT_DDR_STRUCT *ddr, uint8 ch, uint8 cs, uint8 dq, uint32 freq_khz)
{
	uint8 fsp_op;
...
void DDRSS_cpy_dram_wr_vref_r0tor1(BOOT_DDR_STRUCT *ddr,uint8 ch )
{  
    uint8  MR14_value  = 0;
    training_data *training_data_ptr;
    training_data_ptr = (training_data *)(&ddr->training_results.training_data);
    MR14_value = MC_MR_Read  (REG_OFFSET_MC_CH(ch), DDR_CS0, JEDEC_MR_14);
    MC_MR_Write (REG_OFFSET_MC_CH(ch), DDR_CS1, JEDEC_MR_14, MR14_value);
    training_data_ptr->results.wr_dram.wr_dram_vref[ch][1][0][0/*FSP0*/]=MR14_value;
