...
static void usb_transmit_byte(byte chr)
{
  qusb_log__param(HOSTDL_USB_TRANSMIT_BYTE_LOG, chr);
  qusb_fd_tx_byte(chr);
}
...
static boolean usb_active(void)
{
  ...
  * TRUE.
  */
  qusb_log(HOSTDL_USB_ACTIVE_LOG);
  return TRUE;
}

static void ap_rx_complete(struct qusb_urb* urb)
{
  qusb_log(HOSTDL_AP_RX_COMPLETE_LOG);
...
{
  uint32 t_end = 0;
  qusb_log_bus(HOSTDL_USB_INIT_LOG);
  ...
  if ( NULL != dev ) 
  {
    while ( t_end != QUSB_HOSTDL_CONNECTION_TIMEOUT ) 
    {
      ...
      {
        qusb_log_bus(HOSTDL_USB_INIT_SUCCESS_LOG);
        return TRUE;
      }
      qusb_dci_delay_ms(1); 
      t_end++; 
  ...
  qusb_log_err(HOSTDL_USB_INIT_FAILED_LOG);
  return FALSE;
}
...
static void usb_deinit(void)
{
  qusb_log_bus(HOSTDL_USB_DEINIT_LOG);
