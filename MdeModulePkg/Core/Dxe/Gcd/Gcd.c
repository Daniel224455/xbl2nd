...
#if defined(QCOM_EDK2_PATCH)
      // Skip all the DDR banks above 4GB      
      if ((ResourceHob->PhysicalStart) > (EFI_PHYSICAL_ADDRESS)0xFFFFFFFF) {
        continue;
      }      
#endif      
...
#if defined(QCOM_EDK2_PATCH)
      // check if our memory requirements can fit within 4GB      
      if ((TestedMemoryBaseAddress + MinimalMemorySizeNeeded) > (EFI_PHYSICAL_ADDRESS)0xFFFFFFFF) {
        continue;
      }      
      
      TestedMemoryLength      = PageAlignLength  ((EFI_PHYSICAL_ADDRESS)0xFFFFFFFF - TestedMemoryBaseAddress);
