...
void ddr_share_data_to_aop(void)
{
  ...
  size_info[PASR_DDR_INFO_TABLE] = sizeof(pasr_shared_ddr_info);
  size_info[ALC_POLICY_INDEX] = sizeof(ddrsns_share_data->aop_params.alc_policy_index);
  ...
  ddr_part_info.ddr_cs0_addr = ddrsns_share_data->shrm_ddr_ptr->ddr_size_info.ddr_cs0_addr[0];
  ddr_part_info.ddr_cs1_addr = ddrsns_share_data->shrm_ddr_ptr->ddr_size_info.ddr_cs1_addr[0];
  ddr_part_info.ddr_cs0_remapped_addr= ddrsns_share_data->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0];
  ddr_part_info.ddr_cs1_remapped_addr= ddrsns_share_data->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0];
  ddr_part_info.per_ch_density_rank0= ddrsns_share_data->shrm_ddr_ptr->detected_ddr_device[0].device_density[0];
  ddr_part_info.per_ch_density_rank1= ddrsns_share_data->shrm_ddr_ptr->detected_ddr_device[0].device_density[1];
  ddr_part_info.num_channels= ddr_get_max_channels();
  ...
  /* Copy pasr table to smem */
  qmemscpy(smem_ptr+ddr_smem_info_ptr->shared_table_header[PASR_DDR_INFO_TABLE].offset, ddr_smem_info_ptr->shared_table_header[PASR_DDR_INFO_TABLE].size, 
           &ddr_part_info, ddr_smem_info_ptr->shared_table_header[PASR_DDR_INFO_TABLE].size);  
  /* Copy alc_policy_index table to smem */
  qmemscpy(smem_ptr+ddr_smem_info_ptr->shared_table_header[ALC_POLICY_INDEX].offset, ddr_smem_info_ptr->shared_table_header[ALC_POLICY_INDEX].size, 
           &ddrsns_share_data->aop_params.alc_policy_index[0], ddr_smem_info_ptr->shared_table_header[ALC_POLICY_INDEX].size);
