...
--------   ---     ----------------------------------------------------------
02/08/18   sv      Added external charging support for CLS platform.
10/17/17   czq     Executing FG sram reading related WAs after battery missing detection. 
...
pm_sbl_chg_init (void)
{
      ...
      ( (pmic_model == PMIC_IS_PMI8998)) )
   {
	  ...
	  //WA£º Check FG sram healthy. 
	  err_flag |= pm_sbl_check_fg_sram_status(device_index);
	  if(err_flag != PM_ERR_FLAG_SUCCESS)
	  {
		 return err_flag;
	  }
	  //WA: compare vbat gain data between OTP and Sram.
      err_flag = pm_sbl_copy_vbat_gain_from_pbs_otp_to_sram();
      if(err_flag != PM_ERR_FLAG_SUCCESS)
      {
        return err_flag;
      }

      //WA: [8998]WA for Erroneous USBIN auto-power-on(CR2005474)
   ...
   else if(platform_type == DALPLATFORMINFO_TYPE_CLS)
   {
       pm_ext_chgr_weakBatt_init();
   }
...
pm_err_flag_type pm_system_init(void)
{
   ...
   if(pm_get_pmic_model(PMI_INDEX) == PMIC_IS_PMI8998)
   {
...
pm_config_wlan_active_and_retention_level(void)
{
    ...
    err_flag |= pm_comm_write_byte_array(MISC_SLAVE_ADDR, MISC_VSET2_L5_RET_ADDR_LB, 2, retention_level, 1);
    ...
    err_flag |= pm_comm_write_byte_array(MISC_SLAVE_ADDR, MISC_VSET1_L5_ACTIVE_ADDR_LB, 2, active_level, 1);
