...
static struct qusb_dcd_dsc_interface* find_ifc(const struct qusb_dcd_dsc_device* dev)
{
  ...
  {
    qusb_log_err__param(DCD_CH9_NULL_POINTER_ERROR_LOG, __LINE__);
    return NULL;
  }
}
/**
...
static void qusb_dcd_update_packet_size(void)
{
  ...
  qusb_log_bus__param(DCD_UPDATE_PACKET_SIZE_LOG, max_packet_size);
  return;
}
...
{
  qusb_log_bus__param(DCD_NOTIFY_LOG, event);
  if ( NULL != dev->notify ) 
  {
...
static int dcd_set_interface(struct qusb_dcd_dsc_configuration* conf, uint8 ifc_num, uint8 alt)
{
  ...
  qusb_log_bus__param(DCD_SET_INTERFACE_LOG, ifc_num);
...
int qusb_dcd_set_config(struct qusb_dcd_dsc_device* dev, uint8 new_config)
{
  ...
  //another
  qusb_log_bus__param(DCD_SET_CONFIG_LOG, new_config);
...
int qusb_dcd_control_transfer(struct qusb_dcd_dsc_device* dev, int is_tx, void* buff, uint32 len)
{
  ...
  if (is_tx == 1)
  {
    qusb_log_bus__param(DCD_CONTROL_TRANSFER_TX_LOG, len);
  }
  else
  {
    qusb_log_bus__param(DCD_CONTROL_TRANSFER_RX_LOG, len);
  }
...
int handle_get_descriptor(struct qusb_dcd_dsc_device* dev, struct usb_setup_packet* req)
{
  ...
  switch ( dsc->desc_type ) 
  {
    case USB_DESCTYPE_DEVICE:
      ...
      qusb_log_bus__param(HANDLE_GET_DESCRIPTOR_DEVICE_LOG, len);
    break;
    ...
    case USB_DESCTYPE_CONFIGURATION:
    {
      ...
      qusb_log_bus__param(HANDLE_GET_DESCRIPTOR_CONFIG_LOG, len);
    }
    break;
    ...
    case USB_DESCTYPE_BOS:
    {
      ...
      qusb_log_bus__param(HANDLE_GET_DESCRIPTOR_BOS_LOG, len);
    }
    break;
    ...
    case USB_DESCTYPE_STRING:
    {
      ...
      qusb_log_bus__param(HANDLE_GET_DESCRIPTOR_STRING_LOG, req->wIndex);
    }
    break;
    ...
    case USB_DESCTYPE_DEV_QUAL:
    {
        ...
        && (NULL != dev->dev_qual))
      {
        ...
        qusb_log_bus__param(HANDLE_GET_DESCRIPTOR_DEVQUAL_LOG, len);
      }
    }
    break;
    ...
    default:
      qusb_log_bus__param(UNKNOWN_GET_DESCRIPTOR_RECEIVED_LOG, dsc->desc_type);
  ...
  switch ( req->bRequest )
  {
    case USB_SETUP_REQ_GET_STATUS:
      ...
      { /* USB standard expects 2 bytes here */
        ...
        qusb_log_bus__param(HANDLE_SETUP_DEV_GET_STATUS_LOG, dev->setup_data[0]);
    ...
    case USB_SETUP_REQ_CLEAR_FEATURE:
      ...
      qusb_log_bus__param(HANDLE_SETUP_DEV_CLEAR_FEATURE_LOG, req->wValue);
      return error;
    ...
    case USB_SETUP_REQ_SET_FEATURE:
      ...
      qusb_log_bus__param(HANDLE_SETUP_DEV_SET_FEATURE_LOG, req->wValue);
      return error;
    ...
    case USB_SETUP_REQ_SET_ADDRESS:
      if ( 0 == dev->config_value )
      {
        ...
        qusb_log_bus__param(HANDLE_SETUP_DEV_SET_ADDRESS_LOG, dev->address);
        return 0;
      }
    break;
    ...
    case USB_SETUP_REQ_SET_SEL:
      if ( req->wLength == 6 )
      {
        ...
        qusb_log_bus__param(HANDLE_SETUP_DEV_SET_SEL_LOG, req->wLength);
    ...
    case USB_SETUP_REQ_SET_ISOCH_DELAY:
      ...
      qusb_log_bus(HANDLE_SETUP_DEV_SET_ISOCH_DELAY_LOG);
      return 0;

    case USB_SETUP_REQ_GET_DESCRIPTOR:
      qusb_log_bus(HANDLE_SETUP_DEV_GET_DESCRIPTOR_LOG);
      return handle_get_descriptor(dev, req);
    
    case USB_SETUP_REQ_SET_DESCRIPTOR:
      qusb_log_bus(HANDLE_SETUP_DEV_SET_DESCRIPTOR_LOG);
      /* not supported (yet?) */
    break;
    ...
    case USB_SETUP_REQ_GET_CONFIGURATION:
      ...
      { /* USB standard expects 1 byte here */
        ...
        qusb_log_bus__param(HANDLE_SETUP_DEV_GET_CONFIGURATION_LOG, dev->setup_data[0]);
    ...
    case USB_SETUP_REQ_SET_CONFIGURATION:
      qusb_log_bus__param(HANDLE_SETUP_DEV_SET_CONFIGURATION_LOG, (uint8)(req->wValue & 0xff));
  ...
  switch ( req->bRequest )
  {
    case USB_SETUP_REQ_GET_STATUS:
      ...
      { /* USB standard expects 2 bytes here */
        ...
        qusb_log_bus__param(HANDLE_SETUP_IFC_GET_STATUS_LOG, dev->setup_data[0]);
    ...
    case USB_SETUP_REQ_CLEAR_FEATURE:
      qusb_log_bus(HANDLE_SETUP_IFC_CLEAR_FEATURE_LOG);
    break;
    ...
    case USB_SETUP_REQ_SET_FEATURE:
      if (req->wValue == USB_FSEL_IFACE_FUNC_SUSP)
      {
        ...
        qusb_log_bus__param(HANDLE_SETUP_IFC_SET_FEATURE_LOG, dev->remote_wakeup_enabled);  
        return 0;
      }
      else
      {
    ...
    case USB_SETUP_REQ_GET_INTERFACE:
      ...
      { 
        ...
        qusb_log_bus__param(HANDLE_SETUP_IFC_GET_INTERFACE_LOG, dev->setup_data[0]);
        error = qusb_dcd_control_transfer(dev, dev->ctrl_data_dir, dev->setup_data, 1);
        return error;
      }
    break;
    ...
    /* ifc as found above, is irrelevant */
    {
      ...
      if ( conf != NULL )
      {
        qusb_log_bus(HANDLE_SETUP_IFC_SET_INTERFACE_LOG);
    ...
    default:
      ...
      {
        qusb_log_bus(CALL_SETUP_HANDLER_FOR_IFC_LOG);
                          ...
                          struct qusb_dcd_dsc_endpoint* ep, int halt)
{
  ...
  qusb_log_bus__param(HANDLE_EP_HALT_LOG, ep_num);
  ...
  switch ( req->bRequest )
  {
    case USB_SETUP_REQ_GET_STATUS:
      if ( req->wLength >= 2 ) 
      {
        if(dev->setup_data)
        {
          ...
          qusb_log_bus__param(HANDLE_SETUP_EP_GET_STATUS_LOG, dev->setup_data[0]); 
          error = qusb_dcd_control_transfer(dev, dev->ctrl_data_dir, dev->setup_data, 2);
        }
        return error;
      }
    ...
    case USB_SETUP_REQ_CLEAR_FEATURE:
      ...
      { /* feature selector */
        case USB_FSEL_EP_HALT:
          if(NULL != ep)
          {
            qusb_log_bus(HANDLE_SETUP_EP_CLEAR_FEATURE_LOG);
    ...
    case USB_SETUP_REQ_SET_FEATURE:
      ...
      { /* feature selector */
        ...
        if(NULL != ep)
        {
          qusb_log_bus(HANDLE_SETUP_EP_SET_FEATURE_LOG);
    ...
    case USB_SETUP_REQ_SYNCH_FRAME:
      qusb_log_bus(HANDLE_SETUP_EP_REQ_SYNCH_FRAME_LOG);
    break;

    default:
      if ( (NULL != ep) && (NULL != ep->setup) )
      {
        qusb_log_bus(CALL_SETUP_HANDLER_FOR_EP_LOG);
...
static void dcd_callback_reset(struct qusb_dcd_dsc_device* dev)
{
  ...
  qusb_log_bus(DCD_CALLBACK_RESET_LOG);
...
static void dcd_callback_disconnect(struct qusb_dcd_dsc_device* dev)
{
  qusb_log_bus(DCD_CALLBACK_DISCONNECT_LOG);
  dcd_notify(dev,QUSB_DCD_EVT_DISCONNECT);
}

static void dcd_callback_suspend(struct qusb_dcd_dsc_device* dev)
{
  qusb_log_bus(DCD_CALLBACK_SUSPEND_LOG);
...
static void dcd_callback_setup(struct qusb_dcd_dsc_device* dev, void* buffer_ptr)
{
  ...
  { 
    /* error, stall EP0 */
    qusb_log_err__param(DCD_UNSUPPORTED_SETUP_REQUEST_LOG, ((req->bmRequestType >> USB_SETUP_RT_RCP_POS) & USB_SETUP_RT_RCP_SEED ));
...
void qusb_dcd_port_status_changed(struct qusb_dcd_dsc_device* dev)
{
  ...
  qusb_log_bus__param(DCD_PORT_STATUS_CHANGED_LOG, dev->speed);
...
{
  uint32 i = 0; 
  qusb_log_bus__param(DCD_SHUTDOWN_LOG, dev->shutdown_mode);
...
int qusb_submit_urb(struct qusb_urb* urb)
{
  ...
  qusb_log_err__param(DCD_SUBMIT_URB_ERROR_LOG, urb->transfer_status);
  return -1;
}
