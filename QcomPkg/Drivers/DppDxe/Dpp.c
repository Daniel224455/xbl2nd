/** @file Dpp.c
  ...
  Copyright (c) 2012-2017 Qualcomm Technologies, Inc. All rights 
  reserved. 
...
--------   ---      ----------------------------------------------------------- 
07/05/17   gpavithr Replaced error logs with relevant information
    ...
    )
{
  ...
  *FileInfo = NULL;
  ...
  {    
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN,"[%a] AllocatePool(%d) failed. Returning: %r\n",
             __FUNCTION__, *FileInfoSize, Status));
      goto GetFatFileInfoExit;
    }
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN,"[%a] File->GetInfo failed. Returning: %r\n",
             __FUNCTION__, Status));
      goto GetFatFileInfoExit;
    }
  }
  ...
  {
    DEBUG((EFI_D_WARN,"[%a] Failed to get file size. Returning: %r\n",
           __FUNCTION__, Status));
    goto GetFatFileInfoExit;
  }
  else
  {
    ...
    DEBUG((EFI_D_WARN,"[%a] File->GetInfo returned unexpected SUCCESS. "
           "Returning: %r\n", __FUNCTION__, Status));
    goto GetFatFileInfoExit;
  }
  ...
  )
{
      ...
      if(Status == EFI_NOT_FOUND)
      {
        ...
        if(EFI_ERROR(Status))
        {
          DEBUG((EFI_D_WARN,"[%a] Failed to create directories in filepath: (%s). "
                 "Returning: %r\n", __FUNCTION__, pFilePath, Status));
          goto FatSetDPPExit;
        }

        DEBUG((EFI_D_WARN,"Directories in filepath(%s) are created\n", pFilePath));
        Status = FsRoot->Open(FsRoot,
                              &File,
                              pFilePath,                          
                              OpenMode,
  ...
  {
    Status = EFI_NOT_FOUND;
    DEBUG((EFI_D_WARN,"[%a] FsRoot is NULL, OpenVolume returned unexpected "
           "SUCCESS. Returning: %r\n", __FUNCTION__, Status));
    goto FatSetDPPExit;
  }
  ...
  if (File != NULL)
  {
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN,"[%a] Write(%s) failed for length(%d). Returning: %r\n",
             __FUNCTION__, pFilePath, DataSize, Status));
      goto FatSetDPPExit;
    }
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN,"[%a] GetFatFileInfo(%s) failed. Returning: %r\n",
             __FUNCTION__, pFilePath, Status));
      goto FatSetDPPExit;
    }
  ...
  )
{
  ...
  {
    Status = EFI_NOT_FOUND;
    DEBUG((EFI_D_WARN,"%a(): FileInfo:%p File:%p ,GetFatFileInfo returned "
           "unexpected SUCCESS. Returning: %r\n", __FUNCTION__, FileInfo, File, Status));
    goto FatGetDPPExit;
  }
FatGetDPPExit:
  ...
  )
{
  ...
  if(!EFI_ERROR(Status) && (FileInfo != NULL))
  {
    ...
    if (!EFI_ERROR(Status) && (Buffer != NULL))
    {
      ...
      {
        DEBUG((EFI_D_WARN,"[%a] Write(%s) failed for length(%d). Returning: %r\n",
               __FUNCTION__, pFilePath, FileSize, Status));
        goto FatDeleteItemDPPExit;
      }
  ...
  )
{
  ...
  else
  {
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN, "[%a] Unsupported DPP type. Returning: %r\n",
             __FUNCTION__, Status));
      goto EraseDPPExit;
    }
  }
  ...
  )
{
  ...
  if (gDppPartType == DPP_PARTITION_TYPE_BLOCK)
  {
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN, "[%a] BlockSetDPP failed. Returning: %r\n",
             __FUNCTION__, Status));
      goto SetDPPExit;
    }
  }
  else if (gDppPartType == DPP_PARTITION_TYPE_FAT)
  {
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN, "[%a] FatSetDPP failed. Returning: %r\n",
             __FUNCTION__, Status));
      goto SetDPPExit;
    }
  }
  ...
  )
{
  ...
  if (gDppPartType == DPP_PARTITION_TYPE_BLOCK)
  {
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN, "[%a] BlockGetDPP failed. Returning: %r\n",
             __FUNCTION__, Status));
      goto GetDPPExit;
    }
  }
  else if (gDppPartType == DPP_PARTITION_TYPE_FAT)
  {
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN, "[%a] FatGetDPP failed. Returning: %r\n",
             __FUNCTION__, Status));
      goto GetDPPExit;
    }
  }
  ...
  )
{
  ...
  if (gDppPartType == DPP_PARTITION_TYPE_BLOCK)
  {
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN, "[%a] BlockDeleteItemDPP failed. Returning: %r\n",
             __FUNCTION__, Status));
      goto DeleteDPPExit;
    }
  }
  else if (gDppPartType == DPP_PARTITION_TYPE_FAT)
  {
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN, "[%a] FatDeleteItemDPP failed. Returning: %r\n",
             __FUNCTION__, Status));
      goto DeleteDPPExit;
    }
  }
  ...
  )
{
  ...
  {
    Status = EFI_UNSUPPORTED;
    DEBUG((EFI_D_WARN,"\n[%a] Warning: Failed to mount DPP FAT partition. "
           "Returning: %r\n", __FUNCTION__, Status));
    goto InitializeDppProtocolExit;
  }
  ...
  if(!EFI_ERROR(Status) && (PartHandle != NULL))
  {
    ...
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_WARN,"[%a] BlockDppProtocolInit failed. Returning: %r\n",
             __FUNCTION__, Status));
      goto InitializeDppProtocolExit;
    }
  ...
  }else if (Status == EFI_NOT_FOUND)
  {
    ...
    if(!EFI_ERROR(Status) && (PartHandle != NULL))
    {
      ...
      if(EFI_ERROR(Status))
      {
        DEBUG((EFI_D_WARN,"[%a] FatDppProtocolInit failed. Returning: %r\n",
               __FUNCTION__, Status));
        goto InitializeDppProtocolExit;
      }
    ...
    {
      Status = EFI_UNSUPPORTED;
      DEBUG((EFI_D_WARN,"\n [%a] Warning: DPP not found, please check if DPP is "
             "formated. Returning: %r\n", __FUNCTION__, Status));
      goto InitializeDppProtocolExit;
  ...
  {
     Status = EFI_UNSUPPORTED;
     DEBUG((EFI_D_WARN,"\n [%a] Warning: Block DPP unsupported platform type. "
            "Returning: %r\n", __FUNCTION__, Status));
     goto InitializeDppProtocolExit;
  }
  ...
  if(EFI_ERROR(Status))
  {
    DEBUG((EFI_D_WARN,"[%a] InstallMultipleProtocolInterfaces(EfiDppProtocolGuid) "
           "failed. Returning: %r\n", __FUNCTION__, Status));
    goto InitializeDppProtocolExit;
  }
  if (gDppPartType == DPP_PARTITION_TYPE_BLOCK)
