...
typedef struct
{
  ...
  eDcvsModeType      eDcvsMode;               /**< DCVS Mode type (HW_ALL, SW_ALL, SW_ANY) */
...
/**
  ...
  This function switches a clock mux to a new configuration. No dependencies
  (source, voltage, etc...) are handled as part of this function.

  @param *pClockDomain [in] -- Clock domain whose mux to configure.
  @param *pNewConfig [in] -- New mux configuration to use.

  @return
  DAL_SUCCESS -- The mux was successfully configured.
  DAL_ERROR -- The mux configuration failed.
*/
