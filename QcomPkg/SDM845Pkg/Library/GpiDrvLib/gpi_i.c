...
GPI_RETURN_STATUS init_threads(gpi_ctxt *gpi)
{
   ...
   gpi->gpi_thread.priority = 10;  /* Set thread priority */
   gpi->gpi_thread.state = GPI_THREAD_INITIAL;  /* State */
   reset_evt_q(&gpi->gpi_thread.q);  /* Queue */
   if (GPI_STATUS_SUCCESS != gpi_syncinit(&gpi->gpi_thread.q.lock))  /* Spinlock */
   {
      gpi_log(GPI_ERROR, "ERROR - Thread %d gpi_syncinit() failed!\n", i);
      return GPI_STATUS_ERROR;
   }
...
GPI_RETURN_STATUS set_gpii_evt_chan(gpii_info *gpii)
{
   ...
   gpii->evt_ring_info.base_va = (ptr_type)&gpii->evt_ring_elem[0];
   gpii->evt_ring_info.base_pa = (ptr_type)gpi_va_to_pa((void *)gpii->evt_ring_info.base_va);
...
GPI_RETURN_STATUS gpii_init(gpii_info *gpii)
{
   ...
   if (((HWIO_EE_n_GSI_STATUS_INMI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, 
                                   HWIO_EE_n_GSI_STATUS_ENABLED_BMSK)) >> HWIO_EE_n_GSI_STATUS_ENABLED_SHFT) != 0)
   {
      gpii->gpi->gpii_hw_ready_bmsk |= 1 << gpii->id;

      HWIO_EE_n_CNTXT_TYPE_IRQ_MSK_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, ~0x0u);
      HWIO_EE_n_CNTXT_SRC_IEOB_IRQ_MSK_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, ~0x0u);
      HWIO_EE_n_CNTXT_SRC_GSI_CH_IRQ_MSK_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, ~0x0u);
      HWIO_EE_n_CNTXT_SRC_EV_CH_IRQ_MSK_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, ~0x0u);
      HWIO_EE_n_CNTXT_GLOB_IRQ_EN_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, ~0x0u);
      HWIO_EE_n_CNTXT_GSI_IRQ_EN_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, ~0x0u);

      /* Set interrupt type */
      HWIO_EE_n_CNTXT_INTSET_OUTMI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, HWIO_EE_n_CNTXT_INTSET_INTYPE_BMSK, GPI_INT_IRQ << HWIO_EE_n_CNTXT_INTSET_INTYPE_SHFT);

      if (gpii->irq_mode && !gpii->gpi->irq_muxing) 
      {
         if (GPI_STATUS_SUCCESS != register_irq(gpii))
         {
            gpi_log(GPI_ERROR, "ERROR - gpii: %d register_irq() failed , exiting\n", gpii->id);
            return GPI_STATUS_ERROR;
         }
      }

      HWIO_EE_n_CNTXT_MSI_BASE_LSB_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, 0);
      HWIO_EE_n_CNTXT_MSI_BASE_MSB_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, 0);
      HWIO_EE_n_CNTXT_INT_VEC_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, 0);
      HWIO_EE_n_CNTXT_SCRATCH_0_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, 0);
      HWIO_EE_n_CNTXT_SCRATCH_1_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, 0);
      HWIO_EE_n_ERROR_LOG_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, 0x0);
      HWIO_EE_n_ERROR_LOG_CLR_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, ~0x0u);
      HWIO_EE_n_GSI_EE_GENERIC_CMD_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, 0x160001);

      /* Done by manager EE */
      HWIO_GSI_MANAGER_EE_QOS_n_OUTMI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id,
    ...
    HWIO_EE_n_CNTXT_SRC_GSI_CH_IRQ_CLR_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, ~0x0u);
    HWIO_EE_n_CNTXT_SRC_EV_CH_IRQ_CLR_OUTI(gpii->gpi->qup[gpii->qup_type].gsi_pa, gpii->id, ~0x0u);
   }
   else
   {
