...
/*=============================================================================
 ...
 02/14/18 ts       Optimized the timer function to do minimal operation
 01/31/18 ts       Deferred L1ss config to a 100 ms timer callback
 01/23/18 ts       Do not turnoff root ports if link trainig failed
...
EFI_STATUS PcieConfigLibSetupL1ss(UINT32 RootPortIndex)
{
  ...
  if (port->link->trained)
  {
    ...
    TempReg = MmioRead32 (mem[dbi_reg].pa + PcieCapBaseRp + link_capabilities_offset);
    TempReg = TempReg | HWIO_PCIE_0_LINK_CAPABILITIES_REG_PCIE_CAP_ASPM_OPT_COMPLIANCE_BMSK;
    /* clear AS_LINK_PM_SUPT */
    TempReg = TempReg & (~(UINT32)(HWIO_PCIE_0_LINK_CAPABILITIES_REG_PCIE_CAP_ACTIVE_STATE_LINK_PM_SUPPORT_BMSK));
    /* Configure L0s & L1 from settings */
    TempReg = TempReg | (port->link->aspm << HWIO_PCIE_0_LINK_CAPABILITIES_REG_PCIE_CAP_ACTIVE_STATE_LINK_PM_SUPPORT_SHFT);
    MmioWrite32 (mem[dbi_reg].pa + PcieCapBaseRp + link_capabilities_offset, TempReg);
