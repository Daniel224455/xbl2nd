...
/*===========================================================================
**  Function : prep_elf_parsing
** ==========================================================================
*/
/*!
* 
* @brief
*   This function prepares the elf file before parsing it.
*
* @par Dependencies
*   None 
* 
  ...
  )
{
  ...
  if ( elf_format == ELF_CLASS_32 )
  {
    ...
    /* Copy ELF Header */
    CopyMem(meta_base + current_meta_buffer_size, &ehdr_32, sizeof(Elf32_Ehdr));
    /* Move buffer offset to the end of ELF header */
    current_meta_buffer_size += sizeof(Elf32_Ehdr);

    /* Copy Program Headers */
    CopyMem(meta_base + current_meta_buffer_size, ephdr_32, sizeof(Elf32_Phdr) * ehdr_32.e_phnum);
    ...
    if (hash_seg_size != 0)
    {
      CopyMem(meta_base + current_meta_buffer_size, hash_seg_base, hash_seg_size);
    }
  }
  else if ( elf_format == ELF_CLASS_64 )
  {
    ...
    /* Copy ELF Header */
    CopyMem(meta_base + current_meta_buffer_size, &ehdr, sizeof(Elf64_Ehdr));
    /* Move buffer offset to the end of ELF header */
    current_meta_buffer_size += sizeof(Elf64_Ehdr);

    /* Copy Program Headers */
    CopyMem(meta_base + current_meta_buffer_size, ephdr, sizeof(Elf64_Phdr) * ehdr.e_phnum);
    /* Move buffer offset to the end of program headers */
