...
void ddi_add_fail_summary (ddi_fail_summary* fail_summary, uint64* addr)
{
  ...
  if (ddi_get_rank(addr)) {
    fail_summary->rank_1_fail_count++;
  }
  else {
    fail_summary->rank_0_fail_count++;
  }
  switch(ddi_get_channel(addr)) {
    case 0:
      fail_summary->channel_0_fail_count++;
      break;
    case 1:
      fail_summary->channel_1_fail_count++;
      break;
    case 2:
      fail_summary->channel_2_fail_count++;
      break;
    case 3:
      fail_summary->channel_3_fail_count++;
      break;
  }
}
...
void ddi_run_command_rdwr(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  ...
  *input_iter += pattern_size*sizeof(uint64);
  ...
  rdwr_output.fail_summary.total_fail_count = 0;
  rdwr_output.fail_summary.rank_0_fail_count = 0;
  rdwr_output.fail_summary.rank_1_fail_count = 0;
  rdwr_output.fail_summary.channel_0_fail_count = 0;
  rdwr_output.fail_summary.channel_1_fail_count = 0;
  rdwr_output.fail_summary.channel_2_fail_count = 0;
  rdwr_output.fail_summary.channel_3_fail_count = 0;
  rdwr_output.fail_summary.num_fail_records = 0;
