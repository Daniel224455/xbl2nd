...
static uint32 HAL_MDP_DSC_ENC_CalcInitialLines(uint32 uBPC,
                                               uint32 uBPP,
                                               uint32 uSoftSlices,
                                               uint32 uSliceWidth,
                                               uint32 uXmitDelay,
                                               ...
                                               uint32 uChunkSize)
{
   ...
   uint32 uSsmDelay           = 3 * ((uBPC < 10) ? 84 : 92);
   uint32 uTotalPixels        = uSoftSlices * uSsmDelay + uXmitDelay + 47;
   uint32 uFifoSize           = ((2 == uSoftSlices) ? 5610 : 11610);
   uint32 uFirstLineOffset    = (uFirstLineBpgOffset * uPicWidth) / (3 * 8);
   uint32 aInitialLinesTmp[3] = {0};

   uInitialLines          = (uint32) HAL_CEIL((float) uTotalPixels, uSliceWidth);

   aInitialLinesTmp[0]    = uXmitDelay + uFirstLineOffset;
   aInitialLinesTmp[1]    = (uint32) ((2 + 0.75 * uSoftSlices) * uChunkSize);
   aInitialLinesTmp[2]    = (uTotalPixels % uSliceWidth) * (uBPP / 8);

   if ((aInitialLinesTmp[0] + aInitialLinesTmp[1] - aInitialLinesTmp[2]) < uFifoSize)
   {
     uInitialLines++;
   }
...
{
   uint32 uFinalOffset = uModelSize - (uXmitDelay * uOutputBpp) + uExtraMuxBits;
...
static void HAL_MDP_DSC_ENC_PrepareRegInfo(HAL_MDP_DSC_EncoderConfigType *pDSCEncConfig)
{
   ...
   HAL_MDP_DSC_Data            *pDSCData                = &pDSCEncConfig->sDSCData;

   // General Parameters
   pDSCData->uTargetBPP    = pEncParams->uBitsPerPixel;
   pDSCData->uValInputBPC  = pEncParams->uBitsPerComponent;

   pDSCData->uTargetBPPReg = (uint32)(pDSCData->uTargetBPP  << 4);   // format 6.4, 4 fractional bits
   ...
   if(FALSE == pDSCEncConfig->bUserProvidedEncParams)
   {
      ...
      {
         pDSCData->eInputBPP = 0;	/* 8 bits input */
      ...
      if((TRUE == pDSCData->bHardSliceMuxEnable) && (1 < uSoftSlices))
      {
      ...
      pEncParams->uChunkSize    = HAL_MDP_DSC_ENC_CalcChunkSize(pDSCData->uTargetBPP, pEncParams->uSliceWidth);

      // Calculate the extra mux bits
      uMuxWordSize   = HAL_MDP_DSC_ENC_CalcMuxWordSize(pDSCData->uValInputBPC);
      uExtraMuxBits  = HAL_MDP_DSC_ENC_CalcExtraMuxBits(pDSCData->bRgbInput,
                                                        pDSCData->uValInputBPC,
      ...
      uHrdDelay      = HAL_MDP_DSC_ENC_CalcHrdDelay(pDSCData->uTargetBPP,
      ...
      pEncParams->uFinalOffset        = HAL_MDP_DSC_ENC_CalcFinalOffset(pDSCData->uTargetBPP, 
                                                                      pEncParams->uRcModelSize, 
                                                                      pEncParams->uInitialXmitDelay, 
                                                                      uExtraMuxBits);
   ...
   pDSCData->uInitialLines = HAL_MDP_DSC_ENC_CalcInitialLines(pDSCData->uValInputBPC,
                                                              pDSCData->uTargetBPP,
                                                              uSoftSlicesPerEncoder,
                                                              pEncParams->uSliceWidth,
                                                              pEncParams->uInitialXmitDelay,
                                                              pEncParams->uFirstLineBpgOffset,
                                                              pEncParams->uPicWidth,
                                                              pEncParams->uChunkSize);

   pDSCData->uFlatnessDetThreshold  = 2 << (pDSCData->uValInputBPC - 8);
   ...
   case 0:
      {
         pDSCData->eLastGroupSize   = HAL_MDP_DSC_ENC_GROUP_SIZE_3;      // last group has 3 valid pxels
      }
      break;
   case 1:
      {
         pDSCData->eLastGroupSize   = HAL_MDP_DSC_ENC_GROUP_SIZE_1;      // last group has 1 valid pxels
      }
      break;
   case 2:
      {
         pDSCData->eLastGroupSize   = HAL_MDP_DSC_ENC_GROUP_SIZE_2;      // last group has 2 valid pxels
      }
      break;
   default:
      {
                                          ...
                                          HAL_MDP_DSC_EncoderConfigType *pDSCEncConfig)
{
   ...
   // DSC_X_COMMAN_MODE
   uRegValue   = 0x00;
   uRegValue   = HWIO_OUT_FLD(uRegValue, MDP_DSC_0_COMMON_MODE, SPLIT_PANEL, pDSCData->bSplitDisplayEnable);
   uRegValue   = HWIO_OUT_FLD(uRegValue, MDP_DSC_0_COMMON_MODE, MULTIPLEX, pDSCData->bHardSliceMuxEnable);
   uRegValue   = HWIO_OUT_FLD(uRegValue, MDP_DSC_0_COMMON_MODE, VIDEO, pDSCData->bVideoMode);
   out_dword(HWIO_MDP_DSC_0_COMMON_MODE_ADDR + uBlkRegOffset, uRegValue);

   // DSC_X_ENC
   uRegValue   = 0x00;
   uRegValue   = HWIO_OUT_FLD(uRegValue, MDP_DSC_0_ENC, INPUT_10BITS, pDSCData->eInputBPP);
   ...
   uRegValue   = HWIO_OUT_FLD(uRegValue, MDP_DSC_0_ENC, BITS_PER_PIXEL, pDSCData->uTargetBPPReg);
   uRegValue   = HWIO_OUT_FLD(uRegValue, MDP_DSC_0_ENC, SLICE_LAST_GROUP_SIZE, pDSCData->eLastGroupSize);
   uRegValue   = HWIO_OUT_FLD(uRegValue, MDP_DSC_0_ENC, INITIAL_LINES, pDSCData->uInitialLines);
   ...
   uRegValue   = HWIO_OUT_FLD(uRegValue, MDP_DSC_0_FLATNESS, DET_THRESH, pDSCData->uFlatnessDetThreshold);
...
{
   // Update the input configuration parameters in HAL_MDP_DSC_Data data-structure 

   // Data-Path Configuration Mode (SPLIT-MUX or DUAL-PANEL)
   HAL_MDP_DSC_Data     *pDSCData = &pDSCEncConfig->sDSCData;
   
   MDSS_OSAL_MEMZERO(pDSCData, sizeof(*pDSCData));

   // Detect whether the data-path is for SPLIT_MUX or DUAL_PANEL mode
   if(TRUE == pDSCEncConfig->bSplitMuxActive)
   {
        pDSCData->bSplitDisplayEnable     = TRUE;
        pDSCData->bHardSliceMuxEnable     = TRUE;
   }
   else
   {
      ...
      {
         pDSCData->bSplitDisplayEnable     = TRUE;
         pDSCData->bHardSliceMuxEnable     = FALSE;
      }
   }
...
*
****************************************************************************/
void HAL_MDP_DSC_Setup(HAL_MDP_DSCId eDSCEncEngineId, HAL_MDP_DSC_EncoderConfigType *pDSCEncConfig, uint32 uFlags)
{
   HAL_MDP_DSC_Data     *pDSCData = &pDSCEncConfig->sDSCData;

   // Program the DSC Encoder Module's registers
   HAL_MDP_DSC_ENC_ProgramCfgReg(eDSCEncEngineId, pDSCData, pDSCEncConfig);
}
                                          ...
                                          uint32                 uFlags)
{
   ...
   if(NULL != pDSCInfo->pDSCEncoderCfg)
   {
      ...
      HAL_MDP_DSC_ENC_PrepareRegInfo(pDSCInfo->pDSCEncoderCfg);
   }
