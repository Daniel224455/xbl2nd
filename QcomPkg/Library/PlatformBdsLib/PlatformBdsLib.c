...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 04/04/18   mt      Create and Signal EthernetDriverEvent
 03/21/18   kpa     EFIESP load using GUID support
 03/13/18   mt      Send ReadyToBoot event before PXE Start Image
 02/05/18   prema   Change to handle the new error code that indicate no capsules
                    are found in mass-storage media
 02/05/18   prema   Moved the ESRT sync and lock function to ProcessCapsules
                    to avoid creating stubs for use in LA
 02/05/18   prema   Changes to move to FMP capsule processing
 02/02/18   bd      Add PXE boot support
 09/28/17   bd      Use cpu clock to initialize PMU cycle counter
 10/26/17	mt		Calling function to save TZ buffer in exit boot services
