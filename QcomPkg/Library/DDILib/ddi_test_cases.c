...
extern DDR_REGION ddr_region[3];
extern uint8 max_ddr_regions;
...
/*!
*   @brief
*   filter the user input to set active DDR region to test based on
*			user input /default start and end address 
*
*   @param start       -   [IN] starting address of data region
*   @param end         -   [IN] ending address of data region
*
...
ddi_error get_region(uint64 cmd_id,uint64* start, uint64* end, uint64* output_iter)
{
	uint8 i=0,region=0;
	uint8 no_active_region=0;
	
	if(*start == 0)
	{
		*start = ddr_region[0].start_addr;
	}
	if(*end == 0)
	{
		*end = ddr_region[max_ddr_regions-1].end_addr;
	}
  
	if((*start < ddr_region[0].start_addr) || (*start >= ddr_region[max_ddr_regions-1].end_addr) 
		|| (*end <= ddr_region[0].start_addr) || (*end > ddr_region[max_ddr_regions-1].end_addr))
	{	    
		((ddi_output_header*)ddi_int_out)->cmd_id = cmd_id;
	    ((ddi_output_header*)ddi_int_out)->test_status =  DDI_INVALID_INPUT;
	...
	/*Initializing all the region as UNVAILBALE */
	for(i=0;i<max_ddr_regions;i++)
	{
		ddr_region[i].active_region =REGION_UNAVAILABLE;
	}
	
	/*Deciding active region based on user given or default start and end address */
	while (*start >= ddr_region[region].start_addr && (region < max_ddr_regions))
	{
		if(*start <= ddr_region[region].end_addr)
		{
			ddr_region[region].active_region = REGION_ACTIVE;
		}
		region++;
	}
		
	while ((*end >= ddr_region[region].start_addr) && (region < max_ddr_regions))
	{
		ddr_region[region].active_region = REGION_ACTIVE;
		region++;
	}
	
	/*Check if no region mark as active region */
	for(i=0;i<max_ddr_regions;i++)
	{
		if(ddr_region[i].active_region == REGION_UNAVAILABLE)
		{
			no_active_region++;
		}
	}
	...
	if(no_active_region == max_ddr_regions)
	{
		((ddi_output_header*)ddi_int_out)->cmd_id = cmd_id;
...
/*!
*   @brief
*   filter the user input to set start and end based on user input and active DDR region 
*
*   @param user_start       -   [IN] starting address of data region
*   @param user_end         -   [IN] ending address of data region
*
*   @par Dependencies
*
*   @par Side Effects
...
ddi_error get_start_end(uint8 region, uint64* start, uint64* end, uint64* user_start, uint64* user_end) {


  
  if( (region >= max_ddr_regions) || (ddr_region[region].active_region != REGION_ACTIVE))
	   return DDI_INVALID_INPUT;
  
   
   /*Decide Start and end address based on DDR region */
  if(ddr_region[region].start_addr > *user_start)
  {
     *start = ddr_region[region].start_addr;
  ...
  if(ddr_region[region].end_addr < *user_end )
  {
	 *end = ddr_region[region].end_addr;
  }else{
...
/* ============================================================================
**  Function : ddi_voltage_lvl_to_idx
** ============================================================================
*/
/*!
*   @brief
*   API to convert voltage level value to index for eCDT user guide
*
*   @param rail    -   [IN] Voltage plan
*
*   @par Dependencies
*
*   @par Side Effects
*   None
*
*   @retval voltage level index, 0 mean cannot find it.
*
*   @sa None
*/
  ...
  *output_iter += sizeof(ddi_output_header);
  ...
  do{
  	...
  	*output_iter += sizeof(number_of_freq);
      ...
      ecdt_output->vddcx = ddi_voltage_lvl_to_idx(ddrsns_share_data->aop_params.mc_states[freq_plan_index].vddcx);
      ecdt_output->vddmx = ddi_voltage_lvl_to_idx(ddrsns_share_data->aop_params.mc_states[freq_plan_index].vddmx);
      ecdt_output->vdda =  ddi_voltage_lvl_to_idx(ddrsns_share_data->aop_params.mc_states[freq_plan_index].vdda);
      ecdt_output->enable = ddrsns_share_data->aop_params.mc_states[freq_plan_index].freq_enable;
      ...
      boot_log_message(ddi_log_string);
  		...
  		ret = HAL_DDR_Read_eCDT_Parameter_Values(ddrsns_share_data, ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.clock_plan[freq_plan_index].clk_freq_in_khz,
        &(ecdt_output->settings_buf), buffer_size-sizeof(ddi_out_ecdt_settings));  		
#ifdef DDI_DEBUG_PRINT_ENABLE
...
{
  ddi_output_header output_header;
  ddi_safe_mode_info safe_info;
  #if DDI_PRINT_ENABLE
  snprintf(ddi_log_string, sizeof(ddi_log_string), "get safe mode setting");
  boot_log_message(ddi_log_string);
  #endif
  
  safe_info.self_refresh_mode_enable = ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.hw_self_refresh_enable;
  safe_info.refresh_rate_polling_enable     = ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.MR4_polling_enable;
  safe_info.periodic_zq_calibration_enable = ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.periodic_zq_calibration_enable;
  safe_info.periodic_training_enable = ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.periodic_training_enable;
  //TODO: This is place holder until sdm1000 add this variable.
  safe_info.partial_array_self_refresh_enable = 0xff;//ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.partial_array_self_refresh_enable;
  safe_info.power_down_enable = 0xff;//ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.power_down_enable;
  safe_info.clock_stop_enable = 0xff;//ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.clock_stop_enable;
  safe_info.auto_precharge_enable = 0xff;//ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.auto_precharge_enable;
  
  safe_info.low_power_CA_buffer_support_enable     = (uint8)(ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.lpddr4y_support & LPDDR4Y_LOW_SPEED_CA_BUFFER);
  safe_info.single_ended_clock_support_enable = (uint8)(ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.lpddr4y_support & LPDDR4Y_SINGLE_ENDED_CLOCK)>>1;
  safe_info.single_ended_write_DQS_support_enable = (uint8)(ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.lpddr4y_support & LPDDR4Y_SINGLE_ENDED_WRITE_DQS)>>2;
  safe_info.single_ended_read_DQS_support_enable = (uint8)(ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.lpddr4y_support & LPDDR4Y_SINGLE_ENDED_READ_DQS)>>3;

  safe_info.base_logging_enable = ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.base_logging_enable;
  safe_info.llcc_extended_logging_enable = ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.llcc_extended_logging_enable;
  safe_info.mc_extended_logging_enable = ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.mc_extended_logging_enable;
  safe_info.phy_extended_logging_enable = ddrsns_share_data->shrm_ddr_ptr->ddr_runtime.phy_extended_logging_enable;
  ...
  *output_iter += sizeof(ddi_output_header);
  ...
  save_ddi_data2ddr_partition((uint8*)&safe_info, sizeof(ddi_safe_mode_info), *output_iter);
  *output_iter += sizeof(ddi_safe_mode_info);
}
...
void ddi_run_command_rd(uint64 cmd_id, void* params, uint64* input_iter, uint64* output_iter)
{
  ...
  *input_iter += sizeof(ddi_rd_params);
  ddi_error ret = DDI_CMD_COMPLETE;
  uint64* user_start = (uint64*)cparams->start_addr;
  uint64* user_end = (uint64*)cparams->end_addr;
  uint64* start;
  uint64* end;
  ...
  boot_log_message(ddi_log_string);
	...
	if(get_region(cmd_id,(uint64*)&user_start, (uint64*)&user_end, output_iter) != DDI_CMD_COMPLETE) 
		  return;	  
  
  for( region=0; region < max_ddr_regions ;region++)
  {
	if(ddr_region[region].active_region != REGION_ACTIVE)
		  continue;
	  
	ret |= get_start_end(region, (uint64*)&start, (uint64*)&end, (uint64*)&user_start, (uint64*)&user_end);
	
	#if DDI_PRINT_ENABLE
	snprintf(ddi_log_string, sizeof(ddi_log_string), "Test started for DDR Region: %d", region);
	boot_log_message(ddi_log_string);
