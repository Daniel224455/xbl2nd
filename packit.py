...
# --------   ---     -----------------------------------------------------------
# 09/11/18   vk      Copy Tools for HK11
# 07/25/18   kpa     Support QuestBinPkg packout
# 06/01/18   yps     Fixed HK11 pack issue
...
SHAREDTARGET_FOLDER = 'SDMPkg'
CHIP_NUM_REGEX = "\d{3,4}"
...
release_exceptions = [".inf", ".dec", ".dsc", ".fdf", ".cfg", ".h", "sbl1.ld", ".inc", ".json",\
                      ...
                      ".bmp", ".png", ".jpg", ".inc", "manifest.xml", "commons.py", "elf_gen.py", "elf_gen_tools.py", \
                      "GenXBLConfig.py", "XBLconfig_metadata_generator.py","uefi_load_config.bin"]
...
def main():
  release_list = []
  packout_list = ['HY11_1/boot_images/QcomPkg/QnadPkg', 'HY11_1/boot_images/QcomPkg/QuestBinPkg']
  packin_list = []
...
def delete_dirs(path):
  try:
    ...
    print "Error encountered while deleting: " + path
    print "Error info: ", sys.exc_info()[0]
    sys.exit()
  ...
  for root, directories, files in os.walk(base_directory):
    for current_directory in directories:
      ...
      if current_directory.lower().startswith("msm") or \
        ...
        if (current_directory.lower().endswith("pkg")) and (current_directory != SHAREDTARGET_FOLDER):
          if not any(target.lower() in current_directory.lower() for target in target_names.split(',')):
            delete_dirs(os.path.join(root,current_directory))
        elif (current_directory == SHAREDTARGET_FOLDER):
          chipnum = re.compile('^.*'+CHIP_NUM_REGEX+'.*$')
          chip_options = [re.search(CHIP_NUM_REGEX, chip_option).group() for chip_option in filter(chipnum.match,target_names.split(','))]
          for curr_path, sharedpkg_dirs, sharedpkg_files in os.walk(os.path.join(root,current_directory)):
            for currentdir in (currentdir for currentdir in sharedpkg_dirs if re.search(chipnum,str(currentdir))):
              if re.search(chipnum,str(currentdir)).group() not in chip_options:
                print("deleting "+os.path.join(curr_path,currentdir))
                delete_dirs(os.path.join(curr_path,currentdir))
            del sharedpkg_dirs[:]
  return
...
def create_binary_deliverable(base_directory,
  ...
  for target_name in target_names.split(','):
    try:
      ...
      bin_path = os.path.join(base_directory,
                                "boot_images",
                                "QcomPkg",
                                re.sub("\d","",target_name) + "Pkg",
                                re.sub("\D","",target_name),
                                "Bin")
  ...
  # Copy common target tools
  print "Copying QcomPkg/SDMPkg/Tools ..."
  print target_name
  for target_name in target_names.split(','):
    try:
  ...
  # Copy target tools
  print "Copying QcomPkg/SDMPkg/Target/Tools ..."
  print target_name
  for target_name in target_names.split(','):
    try:
      target_tools_path = os.path.join(pack_directory,
