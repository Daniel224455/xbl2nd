...
EFI_STATUS LoadAndAuthenticateFV(EFI_GUID *FvGuid, CHAR16* PartitionLabel)
{
  ...
  if (FvGuid == NULL &&PartitionLabel == NULL)
  {
    DEBUG((EFI_D_ERROR, "FvGuid and Label are not provided\n"));
    return EFI_INVALID_PARAMETER;
  }
  ...
  if(FvGuid != NULL)
  {
    Status = ReadFromPartitionUsingGuid(FvGuid, &ReadBuffer, &ImageSize);
    if (EFI_ERROR(Status) || ReadBuffer == NULL)
    {
      DEBUG((EFI_D_WARN, "Failed to mount FV from partition using Guid %x\n", Status));
    }
    else
    {
     goto next;
  ...
  if (EFI_ERROR(Status) && PartitionLabel != NULL)
  {
    Status = ReadFromPartition(PartitionLabel, &ReadBuffer, &ImageSize);
    if (EFI_ERROR(Status) || ReadBuffer == NULL)
    {
      DEBUG((EFI_D_WARN, "Failed to mount FV from partition using label %x\n", Status));
      return Status;
    }
  }
next:  
