/** @file ScmArmV8.c
  ...
  Copyright (c) 2014-2020 Copyright Qualcomm Technologies, Inc.  All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 12/20/18   shl     Fixed a bug which return fake status and format cleanup
    ...
    )
{
	...
	SMC_ARG_LIST_64       *IndirectParameters = NULL;
...
)
{
    ...
    case  APP_QUERY_APPID_CMD:
		...
		ReqPtr = ShmBridgeProtocol->ShmBridgeAllocate(ShmBridgeProtocol, &ReqLen, ShmBridgeBootSvcData);
		if (ReqPtr == NULL)
		{
			Status = EFI_OUT_OF_RESOURCES;
			goto ErrorExit;
    ...
    case APP_START_EMBEDDED_APP_CMD:
        ...
        StartEmbeddedAppSyscall->app_name = (UINTN)ReqPtr;
