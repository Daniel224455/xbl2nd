...
static uint32 hsuart_detect_rumi = TRUE;
static uint32 uart_fw_protocol = 2;
/*==================================================================================================
...
==================================================================================================*/
static void fw_init(hsuart_context* hsuart_ctxt)
{
   uint32      se_base = hsuart_ctxt->properties->uart_base;
   uint32      qup_common_base = hsuart_ctxt->properties->qup_common_base;
   ...
   if (qupv3_protocol == uart_fw_protocol) { return; }     //UART FW Already Loaded
   
   hsuart_settings_get_fw((uint8**) &hdr, qupv3_hw_ver);
...
void detect_rumi_and_init(hsuart_context* hsuart_ctxt)
{
   uint32 base = hsuart_ctxt->properties->uart_base;
   uint32 se_base = base;

   fw_init(hsuart_ctxt);

   if (hsuart_detect_rumi)
   {
      ...
      if (HWIO_INX(base , QUPV3_SEn_GENI_IOS) != 0)
      {
         ...
         hsuart_ctxt->properties->uart_base = 0xA88000;
         fw_init(hsuart_ctxt);
      }
   }
}
...
hsuart_result hsuart_fw_load(hsuart_context* hsuart_ctxt)
{
   ...
   fw_init(hsuart_ctxt);
#else
   detect_rumi_and_init(hsuart_ctxt);
#endif
