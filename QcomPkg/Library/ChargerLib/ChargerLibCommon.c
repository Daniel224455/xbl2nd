...
/*=============================================================================
 ...
 --------     ---     ---------------------------------------------------------
02/08/18      dc      Add new input argument for ChargerLib_ConfigureUsbCurrentSetting() to handle reinsert case
10/11/17      dc      Rerun APSD and reconfigure USB ICL in every periodical check
10/11/17      dc      Set ICL to 500mA when APSD result is invalid for CC float charger
09/22/17      va      Read logfs partition size as per alloted from target specific file, Enable BMD explicitly
...
EFI_STATUS ChargerLib_ConfigureUsbCurrentSetting(BOOLEAN bReinsert);
...
EFI_STATUS ChargerLib_WasChargerReinserted(BOOLEAN *pChargerReinserted)
{
  ...
  *pChargerReinserted = FALSE;
  ...
  if(EFI_SUCCESS == Status)
  {
    ...
    {
      Status = ChargerLib_ConfigureUsbCurrentSetting(TRUE);
    }
  }
  else
  {
...
EFI_STATUS ChargerLib_ForceSysShutdown(ChgAppSysShutdownType ShutdownType)
{
  ...
  switch (ShutdownType)
  {
    ...
    case CHGAPP_RESET_AFP:
      ...
      Status = ChargerLib_EnableAfpMode();
      if (EFI_SUCCESS == Status)
      {
        CHARGER_DEBUG((EFI_D_WARN, "ChargerLib:: %a AFP Mode performed = %d \r\n", __FUNCTION__, Status));
      }
      else
      {
        ...
        CHARGER_DEBUG((EFI_D_WARN, "ChargerLib:: %a AFP Mode not supported or failed = %d \r\n", __FUNCTION__, Status));
      }
      /* Try to gracefully shutdownd device */
    ...
    case CHGAPP_RESET_SHIPMODE:
      ...
      Status = ChargerLib_EnableShipMode();
      if (EFI_SUCCESS == Status)
      {
        CHARGER_DEBUG((EFI_D_WARN, "ChargerLib:: %a Ship Mode performed = %d \r\n", __FUNCTION__, Status));
      }
      else
      {
        ...
        CHARGER_DEBUG((EFI_D_WARN, "ChargerLib:: %a Ship Mode not supported or failed = %d \r\n", __FUNCTION__, Status));
      }
      /* Try to gracefully shutdown device before HW actully perform Shipmode */
      ResetType = EfiResetShutdown;
      break;
...
EFI_STATUS ChargerLib_InitializeCharging( VOID )
{
  ...
  Status |= ChargerLib_ConfigureUsbCurrentSetting(FALSE);
