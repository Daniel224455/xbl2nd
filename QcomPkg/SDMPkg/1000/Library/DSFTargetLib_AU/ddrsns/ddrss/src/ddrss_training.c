...
extern  uint32 PRFS_BAND_THRESHOLD[NUM_PRFS_BANDS];
...
extern uint8 g_channelMask;

 char g_str_default[] = "DEFAULT"; // define in ddrss_training.c; declare extern in ddrss.h
 char g_str_dcc_pll0[] = "WR_DCC_PLL0";
 char g_str_dcc_pll1[] = "WR_DCC_PLL1";
 char *tmp_string_p;
 //uint16 *tmp_string_array_p;//[] ={0};
 char g_str_write[] = "WRITE";
 char g_str_read[] = "READ";
 char g_str_error[] = "ERROR";
 char g_str_unit_ps[] = "ps";
 char g_str_unit_mV[] = "mV";
 char g_str_print_s[] = "%s";
 char g_str_print_d[] = "%d";
//extern uint8 DDRSS_Channel_Mask(void);
// uint8 *tmp_string_int_p;
                                   ...
                                   ) 
{
  ...
  //uint32 training_freq_table[MAX_TRAINING_FREQ_NUM] = {0};
    ...
    clock_plan_max_idx = ddr->shrm_ddr_ptr->ddr_runtime.ddr_num_clock_levels - 1;
    ...
    { 
      if (prfs_index>=1)
      {
        ...
        {             
	     ...
	     if(ddr->shrm_ddr_ptr->ddr_runtime.clock_plan[training_ddr_freq_indx].clk_freq_in_khz!=768000)
	     {
...
// Memory write
//================================================================================================//
void DDRSS_mem_write ( uint8 ch, uint8 cs)
{
#if DSF_PXI_TRAINING_EN
    uint32 ch_offset = 0; 

#if PXI_WR_BROADCAST_EN
    ch_offset = REG_OFFSET_MC_BROADCAST;
#else
    ch_offset = REG_OFFSET_MC_CH(ch);
#endif

    MC_PXI_Execute_Tables(ch_offset, cs, 0); //Execute PXI_WR
#else    
    ddr_mem_copy (write_in_pattern, (uint32 *)(size_t)training_address[ch][cs], DQ_TRAINING_PATTERN_SIZE(g_channelMask), 1);
#endif
}
                                 ...
                                 )
{
  ...
  if((clk_freq_in_khz > LOWEST_SCALED_FPRS_FREQ) && (clk_freq_in_khz < HIGHEST_TRAINED_FPRS_FREQ))
  {
    ...
    else
    {
      ...
      ddr_printf(DDR_NORMAL,"SCALE ch = %d PHY_byte = %d source freq = %d scaled prfs = %d Coarse = %d Fine = %d\n",
                              ...
                              )
{
  ...
  DDR_PHY_hal_cfg_wrext_ctl_update (ddr_phy_chn_base + DQ0_DDR_PHY_OFFSET + (phy_inx * DDR_PHY_OFFSET),
      ...
      ddr_printf(DDR_NORMAL,"SCALE ch = %d PHY_byte = %d  scaled prfs = %d Coarse = %d Fine = %d Half = %d\n",
	...
	for(ch = 0; ch < NUM_CH; ch++)
        {
  	 	 ...
  	 	 // Channel 
	    training_address[ch][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + (TRAINING_INTERLEAVE_ADDRESS_OFFSET * ch);       
	    training_address[ch][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + (TRAINING_INTERLEAVE_ADDRESS_OFFSET * ch);  
         }
    }
   else if ((ddr->shrm_ddr_ptr->misc.platform_id == MSM_VERSION_POIPU) && ((ddr->shrm_ddr_ptr->misc.chip_version == 0x200)||(ddr->shrm_ddr_ptr->misc.chip_version == 0x201))) 
   {
          if (0xFF == g_channelMask)
          {
       
	      training_address[0][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x0;       
	      training_address[0][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x0;
	      training_address[1][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x200;       
	      training_address[1][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x200;    
	      training_address[2][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x400;       
	      training_address[2][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x400;    
	      training_address[3][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x600;       
	      training_address[3][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x600;    
	      training_address[4][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x100;       
	      training_address[4][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x100;    
	      training_address[5][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x300;       
	      training_address[5][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x300;    
	      training_address[6][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x500;       
	      training_address[6][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x500;    
	      training_address[7][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x700;       
	      training_address[7][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x700;    
         }
         else if (0xF0 == g_channelMask)
         {
	      training_address[4][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x0;       
	      training_address[4][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x0;    
	      training_address[5][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x200;       
	      training_address[5][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x200;    
	      training_address[6][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x400;       
	      training_address[6][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x400;    
	      training_address[7][0] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs0_remapped_addr[0] + 0x600;       
	      training_address[7][1] = ddr->shrm_ddr_ptr->ddr_size_info.ddr_cs1_remapped_addr[0] + 0x600; 
         }
        else
        {
          ;;
...
void DDRSS_cdcref_init_wr(BOOT_DDR_STRUCT *ddr, uint32 cdc_ref[HISTOGRAM_SIZE][4])
{
    int coarse_x    =   0;
    int fine_x      =   0;
    int knob        =   0;
    ...
    for (coarse_x=0; coarse_x< COARSE_CDC_MAX_VALUE; coarse_x++)
    {
        ...
        { 
            knob                    = coarse_x * FINE_STEPS_PER_COARSE  + fine_x ;
            cdc_ref[knob][COARSE_L] = coarse_x  ;
			cdc_ref[knob][FINE_L  ] = fine_x  + TRAIN_WR_CDC_FINE_STEPS_OFFSET; // 4
...
uint8 DDRSS_Get_Freq_Index (BOOT_DDR_STRUCT *ddr, uint32 clk_freq_khz)
{
   ...
   for (prfs_index = 0; (prfs_index < sizeof(PRFS_BAND_THRESHOLD)/sizeof(uint32)); prfs_index++)
   {
      if (clk_freq_khz < PRFS_BAND_THRESHOLD[prfs_index])
         break;
   }
...
//================================================================================================//
uint32 DDRSS_mem_read_Get_Fail_pat ( uint8 ch, uint8 cs)
{
#if DSF_PXI_TRAINING_EN
    uint32  REG_OFFSET_MC_PXI_SECURE       =  REG_OFFSET_MC_PXI_SECURE_CH(ch);
    uint8   reg_num         = 0;
#else
	//uint8 mask = g_channelMask;
    //if (0xFF == mask)
	//{	
    //uint32	read_back_pattern[DQ_TRAINING_PATTERN_SIZE(mask)] __attribute__((aligned(8))) =  {0};
		//uint32	read_back_pattern[64] __attribute__((aligned(8))) =  {0};
	//}
	//else 
   // {
		uint32	read_back_pattern[128] __attribute__((aligned(8))) =  {0};
	//}
...
#if PXI_RD_BROADCAST_EN
    ch_offset = REG_OFFSET_MC_BROADCAST;
#else
    ch_offset = REG_OFFSET_MC_CH(ch);
#endif

    MC_PXI_Execute_Tables(ch_offset, cs, 1); //Execute PXI_RD

    for (word_x=0;word_x <NUM_DQ_PCH;word_x++)
    {
        reg_num = word_x + 2;
        fail_in_word |= HWIO_INXI (REG_OFFSET_MC_PXI_SECURE ,DTTS_RAM_PXI_RD_n_ELOG_TABLE_RD_ONLY, reg_num);
    }//for
#else
    ddr_mem_copy ((uint32 *)(size_t)training_address[ch][cs], read_back_pattern, DQ_TRAINING_PATTERN_SIZE(g_channelMask), 1);
    for (word_x=0;word_x <DQ_TRAINING_PATTERN_SIZE(g_channelMask);word_x++)
    {
...
void DDRSS_MC_cnt_clk_en(BOOT_DDR_STRUCT *ddr,DDR_CHANNEL channel, DDR_CHIPSELECT chip_select) //TODO remove when fix is in place by srini (MC)
{
    uint8  ch                = 0;   
    uint8 cs;
    uint8  index            = 0;
    ...
    for(ch = 0; ch < NUM_CH; ch++)
    {
        ...
        for(cs = 0; cs < NUM_CS; cs++)
        {
             if((chip_select >> cs) & 0x1)
             {
                 ...
                 {        
                     HWIO_OUTXFI2(REG_OFFSET_MC_OSU_CH(ch), MPE_APM_OPT_CFG_m_CS_n, index, cs, IDLE_POWER_DOWN_EN, 0);
                     HWIO_OUTXFI2(REG_OFFSET_MC_OSU_CH(ch), MPE_APM_OPT_CFG_m_CS_n, index, cs, ACTIVE_POWER_DOWN_EN, 0);  
                 }
             }          
        } 

         HWIO_OUTXFI (REG_OFFSET_MC_MPE_CH(ch), MPE_APM_OPT_CTRL_m, 0, CLOCK_STOP_EN, 0);
         HWIO_OUTX (REG_OFFSET_MC_MPE_CH(ch), MPE_LOAD_CONFIG, 0x1); 
         while (HWIO_INX(REG_OFFSET_MC_MPE_CH(ch), MPE_LOAD_CONFIG) != 0x0); 
         
        // HWIO_OUTXF2 (REG_OFFSET_MC_OSU_CH(ch), OSU_QFI_CLK_CTRL, QFI_CLK_SET, QFI_CLK_OVERRIDE_EN, 1, 1);
        // HWIO_OUTX   (REG_OFFSET_MC_OSU_CH(ch), OSU_LOAD_CONFIG, 0x1); 
        // while (HWIO_INX(REG_OFFSET_MC_OSU_CH(ch), OSU_LOAD_CONFIG) != 0x0); 
         
         //dummy MR_read to enable continuous clk
        // MC_MR_Read(channel, chip_select, JEDEC_MR_5);
    }
}

void DDRSS_MC_cnt_clk_dis(BOOT_DDR_STRUCT *ddr,DDR_CHANNEL channel, DDR_CHIPSELECT chip_select)//TODO remove when fix is in place by srini (MC)
{
    uint8 cs;
    uint8  ch        = 0;
    uint8  index            = 0;
    ...
    for(ch = 0; ch < NUM_CH; ch++)
    {
        ...
        for(cs = 0; cs < NUM_CS; cs++)
        {
            if((chip_select >> cs) & 0x1)
            {
                 for (index = 0; index < 4; index++) 
                 {        
                     HWIO_OUTXFI2(REG_OFFSET_MC_OSU_CH(ch), MPE_APM_OPT_CFG_m_CS_n, index, cs, IDLE_POWER_DOWN_EN, 1);
                     HWIO_OUTXFI2(REG_OFFSET_MC_OSU_CH(ch), MPE_APM_OPT_CFG_m_CS_n, index, cs, ACTIVE_POWER_DOWN_EN, 1);  
                 }
            }          
        } 

        //HWIO_OUTXF2 (REG_OFFSET_MC_OSU_CH(ch), OSU_QFI_CLK_CTRL, QFI_CLK_SET, QFI_CLK_OVERRIDE_EN, 0, 0);
        //HWIO_OUTX   (REG_OFFSET_MC_OSU_CH(ch), OSU_LOAD_CONFIG, 0x1);
        //while (HWIO_INX(REG_OFFSET_MC_OSU_CH(ch), OSU_LOAD_CONFIG) != 0x0);      
        HWIO_OUTXFI (REG_OFFSET_MC_MPE_CH(ch), MPE_APM_OPT_CTRL_m, 0, CLOCK_STOP_EN, 1);
        HWIO_OUTX   (REG_OFFSET_MC_MPE_CH(ch), MPE_LOAD_CONFIG, 0x1); 
        while (HWIO_INX(REG_OFFSET_MC_MPE_CH(ch), MPE_LOAD_CONFIG) != 0x0); 
    }
}    
    
int DDRSS_Histogram_Max_Area_Rectangle(training_params_t            *training_params_ptr,
...
int DDRSS_convert_vref_mv_to_steps(BOOT_DDR_STRUCT *ddr, uint32 vref_mv, uint32 freq_khz, uint8 dir_rd_wr, uint8* const out_vref_steps)
{
	...
	{
        if (vref_mv <  WR_RANGE0_OFFSET_MV)
        {
            vref_mv =  WR_RANGE0_OFFSET_MV;
            ret = -1;
        }
        vref_steps = (vref_mv -  WR_RANGE0_OFFSET_MV) / VREF_WR_LP4x_STEP_IN_MV;
...
uint32 DDRSS_convert_vref_steps_to_mv(uint8 steps, uint8 dir_rd_wr, uint32 freq_khz)
{
	...
	{
        vref_mv =  WR_RANGE0_OFFSET_MV + steps * VREF_WR_LP4x_STEP_IN_MV;
	}
      ...
      for (cs=0;cs<NUM_CS;cs++)
      {
        ...
        {          
            ...
            {      
              if (cs == 0)
              {
                trained_coarse_cdc[ch][cs][byte_lane] = HWIO_INXF (dq0_ddr_phy_base + ((byte_remapping_table[ch][byte_lane]) * DDR_PHY_OFFSET), 
                                                                   DDR_PHY_DDRPHY_CMCDCRD_CTL_CFG,
                                                                   HP_COARSE_DELAY_RANK_0); 
                trained_fine_cdc[ch][cs][byte_lane]   = HWIO_INXF (dq0_ddr_phy_base + ((byte_remapping_table[ch][byte_lane]) * DDR_PHY_OFFSET), 
                                                                   DDR_PHY_DDRPHY_CMCDCRD_CTL_CFG,
        ...
        for (fine_cdc = 0;fine_cdc <FINE_STEPS_PER_COARSE; fine_cdc++)
        {
          ...
          for (ch=0;ch<NUM_CH;ch++)
          {
            for (cs=0;cs<NUM_CS;cs++)
            {
                if ((qualified_cs>> cs) & 0x1)
                { 
                    fail_pat[ch][cs]	= DDRSS_mem_read_Get_Fail_pat(ch,cs);
                }
            } // cs
          } // ch   
                                            ...
                                            )
{
  ...
  *output_iter += sizeof(uint16);
  ...
  coarse_cdc_max_value = ((full_cycle_steps + full_cycle_steps/2) > COARSE_CDC_MAX_VALUE) ? COARSE_CDC_MAX_VALUE : (full_cycle_steps + full_cycle_steps/2);
  ...
  ddr_printf(DDR_NORMAL,"SuperIteration Loop Repeat,Frequency,PLL,Vref,WR Coarse,WR Fine,Read/Write,CH0 CS0 P0,CH0 CS0 P1,CH0 CS1 P0,CH0 CS1 P1,CH1 CS0 P0,CH1 CS0 P1,CH1 CS1 P0,CH1 CS1 P1,CH2 CS0 P0,CH2 CS0 P1,CH2 CS1 P0,CH2 CS1 P1,CH3 CS0 P0,CH3 CS0 P1,CH3 CS1 P0,CH3 CS1 P1\n");
    ...
    dq0_ddr_phy_base = REG_OFFSET_DDR_PHY_CH(7) + DQ0_DDR_PHY_OFFSET;
...
uint32 DDRSS_get_sweep_size(uint32 clk_freq_khz)
{
    uint32 sweep_size =0;
    sweep_size= (1000000000/  (FINE_STEP_IN_PS*clk_freq_khz))+800/ FINE_STEP_IN_PS;  // This will change the search range from T+20 to T + Max Tdqs2dq to ensure we can see the full eye
    if (HISTOGRAM_SIZE<sweep_size)
    {
        sweep_size=HISTOGRAM_SIZE;
    }
...
uint8 g_abort_flag = 0; //move to DCB
uint8 g_sed_flag = 1; //move to DCB

sed_print_struct_t g_sed_struct_val;
...
g_sed_print_struct_strg;

static void DDRSS_SED_prints_struct_helper(char *string, uint32 value)
{
    if(value == DC )
    {
        ddr_external_snprintf(string, 10, "%s", "NA");
    }
    else
    {
        ddr_external_snprintf(string, 10, "%u", value);
    }
}

void DDRSS_SED_prints_struct()
{
    if(!(g_sed_flag || g_abort_flag))
    {
        return;
    }
    
    DDRSS_SED_prints_struct_helper(g_sed_print_struct_strg.vref_steps, g_sed_struct_val.vref_steps);
    DDRSS_SED_prints_struct_helper(g_sed_print_struct_strg.vref_value, g_sed_struct_val.vref_value);
    DDRSS_SED_prints_struct_helper(g_sed_print_struct_strg.ch, g_sed_struct_val.ch);
    DDRSS_SED_prints_struct_helper(g_sed_print_struct_strg.cs, g_sed_struct_val.cs);
    DDRSS_SED_prints_struct_helper(g_sed_print_struct_strg.byte_lane, g_sed_struct_val.byte_lane);
    
    if(g_abort_flag)
    {
        ddr_external_snprintf(g_sed_print_struct_strg.prefix, 10, "%s", "DDR ABORT");
		//g_abort_flag = 0;
    }
    else
    {
        ddr_external_snprintf(g_sed_print_struct_strg.prefix, 10, "%s", "SLT Print");
    }
    
    ddr_printf(DDR_NORMAL, "\n %s,   %s, %s, %d, %d, %s, %s, %s, %s, %d, %s, %s, %s ",
        g_sed_print_struct_strg.prefix,
        g_sed_struct_val.training,
        g_sed_struct_val.param_name,
        g_sed_struct_val.param_steps,
        g_sed_struct_val.param_value,
        g_sed_struct_val.param_units,
        g_sed_struct_val.vref_type,
        g_sed_print_struct_strg.vref_steps,
        g_sed_print_struct_strg.vref_value,
        g_sed_struct_val.freq,
        g_sed_print_struct_strg.ch,
        g_sed_print_struct_strg.cs,
        g_sed_print_struct_strg.byte_lane);
    g_abort_flag = 0;		
} 
