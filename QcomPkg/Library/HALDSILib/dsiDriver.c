...
MDP_Status DSIDriver_SetMode(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  for (uDevIdx = 0; uDevIdx < pDriverCtx->uNumDSIControllers; uDevIdx++)
  {
        ...
        (0    != pPanelConfig->uAttrs.sDsi.uDSCCompressionRatio))
    {
      ...
      pPanelConfig->uAttrs.sDsi.sDSCEncCfg.uBytePerPkt = pPanelConfig->uAttrs.sDsi.uDSCSlicePerPacket * pPanelConfig->uAttrs.sDsi.uDSCSliceWidth;
    }
        ...
        (pPanelConfig->ePhysConnect == MDP_DISPLAY_CONNECT_SECONDARY_DSI_VIDEO))
    {
      ...
      if (pPanelConfig->uAttrs.sDsi.bDSCEnable)
      {
        if (pPanelConfig->uAttrs.sDsi.uDSCCompressionRatio)
        {
          ...
          sDisplayConfig.sDisplayInfo.iVisWidthPx                        = HAL_DSI_Phy_Roundup((float)sDisplayConfig.sDisplayInfo.iVisWidthPx / pPanelConfig->uAttrs.sDsi.uDSCCompressionRatio);
