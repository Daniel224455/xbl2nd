...
#
# Copyright 2017-2019 by Qualcomm Technologies, Inc.
#                 All Rights Reserved.
...
# --------   ---     -----------------------------------------------------------
# 09/25/19   am       Handle multi-line env variables in parse_and_set_edk_env
# 01/24/19   kpa      Added variables SILICON_PATH and EDK2_CORE_PATH
# 01/17/19   kpa      Support cleaning custom build artifacts
...
# clean_dir
##############################################################################
def clean_dir(del_dir, log_flag):
  try:
  #Preserve input root dir. Delete all contents within.
    for del_conf_file in os.listdir(del_dir):
      del_file = os.path.join(del_dir, del_conf_file)
      if os.path.isdir(del_file):
        shutil.rmtree(del_file)
        if log_flag == "log":
          print "[buildex.py] Removed " + del_file      
      else:
        os.remove(del_file)
        if log_flag == "log":
          print "[buildex.py] Removed " + del_file
    ...
    print error
    sys.exit(BUILD_CLEAN_ERROR_CODE)        
     
##############################################################################
# clean_build_artifacts
##############################################################################
def clean_build_artifacts(target_paths):
  try:  
    del_build_dir = os.path.join(os.environ["WORKSPACE"], "BaseTools")
    # Clean build tools if it already exists
    if os.path.isdir(os.path.join(os.environ["WORKSPACE"],"BaseTools","Source","C", "bin")):
      subprocess.call(["make", "-C", del_build_dir, "clean"])

    print "\n --- Clean Custom Build Artifacts --- \n" 
    
    # Remove workspace file if it exists. it will force Basetools to be rebuilt next time
    ...
    if os.path.isfile(workspace_file):
      ...
      print "[buildex.py] Removed " + workspace_file
    
    # Remove temporary configuration files
    del_build_dir = os.path.join(os.environ["WORKSPACE"], "Conf")
    clean_dir(del_build_dir, "log")
      
    # Remove .../boot_images/Build/* directory contents  
    del_build_dir = os.path.join(os.environ["WORKSPACE"], "Build")
    clean_dir(del_build_dir, "log")
    
    # Also Remove .../boot_images/Build/ directory
    if os.path.isdir(del_build_dir):
      shutil.rmtree(del_build_dir)
      print "[buildex.py] Removed " + del_build_dir

    # Remove .../boot_images/QcomPkg/<target>/Bin/ directory contents  
    for target_path in target_paths:
      del_build_dir = os.path.join(target_path, "Bin")
      clean_dir(del_build_dir, "log")
...
def main():
  ...
  for target_path in target_paths:
    ...
    while (index < len(variants) ) or (index == 0):
      ...
      for release_mode in options.release.split(','):
        ...
        for image in images:
          variant_flavor = build_flavor(str(os.environ["TARGETMSM"])+'Pkg',
                                        image,
                                        variant.upper(),
                                        release_mode.upper(),
                                        "",
  ...
  if "clean" in options.build_flags:
    clean_build_artifacts(target_paths)
    print "[buildex.py] Clean successfull."
  else:
...
#####################################################################
def check_components_status(dsc_path,release_mode):
  if release_mode in components_status:
    if dsc_path in components_status[release_mode]:
      if  components_status[release_mode][dsc_path] == "processed":
        return 1
  else:
      components_status.setdefault(release_mode,{})
  components_status[release_mode].setdefault(dsc_path,"")
  return 0

#####################################################################
# set_components_status
#####################################################################
def set_components_status(dsc_path,release_mode):
  if dsc_path in components_status[release_mode]:
     components_status[release_mode][dsc_path] = "processed"
  return 1
#####################################################################
# process_components
#####################################################################
...
def process_component(options,variant_flavor,component,process_dirt,build_dirt,tools_dirt):
  ...
  for i in range(len(component_dirt['Process'])):
    ...
    if build_cmd[0] == "build":
      ...
      if ((options.components and component_dirt['Name'].encode('ascii').upper() in options.components.upper()) or options.components == "") and not check_components_status(dsc_path, variant_flavor.release_mode):
        ...
        set_components_status(dsc_path, variant_flavor.release_mode)
      else:
         return dsc_path
...
def search_valid_dirs(search_path, regex):
  ...
  for f in os.listdir(search_path):
    if re.search(regex, f):
      # Omit auto addition of QuestBinPkg
      # To be removed once QuestBinPkg can compile independently
      if not re.search(SKIP_REGEX, f):
...
def setup_environment():
  ...
  # Update SILICON_PATH and EDK2_CORE_PATH to point to Workspace Dir. 
  # OEM's may update the variables to add their custom paths
  if "SILICON_PATH" not in os.environ:  
    os.environ["SILICON_PATH"] = os.environ["WORKSPACE"]

  if "EDK2_CORE_PATH" not in os.environ:  
    os.environ["EDK2_CORE_PATH"] = os.environ["WORKSPACE"]
...
def search_all_target_path(option_target = None):
  ...
  if option_target:
    for target in option_target.split(","):
      ...
      target_num = re.search(CHIP_NUM_REGEX,target_regex)
      if target_match:
        target_paths.append(target_match[0])
      elif target_num: ############### added for sdmpkg
        shared_pkg = re.sub(CHIP_NUM_REGEX,'',target_regex)
        split_pkg = search_dir(os.path.join(search_path,shared_pkg),target_num.group())
        target_paths.append(split_pkg[0])
      else:
    ...
    for target_match in search_valid_dirs(search_path, target_regex):
      if os.path.basename(target_match)==DEFAULT_SHAREDPKG_REGEX: ############### added for sdmpkg
          for target_num_match in search_valid_dirs(target_match, CHIP_NUM_REGEX):
