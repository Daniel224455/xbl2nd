...
----------   ---     ----------------------------------------------------------
08/29/18     vk      Move register region to main table
...
void sbl1_relocate_page_table_to_ddr(bl_shared_data_type *bl_shared_data)
{
  ...
  {
    boot_log_message("Relocate Pagetable to DDR, Start");
    ...
    boot_log_message("Relocate Pagetable to DDR, End");
...
* 
*/
static boot_boolean sbl1_pagetable_add_MMU_entry(bl_shared_data_type* bl_shared_data)
{
  boot_boolean result;  
...
boot_boolean download_mode_mmemmap_init(void)
{
  ...
  { SCL_IMEM_BASE, SCL_IMEM_BASE, SCL_TZ_OCIMEM_BUFFER >> 10, MMU_L3_SECTION_MAPPING, MMU_PAGETABLE_MEM_READ_WRITE, MMU_PAGETABLE_DEVICE_SHARED, MMU_PAGETABLE_NON_EXECUTABLE_REGION }; 
...
#define ROM_BASE 0x300000
  ...
  /* Unmap XBL_SEC */
#define IMEM_XBL_SEC_BASE 0x14698000
  result = boot_mmu_find_entry_and_unmap ((uintnt*)mmu_get_page_table_base(), IMEM_XBL_SEC_BASE, SIZE_88KB);
  BL_VERIFY(result , BL_ERR_MMU_PGTBL_MAPPING_FAIL|BL_ERROR_GROUP_BOOT);  
