  ...
  IN UINTN BufLength)
{
  ...
  for ( offset = 0; (offset + 2*sizeof(UINT16)) < BufLength; ){
    Length = *(UINT16*) (pNicDevice->pBulkInBuff + offset);
    LengthBar = *(UINT16*) (pNicDevice->pBulkInBuff + offset +2);

    Length &= 0x7ff;
    LengthBar &= 0x7ff;
    LengthBar |= 0xf800;

    if ((Length ^ LengthBar ) != 0xFFFF) {
      DEBUG (( EFI_D_ERROR , "Ax88772 - Pkt length error. BufLength = %d\n", BufLength));
      pNicDevice->Leftover = 0;
      return EFI_DEVICE_ERROR;
    }
    
    if ((offset + 2*sizeof(UINT16) + Length) > BufLength) {
      break;
    }
    ...
    if ( NULL == pRxPacket ) {
      ...
      if ( !EFI_ERROR ( Status )) {
        ...
        DEBUG (( EFI_D_ERROR , "Ax88772 - Failed to allocate\n", BufLength));
        pNicDevice->Leftover = 0;
        return EFI_OUT_OF_RESOURCES;
      }
    }

    pRxPacket->Length = Length;
    pRxPacket->LengthBar = LengthBar;

    pData = pNicDevice->pBulkInBuff + offset + 2*sizeof(UINT16);
    CopyMem (&pRxPacket->Data[0], pData, Length);
    ...
    offset += (((Length + 1)&0xFFFE) + 2*sizeof(UINT16));
  }
  pNicDevice->Leftover = BufLength - offset;
  if (offset < BufLength) {
    if (pNicDevice->Leftover > MAX_BULKIN_SIZE) {
      DEBUG (( EFI_D_ERROR , "Ax88772: no room to hold partial buffer"));
      pNicDevice->Leftover = 0;
      return EFI_OUT_OF_RESOURCES;
    } else {
      CopyMem (pNicDevice->pBulkInBuff, pNicDevice->pBulkInBuff + offset, pNicDevice->Leftover);
    }
  }
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  //
  SetMem (&pNicDevice->pBulkInBuff[pNicDevice->Leftover], MAX_BULKIN_SIZE, 0);
  pUsbIo = pNicDevice->pUsbIo;
  Status = pUsbIo->UsbBulkTransfer ( pUsbIo,
                                     USB_ENDPOINT_DIR_IN | BULK_IN_ENDPOINT,
                                     &pNicDevice->pBulkInBuff[pNicDevice->Leftover],
                                     &LengthInBytes,
                                     2,
                                     &TransferStatus );
  if ( !EFI_ERROR (Status) && LengthInBytes > 0 ) {
    Status = FillPkt2Queue(pNicDevice, LengthInBytes + pNicDevice->Leftover);
  }
