/** @file Sec.c
  ...
  Portions Copyright (c) 2008 - 2020, Apple Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 09/13/19   am      Added Total installed SDRAM memory print
...
#if ENABLE_AUTO_BOOT_OPTIMIZATION
/*{D14C51ED-4F4D-2F2D-EAE4-77914692D046}*/
extern EFI_GUID gCompressedSecondaryFvGuid;

/*{9E21FD93-9C72-4c15-8C4B-E77F1DB2D792}*/
extern EFI_GUID gCompressedFirstFvGuid;
#endif
...
PrintInstalledSDRAMPartitionSize (VOID)
{
  ...
  Status = GetInstalledSDRAMMemory (&TotalRamSize);
  if (EFI_ERROR (Status))
  {
    return Status;
  }
  
  SerialPrint("Total Installed RAM : %d MB (0x%010lx)\n", TotalRamSize / BYTES_TO_MB, TotalRamSize);
...
void UefiBootContinue (void* arg)
{
  ...
  UINT32           UEFIBootupCore = 0;
  
#ifdef ENABLE_AUTO_BOOT_OPTIMIZATION
  UINT32           BeforeCount, AfterCount, DeltaTime;
  UINT64           UefiFdBase = 0;
  UINT64           UefiFdSize = 0;
#endif
  
  Status = GetConfigValue ("UEFIBootupCore", (UINT32*) &UEFIBootupCore);
  if ((Status == EFI_SUCCESS) && (UEFIBootupCore != 0))
  {
    if (mpcore_get_avail_cpu_count() >= (UEFIBootupCore+1))
    {
    /* TODO: Check if core UEFIBootupCore is up with active_cpus mask, before pinning to that core.
     * But currently there is performance impact of around 10ms if we wait for core to be up.
     * So pinned core now and as soon as core is up, present thread will be switched to that core
     */
      gthread_set_pinned_cpu(thread_get_current(), UEFIBootupCore);
      thread_sleep(1);
    }
  }
  ...
  */
  Status = ProcessFvDecompression((void *)UefiFdBase, UefiFdSize, &gCompressedFirstFvGuid, FALSE );
  ASSERT_EFI_ERROR(Status);  
  
  Status = AddFvInfoToHob(&gCompressedFirstFvGuid);  
  AfterCount = GetTimerCountms();
  DeltaTime = AfterCount - BeforeCount;
  DEBUG((EFI_D_ERROR, "First FV Decompress time %d ms\r\n",DeltaTime));  
  ASSERT_EFI_ERROR(Status);  

  BeforeCount = GetTimerCountms(); 
  Status = ProcessFvDecompression((void *)UefiFdBase, UefiFdSize, &gCompressedSecondaryFvGuid, FALSE );
  ASSERT_EFI_ERROR(Status);  

  AfterCount = GetTimerCountms();
  DeltaTime = AfterCount - BeforeCount;
  DEBUG((EFI_D_ERROR, "Second FV Decompress time %d ms (Decompress Thread)\r\n",DeltaTime));  
#else
  DecompressFirstFv ();
#endif
