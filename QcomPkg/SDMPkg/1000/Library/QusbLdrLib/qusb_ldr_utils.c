 ...
 */
// ===========================================================================
char* qusb_fedl_get_role_str(pm_schg_usb_typec_port_role_type port_role)
{
  switch (port_role)
  {
    case PM_SCHG_USB_TYPEC_ROLE_NONE:     return " NONE";
    case PM_SCHG_USB_TYPEC_ROLE_DFP:      return " DFP";
    case PM_SCHG_USB_TYPEC_ROLE_UFP:      return " UFP";
    case PM_SCHG_USB_TYPEC_ROLE_INVALID:  return " INVALID";
 ...
 */
// ===========================================================================
static boolean qusb_fedl_is_ufp(void)
{
  ...
  pm_schg_usb_typec_port_role_type  port_role     = {0};

  if (PM_ERR_FLAG__SUCCESS != pm_get_pmic_info(QUSB_DCI_PMI_INDEX, &pmic_device_info))
  {    
  ...
  while (--charger_det_retry_cnt)
  {
    ...
    pm_err = pm_schg_usb_get_typec_status(QUSB_DCI_PMI_INDEX, &typec_status);
    ...
    // Normally, FEDL check can only be done after APSD charger detection
    // But on this target, APSD is disabled until UEFI
    // So, FEDL check just needs to be done for UFP with VBUS ON.
    pm_err = pm_schg_usb_typec_get_port_role(QUSB_DCI_PMI_INDEX, &port_role);
    if (PM_ERR_FLAG__SUCCESS != pm_err)
    {
      qusb_uart_log("fedl: role_det_pm_err", 0);
      qusb_error_log(DCI_SKIP__CHGR_PORT_DET_FAIL_LOG, 0, pm_err);
      return FALSE;
    }

    if (PM_SCHG_USB_TYPEC_ROLE_INVALID != port_role)
    {
      break;
    }
    qusb_dci_delay_ms(5);
  ...
  qusb_uart_w_str_log("usb: role", qusb_fedl_get_role_str(port_role));
 
  if (PM_SCHG_USB_TYPEC_ROLE_INVALID == port_role)
  {
    qusb_uart_log("fedl, role_det_timeout", 0);
    qusb_error_log(DCI_SKIP__CHGR_PORT_DET_FAIL_LOG, 0, 0);
    return FALSE;
  }

  if (PM_SCHG_USB_TYPEC_ROLE_UFP != port_role)
  {
    qusb_error_log(DCI_SKIP__CHGR_PORT_NOT_SDP_LOG, 0, port_role);
    return FALSE;
  }
...
boolean qusb_ldr_utils_fedl_check(void)
{
  ...
  // Skip rest of the code including EUD PHY tuning if EUD is already active
  if (usb_shared_is_eud_active())
  {
    /*  Test code to wait for EUD to enumerate
    for (int i=0; i>10; i++)
    {
      qusb_uart_log("waiting_1s_for_eud_enum", i);
