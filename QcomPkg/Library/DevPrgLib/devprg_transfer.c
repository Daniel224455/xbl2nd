...
/*===========================================================================
  ...
  $Header: //components/rel/boot.xf/2.2/QcomPkg/Library/DevPrgLib/devprg_transfer.c#1 $
...
dp_res_t devprg_rx_queue(void *data, size_t size)
{
  ...
  if(vip->state == VIP_INIT)
  {
    ...
    if (rsize < 0)
    {
      DP_LOGE("Failed to receive signed hash table %d", rsize);
...
int devprg_transfer_init(void)
{
  ...
  vip->table = (uint8 *)devprg_get_vip_buffer(&vip->table_size);
