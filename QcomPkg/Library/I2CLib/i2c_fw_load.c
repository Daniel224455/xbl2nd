...
--------   ---     --------------------------------------------------------
30/01/17   ska       Added check FW loading, if already loaded
...
#define I2C_PROTOCOL_FW_PROTO 3
...
i2c_status fw_init(plat_device_config *dcfg)
{
   ...
   uint32      qupv3_proto  = HWIO_INXF(se_base, QUPV3_SEn_GENI_FW_REVISION_RO, PROTOCOL);
   uint32 mask,i,result;
   
   I2C_LOG(LEVEL_PERF, "\n[I2C] qupv3_proto val 0x%x", qupv3_proto);
   
   if(qupv3_proto == I2C_PROTOCOL_FW_PROTO)
   {
       return I2C_SUCCESS;
   }
