...
typedef struct ufs_host_info {
   ...
   uint32_t             max_hsgear_device;   /* Maximum HS Gear allowed */   
   uint8_t              has_hsg4_check;      /* Check for HSG4 support done? */
   uint8_t              volt_sw_done;        /* Lowering of Vcc for UFS 3.0 done? */
   uint8_t              ratea_override;      /* Use Rate A for HS */
} ufs_host_t;
