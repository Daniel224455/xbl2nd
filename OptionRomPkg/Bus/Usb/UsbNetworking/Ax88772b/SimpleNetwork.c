  ...
  IN UINTN BufLength)
{
  ...
  for ( offset = 0; (offset + 2*sizeof(UINT16)) < BufLength; ){
      ...
      *pLengthBar |= 0xf800;
      ...
      if ((offset + 2*sizeof(UINT16) + *pLength) > BufLength) {
        break;
      }
      if (TRUE == pNicDevice->pNextFill->f_Used) {
        break;
          ...
          offset += (((*pLength + 1)&0xFFFE) + 2*sizeof(UINT16));
  ...
  if (offset < BufLength) {
    ...
    if (pNicDevice->Leftover > MAX_BULKIN_SIZE) {
      DEBUG (( EFI_D_ERROR , "Ax88772b: no room to hold partial buffer"));
      pNicDevice->Leftover = 0;
    } else {
    ...
    if ( EfiSimpleNetworkInitialized == pMode->State ) {
      ...
      if ( pMode->MediaPresent && pNicDevice->bComplete) {
        ...
        if (pNicDevice->PktCntInQueue == 0 ){
            ...
            //
            SetMem (&pNicDevice->pBulkInBuff[pNicDevice->Leftover], 4, 0);
            ...
            if (LengthInBytes != 0 && !EFI_ERROR(Status) && !EFI_ERROR(TransferStatus) ){
                FillPkt2Queue(pSimpleNetwork, LengthInBytes + pNicDevice->Leftover);
            }
        }
