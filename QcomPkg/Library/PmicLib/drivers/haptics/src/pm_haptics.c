...
pm_err_flag_type pm_haptics_set_actuator_type(uint32 pmic_chip, pm_haptics_actuator_type actuator_type)
{
  ...
  mask = 0x80;
  err_flag |= pm_comm_write_byte_mask(haptics_ptr->comm_ptr->slave_id, reg_auto_res_ctrl, mask, data, 0);
...
pm_err_flag_type pm_haptics_play(uint32 pmic_chip, pm_haptics_vibrate_type vibrate)
{
  ...
  err_flag = pm_comm_write_byte(haptics_ptr->comm_ptr->slave_id,reg_haptics_vibrate_on_off,data,0);
