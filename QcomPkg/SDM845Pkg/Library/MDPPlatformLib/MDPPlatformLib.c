...
/* SDM845 Main Panel CDP specific defines
*/
...
#define EFI_PLATFORMINFO_SUBTYPE_RB3                7     // The subtype for RB3 MTP device
...
/* SDM845 Main Panel CDP Functions
*/
...
/* SDM845 Main Panel CLS Functions
*/
...
static MDP_Status Panel_CLS_Reset(void);
...
*/
static MDP_Status ExternalPanel_CDP_PowerUp(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams);
 ...
 *                   Cmd mode                                     *
 ******************************************************************/
const static int8 NT36850_Truly_WQHD_cmd_xmldata[] =
"<?xml version=\"1.0\" encoding=\"utf-8\"?>"
...
/******************************************************************
*              CLS 12.6"  2880 x 1920 SN65DSIx6                        *
*              2880X1920                                          *
*              Dual-DSI-->SN65DSIx6(bridge chip)-->eDP(NV126A1M-N52)  *
*              Video mode                                         *
******************************************************************/
const static int8 CLS_SN65DSIx6_WQHD_DSI_xmldata[] =
"<?xml version=\"1.0\" encoding=\"utf-8\"?>"
"<PanelName>SN65DSIx6</PanelName>"
"<PanelDescription>SN65DSIx6 Dual DSI to eDP (2880x1920 24bpp)</PanelDescription>"
"<Group id=\"Active Timing\">"
" <HorizontalActive>2880</HorizontalActive>"
" <HorizontalFrontPorch>48</HorizontalFrontPorch>"
...
" <DSICmdSwapInterface>False</DSICmdSwapInterface>"
" <DSICmdUsingTrigger>False</DSICmdUsingTrigger>"
" <DSIInitMasterTime>30</DSIInitMasterTime>"
" <DSIControllerMapping>\n"
"  00 01\n"
" </DSIControllerMapping>\n"
"</Group>"
...
static PlatformDSIDetectParams panelList_0[] = {    /* subtype == 1 */
    ...
    {  
      ...
      Truly_TFT2P2827_E_cmd_dsc_xmldata,                     // psPanelCfg (panel configuration)
      sizeof(Truly_TFT2P2827_E_cmd_dsc_xmldata),             // uPanelCfgSize
...
static PlatformDSIDetectParams panelList_3[] = {    /* subtype == 3 */
    // Panel #1 - Truly DSI Command Mode Panel (1440x2560 24bpp
    {  
      0xFF,                                                  // uCmdType
      ...
      0,                                                     // Lane remap order {0, 1, 2, 3}
      dummy_xmldata,                                         // psPanelCfg (panel configuration)
      sizeof(dummy_xmldata),                                 // uPanelCfgSize
...
const PanelInfoType gPanelList[] =
{
 ...
 PANEL_CREATE_ENTRY("truly_wqxga_dual_cmd",       MDPPLATFORM_PANEL_TRULY_WQXGA_DUALDSI_CMD,   "dsi_dual_nt35597_truly_cmd_display:",   DISP_INTF_DSI, Truly_TFT2P2827_E_cmd_xmldata,         DISP_TOPOLOGY_CONFIG_NONE,  DISP_TIMING_CONFIG_NONE,    PLL_OVERRIDE_NONE, DISP_MODE_DUAL_DSI,                                DISP_MODE_DUAL_DSI | DISP_MODE_PING_PONG_SPLIT,       DISP_MODE_DUAL_DSI),
 
 /*Truly 2k NT35597 video*/
 PANEL_CREATE_ENTRY("truly_wqxga_dual_vid",       MDPPLATFORM_PANEL_TRULY_WQXGA_DUALDSI_VIDEO,  "dsi_dual_nt35597_truly_video_display:",DISP_INTF_DSI, Truly_TFT2P2827_E_video_xmldata,       DISP_TOPOLOGY_CONFIG_NONE,  DISP_TIMING_CONFIG_NONE,    PLL_OVERRIDE_NONE, DISP_MODE_DUAL_DSI,                                DISP_MODE_DUAL_DSI | DISP_MODE_PING_PONG_SPLIT,       DISP_MODE_DUAL_DSI),

 /*NT36850 Truly WQHD CMD */
 PANEL_CREATE_ENTRY("nt36850_truly_wqhd_cmd",     MDPPLATFORM_PANEL_NT36850_TRULY_WQHD_CMD,     "dsi_nt36850_truly_wqhd_cmd_display:",  DISP_INTF_DSI, NT36850_Truly_WQHD_cmd_xmldata,        DISP_TOPOLOGY_CONFIG_NONE,  DISP_TIMING_CONFIG_NONE,    PLL_OVERRIDE_NONE, DISP_MODE_DUAL_DSI,                                DISP_MODE_DUAL_DSI,                                   DISP_MODE_DUAL_DSI),
...
/* ---------------------------------------------------------------------- */
/**
** FUNCTION: Panel_CDP_backlight_enable()
**
** DESCRIPTION: configure wled registers and enable it
**
**/
static MDP_Status Panel_CDP_backlight_enable(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
    ...
    else
    {
        ...
        if (pPowerParams->ePMICSecondaryPower[eDisplayId] == MDP_PMIC_MODULE_CONTROLTYPE_IBB_LAB_OLED)
        {
              ...
              if (MDPPLATFORM_LABIBB_MODE_SWIRE == pPowerParams->eOLEDPowerMode)
              {
                ...
                //if (EFI_SUCCESS != PmicWledProtocol->SetDormantSwireVolt(PMIC_PMI_DEV_INDEX, 7600))
                //{
                //  DEBUG((EFI_D_ERROR, "Wled Protocol SetDormantSwireVolt failed\n"));
                //}
...
static MDP_Status Panel_CLS_PowerUp(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    */
    ...
    /* Enable backlight PWM signal: config GPIO_8 as high output always first */
    ...
    if (EFI_SUCCESS != TLMMProtocol->GpioOut((UINT32)EFI_GPIO_CFG(CLS_GPIO_BACKLIGHT_SOURCE_EN, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_16MA), GPIO_HIGH_VALUE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: GPIO %d line high failed!\n", CLS_GPIO_BACKLIGHT_SOURCE_EN));
    }
  }
...
**
** DESCRIPTION:
**        Panel power down sequence for CLS
**
**/
static MDP_Status Panel_CLS_PowerDown(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    /* Disable VCC/VCCA 1.2V to bridge chip, active high */
    ...
    /* Disable 3.0V to LCD panel, active high */
...
**
** DESCRIPTION:
**        Panel reset sequence for CLS
**
**/
MDP_Status Panel_CLS_Reset(void)
{
  MDP_Status                    Status       = MDP_STATUS_OK;
  EFI_TLMM_PROTOCOL            *TLMMProtocol = NULL;
  ...
  case MDPPLATFORM_CONFIG_SW_RENDERER:
    ...
    else if (TRUE == sPlatformInfo.bSWRender)
    {
      ...
      pPlatformParams->sPlatformInfo.bSWRender = TRUE;
             ...
             (EFI_PLATFORMINFO_SUBTYPE_RB3  == sPlatformInfo.sEFIPlatformType.subtype)            ||
