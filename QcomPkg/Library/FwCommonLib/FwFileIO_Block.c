...
{ 
    EFI_STATUS              Status;
    EFI_BLOCK_IO_PROTOCOL   *BlockIo;
    UINTN                   BlockSize = 0, LengthUnAligned = *Length;
    EFI_STATUS (*BlockOperation)() = NULL;
    ...
    ALIGN_TO_BLOCK(LengthUnAligned, BlockSize, *Length);

    //Set Read and Write operations.
    if(Read)
    {
        BlockOperation = BlockIo->ReadBlocks;
        Status = AllocateAndZero(EfiBootServicesData, *Length, Buffer);
    ...
    {
        BlockOperation = BlockIo->WriteBlocks;
        Status = FwUpdateBlockFileAlignFileBuffer(EfiBootServicesData, LengthUnAligned, Buffer, *Length);
