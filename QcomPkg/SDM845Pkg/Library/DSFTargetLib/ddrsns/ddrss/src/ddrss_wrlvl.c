...
$Header: //components/rel/boot.xf/2.0/QcomPkg/SDM845Pkg/Library/DSFTargetLib/ddrsns/ddrss/src/ddrss_wrlvl.c#14 $
$DateTime: 2018/02/02 03:33:01 $
$Author: pwbldsvc $
================================================================================
...
{  
  ...
  freq_eq_max_training          = WRLVL_Training_Switchboard[prfs_index][_WRLVL_CF];
    ...
    if (freq_eq_max_training)
    {
        ...
        // Get MR2 opcode for wrlvl frequency
...
void DDRSS_wrlvl_ca(BOOT_DDR_STRUCT *ddr, uint8 ch, uint8 cs, uint32 freq, training_params_t *training_params_ptr, uint8 prfs_index, uint8 MR2_opcode)
{
  ...
  //-------------------------------------------------------------------------------------
    ...
    { 
        ...
        for (index = prfs_index; index >= LOWEST_WRLVL_COPY_PRFS; index--)
        {
