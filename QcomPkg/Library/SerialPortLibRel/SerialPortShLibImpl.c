...
UINTN gFbAddr = FixedPcdGet32(PcdMipiFrameBufferAddress);
...
UINTN gBpp    = FixedPcdGet32(PcdMipiFrameBufferBpp);
UINTN delay = 10000;
...
newline:
  ...
  if (m_Position.y >= m_MaxPosition.y - scale_factor) {
    ZeroMem ((VOID *)gFbAddr, 4 * gWidth * gHeight);
    FbConFlush();
    m_Position.y = 0;
...
void FbConScrollUp(void)
{
  unsigned short *dst   = (void *)gFbAddr;
