  ...
  *Message = UsbCfgPortMsgNone;
  ...
  if ((PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_RUMI) ||
    ...
    (PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_UNKNOWN) ||
     (PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_OEM)) {
  ...
  if (gUsbPortCfg[PortIndex].Type == USB_CONNECTOR_TYPE_C) { // For Type-C ports, primary and secondary
  ...
  else { // For the Type-A ports (connected to multi-port controller)
    Status = EFI_INVALID_PARAMETER;
    goto ON_EXIT;
  }
  ...
  UsbConfigPortsPrintPortStatus(EFI_D_INFO, UsbPort, *Message, PortIndex);
