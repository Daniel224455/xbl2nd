...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 08/20/18   yw       fix bug which cause assertion from freeing OutputBuffe if ProduceFVBProtocolOnBuffer() fails
 07/31/18   yw       update to support FV loading by GUID
 01/23/18   kpa      Updates to support authenticated FV loading
  ...
  *OutputBuffer = NULL;
  ...
  while (ParsedLength < SectionSize)
  {
    ...
    else if (Section->Type == EFI_SECTION_GUID_DEFINED)
    {
      ...
      //
      OrigDstBufferPage = EFI_SIZE_TO_PAGES (DstBufferSize) + 1;
      OrigDstBuffer = (VOID *)(UINTN)AllocatePages (OrigDstBufferPage);
      if (OrigDstBuffer == NULL)
        return EFI_OUT_OF_RESOURCES;
      ...
      //
      DstBuffer = (UINT8 *)OrigDstBuffer + EFI_PAGE_SIZE - sizeof (EFI_COMMON_SECTION_HEADER);
      //
  ...
  )
{
  ...
  if (OrigDstBuffer != NULL && OrigDstBufferPage != 0)
  {
      FreePages(OrigDstBuffer, OrigDstBufferPage);
      OrigDstBuffer = NULL;
      OrigDstBufferPage = 0;
  }
...
/**
  ...
  @param[out] ReadBuffer        Buffer cointaining loaded FV
        ...
        [in]  Attrib            Attributes of partitioned to be opened
    ...
    )
{
  ...
  {
    DEBUG ((EFI_D_ERROR, "ReadFromPartitionWithAttrib requires either partition label OR Guid\n"));
    return EFI_INVALID_PARAMETER;
  }
  ...
  Status = GetBlkIOHandlesExt(Attrib,
                            &FilterData,
                            HandleInfoList,
                            &HandleCount);
#else
...
EFI_STATUS ReadFromPartition (CHAR16* PartitionLabel, VOID **ReadBuffer, UINTN *ImageSize)
{
  ...
  return ReadFromPartitionWithAttrib(NULL, PartitionLabel, ReadBuffer, ImageSize, Attrib);
...
/** 
  ...
  @param[in]   PartitionGuid   Partition GUID
...
EFI_STATUS ReadFromPartitionByGuid (EFI_GUID* PartitionGuid, VOID **ReadBuffer, UINTN *ImageSize)
{
  UINT32                 Attrib;
  ...
  return ReadFromPartitionWithAttrib(PartitionGuid, NULL, ReadBuffer, ImageSize, Attrib);
}
